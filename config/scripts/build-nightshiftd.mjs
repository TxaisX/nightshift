#!/usr/bin/env node
/**
 * Bundle `nightshiftd` — the Nightshift runtime served from plain Node, no Electron.
 *
...
} from '../../src/shared/nightshiftd-artifacts.ts'

const ROOT = join(import.meta.dirname, '..', '..')
const OUT_DIR = join(ROOT, 'out', 'nightshiftd')
const ENTRY = join(ROOT, 'src/main/nightshiftd/main.ts')
// Why beside nightshiftd.js: the watcher runs in a forked child so a native @parcel/watcher
...
// Why beside nightshiftd.js: nightshiftd forks the terminal daemon so PTYs outlive the runtime process,
// and `getDaemonEntryPath()` probes the app root for this exact filename. Without it every
// nightshiftd restart would SIGKILL every running terminal.
...
const OUT_FILE = join(OUT_DIR, 'nightshiftd.js')
...
// Why only these: measured, not guessed. `node-pty` is a hard `require.resolve` — nightshiftd
 ...
 *  directory under `outdir`, and both children must land flat beside nightshiftd.js — that is where
...
const output = Object.values(result.metafile.outputs).find(
  (o) => o.entryPoint === 'src/main/nightshiftd/main.ts'
)
...
if (graphErrors.length > 0) {
  console.error(`[build-nightshiftd] ${graphErrors.join('\n')}`)
  // Why this can exceed the ratchet baseline: the ratchet measures the graph reachable
  // from nightshift-runtime + runtime-rpc, but this entry also imports ipc/pty directly to
  // install the PTY controller. Once nightshiftd ships, it should become a ratchet entry
  ...
  // smoke-loads its entries for exactly this reason, and nightshiftd cannot join that guard
  ...
  const smoke = spawnSync(process.execPath, [OUT_FILE, '--nightshiftd-smoke-load-check'], {
    encoding: 'utf8',
    timeout: 60_000
  })
  ...
  if (smoke.error || smoke.signal || smoke.status !== 0) {
    console.error(
      `[build-nightshiftd] the bundle did not load under plain Node.\n` +
  ...
  // token and a PTY — `smoke:nightshiftd-terminal` does that end to end, through nightshiftd.
  ...
  const daemonSmoke = spawnSync(
    ...
    ],
    {
      encoding: 'utf8',
      timeout: 60_000,
      env: { ...process.env, NIGHTSHIFT_DAEMON_ENTRY_LOAD_CHECK: '1' }
    }
  )
  ...
  if (daemonSmoke.error || daemonSmoke.signal || daemonSmoke.status !== 0) {
    console.error(
      `[build-nightshiftd] the daemon child did not load under plain Node.\n` +
  ...
  if (watcherFailure) {
    console.error(
      `[build-nightshiftd] the watcher child did not run under plain Node.\n${watcherFailure}`
    )
    process.exitCode = 1
  }
}

// Why a content hash and not NIGHTSHIFTD_VERSION alone: the remote install directory is keyed on
...
if (process.exitCode !== 1) {
  ...
  for (const filename of nightshiftdArtifactFilenames()) {
    const artifactPath = join(OUT_DIR, filename)
    if (!existsSync(artifactPath)) {
      throw new Error(
        `nightshiftd declares ${filename} in NIGHTSHIFTD_ARTIFACTS but never emitted it. Add the build ` +
          'step, or drop it from src/shared/nightshiftd-artifacts.ts.'
      )
    }
    hash.update(readFileSync(artifactPath))
  }
  const fullVersion = `${NIGHTSHIFTD_VERSION}+${hash.digest('hex').slice(0, 12)}`
  writeFileSync(join(OUT_DIR, NIGHTSHIFTD_VERSION_FILENAME), fullVersion)
  console.log(
    `[build-nightshiftd] ok — ${fullVersion}, ${(output.bytes / 1024 / 1024).toFixed(2)} MB, ${Object.keys(output.inputs).length} modules, zero electron and node:sqlite imports.`
  )
}
...
async function smokeLoadWatcherChild() {
  const probeDir = mkdtempSync(join(tmpdir(), 'nightshiftd-watcher-smoke-'))
