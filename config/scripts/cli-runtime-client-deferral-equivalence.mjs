...
function run(entry, argv, env) {
  ...
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT') {
      throw new Error(
        `nightshift ${argv.join(' ')} did not exit within ${RUN_TIMEOUT_MS} ms — it reached a blocking command`
      )
    }
    throw result.error
  }
...
function buildCases(isolatedUserData) {
  const isolated = { NIGHTSHIFT_USER_DATA_PATH: isolatedUserData }
  const cases = [
    ...
    [['no-such-command'], {}],
    [['no-such-command'], { NIGHTSHIFT_PAIRING_CODE: 'garbage' }],
    [['wrktree', 'list'], {}],
    [['agent-context'], {}],
    [['agent-context', '--json'], {}],
    ...
    [['status', '--pairing-code', 'nightshift://pair?code=zzzz'], isolated],
    ...
    [['status', '--json'], { ...isolated, NIGHTSHIFT_PAIRING_CODE: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, NIGHTSHIFT_REMOTE_PAIRING: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, NIGHTSHIFT_ENVIRONMENT: 'no-such-environment' }],
    ...
    // NIGHTSHIFT_USER_DATA_PATH. They are deliberately NOT invoked here. The
    ...
    [
      ...
      { ...isolated, NIGHTSHIFT_ENVIRONMENT: 'no-such-environment' }
    ],
    [
      ['environment', 'list', '--json'],
      { ...isolated, NIGHTSHIFT_PAIRING_CODE: 'not-a-pairing-code' }
    ],
    [['agent-context', '--json'], { ...isolated, NIGHTSHIFT_PAIRING_CODE: 'not-a-pairing-code' }],
...
//
//   1. FOREGROUND — `nightshift serve` runs Nightshift until Ctrl+C and `nightshift open` /
...
//   2. MUTATING — writes outside NIGHTSHIFT_USER_DATA_PATH (`agent hooks off` parks
...
const UNSAFE_TOKENS = new Map([
  ...
  ...MUTATING_TOKENS.map((token) => [token, 'can write outside NIGHTSHIFT_USER_DATA_PATH'])
])
