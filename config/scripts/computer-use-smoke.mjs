...
const cliPath =
  process.env.NIGHTSHIFT_COMPUTER_SMOKE_CLI_PATH ?? resolve(repoRoot, 'out', 'cli', 'index.js')
...
const preferredApps = (
  requestedApps ??
  process.env.NIGHTSHIFT_COMPUTER_SMOKE_APPS ??
  ...
  const child = spawnSync(process.execPath, [cliPath, ...cliArgs], {
    ...
    env: {
      ...
      NIGHTSHIFT_USER_DATA_PATH:
        process.env.NIGHTSHIFT_COMPUTER_SMOKE_USER_DATA_PATH ?? defaultDevUserDataPath()
    }
  })
  if (child.status !== 0) {
...
function defaultDevUserDataPath() {
  if (process.platform === 'darwin') {
    return resolve(homedir(), 'Library', 'Application Support', 'nightshift-dev')
  }
  ...
  return resolve(process.env.XDG_CONFIG_HOME ?? resolve(homedir(), '.config'), 'nightshift-dev')
}
