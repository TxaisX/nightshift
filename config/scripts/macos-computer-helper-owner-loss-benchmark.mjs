...
const INTERNAL_ENV = 'NIGHTSHIFT_COMPUTER_HELPER_OWNER_BENCH_INTERNAL'
const EXPECTATION_ENV = 'NIGHTSHIFT_COMPUTER_HELPER_OWNER_BENCH_EXPECTATION'
const HELPER_RECORD_PATH_ENV = 'NIGHTSHIFT_COMPUTER_HELPER_OWNER_BENCH_HELPER_RECORD_PATH'
const RESULT_PATH_ENV = 'NIGHTSHIFT_COMPUTER_HELPER_OWNER_BENCH_RESULT_PATH'
...
const helperPath = path.join(helperAppPath, 'Contents', 'MacOS', 'nightshift-computer-use-macos')
...
function startSidecar() {
  ...
  const child = fork(sidecarPath, [], {
    ...
    env: {
      ...
      NIGHTSHIFT_COMPUTER_SIDECAR: '1',
      NIGHTSHIFT_COMPUTER_MACOS_HELPER_APP_PATH: helperAppPath
    }
  })
...
function runTrial(executable, expectation) {
  ...
  let trialOutput = ''
  try {
    launcherDir = mkdtempSync(path.join(tmpdir(), 'nightshift-helper-owner-bench-launcher-'))
    trialTempDir = mkdtempSync(path.join(path.sep, 'tmp', 'nightshift-owner-bench-'))
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'nightshift-helper-owner-benchmark', main: 'main.cjs' })
    )
    writeFileSync(
