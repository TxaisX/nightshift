...
// Why: nightshift-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'nightshift-cli.md')
const stubPath = join(projectDir, 'skills', 'nightshift-cli', 'SKILL.md')
// Why: orchestration and nightshift-emulator also ship hybrid stubs now, so their version-sensitive
...
const emulatorSkillPath = join(projectDir, 'skill-guides', 'nightshift-emulator.md')

function readSkill(path = guidePath) {
  return readFileSync(path, 'utf8')
}

describe('nightshift CLI skill guidance', () => {
  it('keeps external browser routing at the OS/page boundary', () => {
    ...
    expect(skill).toContain(
      'For external Chrome/Safari/webviews or Nightshift app chrome/settings, use the Computer Use skill/tool only when the task requires OS/window-level control'
    )
    expect(skill).toContain(
      "Use `nightshift-cli` for Nightshift's embedded pages and a page-automation tool such as Playwright or CDP for external pages"
    )
  })
  ...
  it('keeps independent worktree lineage separate from Git base selection', () => {
    ...
    expect(skill).toContain('`--no-parent` only controls Nightshift lineage')
    expect(skill).toContain('omit `--base-branch` so Nightshift uses the repo default base')
  ...
  it('documents non-lifecycle full handoffs and custom Codex model fallback', () => {
    ...
    expect(skill).toContain(
      'Do not use `nightshift orchestration task-create`, `nightshift orchestration dispatch --inject`, or `nightshift orchestration check --wait` for full handoffs.'
    )
    expect(skill).toContain(
      '`task-create` is also forbidden because it records coordinator-owned tracking state'
    )
    expect(skill).toContain(
      'NIGHTSHIFT worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
  ...
  // behind `skills get nightshift-cli --reference` so they are not charged to every turn, with
  ...
  it('gates the reconstructible command catalogs behind bundled references', () => {
    ...
    expect(skill).toContain('NIGHTSHIFT skills get nightshift-cli --reference references/<file>.md')
    expect(skill).toContain(
      'If the CLI rejects `--reference`, run `NIGHTSHIFT skills get nightshift-cli --full`'
    )
    for (const reference of [
      ...
      expect(
        readSkill(join(projectDir, 'skill-guides', 'nightshift-cli', reference)).trim()
      ).not.toBe('')
    }
    expect(skill).not.toContain('NIGHTSHIFT automations create')
    expect(skill).not.toContain('NIGHTSHIFT artifacts share <file>')
    expect(skill).not.toContain('NIGHTSHIFT goto --url')
  })
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`nightshift eval` expressions, or `nightshift exec` commands')
...
describe('nightshift CLI install stub', () => {
  ...
  it('does not fall through to another executable on a resolution failure', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong Nightshift build.
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('NIGHTSHIFT automations create')
