...
// Why: nightshift-linear and its legacy linear-tickets alias now ship hybrid discovery stubs, so
...
const canonicalGuidePath = join(projectDir, 'skill-guides', 'nightshift-linear.md')
const legacyGuidePath = join(projectDir, 'skill-guides', 'linear-tickets.md')
const canonicalStubPath = join(projectDir, 'skills', 'nightshift-linear', 'SKILL.md')
...
describe('nightshift-linear skill guidance', () => {
  it('keeps canonical and legacy Linear guide bodies from drifting', () => {
    ...
    expect(canonical).toContain('name: nightshift-linear')
  ...
  it('documents targeted project discovery in both skill names', () => {
    ...
    for (const skill of [canonical, legacy]) {
      expect(skill).toContain('NIGHTSHIFT linear project list --query <project-name>')
      expect(skill).toContain('Run only the command for the metadata you need')
    }
  })

  // Why: a bare `nightshift` at line start resolves to the GNOME Orca screen reader on Linux and
  // starts speech on the user's machine, so guide examples use the resolved-executable
  // placeholder instead.
  it('keeps Linear guide examples off a bare nightshift command name', () => {
    for (const guidePath of [canonicalGuidePath, legacyGuidePath]) {
      ...
      expect(skill, guidePath).toContain(
        '`NIGHTSHIFT` is a placeholder for the executable you resolved in the stub'
      )
      expect(skill, guidePath).not.toMatch(/^nightshift /mu)
      expect(skill, guidePath).not.toMatch(/\$NIGHTSHIFT(?:_|\b)/u)
    }
  })
...
describe('nightshift-linear install stubs', () => {
  const cases = [
    { name: 'nightshift-linear', stubPath: canonicalStubPath, guidePath: canonicalGuidePath },
  ...
  for (const { name, stubPath, guidePath } of cases) {
    it(`points ${name} at the version-matched guide and preserves the safe resolver`, () => {
      ...
      expect(stub).toContain(`NIGHTSHIFT skills get ${name}`)
    ...
    it(`drops the changing command reference from the installable ${name} file`, () => {
      ...
      expect(stub).not.toMatch(/\bnightshift linear search\b/iu)
      expect(stub).not.toMatch(/\bnightshift linear comment\b/iu)
