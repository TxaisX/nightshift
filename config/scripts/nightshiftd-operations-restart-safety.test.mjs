...
const operationsGuide = readFileSync('docs/reference/nightshiftd-operations.md', 'utf8')
const operationsProse = operationsGuide.replace(/\s+/g, ' ')

describe('nightshiftd operations restart safety', () => {
  ...
  it('fails closed before cgroup-wide maintenance', () => {
    ...
    expect(operationsGuide).not.toContain('sudo -Hu nightshift nightshift-ide terminal list --json')
    expect(operationsProse).toContain(
    ...
    expect(operationsProse).toContain(
      'Nightshift does not yet provide an atomic census-and-stop fence'
    )
  })

  it('does not refer to the unavailable shipping design', () => {
    expect(operationsGuide).not.toContain('docs/design/shipping-nightshiftd.html')
  })
})
