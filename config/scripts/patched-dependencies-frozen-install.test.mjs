...
describe('patched dependencies', () => {
  it('installs with --frozen-lockfile, which is what validates every patch hash', () => {
    ...
    const scratch = mkdtempSync(join(tmpdir(), 'nightshift-frozen-install-'))
    try {
  ...
  it('materializes the patched command-line reader on a real install', () => {
    ...
    const scratch = mkdtempSync(join(tmpdir(), 'nightshift-patch-apply-'))
    try {
      ...
      writeFileSync(
        ...
        `${JSON.stringify(
          {
            name: 'nightshift-patch-apply-probe',
            version: '1.0.0',
