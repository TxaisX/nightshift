...
describe('PR workflow parallelism', () => {
  ...
  it('reuses native preparation after the dependency action gate', () => {
    ...
    expect(packageStep.env.NIGHTSHIFT_REUSE_PREPARED_NATIVE_RUNTIME).toBe('1')
  })
  ...
  it('keeps verify as the aggregate required check', () => {
    ...
    // NIGHTSHIFT_BROWSER_EXECUTABLE, so it only guards anything if verify actually reads it.
    expect(verifyStep.env.NIGHTSHIFTD_BROWSER).toBe('${{ needs.nightshiftd_browser.result }}')
    expect(verifyStep.run).toContain('"$NIGHTSHIFTD_BROWSER"')
