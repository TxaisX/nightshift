...
// Why: under a Git Bash setup runner, Nightshift exports NIGHTSHIFT_WORKTREE_PATH in MSYS form (/c/...), which
...
export function runInternalDevSetup({
  ...
  const setupPath = env.NIGHTSHIFT_INTERNAL_DEV_SETUP?.trim()
  ...
  spawnOptionalSetup(spawn, setupPath, env.NIGHTSHIFT_WORKTREE_PATH || cwd, platform, env)
