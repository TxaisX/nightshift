...
#
# Why a separate repo rather than reusing nightshift-hourly: the daily channel is a
...
if gh api "repos/$DAILY_REPO" --jq '.full_name' >/dev/null 2>&1; then
  ...
  gh repo create "$DAILY_REPO" \
    --public \
    --description "Daily macOS dev builds of Nightshift, cut from main each morning. Not a source repo." \
    --add-readme \
    --disable-issues \
    --disable-wiki ||
...
Could not do it from here${INSTALL_ID:+ (needs an Organization Owner)}. Do it in the browser:
  ...
  3. Repository access  ->  Only select repositories  ->  add $DAILY_REPO
     (keep nightshift-hourly and nightshift-adhoc selected; all dev channels use this one App)
  4. Save.
EOF
fi
