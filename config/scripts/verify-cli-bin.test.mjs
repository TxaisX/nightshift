...
/** Builds a temporary Nightshift-style project fixture with a compiled CLI entrypoint. */
function makeProjectWithCli(
  content,
  { mode = 0o755, rootPackageType, writeOutPackageJson = true } = {}
) {
  const projectDir = mkdtempSync(path.join(tmpdir(), 'nightshift-cli-bin-'))
  ...
  writeFileSync(
    ...
    JSON.stringify({ bin: { nightshift: './out/cli/index.js' }, type: rootPackageType }),
    'utf8'
  )
...
describe('verifyPackageCliBin', () => {
  ...
  it('rejects an empty package bin target', () => {
    ...
    expect(() => verifyPackageCliBin({ projectDir })).toThrow('bin.nightshift target is empty')
  })

  it('rejects package bin targets without a Node shebang', () => {
    const { projectDir } = makeProjectWithCli('console.log("nightshift")\n')
  ...
  it('writes a CommonJS package boundary for the compiled CLI directory', () => {
    ...
    expect(JSON.parse(readFileSync(outPackageJsonPath, 'utf8'))).toEqual({
      name: 'nightshift-compiled-output',
      type: 'commonjs',
      private: true
    })
  })
