...
function createGitRepo() {
  const repoDir = mkdtempSync(path.join(tmpdir(), 'nightshift-wayland-gpu-repo-'))
...
async function runValidation(mode) {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'nightshift-wayland-gpu-userdata-'))
  ...
  try {
    ...
    app.process().stderr?.on('data', (chunk) => {
      ...
      if (process.env.NIGHTSHIFT_WAYLAND_GPU_VERBOSE === '1') {
