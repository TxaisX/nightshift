...
describe('verify packaged plugin resources', () => {
  it('accepts exact launch bytes copied into a packaged resources directory', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'nightshift-packaged-plugins-'))
    try {
      await cp(
  ...
  it('rejects mutated bytes in the packaged output', async () => {
    ...
    try {
      ...
      await writeFile(
        join(launchRoot, 'txais.nightshift-navigation-shortcuts', 'extra.json'),
        '{"mutated":true}\n'
      )

      expect(() => verifyPackagedPluginResources(resourcesDir)).toThrow(
        'packaged bytes do not match txais.nightshift-navigation-shortcuts'
      )
    } finally {
      await rm(resourcesDir, { recursive: true, force: true })
    }
