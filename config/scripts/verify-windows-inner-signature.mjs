...
$signature = Get-AuthenticodeSignature -FilePath $env:NIGHTSHIFT_WINDOWS_INNER_EXECUTABLE
...
export function parseExpectedSigners(value = process.env.NIGHTSHIFT_WINDOWS_EXPECTED_SIGNERS) {
...
export function parseExpectedThumbprints(
  value = process.env.NIGHTSHIFT_WINDOWS_EXPECTED_THUMBPRINTS
) {
...
export function validateExecutablePath(executablePath) {
  if (typeof executablePath !== 'string' || executablePath.trim() === '') {
    throw new Error(
      'Usage: node config/scripts/verify-windows-inner-signature.mjs <Nightshift.exe>'
    )
  }
...
export function getPowerShellSignatureJson(executablePath, spawnSyncImpl = spawnSync) {
  ...
  const result = spawnSyncImpl(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        NIGHTSHIFT_WINDOWS_INNER_EXECUTABLE: executablePath
      }
    }
  )
