 ...
 * graceful-then-immediate kill pair emitted when Nightshift closes a workspace.
...
const iterations = Number(process.env.NIGHTSHIFT_WINDOWS_DAEMON_CLOSE_ITERATIONS ?? 25)
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'nightshift-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\nightshift-daemon-close-${process.pid}-${randomUUID()}`
  ...
  const child = fork(
    ...
    ['--socket', socketPath, '--token', tokenPath, '--log-file', daemonLogPath],
    {
      ...
      windowsHide: true,
      env: { ...process.env, NIGHTSHIFT_USER_DATA_PATH: scratch }
    }
  )
