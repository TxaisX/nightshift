...
- Nightshift implementation: `skills-share` through `607e31dd04`; no PR.
- Nightshift Cloud: bundle smoke PR `#329` merged as `eddb144afe`; generation-aware recovery PR `#330`
...
- Final bearer-link changes pass 143 Nightshift Cloud API tests with one opt-in skip; 58 focused Nightshift
...
- The opt-in desktop staging harness now publishes a bearer share without retired audience fields;
  Node typecheck and Playwright test discovery pass without starting Nightshift or a browser login flow.
- Local Node and web typechecks, changed-code quality gates, 94 skill-domain files with 770 tests
  passed and 3 skipped, 134 Nightshift Cloud API tests with one opt-in integration skip, the full Cloud
  monorepo test/typecheck/lint/build gates, and isolated Terraform formatting and validation.
- Final Nightshift validation passed repository-wide lint, all Node/CLI/web typechecks, the production
  ...
  after removing Nightshift-injected Git config variables and rerunning the slow macOS PTY case.
...
- Standalone and bundled package names reject Windows reserved device names in both Nightshift and Cloud,
...
- The final combined physical `windows 2` run at `44d1266641` enabled native-Windows, real-process,
  ...
  aliases with `ln -sT`. The checkout remained clean and no `nightshift-skill-*` directory remained in
  WSL `/tmp`.
...
- The exact x64 Electron directory package at `f1dccb4f42` passed its after-pack ABI scan for all
  ...
  `nightshift-ide` executable and `resources/node_modules` layout. A native ARM64 package built from
...
- Real paired-runtime installation passed from an isolated Electron client to both a headed
  desktop host and headless `nightshift serve` host. The host owned global and Git-worktree installs;
  the headed host also owned a plain-folder install. The client's home remained untouched.
- A fresh post-resilience E2E run passed all three headed paired-runtime, headless `nightshift serve`,
...
- Isolated staging bucket, IAM, `nightshift_skills` database and principal, enabled secret version, 11
  log metrics, four alerts, and one dashboard in `nightshift-cloud-staging`. The complete targeted
...
- The route-disabled baseline, API revision `nightshift-cloud-api-staging-00039-rek`, first proved the
...
- Auth deploy run `31535179937` promoted revision `nightshift-cloud-auth-staging-00017-dug` at 100%
...
- The historical ACL-era API deploy run `31535438327` promoted revision
  `nightshift-cloud-api-staging-00042-hef` at 100%
...
- Cloud PR `#317` passed both required checks and merged as `be00db10`. Its historical ACL-era
  ...
  promoted revision `nightshift-cloud-api-staging-00048-xom` at 100% traffic with
...
- Guarded wake run `31564370807` prepared the reviewed staging topology. Deploy run `31564803943`
  promoted `nightshift-cloud-api-staging-00051-yoq` at 100% traffic with immutable image digest
...
- Cloud PR `#330` passed required run `31578720429`, merged as `8045c85dad`, and passed merged-main
  ...
  deploy `31579844413` promoted `nightshift-cloud-api-staging-00057-kat` at 100% traffic with immutable
...
- Auth deploy `31584318896` promoted `nightshift-cloud-auth-staging-00021-tuq` with digest
...
- Guarded wake `31589384191` restored SQL and the two configured Relay cells without waking C3.
  Auth deploy `31589963244` promoted `nightshift-cloud-auth-staging-00025-zuz` at 100% traffic with
...
- Guarded wake `31603249983` restored only the two configured staging cells. A disposable
  ...
  package deletion. The remote install, Nightshift SSH target, encrypted credential artifact, one-time
...
- Guarded wake `31605729090` restored the same configured topology. An isolated headless Nightshift host
...
- Cloud PR `#355` removed internal publisher and organization identifiers from anonymous skill
  ...
  `31640677572` promoted `nightshift-cloud-api-staging-00060-qay` at 100% traffic with immutable digest
...
- [x] A user can select one or many private local skills, publish one immutable Skill Bundle, and
      receive one durable Nightshift share URL.
- [x] Anyone with an active unlisted link can inspect a bundle without signing in, choose all or a
      ...
      workspace.
- [x] Installation works on local macOS, Linux, native Windows, WSL, paired Nightshift runtimes, and
      supported SSH targets.
- [x] The portable archive root conforms to Agent Plugins 1.0.0 for skills-only packages and keeps
      Nightshift integrity metadata in `dev.nightshift.skill-sharing/manifest.json`.
...
- [x] Decide Nightshift will not depend on the upstream CLI or an unsupported programmatic API.
- [x] Decide V1 accepts Nightshift package sources only; Git, npm, and community registries remain with
      existing tools.
...
- [x] Treat the archive as a Skill Bundle, not a Nightshift plugin product or executable plugin runtime.
...
- [x] Confirm private means unlisted and protected by an unpredictable revocable bearer link, not
      end-to-end encryption from Nightshift Cloud operators.
...
- [x] Replace the unpublished single-skill envelope with root `plugin.json`, `skills/<name>/`, and
      `dev.nightshift.skill-sharing/manifest.json`; do not add an outer `bundle/` directory.
...
- [x] Create fresh staging roots and reject unknown, malformed, or conflicting imported
      `dev.nightshift.skill-sharing` namespaces without overwriting them. The extractor now exclusively
...
- [x] Generate deterministic `plugin.json`, `skills/`, and Nightshift extension metadata.
...
- [x] Validate the portable root, Nightshift extension namespace, bundle manifest, and every selected
...
- [x] Allow only configured Nightshift skill-bucket origins and reject credential-bearing cross-host
      redirects.
...
- [x] Permit clean updates only when installed bytes still match Nightshift provenance.
- [x] Return a modified conflict when installed bytes drift from their receipt.
- [x] Return an unowned conflict when a destination exists without Nightshift provenance.
...
- [x] Add data-only records for Nightshift agent ID, canonical-root support, global/project resolvers,
...
- [x] Repair broken Nightshift-owned aliases.
...
- [x] Remove the canonical copy only when Nightshift owns it, it is unmodified, and no retained receipt
      depends on it.
...
- [x] Inspect the active `nightshift-cloud` project read-only and record the existing Cloud Run, GCS,
      Cloud SQL, IAM, and enabled-service baseline.
- [x] Locate the authoritative Nightshift Cloud Terraform worktree, state, modules, and deployment
      pipeline.
...
- [x] Confirm production names and quotas, including availability of
      `nightshift-cloud-skill-packages`. A read-only 2026-08-12 check returned `404` for the global
      bucket name, found no production `nightshift_skills` database or skill-database secret collision,
...
- [x] Declare private bucket `nightshift-cloud-skill-packages`, subject to Terraform validation.
...
- [x] Add exact approved production and development Nightshift origins to CORS; never use `*`.
...
- [x] Declare database `nightshift_skills` on existing regional PostgreSQL 17 instance
      `nightshift-cloud-auth-db`.
- [x] Declare dedicated principal `nightshift_skills_app` with access only to `nightshift_skills`.
- [x] Store its connection URL in Secret Manager as `nightshift-cloud-skills-database-url`.
- [x] Attach the existing Cloud SQL instance to `nightshift-cloud-api` without replacing the service.
- [x] Inject only the skill database secret into the API service.
- [x] Verify backups and point-in-time recovery cover the new database. `nightshift_skills` shares the
...
- [x] Grant `nightshift-cloud-api@nightshift-cloud.iam.gserviceaccount.com` bucket-scoped
...
- [x] Extend `nightshift-cloud-api` in `us-central1`; do not create a separate V1 worker service.
...
- [x] Make immutable API candidates atomically replace the complete Terraform-owned literal
      environment and the exact `NIGHTSHIFT_SKILLS_DATABASE_URL` Secret Manager reference.
...
- [x] Test tenant-scoped deduplication without cross-tenant timing or response disclosure. Nightshift
...
- [x] Return a copyable durable Nightshift URL after publication.
...
- [x] Show whether a package came from Nightshift Cloud and its accessible version history.
...
- [x] Prefer destination-runtime direct download when it can reach approved Nightshift Cloud storage.
...
- [x] Run the same contract against a paired headless `nightshift serve` host for global and
...
The connected Nightshift environment `windows 2` was reachable on 2026-08-11 and ra Nightshift `1.4.180` with
runtime ID `68b5e70d-baaf-40a5-b384-be09cc088880`. Validation used the isolated checkout
`C:\Users\neil\nightshift\skills-share-validation` and WSL distro `Ubuntu-24.04`; no production skill
...
`C:\Users\neil\nightshift\workspaces\nightshift\skills-share-staging-validation` on branch
`NightshiftWin/skills-share-staging-validation`. At `44d1266641`, native-Windows, real-process, and real
...
distro filesystem. No standalone Nightshift CLI is installed inside that distro; WSL skill operations
are owned by the connected Windows Nightshift runtime. A reproducible second-distro setup is
`wsl --install -d Debian --no-launch`, followed by first launch and an isolated Nightshift test user/home
...
matching Nightshift's supported Windows placement topology.
...
- [x] Confirm `windows 2` is saved and reachable through `nightshift environment list` and
      `nightshift status --environment "windows 2" --json`.
...
- [x] Discover installed WSL version and distros, each distro's running state, default user, home,
      filesystem, and Nightshift host support.
...
- [x] Record exact Nightshift client/server versions and capabilities with each test run.
...
- [x] Test an unowned junction, external link, broken junction, and provider parent that is already
      linked. All four real NTFS junction cases passed on `windows 2` at `c7c8a2e32d`; Nightshift repairs
...
- [x] Temporary local test installations, workspaces, and packages are removed through verified,
      recoverable cleanup. The final checkout was clean and WSL `/tmp` had no `nightshift-skill-*` debris.

## 11. Implement and validate SSH targets

The 2026-08-11 live Nightshift inventory contained only the connected `windows 2` environment and no
non-local worktree or repository. Later staging journeys registered disposable Ubuntu 20.04 and
native macOS ARM64 hosts directly through Nightshift's SSH provider and removed them afterward. A
...
- [x] Add signed-policy/URL generation and IAM Credentials failure alerts. Grant-signing failures
      ...
      Data Access audit logging; evidence is in Nightshift Cloud `69b388e`.
- [ ] Add budget alerts for GCS storage/egress, Cloud Run growth, and Cloud SQL storage.

Dashboard evidence in Nightshift Cloud `41ef335` and `f67ee2c`: upload-grant, finalize, share, and
...
- [x] Test malicious redirects, DNS/host confusion, expired grants, mismatched generations, and
      ...
      GCS generation pinning are covered in Nightshift `534abb8908` and Nightshift Cloud `81a581d`.
...
- [ ] Test canonical/provider paths as regular files, directories, aliases, junctions, external
      ...
      Nightshift-owned broken-link repair, aliases, and verified copy fallback. Physical Windows
...
- [x] Run Linux against Ubuntu 20.04/glibc 2.31 and verify bundled native binaries respect the
      ...
      `nightshift-node-pty-floor-ok` from an ARM64 Ubuntu 20.04 container on native `aarch64` hardware.
...
- [x] Modify installed bytes, publish an update, and prove Nightshift refuses silent replacement. The
...
- [x] Add deterministic fixtures authored for Nightshift without copying upstream fixtures.
...
- [x] Deploy `nightshift-cloud-api` skill routes disabled by server-side flags.
...
- [x] Deploy the API with all skill flags disabled. Bootstrap revision `nightshift-cloud-api-00016-yem`
...
- [x] After every first-release gate passes, enable the feature for all accounts in one launch.
      ...
      artifact plus skill smoke and promoted `nightshift-cloud-api-00025-qup` at 100% traffic with digest
...
- [ ] Review error budgets, cost, authorization denials, saturation, orphan counts, and support
      ...
      alerts route to the enabled dedicated `Nightshift skill sharing alerts` channel, and aggregate
      monitor execution `nightshift-cloud-skill-storage-monitor-lfd26` passed on the serving API digest
...
## 16. Agent and Nightshift CLI publishing
...
- [x] Add `nightshift skills installed --json` with discovery IDs and names but no local filesystem paths.
- [x] Add `nightshift skills share` with repeated explicit `--skill` selectors, a required bundle name,
