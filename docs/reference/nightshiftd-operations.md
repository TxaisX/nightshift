...
`nightshiftd` is the Nightshift runtime served from plain Node. This is the contract between it and
...
A deployment is **nightshiftd** plus **the terminal daemon**.

|            | nightshiftd                            | terminal daemon                       |
| ---------- | -------------------------------- | ------------------------------------- |
| Started by | the supervisor                   | nightshiftd, detached                       |
| Owns       | RPC, git, worktrees, persistence | every local PTY                       |
| Lifetime   | one supervised run               | detached from nightshiftd, not its service  |
| Endpoint   | `ws://<bind>:<port>`             | `<data-root>/daemon/daemon-v<N>.sock` |

nightshiftd detaches the daemon and calls `disconnectDaemon()`, never `shutdownDaemon()`. The
built-in remote deployment path stops only the recorded nightshiftd PID, so the daemon and its PTYs
...
Process detachment is not service isolation. A daemon forked by nightshiftd, and every PTY it owns,
...
`nightshift serve`'s wide default, and a startup where some device has connected before — and an
...
Under the shipping design a client reaches a remote nightshiftd over an SSH local port-forward, so
loopback is the correct default and the pairing credential travels over SSH.

## Data root and the instance lock

The data root is `$NIGHTSHIFT_USER_DATA`, else `$XDG_DATA_HOME/Nightshift`, else `~/.nightshift`.

Before the profile index or the store is touched, nightshiftd takes `<data-root>/nightshiftd.lock`.
It refuses to start when:

| Code                                   | Meaning                                                       |
| -------------------------------------- | ------------------------------------------------------------- |
| `nightshiftd_data_root_wrong_owner`          | the root is owned by another uid (POSIX)                      |
| `nightshiftd_data_root_shared`               | the root is group/world accessible and could not be tightened |
| `nightshiftd_instance_lock_held`             | another live nightshiftd owns this root                             |
| `nightshiftd_instance_lock_foreign_identity` | the lock belongs to a different identity                      |
| `nightshiftd_data_root_unusable`             | the root cannot be created, stat'd or written                 |

A root that is merely too permissive and that we own is tightened to `0700` rather than
refused — nightshiftd stores credentials there unsealed (no OS keyring on this host), so the goal
...
Before a cgroup-wide stop, obtain a fresh `nightshift-ide terminal list --json` result using the same OS
...
`sudo -Hu nightshift /home/nightshift/.local/bin/nightshift-ide terminal list --json`). Replace both `nightshift` and
`/home/nightshift` with the service account and home used by the unit; an extracted deployment may use
its absolute `resources/bin/nightshift-ide` launcher instead. A safe empty census is untruncated, has an explicit `hostScope`, covers every
...
not admit new work after the census. Nightshift does not yet provide an atomic census-and-stop fence.

### Who supervises nightshiftd

An external supervisor (systemd, launchd, a process manager). nightshiftd conforms to it:

- **Readiness.** One JSON line on stdout (`--json`), `type: "nightshift_server_ready"`, published
...
- **Shutdown.** `SIGTERM` or `SIGINT` starts a graceful stop. A **second** signal exits
  ...
  the one teardown that skips the daemon handoff. nightshiftd also imposes its own 15s deadline
...
- **Logs.** nightshiftd writes human-readable diagnostics to **stderr** and its readiness contract
  ...
  `NIGHTSHIFT_DIAGNOSTICS_DISABLED=1`). Rotation of that file is not implemented — see
  [What is not covered](#what-is-not-covered).

### nightshiftd supervising the daemon

- **Launch.** Forked detached from `daemon-entry.js` beside `nightshiftd.js`, with its own PID
...
- **Crash-loop containment.** At most **5 launches per 60s rolling window** per nightshiftd run;
  ...
  recovers without restarting nightshiftd. An operator-initiated daemon restart clears it — that
  is the deliberate "try again".
- **No macOS login-session watch.** That watch retires the daemon when the spawning GUI login
  session dies. A nightshiftd daemon must survive its SSH session ending.
- **Shutdown.** nightshiftd never stops the daemon. A daemon that was never adopted retires itself
  after its adoption window; an adopted one stays resident (see Decommissioning).

### Decommissioning

After a PID-scoped stop, an adopted daemon stays resident so the next nightshiftd can reattach.
A combined-unit systemd stop kills it instead. To retire a process-scoped deployment, apply
the census rule above, stop nightshiftd, then stop the daemon named by `health.terminalDaemon.pid`.
...
```
buildHash    sha256 (16 hex) of the running nightshiftd bundle — build identity that a version
             string cannot give, so a rollback that did not replace the file is visible
buildVersion NIGHTSHIFT_VERSION
...
terminalDaemon:
  ...
  buildVersion       the build the LIVE daemon was forked from (may legitimately predate
                     this nightshiftd after an update — reporting nightshiftd's version for both would
                     hide exactly that)
...
processes: nightshiftd drives it, the daemon performs it, the verdict crosses the socket.
...
because those terminals die with nightshiftd. A daemon that answered and then failed its spawn
...
- **A continuous health endpoint.** `health` is published once, in the readiness payload. A
  ...
  `collectNightshiftdHealth()`; that surface does not exist yet.
- **Systemd-isolated daemon supervision.** nightshiftd and its daemon currently share one service
