...
Nightshift can put one skill or a bundle of skills behind one unlisted, revocable link. Shared bundles
...
Publishing and link management require a Nightshift account in the desktop app.
...
Nightshift publishes an immutable version. Later changes do not silently alter a link's current bytes;
...
Opening a Nightshift skill link shows a preview before changing any files. You can also open **Skills**,
...
Supported destinations include the local machine, paired Nightshift runtimes, WSL, and SSH hosts. The
...
Nightshift keeps one canonical installed copy and places it where supported agents can discover it.
...
**Keep local** is the default when an existing skill differs. Nightshift replaces modified content only
...
Nightshift updates only the selected skills that still exist in that version.

An interrupted install is recovered on restart. If Nightshift reports a conflict or partial result,
review the named skill and retry; completed skills do not need to be installed again.

## Remove an installed skill

Use **Skills → Manage installs → Remove**. Nightshift removes only copies and provider placements that it
...
executables may run later when a person or agent uses the skill. Nightshift validates the package and
...
Nightshift records bounded operational identifiers and outcomes. Normal logs, telemetry, and support
...
links intentionally show the same response so Nightshift does not disclose private package existence.
