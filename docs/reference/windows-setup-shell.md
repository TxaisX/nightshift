...
On native Windows, Nightshift writes the `nightshift.yaml` setup script (and the issue command) to a generated
...
- Two people on the same repo got different interpreters for the same `nightshift.yaml`, so no project
...
A `#!` line only takes effect when Nightshift can actually launch bash from the configured terminal — the
