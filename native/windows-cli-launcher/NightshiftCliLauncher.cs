...
internal static class NightshiftCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(NightshiftCliLauncher).Assembly.Location);
            ...
            string electronPath = Path.Combine(appDirectory, "Nightshift.exe");
            ...
            {
                Console.Error.WriteLine("Unable to locate Nightshift.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Nightshift CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
            ...
            MoveEnvironmentVariable("NODE_OPTIONS", "NIGHTSHIFT_NODE_OPTIONS");
            MoveEnvironmentVariable("NODE_REPL_EXTERNAL_MODULE", "NIGHTSHIFT_NODE_REPL_EXTERNAL_MODULE");
        ...
        {
            Console.Error.WriteLine("Unable to start the Nightshift CLI: {0}", error.Message);
            return 1;
        }
    }
