...
Use this skill for desktop UI through `nightshift computer`. For a website or web app, use it only when the page is in an external desktop browser window that needs desktop-level control. Do not use it for page-only automation: use `nightshift-cli` for Nightshift's embedded pages and a page-automation tool such as Playwright or CDP for external pages.
...
```text
NIGHTSHIFT computer capabilities --json
```

## Core Loop

```text
NIGHTSHIFT computer list-apps --json
NIGHTSHIFT computer get-app-state --app com.spotify.client --json
NIGHTSHIFT computer click --app com.spotify.client --element-index 42 --json
```
...
```text
NIGHTSHIFT computer get-app-state --app com.microsoft.edgemac --json
NIGHTSHIFT computer get-app-state --app Spotify --json
NIGHTSHIFT computer get-app-state --app pid:12345 --json
```
...
```text
NIGHTSHIFT computer permissions --json
NIGHTSHIFT computer capabilities --json
NIGHTSHIFT computer list-apps --json
NIGHTSHIFT computer list-windows --app <app> --json
NIGHTSHIFT computer get-app-state --app <app> --json
NIGHTSHIFT computer get-app-state --app <app> --restore-window --json
NIGHTSHIFT computer click --app <app> --element-index <index> --json
NIGHTSHIFT computer click --app <app> --x 100 --y 100 --json
NIGHTSHIFT computer click --app <app> --x 100 --y 100 --modifiers CmdOrCtrl+Shift --json
NIGHTSHIFT computer click --app <app> --element-index <index> --mouse-button right --json
NIGHTSHIFT computer click --app <app> --element-index <index> --mouse-button middle --json
NIGHTSHIFT computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
NIGHTSHIFT computer set-value --app <app> --element-index <index> --value "text" --json
NIGHTSHIFT computer type-text --app <app> --text "text" --json
NIGHTSHIFT computer press-key --app <app> --key Return --json
NIGHTSHIFT computer hotkey --app <app> --key CmdOrCtrl+A --json
NIGHTSHIFT computer paste-text --app <app> --text "text" --json
NIGHTSHIFT computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
NIGHTSHIFT computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
NIGHTSHIFT computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | NIGHTSHIFT computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```text
NIGHTSHIFT computer get-app-state --app com.microsoft.edgemac --restore-window --json
NIGHTSHIFT computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
NIGHTSHIFT computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `NIGHTSHIFT computer ... --app Gmail` unchanged because `nightshift computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `NIGHTSHIFT computer permissions --id screenshots --json`.
- `accessibility_error`: run `NIGHTSHIFT computer capabilities --json`; if the message names Accessibility permission, run `NIGHTSHIFT computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `NIGHTSHIFT computer permissions --json`, or `NIGHTSHIFT computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.
