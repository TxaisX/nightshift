...
`NIGHTSHIFT emulator exec --command "<serve-sim command>"`, which forwards the string to serve-sim
...
- An active session for the worktree before any input verb: run `NIGHTSHIFT emulator attach` or
...
Nightshift reports a clear error when the host is missing macOS or the Xcode tools.
...
| ------------------------ | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| List available / running | `NIGHTSHIFT emulator list --json`                                 | Nightshift-managed sessions plus raw serve-sim streams. Use its ids for `--device` / `--emulator`.                                                                          |
| List devices everywhere  | `NIGHTSHIFT emulator devices --json`                              | Every backend's devices with a platform column, booted and shutdown.                                                                                                  |
| Attach / make active     | `NIGHTSHIFT emulator attach "iPhone 16 Pro" --json`               | Starts the helper if needed and makes the device active for the worktree. `--focus` switches the UI; it does not by default.                                          |
| Single tap               | `NIGHTSHIFT emulator tap <x> <y> --json`                          | Normalized 0..1 coordinates.                                                                                                                                          |
| Multi-step gesture       | `NIGHTSHIFT emulator gesture '<json>' --json`                     | Begin/move/end points. Use `tap` for a single tap.                                                                                                                    |
| Type text                | `NIGHTSHIFT emulator type "text" --json`                          | US-ASCII only.                                                                                                                                                        |
| Hardware button          | `NIGHTSHIFT emulator button home --json`                          | `home` and `side_button` are documented by the CLI spec; other names such as `swipe_home`, `app_switcher`, `lock`, and `siri` are forwarded to serve-sim unvalidated. |
| Rotate device            | `NIGHTSHIFT emulator rotate landscape_left --json`                | The orientation persists for subsequent gestures.                                                                                                                     |
| Accessibility tree       | `NIGHTSHIFT emulator ax --json`                                   | serve-sim node tree, capped at 500 nodes, frames normalized 0..1 with a top-left origin. Needs an active session.                                                     |
| Raw passthrough          | `NIGHTSHIFT emulator exec --command "ca-debug blended on" --json` | serve-sim subcommand string, without a `serve-sim` prefix.                                                                                                            |
| Stop the helper          | `NIGHTSHIFT emulator kill --json`                                 | Leaves the device booted.                                                                                                                                             |
| Stop and power off       | `NIGHTSHIFT emulator shutdown --json`                             | Stops the helper and shuts the simulator device down.                                                                                                                 |
...
NIGHTSHIFT emulator tap 0.5 0.8 --json
NIGHTSHIFT emulator type "user@example.com" --json
NIGHTSHIFT emulator button home --json
NIGHTSHIFT emulator ax --json
NIGHTSHIFT emulator exec --command "ca-debug blended on" --json
NIGHTSHIFT emulator kill --device "iPhone 16 Pro" --json
```

See also: `nightshift-emulator-android` for Android devices, `nightshift-cli` for terminals, worktrees,
