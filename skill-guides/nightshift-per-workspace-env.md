...
Inside the lifecycle scripts the placeholder does not apply: `nightshift serve` written there runs on
the remote machine's own binary.

## Autonomy envelope

Without asking again you may read the repo and its `nightshift.yaml`, detect provider CLIs and their
login state, scaffold and edit files under `scripts/nightshift-vm/`, and run `NIGHTSHIFT vm recipe doctor`
...
done. Never create a Nightshift workspace except for the step-10 test the user asked for. Do not create
...
In **Nightshift-server** mode `create` runs `nightshift serve` in the environment and emits a `pairingCode`. In
**SSH** mode `create` runs no server and emits a `connection.type:"ssh"` block Nightshift dials into.
Settle this first; it changes the `create` output and half the templates.

Keep Nightshift's checkout behavior unchanged by default: omit `checkoutMode`, emit schema version 1, and
let Nightshift create a linked worktree. Use `checkoutMode: provisioned-root` only when the user
...
1. **Inspect the repo** for an existing `environmentRecipes` entry, `scripts/nightshift-vm/`, a state
...
2. **Interview the user up front.** Gather these choices and confirm them back before scaffolding
   ...
   - **Connection mode:** a Nightshift server or SSH, as above. Settle it first.
   ...
   - **Provider:** Vercel Sandbox, Fly, Modal, an existing SSH host, and so on. For a non-obvious
     ...
     Nightshift's SSH mode needs the former.
...
7. **Wire the recipe** so `nightshift.yaml` points create, suspend, resume, and destroy at the scripts.
   ...
   any branch; the picker needs `nightshift.yaml` on the primary branch.
...
- Trap errors and remove the half-built environment, so a crash does not leave a paid resource
  running.
- **Never snapshot a machine on which the Nightshift runtime has already run.** The first `nightshift serve`
  ...
  (`nightshift-devices.json`, `nightshift-e2ee-keypair.json`), `agent-session-authority.key`, and the build
  ...
  dedicated to Nightshift runtime data, never `/`, the home directory, or an ancestor of home. Refuse
  empty or relative paths. Remove only that verified directory, not an unchecked environment value.
  That matches Nightshift's Linux precedence for custom and default paths; deleting a named file list
  drifts as Nightshift adds state.
...
Section 3's rule still applies: if you ran `nightshift serve` on this machine to smoke-test it, delete
...
A repo-local JSON file such as `scripts/nightshift-vm/<provider>-state.json` threads non-secret values
...
Scaffold under `scripts/nightshift-vm/`. These are shapes; fill in the provider's real commands. **Every
...
You run this by hand, not via `nightshift.yaml`, after exporting the first-run inputs state does not have
...
# name = nightshift-${NIGHTSHIFT_RECIPE_ID}-${NIGHTSHIFT_VM_INSTANCE_ID} (sanitized, length-capped)
...
# 3. Nightshift-server mode only: remote exec starting nightshift serve and reading the recipe JSON it writes
...
#!/usr/bin/env bash
set -euo pipefail
payload="$(cat)"                       # Nightshift passes lifecycle JSON on stdin
...
# destroy: provider remove "$resource_id"   (or set destroy: none in nightshift.yaml)
```
...
Define recipes in `nightshift.yaml`:

```yaml
environmentRecipes:
  - id: cloud-sandbox
    name: Cloud Sandbox
    create: ./scripts/nightshift-vm/cloud-sandbox-create.sh
    suspend: ./scripts/nightshift-vm/cloud-sandbox-suspend.sh
    resume: ./scripts/nightshift-vm/cloud-sandbox-resume.sh
    destroy: ./scripts/nightshift-vm/cloud-sandbox-destroy.sh
```
...
The base result, which is what Nightshift-server mode prints:

```json
{
  "schemaVersion": 1,
  "pairingCode": "nightshift-pairing-code-or-url",
...
- **`nightshift serve --recipe-json` output** is this same object without `userData`. Merge your own
...
- **SSH mode** replaces `pairingCode` and `projectRoot` with a `connection` block whose `type` is
  `"ssh"`, and does not run `nightshift serve`. The exact target shape is in `references/ssh-host.md`.
- **Provisioned root** applies only to direct SSH and only when the user explicitly asked for it. Add
  `checkoutMode: provisioned-root` to the recipe, require `NIGHTSHIFT_RECIPE_RESULT_SCHEMA_VERSION=2`, and
...
### The `nightshift serve` invocation

Inside the environment, in Nightshift-server mode, run exactly this. These flags are verified; do not
improvise them.
...
In an environment built from source, run it as `pnpm exec nightshift-dev serve …` from the repo root;
`nightshift-dev` is the in-repo entrypoint. Plain `nightshift serve …` is the same command when the built CLI is
...
`NIGHTSHIFT vm recipe doctor <recipe-id> --repo-path <repo> --json` validates static wiring only; it boots
...
run `NIGHTSHIFT skills get nightshift-per-workspace-env --reference references/<file>.md` and read only that
document; `--references` lists the names. Read the reference at the gate, not before. If the CLI
rejects `--reference`, run `NIGHTSHIFT skills get nightshift-per-workspace-env --full` once instead: it returns
...
| The recipe connects over SSH instead of starting `nightshift serve`, including provisioned root | `references/ssh-host.md`        |
