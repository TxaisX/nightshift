...
- When committing from an interactive shell, force the runtime entrypoint back to `sshd`:
  `docker commit --change='ENTRYPOINT ["/usr/local/bin/nightshift-docker-ssh-entrypoint"]' …`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "NIGHTSHIFT_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
