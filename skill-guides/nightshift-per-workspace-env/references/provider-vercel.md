...
This is the Nightshift-server connection mode: the recipe emits a pairing URL. If the user chose SSH in
...
verdict="$(vercel sandbox exec "$auth" "${vercel_args[@]}" --timeout 30s \
  -- bash -lc 'if codex login status >/dev/null 2>&1; then echo NIGHTSHIFT_AGENT_LOGGED_IN; else echo NIGHTSHIFT_AGENT_LOGGED_OUT; fi')"
case "$verdict" in
  *NIGHTSHIFT_AGENT_LOGGED_IN*) ;;
...
recipe_id="${NIGHTSHIFT_RECIPE_ID:-vercel-sandbox}"
recipe_id="${recipe_id//./-}"  # Vercel names forbid dots.
instance_id="${NIGHTSHIFT_VM_INSTANCE_ID:-$(date +%s)}"
max_recipe_id_length=$((128 - ${#instance_id} - 6))  # Preserve the unique instance suffix.
[ "$max_recipe_id_length" -gt 0 ] || { echo "NIGHTSHIFT_VM_INSTANCE_ID is too long for a Vercel sandbox name" >&2; exit 1; }
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "NIGHTSHIFT_PROJECT_ROOT=$project_root" \
  --env "NIGHTSHIFT_REPO_URL=$repo_url" --env "NIGHTSHIFT_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$NIGHTSHIFT_PROJECT_ROOT"; \
    ...
    git fetch origin "$NIGHTSHIFT_REPO_REF"; \
    git checkout -B "$NIGHTSHIFT_REPO_REF" FETCH_HEAD; \
    rm -f /tmp/askpass.sh; \
    c="$(git rev-parse HEAD)"; [ -f .nightshift-built ] && [ "$(cat .nightshift-built)" = "$c" ] || { \
      ...
      printf "%s" "$c" > .nightshift-built; }' >&2

# 3. (remote) start nightshift serve in the background, writing recipe JSON to a file; poll until it parses
recipe_json="$(vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 60s \
  --env "NIGHTSHIFT_PORT=$port" --env "NIGHTSHIFT_PROJECT_ROOT=$project_root" --env "NIGHTSHIFT_PAIRING_ADDRESS=$pairing_ws" \
  -- bash -lc 'set -euo pipefail; cd "$NIGHTSHIFT_PROJECT_ROOT"; rm -f /tmp/nightshift-recipe.json /tmp/nightshift-serve.log; \
    nohup pnpm exec nightshift-dev serve --port "$NIGHTSHIFT_PORT" --project-root "$NIGHTSHIFT_PROJECT_ROOT" \
      --pairing-address "$NIGHTSHIFT_PAIRING_ADDRESS" --recipe-json >/tmp/nightshift-recipe.json 2>/tmp/nightshift-serve.log </dev/null & \
    pid=$!; for _ in $(seq 1 80); do \
      node -e "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/nightshift-recipe.json\",\"utf8\"))" >/dev/null 2>&1 && { cat /tmp/nightshift-recipe.json; exit 0; }; \
      kill -0 "$pid" 2>/dev/null || { cat /tmp/nightshift-serve.log >&2; exit 1; }; sleep 0.25; \
    done; cat /tmp/nightshift-serve.log >&2; echo "serve recipe JSON timed out" >&2; exit 1')"
