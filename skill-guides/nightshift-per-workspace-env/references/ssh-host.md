...
Load this when the recipe connects over SSH instead of starting `nightshift serve`, and when the user has
explicitly asked for `checkoutMode: provisioned-root`.

SSH mode is a different shape, not the Nightshift-server templates relabeled. `create` runs no
`nightshift serve` and emits no `pairingCode`. Nightshift connects over its SSH relay, brings up the git and
filesystem providers, and imports the repo. The script only readies the host and prints the SSH
details Nightshift dials.

## The result shape

Nightshift rejects anything else. Required fields only; add optionals from the next section as the
network needs them.
...
These describe how the user's desktop reaches the box; there is no `nightshift serve` URL in SSH mode.
...
- `relayGracePeriodSeconds` bounds how long Nightshift keeps the SSH relay alive after the workspace
...
# 1. ensure the repo is present and at the right commit on the host (NO nightshift serve here).
...
# 2. print the SSH connection block (NO pairingCode, NO nightshift serve). host/port/username tell Nightshift's
...
and resume. Nightshift still disconnects and reconnects its own SSH relay on sleep, wake, and delete, which
...
`NIGHTSHIFT_RECIPE_RESULT_SCHEMA_VERSION`, `NIGHTSHIFT_REPO_URL`, `NIGHTSHIFT_REPO_REF`, `NIGHTSHIFT_REPO_REF_HEAD`, and
`NIGHTSHIFT_REPO_BRANCH`. Use `NIGHTSHIFT_REPO_REF` to fetch the selected source, but create `NIGHTSHIFT_REPO_BRANCH`
at the exact `NIGHTSHIFT_REPO_REF_HEAD` commit, because resolving the symbolic ref again can race with an
upstream update. `NIGHTSHIFT_REPO_URL` and `NIGHTSHIFT_REPO_REF` are a matched fetch pair, and the URL is the
remote Nightshift resolved the base ref against, which is not necessarily named `origin` on the desktop.
Fetch from the URL the pair supplies:

```bash
[ -n "${NIGHTSHIFT_REPO_REF_HEAD:-}" ] || { echo "missing pinned source commit" >&2; exit 1; }
git fetch "$NIGHTSHIFT_REPO_URL" "$NIGHTSHIFT_REPO_REF"
git cat-file -e "${NIGHTSHIFT_REPO_REF_HEAD}^{commit}"
git checkout -B "$NIGHTSHIFT_REPO_BRANCH" "$NIGHTSHIFT_REPO_REF_HEAD"
```
