...
`.sh` will not execute there. Either require WSL or Git Bash and point `nightshift.yaml` at a launcher such
as `bash ./scripts/nightshift-vm/<name>.sh` through a `.cmd` file, or scaffold PowerShell equivalents.
...
# Nightshift-server mode: @{ schemaVersion=1; pairingCode=$pairingCode; projectRoot=$projectRoot; userData=@{...} }
