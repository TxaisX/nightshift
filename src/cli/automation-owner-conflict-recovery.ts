...
const RECOVERY: Record<string, readonly string[]> = {
  ...
  [AUTOMATION_OWNER_CONFLICT_CODES.targetRemoved]: [
    ...
    'Re-add that SSH host, or delete the automation with `nightshift automations remove --id <id>`.'
  ],
  [AUTOMATION_OWNER_CONFLICT_CODES.fencingRequired]: [
    ...
    'Update Nightshift on the host, then run the command again.'
  ],
  [AUTOMATION_OWNER_CONFLICT_CODES.invalidDestination]: [
    ...
    'Run `nightshift automations show --id <id>` to see where it lives, then target a host that authority knows.'
  ]
}
