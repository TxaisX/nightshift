...
describe('orchestration gate commands carry caller identity', () => {
  ...
  afterEach(() => {
    ...
    restoreEnv('NIGHTSHIFT_PANE_KEY', originalPaneKey)
    process.exitCode = 0
  })
  ...
  it('sends the bound coordinator handle to gateCreate', async () => {
    process.env.NIGHTSHIFT_TERMINAL_HANDLE = 'term_coord'
    queueFixtures(
      callMock,
  ...
  it('remints a stale environment handle before authorizing gateCreate', async () => {
    process.env.NIGHTSHIFT_TERMINAL_HANDLE = 'term_stale'
  ...
  it('reports idempotent recovery when a mutation connection drops', async () => {
    ...
    callMock
      ...
      .mockRejectedValueOnce(
        new RuntimeClientError(
          'runtime_unavailable',
          'The Nightshift runtime closed the connection before responding. Restart Nightshift and try again. Orchestration mutation request ID: mutation_1.',
          {
    ...
    expect(output.error.message).not.toMatch(/restart Nightshift/i)
