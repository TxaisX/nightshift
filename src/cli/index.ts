...
// Why: the SSH relay bridge executes this CLI on the Nightshift host while the
// caller's shell cwd lives on the remote machine (which cannot be chdir'd
// into). NIGHTSHIFT_CLI_CWD carries that remote cwd so cwd-based selectors like
// `--worktree active` resolve against the caller's directory.
function resolveInvocationCwd(): string {
  const override = process.env.NIGHTSHIFT_CLI_CWD
...
export async function main(
  ...
  if (argv.length === 1 && (argv[0] === '--version' || argv[0] === '-v')) {
    const version = readNightshiftCliVersion()
    if (!version) {
      process.stderr.write('Could not determine the Nightshift version for this build.\n')
      process.exitCode = 1
      return
    }
  ...
  try {
    ...
    // lookup so users do not get misleading "Nightshift is not running" failures for
    ...
    // Why: only the explicit flag is asserted eagerly. An ambient NIGHTSHIFT_ENVIRONMENT is background
    ...
    // An ambient NIGHTSHIFT_ENVIRONMENT is checked for disagreement too — silently
      ...
      : await resolveHostFlagEnvironmentId(parsed.flags, {
          ...
          environmentSelector:
            typeof environmentSelector === 'string'
              ...
              : process.env.NIGHTSHIFT_ENVIRONMENT
                ? { value: process.env.NIGHTSHIFT_ENVIRONMENT, label: 'NIGHTSHIFT_ENVIRONMENT' }
                : null
        })
    ...
    // NIGHTSHIFT_PAIRING_CODE / NIGHTSHIFT_ENVIRONMENT env-var fallback for commands
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `nightshift claude-teams` belongs to Claude Code, not
    // Nightshift's own flag parser, so new Claude flags work without Nightshift changes.
...
async function runAgentTeamsTmuxShim(argv: string[]): Promise<void> {
  try {
    ...
    const response = await client.call<{
      ...
      {
        teamId: process.env.NIGHTSHIFT_AGENT_TEAMS_TEAM_ID,
        token: process.env.NIGHTSHIFT_AGENT_TEAMS_TOKEN,
