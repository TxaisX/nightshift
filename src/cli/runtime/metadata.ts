  ...
  try {
    ...
    if (!metadata || !findTransport(metadata, 'unix', 'named-pipe') || !metadata.authToken) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        `Nightshift runtime metadata is incomplete at ${metadataPath}`
      )
    }
    return metadata
  } catch (error) {
    ...
    throw new RuntimeClientError(
      'runtime_unavailable',
      `Could not read Nightshift runtime metadata at ${metadataPath}. Start the Nightshift app first.`
    )
  }
}
...
export function getDefaultUserDataPath(
  ...
  // Why: in dev mode (and for parallel Nightshift instances), the Electron app writes
  // runtime metadata to a separate userData directory (e.g. `nightshift-dev`) to avoid
  ...
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Nightshift runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'nightshift')
  }
