...
describe('orchestration recovery command identity', () => {
  it.each([
    ['configured dev', { NIGHTSHIFT_CLI_COMMAND: 'nightshift-dev' }, 'darwin', 'nightshift-dev'],
    ['configured WSL', { NIGHTSHIFT_CLI_COMMAND: 'nightshift-ide' }, 'linux', 'nightshift-ide'],
    ['dev checkout', { NIGHTSHIFT_DEV_REPO_ROOT: '/repo' }, 'darwin', 'nightshift-dev'],
    ['packaged Linux', {}, 'linux', 'nightshift-ide'],
    ['local macOS', {}, 'darwin', 'nightshift'],
    ['local Windows', {}, 'win32', 'nightshift']
  ...
  it.each([
    ...
    expect(buildOrchestrationRecoveryCommand(method, {}, 'nightshift', args)).toEqual([
      'nightshift',
      ...args
    ])
  })
  ...
  it.each([
    ...
    expect(
      buildOrchestrationRecoveryCommand('orchestration.send', {}, 'nightshift', args)
    ).toBeUndefined()
  })

  it('supports the explicit executable-first form', () => {
    expect(
      buildOrchestrationRecoveryCommand('nightshift-ide', 'orchestration.workerStop', {
        dispatch: 'dispatch_1'
      })
    ).toEqual(['nightshift-ide', 'orchestration', 'worker-stop', '--dispatch', 'dispatch_1'])
  })
