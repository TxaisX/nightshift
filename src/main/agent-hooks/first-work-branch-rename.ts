...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for Nightshift-created worktrees whose branch Nightshift is allowed to rename. */
  canRenameNightshiftCreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameNightshiftCreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch Nightshift auto-named — never one the user chose.
