...
async function createTempHome(): Promise<string> {
  const home = await mkdtemp(join(tempRoot, 'nightshift-managed-hook-runtime-'))
  tempHomes.push(home)
  return home
}
...
describe.runIf(process.platform !== 'win32')('resolveRelayGrokHome', () => {
  it('uses the login-shell GROK_HOME and normalizes trailing separators', async () => {
    ...
    await expect(resolveRelayGrokHome('/home/nightshift')).resolves.toBe('/srv/grok')
  ...
  it('falls back when the login-shell GROK_HOME is not an absolute POSIX path', async () => {
    ...
    await expect(resolveRelayGrokHome('/home/nightshift')).resolves.toBe('/home/nightshift/.grok')
  })
...
describe.runIf(process.platform !== 'win32')('installManagedHooks', () => {
  it.each([
    ...
    async (_label, options) => {
      ...
      // Why: no agent config home, no ~/.nightshift install lock, and no GROK_HOME login-shell probe.
