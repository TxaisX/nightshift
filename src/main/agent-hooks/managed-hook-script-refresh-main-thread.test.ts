...
vi.mock('electron', () => ({ app: { getPath: () => '/tmp/nightshift-user-data' } }))
...
describe('managed hook script refresh stays off the main thread', () => {
  beforeEach(async () => {
    state.home = await mkdtemp(join(tmpdir(), 'nightshift-hook-refresh-main-thread-'))
    state.syncCalls = []
  ...
  it('uses no synchronous HOME filesystem calls for missing or stale scripts', async () => {
    const hooksDir = join(state.home, '.nightshift', 'agent-hooks')
