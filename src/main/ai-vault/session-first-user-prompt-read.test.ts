...
describe('readAiVaultFirstUserPrompt', () => {
  it('returns the full first user prompt without preview truncation', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-first-prompt-'))
    tempRoots.push(root)
  ...
  it('extracts full Codex input_text content blocks (not preview-capped)', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-first-prompt-codex-'))
    tempRoots.push(root)
  ...
  it('skips meta/harness user turns and returns the first real ask', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-first-prompt-meta-'))
    tempRoots.push(root)
    const projectDir = join(root, 'project')
    await mkdir(projectDir, { recursive: true })
    const filePath = join(projectDir, 'session.jsonl')
  ...
  it('resolves null instead of rejecting when the transcript is corrupt', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-first-prompt-corrupt-'))
    tempRoots.push(root)
  ...
  it('returns null for a non-local execution host', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-first-prompt-remote-'))
    tempRoots.push(root)
