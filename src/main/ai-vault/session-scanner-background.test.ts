...
const originalBackend = process.env.NIGHTSHIFT_AI_VAULT_SERVICE_PROCESS
const originalNodeEnv = process.env.NODE_ENV

afterEach(() => {
  if (originalBackend === undefined) {
    delete process.env.NIGHTSHIFT_AI_VAULT_SERVICE_PROCESS
  } else {
    process.env.NIGHTSHIFT_AI_VAULT_SERVICE_PROCESS = originalBackend
  }
...
describe('shouldUseAiVaultServiceProcess', () => {
  ...
  it.each([
    ...
    process.env.NIGHTSHIFT_AI_VAULT_SERVICE_PROCESS = value
