...
describe('scanAiVaultSessions', () => {
  it('indexes Claude and Codex transcripts with resume commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-'))
    tempRoots.push(root)
  ...
  it('indexes Codex sessions from Nightshift runtime homes with resumable commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-codex-runtime-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
      ...
      jsonLines([
        {
          ...
          payload: {
            ...
            cwd: '/Users/nwparker/nightshift/workspaces/nightshift/mem4'
          }
        },
        {
    ...
    expect(result.sessions[0]).toMatchObject({
      ...
      resumeCommand: `cd '/Users/nwparker/nightshift/workspaces/nightshift/mem4' && CODEX_HOME='${runtimeHome}' codex resume '019e9693-64fc-7370-9c18-7e625c595d0f'`
    })
  })

  it('indexes WSL home session roots', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-wsl-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    const wslHome = join(root, 'wsl', 'Ubuntu', 'home', 'ada')
    await mkdir(join(wslHome, '.claude', 'projects', 'repo'), { recursive: true })
    await mkdir(
      join(wslHome, '.local', 'share', 'nightshift', 'codex-runtime-home', 'home', 'sessions'),
      {
        recursive: true
      }
    )
    ...
    expect(result.sessions.find((session) => session.agent === 'codex')?.codexHome).toBe(
      join(wslHome, '.local', 'share', 'nightshift', 'codex-runtime-home', 'home')
    )
  })

  it('skips hidden Codex context blocks when choosing session titles', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-codex-hidden-context-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('indexes every supported agent transcript format with native resume commands', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-all-agents-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('captures an in-progress OMP model from model_change before any assistant reply', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-omp-mc-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
  ...
  it('strips newline-heavy Grok user_query envelopes without regex matching', async () => {
    ...
    const root = await mkdtemp(join(tmpdir(), 'nightshift-ai-vault-grok-large-'))
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
