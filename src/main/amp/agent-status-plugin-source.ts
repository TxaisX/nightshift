export const AMP_PLUGIN_FILE = 'nightshift-agent-status.ts'
export const AMP_PLUGIN_MARKER = 'Managed by Nightshift. Do not edit; changes may be overwritten.'
...
export function getAmpPluginSource(): string {
  return [
    ...
    '  const endpointPath = process.env.NIGHTSHIFT_AGENT_HOOK_ENDPOINT',
    ...
    "      if (match[1] === 'NIGHTSHIFT_AGENT_HOOK_PORT') out.port = value",
    "      if (match[1] === 'NIGHTSHIFT_AGENT_HOOK_TOKEN') out.token = value",
    "      if (match[1] === 'NIGHTSHIFT_AGENT_HOOK_ENV') out.env = value",
    "      if (match[1] === 'NIGHTSHIFT_AGENT_HOOK_VERSION') out.version = value",
    '    }',
    ...
    "      console.warn('[nightshift-hook] failed to parse Amp endpoint file:', (error as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '  // Why: Amp sessions can outlive a Nightshift restart; the endpoint file is',
    '  // rewritten on each start, so read it per event before falling back to env.',
    '  const fileEnv = readEndpointFile() ?? {}',
    '  return {',
    '    port: fileEnv.port || process.env.NIGHTSHIFT_AGENT_HOOK_PORT,',
    '    token: fileEnv.token || process.env.NIGHTSHIFT_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.env || process.env.NIGHTSHIFT_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.version || process.env.NIGHTSHIFT_AGENT_HOOK_VERSION || ""',
    '  }',
    '}',
    '',
    ...
    '    })',
    '  } catch {',
    '    // Why: Nightshift status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when Nightshift is down and each POST waits for timeout.',
