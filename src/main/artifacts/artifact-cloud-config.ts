...
const PRODUCTION_ARTIFACTS_API_URL = 'https://share.nightshift.invalid'
...
export function resolveArtifactCloudApiUrl(
  ...
  const candidate = override?.trim() || env.NIGHTSHIFT_ARTIFACTS_API_URL?.trim()
  const url = new URL(candidate || PRODUCTION_ARTIFACTS_API_URL)
  const loopback = ['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname)
  const firstParty = url.hostname === 'nightshift.invalid' || url.hostname.endsWith('.nightshift.invalid')
  ...
  if (!firstParty && !loopback) {
    throw new Error('Artifact API URLs must use a nightshift.invalid or loopback host.')
  }
