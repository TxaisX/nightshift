...
describe('Azure DevOps API request (STA-3494)', () => {
  beforeEach(() => {
    process.env = { ...OLD_ENV, NIGHTSHIFT_AZURE_DEVOPS_TOKEN: 'pat-token' }
    delete process.env.NIGHTSHIFT_AZURE_DEVOPS_API_BASE_URL
    _resetAzureDevOpsPreviewApiVersionCache()
  })
  ...
  it('uses the remote-derived project base for Git endpoints when the configured base shares its origin', async () => {
    process.env.NIGHTSHIFT_AZURE_DEVOPS_API_BASE_URL = SERVER_BASE
    const paths: string[] = []
    globalThis.fetch = vi.fn(async (input: string | URL | Request) => {
  ...
  it('keeps a cross-origin configured base URL as an override for Git endpoints', async () => {
    process.env.NIGHTSHIFT_AZURE_DEVOPS_API_BASE_URL = 'http://127.0.0.1:8123/acme/Project'
  ...
  it('keeps a same-origin non-ancestor base URL as a Git endpoint override', async () => {
    process.env.NIGHTSHIFT_AZURE_DEVOPS_API_BASE_URL =
      'https://ado.example.com:8443/rewrite/MyProject'
    const paths: string[] = []
    globalThis.fetch = vi.fn(async (input: string | URL | Request) => {
      paths.push(new URL(String(input)).pathname)
      return Response.json({ id: 'repo-guid' })
