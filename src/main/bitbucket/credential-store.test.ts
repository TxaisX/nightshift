...
beforeEach(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'nightshift-bitbucket-store-'))
...
describe('Bitbucket credential store', () => {
  ...
  it('writes both credential files 0600', async () => {
    ...
    for (const file of ['bitbucket-credential.enc', 'bitbucket-credential.json']) {
      expect(statSync(join(tempHome, '.nightshift', file)).mode & 0o777).toBe(0o600)
    }
  })
  ...
  it('re-tightens permissions when overwriting an existing credential', async () => {
    ...
    for (const file of ['bitbucket-credential.enc', 'bitbucket-credential.json']) {
      chmodSync(join(tempHome, '.nightshift', file), 0o644)
    }
    save('second')
  ...
  it('rejects non-string fields from hand-edited metadata and secret files', async () => {
    ...
    writeFileSync(
      join(tempHome, '.nightshift', 'bitbucket-credential.json'),
      JSON.stringify({ version: 1, authMode: 'basic', email: { evil: true }, account: 42 })
    )
    writeFileSync(
      join(tempHome, '.nightshift', 'bitbucket-credential.enc'),
  ...
  it('keeps the previous credential intact when the secret write fails (STA-3941)', async () => {
    ...
    const before = readFileSync(join(tempHome, '.nightshift', 'bitbucket-credential.enc'))
    ...
    expect(readFileSync(join(tempHome, '.nightshift', 'bitbucket-credential.enc'))).toEqual(before)
    expect(existsSync(join(tempHome, '.nightshift', 'bitbucket-credential.enc.tmp'))).toBe(false)
  })
  ...
  it('clears both files and in-memory state on disconnect', async () => {
    ...
    expect(existsSync(join(tempHome, '.nightshift', 'bitbucket-credential.enc'))).toBe(false)
    expect(existsSync(join(tempHome, '.nightshift', 'bitbucket-credential.json'))).toBe(false)
  })
  ...
  it('surfaces a non-ENOENT delete failure instead of silently keeping the files', async () => {
    ...
    expect(existsSync(join(tempHome, '.nightshift', 'bitbucket-credential.enc'))).toBe(true)
  })
