...
export function storedAuthConfig(
  ...
  return {
    // Why: an explicit NIGHTSHIFT_BITBUCKET_API_BASE_URL still wins even when the
    // credential itself is stored — env precedence is per-setting, not all-or-nothing.
    baseUrl: envValue('NIGHTSHIFT_BITBUCKET_API_BASE_URL') ?? baseUrl ?? DEFAULT_API_BASE_URL,
