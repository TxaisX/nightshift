...
describe('AgentBrowserBridge', () => {
  ...
  function ownSocketDirectory(): void {
    Object.assign(bridge, {
      ...
      agentBrowserEnv: { AGENT_BROWSER_SOCKET_DIR: '/tmp/nightshift-ab-test' }
    })
  }
  ...
  it('snapshots a fresh owned session without launching a helper just to close it', async () => {
    ...
    expect(lstatSyncMock).toHaveBeenCalledWith('/tmp/nightshift-ab-test/nightshift-tab-tab-1.sock')
  })
  ...
  it('fails closed when stale agent-browser session ownership cannot be reset', async () => {
    ...
    try {
      ...
      const rejection = expect(promise).rejects.toMatchObject({
        ...
        message:
          'Could not reset stale helper session nightshift-tab-tab-1; retry after agent-browser exits'
      })
  ...
  it('uses the cleanup timeout when a target swap retires its session', async () => {
    ...
    ).restartSessionForTarget('nightshift-tab-tab-1', 'tab-1', 100, { recreate: false })
  ...
  it('waits for pending session destruction before recreating the same session', async () => {
    ...
    ).destroySession('nightshift-tab-tab-1')
  ...
  it('tears down a session that finishes creating after destruction starts', async () => {
    ...
    ).ensureSession('nightshift-tab-tab-1', 'tab-1', 100)
  ...
  it('replays intercept routes after the daemon idles out', async () => {
    ...
    const sessions = (bridge as unknown as { sessions: Map<string, { lastCommandAt: number }> })
      .sessions
    const session = sessions.get('nightshift-tab-tab-1')!
  ...
  it('bounds concurrent helper retirements during runtime shutdown', async () => {
    ...
    for (let index = 0; index < 6; index++) {
      sessions.set(`nightshift-tab-tab-${index}`, {
  ...
  it('does not recreate a session after shutdown observes its pending retirement', async () => {
    ...
    ).restartSessionForTarget('nightshift-tab-tab-1', 'tab-1', 100)
