...
describe('agent-browser process environment', () => {
  it('bounds Unix socket paths independently of a long profile path', () => {
    ...
    expect(socketDirectory).toMatch(/^\/tmp\/nightshift-ab-[0-9a-f]{16}$/)
    expect(
      `${socketDirectory}/nightshift-tab-00000000-0000-4000-8000-000000000000.sock`.length
    ).toBeLessThan(104)
  })

  // Why ownsSocketDirectory is false for both: a directory Nightshift did not derive can be shared with
  // another Nightshift profile, so `session list` under it is no proof of ownership.
  it('preserves explicit overrides and leaves Windows socket routing unchanged', () => {
    ...
    const windows = createAgentBrowserProcessEnvironment({
      ...
      platform: 'win32',
      userDataPath: 'C:\\Users\\Nightshift'
    })
  ...
  // Why: the only daemon bound that survives a SIGKILL'd Nightshift, so it must be set on every platform.
