...
export function applyPendingBrowserCookieImports({
  resolveMetadataPath,
  defaultPartition,
  activeNightshiftProfileId
}: PendingCookieImportTarget & { activeNightshiftProfileId: string }): void {
  try {
    ...
    for (const profile of meta.profiles) {
      if (isValidPersistedBrowserSessionProfile(profile, activeNightshiftProfileId)) {
