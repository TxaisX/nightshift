...
export class CdpTargetDiscovery {
  ...
  handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (url === '/json/version' || url === '/json/version/') {
      ...
      // "Nightshift/CdpWsProxy" leaks that this is an embedded automation surface, which
  ...
  handleCommand(
    ...
    if (msg.method === 'Browser.getVersion') {
      // Why: returning "Nightshift/Electron" identifies this as an embedded automation
  ...
  private buildTargetInfo(): Record<string, unknown> {
    ...
    return {
      targetId: 'nightshift-proxy-target',
      type: 'page',
