...
const CLAUDE_SERVICE_TEST_ROOT = join(tmpdir(), 'nightshift-claude-service-add-test')
...
describe('ClaudeAccountService credential capture', () => {
  ...
  it('rejects adding a Claude account whose identity already exists', async () => {
    ...
    const existingMarkerPath = join(existingAuthPath, '.nightshift-managed-claude-auth')
  ...
  it('adds a Claude account with the same email under a different organization', async () => {
    ...
    mkdirSync(existingAuthPath, { recursive: true })
    writeFileSync(
      join(existingAuthPath, '.nightshift-managed-claude-auth'),
      'existing-account\n',
      'utf-8'
    )
    let settings = {
