...
export class ClaudeManagedAuthStorage {
  async create(
    ...
    writeFileSync(join(managedAuthPath, '.nightshift-managed-claude-auth'), `${accountId}\n`, {
      encoding: 'utf-8',
      mode: 0o600
    })
    return {
  ...
  async assertOwned(candidatePath: string, expectedAccountId?: string): Promise<string> {
    ...
    if (!trustedPath) {
      throw new Error('Managed Claude auth storage is not owned by Nightshift.')
    }
    return trustedPath
  }
  ...
  private async tryCreateWsl(
    ...
    const linuxPath = `${home.replace(/\/$/, '')}/.local/share/nightshift/claude-accounts/${accountId}/auth`
    const created = await runWslProcess({
      distro,
      loginPath: 'none',
      shell: 'bash',
      script:
        'umask 077; mkdir -p "$1" && printf \'%s\\n\' "$2" > "$1/.nightshift-managed-claude-auth"',
  ...
  private async assertOwnedWsl(
    ...
    if (
      !wslInfo.linuxPath.includes('/.local/share/nightshift/claude-accounts/') ||
      !wslInfo.linuxPath.endsWith('/auth')
    ) {
      throw new Error('Managed WSL Claude auth storage is outside Nightshift account storage.')
    }
    if (process.platform !== 'win32') {
      if (
        !existsSync(candidatePath) ||
        !existsSync(join(candidatePath, '.nightshift-managed-claude-auth'))
      ) {
        throw new Error('Managed Claude auth storage is not owned by Nightshift.')
      }
    ...
    try {
      const expected = expectedAccountId
        ? `test "$(cat "$candidate_real/.nightshift-managed-claude-auth")" = ${shellQuote(expectedAccountId)}`
        : 'test -n "$(cat "$candidate_real/.nightshift-managed-claude-auth")"'
      const owned = await runWslProcess({
        ...
        script: [
          ...
          'managed_root="${HOME%/}/.local/share/nightshift/claude-accounts"',
          'candidate_real=$(readlink -f -- "$candidate")',
          'managed_root_real=$(readlink -f -- "$managed_root")',
          'test -f "$candidate_real/.nightshift-managed-claude-auth"',
          expected,
