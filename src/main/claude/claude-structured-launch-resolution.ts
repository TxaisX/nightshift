 ...
 * emits it — and Nightshift's send acknowledgement depends on the replay.
 */
...
export type ClaudeStructuredLaunch = {
  /** Always Nightshift's resolved user CLI: the SDK's bundled binaries are excluded from the install. */
...
export function claudeSessionIdForNightshiftSession(sessionId: string): string {
  const bytes = createHash('sha256')
    .update(`nightshift-claude:${sessionId}`)
    .digest()
    .subarray(0, 16)
  ...
  return async ({ identity }) => {
    ...
    const providerSessionId =
      head?.handle.provider === 'claude'
        ...
        : claudeSessionIdForNightshiftSession(identity.sessionId)
    ...
    const env = withCliRuntimeOnPath(
      command,
      // Only a dispatched structured worker gets the orchestration identity and the Nightshift CLI on
