...
export const CLAUDE_EVENTS = [
  ...
  // StopFailure instead; without this hook Nightshift leaves the turn spinning.
  {
...
export function getConfigPath(settings = CLAUDE_HOOK_SETTINGS): string {
  // fork: Claude Code honors CLAUDE_CONFIG_DIR; upstream always wrote ~/.claude, which left the
  // hooks dangling on machines whose Claude config lives elsewhere (agent status then never updates).
  if (settings.configDirName === '.claude') {
    return join(defaultClaudeConfigDir(), 'settings.json')
  }
...
export function getWindowsManagedLifecycleHook(
  ...
  const quotedRelativePath = quotePowerShellString(`.nightshift\\agent-hooks\\${scriptFileName}`)
