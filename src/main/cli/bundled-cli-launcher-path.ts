...
// Why `nightshift-ide` on Linux: GNOME Orca ships /usr/bin/orca, so the CLI never claims that name.
export const LINUX_CLI_COMMAND_NAME = 'nightshift-ide'
...
export function getBundledLauncherPath(
  ...
  if (platform === 'darwin') {
    return join(resourcesPath, 'bin', 'nightshift')
  }
  if (platform === 'linux') {
    return join(resourcesPath, 'bin', LINUX_CLI_COMMAND_NAME)
  }
  if (platform === 'win32') {
    return join(resourcesPath, 'bin', 'nightshift.exe')
  }
  return null
}
