...
export abstract class CliInstallLocation {
  ...
  protected get commandName(): string {
    ...
    // Why: packaged Linux uses `nightshift-ide` to avoid shadowing GNOME Orca's /usr/bin/orca.
    return this.platform === 'linux' ? LINUX_CLI_COMMAND_NAME : 'nightshift'
  }
  ...
  constructor(options: CliInstallerOptions = {}) {
    ...
    this.commandPathOverride =
      options.commandPathOverride ?? process.env.NIGHTSHIFT_CLI_INSTALL_PATH ?? null
    ...
    this.macCommandPath = existsSync(dirname(candidateMacPath))
      ? candidateMacPath
      : join(this.homePath, '.local', 'bin', 'nightshift')
    ...
    for (const commandPath of this.getPathCommandCandidates(defaultCommandPath)) {
      ...
      if (status.state !== 'not_installed') {
        if (reachedDefaultCommandPath && !isDefaultCommandPath && status.state === 'conflict') {
          // Why: a non-Nightshift command after an empty default slot can be shadowed by installing there; no user file replaced.
          continue
        }
  ...
  protected resolveCommandPath(): string | null {
    ...
    if (this.platform === 'linux') {
      ...
      // Why `nightshift-ide`: GNOME Orca ships /usr/bin/orca, so avoid shadowing that screen reader.
