...
import { installLinuxBareNightshiftDispatcher } from './linux-bare-nightshift-dispatcher'
...
async function makeFixture(): Promise<{ homePath: string; resourcesPath: string }> {
  const root = await mkdtemp(join(tmpdir(), 'nightshift-bare-dispatcher-'))
  created.push(root)
  const resourcesPath = join(root, 'resources')
  // The bundled nightshift-ide launcher must exist for the dispatcher to be written.
  await mkdir(join(resourcesPath, 'bin'), { recursive: true })
  await writeFile(join(resourcesPath, 'bin', 'nightshift-ide'), '#!/usr/bin/env bash\n', 'utf8')
...
describe('installLinuxBareNightshiftDispatcher', () => {
  it('uses the mounted bundled launcher when only APPDIR is inherited', async () => {
    ...
    const result = await installLinuxBareNightshiftDispatcher({ resourcesPath, homePath })

    expect(result.state).toBe('installed')
    expect(result.target).toBe(join(resourcesPath, 'bin', 'nightshift-ide'))
  })

  it('writes an executable bare-nightshift dispatcher that execs the bundled nightshift-ide launcher', async () => {
    const { homePath, resourcesPath } = await makeFixture()

    const result = await installLinuxBareNightshiftDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })

    const expectedTarget = join(resourcesPath, 'bin', 'nightshift-ide')
    expect(result.state).toBe('installed')
    expect(result.target).toBe(expectedTarget)
    expect(result.dispatcherPath).toBe(join(homePath, '.local', 'bin', 'nightshift'))
  ...
  it('is idempotent — a second install rewrites its own dispatcher without throwing', async () => {
    ...
    const first = await installLinuxBareNightshiftDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
    const second = await installLinuxBareNightshiftDispatcher({
      resourcesPath,
      homePath,
      appImagePath: null
    })
  ...
  it('publishes when the home filesystem does not support hard links', async () => {
    ...
    await expect(readFile(result.dispatcherPath, 'utf8')).resolves.toContain(
      '# nightshift-serve-bare-nightshift-dispatcher'
    )
  })

  it('restores a displaced foreign dispatcher when hard links are unsupported', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    await installLinuxBareNightshiftDispatcher({ resourcesPath, homePath, appImagePath: null })
  ...
  it('quotes a resources path containing spaces so the exec line cannot be split', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-bare-dispatcher-space-'))
    created.push(root)
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const { homePath, resourcesPath } = await makeFixture()
      const appImagePath = join(homePath, 'Nightshift.AppImage')
      ...
      const result = await installLinuxBareNightshiftDispatcher({
        ...
        appImageExtractRunner: async (_appImagePath, cwd) => {
          ...
          await writeFile(join(launcherDir, 'nightshift-ide'), '', {
            encoding: 'utf8',
            mode: 0o755
          })
        }
  ...
  it('skips (does not clobber) a user-owned nightshift already at ~/.local/bin', async () => {
    const { homePath, resourcesPath } = await makeFixture()
    const dispatcherPath = join(homePath, '.local', 'bin', 'nightshift')
    const appImagePath = join(homePath, 'Nightshift.AppImage')
    await mkdir(join(homePath, '.local', 'bin'), { recursive: true })
    await writeFile(dispatcherPath, '#!/bin/sh\necho my own nightshift\n', 'utf8')
    await writeFile(appImagePath, '#!/usr/bin/env bash\n', 'utf8')
    const extract = vi.fn()
  ...
  it('preserves a foreign dispatcher created while AppImage extraction is in flight', async () => {
    ...
    const installation = installLinuxBareNightshiftDispatcher({
      resourcesPath,
      homePath,
      appImagePath,
      appImageCacheRootPath: cacheRootPath,
  ...
  it('prunes old owner generations without touching a sibling namespace', async () => {
    ...
    await installLinuxBareNightshiftDispatcher(options)
    ...
    const installation = installLinuxBareNightshiftDispatcher(options)
  ...
  it('skips when the bundled nightshift-ide launcher is missing from the build', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-bare-dispatcher-nolauncher-'))
    created.push(root)
