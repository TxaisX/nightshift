...
import { buildBareNightshiftCliScript } from './linux-bare-nightshift-dispatcher'
import { quoteShell } from './cli-install-path-format'

const SHIM_DIR_NAME = 'linux-nightshift-cli-shim'

export type LinuxTerminalNightshiftCliShimOptions = {
  userDataPath: string
...
// Why: on Linux the CLI installs as `nightshift-ide` so it never shadows the GNOME
// Orca screen reader at /usr/bin/orca — but agent-facing surfaces (skills,
// dispatch preambles, CLI hints) all invoke bare `nightshift`, so on stock Ubuntu an
// agent inside a Nightshift terminal would launch the screen reader instead
// (TxaisX/nightshift#7904). Prepending this userData-scoped shim dir to managed-PTY
// PATH makes bare `nightshift` resolve to the Nightshift CLI inside Nightshift terminals only,
// leaving the user's own shells (and their screen reader) untouched.
export function ensureLinuxTerminalNightshiftCliShimDir(
  options: LinuxTerminalNightshiftCliShimOptions
): string | null {
  const resourcesPath =
...
fail() {
  printf 'Nightshift CLI is unavailable; reopen Nightshift or register the CLI again.\\n' >&2
  exit 1
}
...
function ensureShimForLauncher(userDataPath: string, launcherPath: string): string | null {
  const script = buildBareNightshiftCliScript(launcherPath)
