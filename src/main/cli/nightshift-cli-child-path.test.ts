...
vi.mock('./linux-terminal-nightshift-cli-shim', () => shim)

import { prependNightshiftCliDirToChildPath } from './nightshift-cli-child-path'
...
describe('prependNightshiftCliDirToChildPath', () => {
  it('leads packaged Linux PATH with the bare-nightshift shim dir', () => {
    // Why this matters at all: the Linux CLI installs as `nightshift-ide` so it never claims
    // GNOME Orca's /usr/bin/orca screen reader, so bare `nightshift` only works through this shim.
    ...
    expect(shim.ensureLinuxTerminalNightshiftCliShimDir).toHaveBeenCalledWith({
      userDataPath: USER_DATA
    })
  })
  ...
  it('leaves packaged Linux PATH untouched when no shim could be written', () => {
    shim.ensureLinuxTerminalNightshiftCliShimDir.mockReturnValue(null)
