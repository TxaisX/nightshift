...
describe('resolveCodexCommand', () => {
  ...
  it('prefers Codex already present on PATH', () => {
    const root = mkdtempSync(join(tmpdir(), 'nightshift-codex-command-'))
...
describe('resolveClaudeCommand', () => {
  ...
  it('prefers claude already present on PATH', () => {
    const root = mkdtempSync(join(tmpdir(), 'nightshift-claude-command-'))
    const pathDir = join(root, 'bin')
...
describe('resolveCliCommands', () => {
  ...
  it('resolves a batch from PATH and install directories', () => {
    const root = mkdtempSync(join(tmpdir(), 'nightshift-cli-commands-'))
    const pathDir = join(root, 'bin')
...
describe('getVersionManagerBinPaths', () => {
  it('includes volta, asdf, fnm, mise, pnpm, yarn, and bun directories', () => {
    const root = mkdtempSync(join(tmpdir(), 'nightshift-vm-paths-'))
...
describe('withCliRuntimeOnPath', () => {
  it('pairs a version-manager CLI with its sibling node (TxaisX/nightshift#10932)', () => {
    const root = mkdtempSync(join(tmpdir(), 'nightshift-pair-'))
