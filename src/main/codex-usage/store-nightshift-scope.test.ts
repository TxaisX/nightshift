...
describe('CodexUsageStore', () => {
  ...
  it('reports no data for Nightshift scope when only non-Nightshift Codex usage exists', async () => {
  ...
  it('uses only Nightshift-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('nightshift', '30d', 'model')
    const recentSessions = await store.getRecentSessions('nightshift', '30d', 10)
