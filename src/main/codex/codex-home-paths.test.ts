...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'nightshift-codex-resource-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'nightshift-codex-resource-user-data-'))
...
describe('syncSystemCodexResourcesIntoManagedHome', () => {
  it('uses NIGHTSHIFT_USER_DATA_PATH when Electron cannot be required', async () => {
    vi.resetModules()
    vi.doMock('electron', () => {
    ...
    process.env.NIGHTSHIFT_USER_DATA_PATH = userDataDir
    try {
      const { getNightshiftManagedCodexHomePath: getCliSafeManagedPath } =
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(managedHomePath, '.nightshift-resource-copies', 'AGENTS.md.json'), {
        recursive: true
      })
  ...
  it('removes an unowned copy when recording copy ownership fails', () => {
    ...
    mkdirSync(managedHomePath, { recursive: true })
    writeFileSync(join(managedHomePath, '.nightshift-resource-copies'), 'blocks marker directory\n')
