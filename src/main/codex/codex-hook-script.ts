...
export function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    // Why: sourcing refreshes PORT/TOKEN/ENV/VERSION from the current Nightshift so a surviving PTY keeps reporting after a restart (see claude/hook-service.ts).
    'load_hook_endpoint() {',
    '  endpoint_path="$1"',
    '  unset NIGHTSHIFT_AGENT_HOOK_TRANSPORT',
    '  case "$endpoint_path" in',
    '    *.cmd)',
    // Why: Windows passes endpoint.cmd into WSL via WSLENV; parse only Nightshift's known assignments since cmd.exe `set` lines aren't shell syntax.
    ...
    '          "set NIGHTSHIFT_AGENT_HOOK_PORT="*) NIGHTSHIFT_AGENT_HOOK_PORT=${endpoint_line#*=} ;;',
    '          "set NIGHTSHIFT_AGENT_HOOK_TOKEN="*) NIGHTSHIFT_AGENT_HOOK_TOKEN=${endpoint_line#*=} ;;',
    '          "set NIGHTSHIFT_AGENT_HOOK_ENV="*) NIGHTSHIFT_AGENT_HOOK_ENV=${endpoint_line#*=} ;;',
    '          "set NIGHTSHIFT_AGENT_HOOK_VERSION="*) NIGHTSHIFT_AGENT_HOOK_VERSION=${endpoint_line#*=} ;;',
    '          "set NIGHTSHIFT_AGENT_HOOK_TRANSPORT="*) NIGHTSHIFT_AGENT_HOOK_TRANSPORT=${endpoint_line#*=} ;;',
    '        esac',
    '      done < "$endpoint_path"',
    '      ;;',
    '    *)',
    ...
    '  load_hook_endpoint "$NIGHTSHIFT_AGENT_HOOK_ENDPOINT"',
    'fi',
