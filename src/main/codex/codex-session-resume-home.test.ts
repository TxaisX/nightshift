...
describe('resolveTrustedCodexSessionResumeHome', () => {
  ...
  it('requires the transcript provenance to name a regular rollout file', () => {
    const homePath = mkdtempSync(join(tmpdir(), 'nightshift-codex-resume-home-'))
...
describe('findTrustedCodexSessionResume legacy-rescan home ranking', () => {
  ...
  it('ranks the real system home above the others even when its path sorts last', async () => {
    const lateSortingSystemHome = join('/var', 'lib', 'nightshift', '.codex')
    await expect(
      findTrustedCodexSessionResume({
        sessionId,
        transcriptPath: undefined,
  ...
  it('ranks Windows homes case-insensitively and keeps the caller path spelling', async () => {
    ...
    const windowsAccountAHome = `${windowsRoot}\\AppData\\Roaming\\Nightshift\\codex-accounts\\a\\home`
    const windowsAccountBHome = `${windowsRoot}\\AppData\\Roaming\\Nightshift\\codex-accounts\\b\\home`
...
describe('claimsCodexRolloutLayout', () => {
  ...
  it('is true for a rollout under a home Nightshift no longer trusts, so resume cannot silently fall through to the selected account', () => {
    expect(
...
describe('resolveCodexSessionResumeProvenance', () => {
  function writeRollout(sessionId: string): { homePath: string; rolloutPath: string } {
    const homePath = mkdtempSync(join(tmpdir(), 'nightshift-codex-resume-provenance-'))
    tempRoots.push(homePath)
  ...
  it('starts fresh for a rollout file that really exists under a home Nightshift no longer trusts', async () => {
    ...
    const trusted = mkdtempSync(join(tmpdir(), 'nightshift-codex-resume-provenance-'))
