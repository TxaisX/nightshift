 ...
 *
 * Non-message items fall back to the `nightshift` namespace keyed by the Codex item
...
export function codexItemIdentity(input: {
  ...
  return { provider: 'nightshift', clientMessageId: `codex-item:${input.threadId}:${item.id}` }
}
