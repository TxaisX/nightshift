...
const LEGACY_NIGHTSHIFT_PROFILE_LINES = [
  '# BEGIN NIGHTSHIFT AGENT STATUS HOOKS',
  '[[hooks.PermissionRequest]]',
  '[[hooks.PermissionRequest.hooks]]',
  'type = "command"',
  'command = "codex-hook"',
  '# END NIGHTSHIFT AGENT STATUS HOOKS',
  ''
]
...
describe('CodexHookService', () => {
  it('removes legacy Nightshift-managed hooks from system ~/.codex during install', async () => {
  ...
  it('removes very large legacy Nightshift-managed hook lists from system ~/.codex', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyCommand = legacyManagedHookCommand()
    mkdirSync(systemCodexHome, { recursive: true })
  ...
  it('removes the legacy Nightshift Codex profile file when it only contains managed hooks', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'nightshift-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(profilePath, LEGACY_NIGHTSHIFT_PROFILE_LINES.join('\n'), 'utf-8')
  ...
  it('removes only the legacy Nightshift block from a user-edited Codex profile file', async () => {
    const systemCodexHome = join(homes.tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'nightshift-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
      profilePath,
      ['model = "gpt-5.5"', '', ...LEGACY_NIGHTSHIFT_PROFILE_LINES].join('\n'),
      'utf-8'
    )
    ...
    expect(profileConfig).not.toContain('NIGHTSHIFT AGENT STATUS HOOKS')
  ...
  it('cleans duplicate Codex hook representations while keeping status hooks in runtime CODEX_HOME', async () => {
    ...
    const legacyProfilePath = join(systemCodexHome, 'nightshift-agent-status.config.toml')
    const legacyCommand = legacyManagedHookCommand()
    ...
    writeFileSync(legacyProfilePath, LEGACY_NIGHTSHIFT_PROFILE_LINES.join('\n'), 'utf-8')
