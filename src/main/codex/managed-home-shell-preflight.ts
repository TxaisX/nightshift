...
function resolveAccountManagedHome(candidate: string, userDataPath: string): string | null {
  ...
  try {
    return readFileSync(join(candidate, '.nightshift-managed-home'), 'utf-8').trim() === accountId
      ? expectedHome
      : null
  } catch {
    return null
...
export function resolveManagedCodexShellPreflightHome(
  ...
  if (!codexHome || !nightshiftCodexHome || !pathsEqual(codexHome, nightshiftCodexHome)) {
    return null
  }
...
export async function prepareManagedCodexHomeBeforeShellLaunch(args: {
  ...
  const env = args.env ?? {
    ...
    NIGHTSHIFT_CODEX_HOME: process.env.NIGHTSHIFT_CODEX_HOME
  }
