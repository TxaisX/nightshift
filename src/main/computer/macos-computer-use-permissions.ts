...
const DEFAULT_COMPUTER_USE_BUNDLE_ID = 'com.txais.nightshift.computer-use'
...
async function openComputerUsePermissionsAsync(
  ...
  if (!helperAppPath) {
    throw new RuntimeClientError('accessibility_error', 'Nightshift Computer Use.app was not found')
  }
...
function closeExistingPermissionHelpers(): void {
  ...
  const setupHelperPatterns = [
    'nightshift-computer-use-macos[[:space:]]+--permission([[:space:]]|$)',
    'nightshift-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'
  ]
...
function nextPermissionStep(
  ...
  return `Grant ${missing.id === 'accessibility' ? 'Accessibility' : 'Screen Recording'} to Nightshift Computer Use, then retry get-app-state.`
}
