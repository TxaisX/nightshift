...
describe('connectMacOSProviderSocket', () => {
  it('removes socket listeners after a connection error', async () => {
    ...
    try {
      ...
      const promise = connectMacOSProviderSocket('/tmp/nightshift-computer.sock', 50)
  ...
  it('destroys the pending socket and removes listeners when aborted', async () => {
    ...
    const promise = connectMacOSProviderSocket('/tmp/nightshift-computer.sock', 5_000, abort.signal)
    await Promise.resolve()
    abort.abort()
