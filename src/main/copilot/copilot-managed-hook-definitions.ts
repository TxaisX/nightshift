...
// names, which match the event vocabulary already normalized by Nightshift's hook
...
export const COPILOT_EVENTS = [
  ...
  // Nightshift can normalize it without depending on a PascalCase payload.
  'subagentStart',
  'SubagentStop',
  'PreCompact',
  'Stop',
...
export function getManagedCommand(scriptPath: string, eventName: string): string {
  if (process.platform !== 'win32') {
    return wrapPosixHookCommand(scriptPath, { NIGHTSHIFT_COPILOT_HOOK_EVENT: eventName })
  }
  return wrapWindowsHookCommand(scriptPath, { NIGHTSHIFT_COPILOT_HOOK_EVENT: eventName })
}
