...
export function buildDaemonHostManifest(sources: DaemonHostSources): CopyOp[] {
  ...
  // Native addon trees (node-pty + the Windows-only addons the daemon bare-requires), mirrored so
  // require() resolves them; filtered to drop unused .pdb/other-arch prebuilds.
  ops.push(
    ...daemonNativeAddonCopyOps(resourcesPath, appDir, toPosixRelative, isRuntimeNodePtyPath)
  )
