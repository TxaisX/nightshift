...
async function spawnExitingPty(index: number): Promise<void> {
  const proc = pty.spawn('/bin/sh', ['-c', 'exit 0'], {
    ...
    rows: 24,
    cwd: process.cwd(),
    env: { ...process.env, NIGHTSHIFT_FD_LEAK_TEST_INDEX: String(index) }
  })
...
describeOnDarwin('node-pty macOS spawn fd handling', () => {
  ...
  it('does not leak fds when native posix_spawn setup fails', async () => {
    ...
    const hiddenHelperPath = `${helperPath}.nightshift-test-hidden`
      ...
      for (let i = 0; i < 20; i++) {
        expect(() =>
          pty.spawn('/bin/sh', ['-c', 'exit 0'], {
            ...
            rows: 24,
            cwd: process.cwd(),
            env: { ...process.env, NIGHTSHIFT_FD_LEAK_TEST_INDEX: String(i) }
          })
