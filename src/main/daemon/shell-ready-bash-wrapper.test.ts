...
describePosix('daemon shell-ready bash wrapper', () => {
  ...
  it('emits OSC 133 C/D markers in the daemon bash wrapper', async () => {
    ...
    expect(bashRc).toContain('__nightshift_append_prompt_command "__nightshift_osc133_epilogue"')
    ...
    expect(bashRc.lastIndexOf("trap '__nightshift_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('__nightshift_append_prompt_command "__nightshift_osc133_epilogue"')
    )
  ...
  itWithBash(
    ...
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshDaemonBashRcfile()
      // Minimal bash-preexec imitation: re-arms its own DEBUG trap from PROMPT_COMMAND at first prompt, silencing Nightshift's trap.
      writeFileSync(
        join(userDataPath, '.bash_profile'),
        [
          'preexec_functions=()',
  ...
  itWithBash(
    'dispatches a non-empty preexec_functions against the real command, not Nightshift hooks',
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshDaemonBashRcfile()
      ...
      expect(output).not.toContain('USER_PREEXEC:__nightshift_osc133')
