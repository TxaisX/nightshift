...
import { RuntimeEmulatorCommands } from '../runtime/nightshift-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous Nightshift-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous Nightshift-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills Nightshift-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
