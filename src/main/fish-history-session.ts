 ...
 * Nightshift's history tree. That is why deletion needs its own path here instead of
 ...
 */
const SESSION_PREFIX = 'nightshift_'
const RELAY_SESSION_PREFIX = 'nightshift_relay_'
const SAFE_SESSION_NAME = /^nightshift_(?:relay_)?[0-9a-f]{1,64}$/
/** Deliberately does NOT match the relay prefix. A relay host keyed by its
 *  CLIENT's worktree ids shares the one fish data dir with any desktop Nightshift on
...
const NIGHTSHIFT_HISTORY_FILE = /^nightshift_([0-9a-f]{1,64})_history$/
...
/** Drop a `fish_history` this process inherited from an outer Nightshift.
 *  fish EXPORTS the variable, so a Nightshift launched from a fish pane hands the
 *  LAUNCHING worktree's session name to every pane of the nested app, which then
 *  writes into that worktree's history file (STA-4682). Only Nightshift-minted names
...
export function dropInheritedNightshiftFishHistory(env: Record<string, string | undefined>): void {
 ...
 *  Resolve this from the PTY's SPAWN env: Nightshift can be launched with a different
 ...
 *  the two disagree whenever Nightshift's environment differs from the PTY's. */
...
/**
 * Delete every Nightshift-minted fish history file whose worktree is gone.
 *
 ...
 * scan of the data dir can find those. Matching on Nightshift's own `nightshift_<hex>_`
 ...
 * outside Nightshift's own directory.
 */
export function sweepOrphanedFishHistoryFiles(
  ...
  for (const dir of new Set(dirs)) {
    ...
    for (const entry of entries) {
      const hash = NIGHTSHIFT_HISTORY_FILE.exec(entry)?.[1]
