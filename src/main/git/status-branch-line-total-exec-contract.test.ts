...
async function createFixtureRepo(): Promise<{ repo: string; mergeBase: string }> {
  const root = await mkdtemp(path.join(tmpdir(), 'nightshift-branch-line-total-exec-'))
  tempRoots.push(root)
...
describe('branch line total completeness', () => {
  ...
  it('omits the total for a directory that is not a git worktree', async () => {
    const root = await mkdtemp(path.join(tmpdir(), 'nightshift-branch-line-total-folder-'))
    tempRoots.push(root)
