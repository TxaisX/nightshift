...
// already skips rows where that's set), but Nightshift's compatibility baseline is Git 2.25, which predates it —
...
//  - Nightshift-driven remove/move: explicit calls below (worktree-removal.ts, worktree-move.ts).
...
/** Drop one worktree's cached state; call when Nightshift itself removes or moves a worktree path. */
