...
function getAuthConfig(): GiteaAuthConfig {
  const apiBaseUrl = envValue('NIGHTSHIFT_GITEA_API_BASE_URL')
  return {
    apiBaseUrl: apiBaseUrl ? normalizeGiteaApiBaseUrl(apiBaseUrl) : null,
    token: envValue('NIGHTSHIFT_GITEA_TOKEN')
  }
}
...
/** Invalidate the shared /pulls scan after Nightshift itself creates a PR so the
