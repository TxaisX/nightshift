...
describe('GitHub issue open-by-number origin preference', () => {
  ...
  it('pins typed issue metadata to explicit origin preference', async () => {
    const source = { owner: 'fork', repo: 'nightshift', host: 'github.com' }
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        ...
        state: 'open',
        labels: [],
        url: 'https://github.com/fork/nightshift/issues/7',
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/fork/nightshift/issues/7'],
  ...
  it('skips the issue probe on untyped open when origin identity is unresolved', async () => {
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        ...
        labels: [],
        isDraft: false,
        url: 'https://github.com/fork/nightshift/pull/7',
        baseRefName: 'main',
