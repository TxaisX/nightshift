...
describe('GitHub issue source split', () => {
  ...
  it('uses upstream for issues and origin for PRs in mixed recent results', async () => {
    ...
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'nightshift' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            title: 'Fork PR',
            state: 'open',
            html_url: 'https://github.com/fork/nightshift/pull/42',
            labels: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/nightshift'), {
      cwd: '/repo-root'
    })
  })
  ...
  it('lists SSH repo work items with explicit owner/repo and no local cwd', async () => {
    ...
    await listWorkItems('/home/jinwoo/nightshift', 10, undefined, undefined, 'auto', 'openclaw-2')

    expect(resolveIssueSourceMock).toHaveBeenCalledWith(
      '/home/jinwoo/nightshift',
      'auto',
      'openclaw-2',
      {}
    )
    expect(getOwnerRepoMock).toHaveBeenCalledWith('/home/jinwoo/nightshift', 'openclaw-2', {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('TxaisX/nightshift'), {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/nightshift'), {})
  })
  ...
  it('uses upstream for issue-only queries and origin for PR-only queries', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'fork/nightshift']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it("uses upstream for recent PRs when preference='upstream'", async () => {
    ...
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'nightshift' })
    mockUpstreamCandidate({ owner: 'txais', repo: 'nightshift' })
    ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
      stdout: '[]'
    })
  ...
  it("falls back to origin for PRs when preference='upstream' and upstream is missing", async () => {
    ...
    expect(result.sources).toEqual({
      issues: { owner: 'fork', repo: 'nightshift' },
      prs: { owner: 'fork', repo: 'nightshift' },
      originCandidate: { owner: 'fork', repo: 'nightshift' },
  ...
  it('counts default work items across upstream issues and origin PRs', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:fork/nightshift is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it('probes the upstream repository for a typed fork PR before origin', async () => {
    const upstream = { owner: 'txais', repo: 'nightshift', host: 'github.com' }
  ...
  it('pins typed PR metadata to explicit origin when upstream has the same number', async () => {
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some((call) =>
        (call[0] as string[]).some((arg) => arg.includes('upstream/nightshift'))
      )
    ).toBe(false)
  ...
  it('does not probe a second PR repository after a non-not-found failure', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        ...
        { owner: 'fork', repo: 'nightshift', host: 'github.com' }
      ],
      headRepo: { owner: 'fork', repo: 'nightshift', host: 'github.com' }
    })
    ghExecFileAsyncMock
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some(([args]) =>
        (args as string[]).some((arg) => arg.includes('fork/nightshift'))
      )
    ).toBe(false)
  })
  ...
  describe('per-repo issue-source preference', () => {
    ...
    it("preference='auto' + no upstream → queries origin", async () => {
      ...
      getOwnerRepoMock.mockResolvedValueOnce({ owner: 'solo', repo: 'nightshift' })
      ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
        stdout: '[]'
      })
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('solo/nightshift'), {
        cwd: '/repo-root'
      })
    })
    ...
    it("preference='auto' + upstream exists → PRs query upstream too", async () => {
      ...
      expect(result.sources).toEqual({
        ...
        originCandidate: { owner: 'fork', repo: 'nightshift' },
        upstreamCandidate: { owner: 'txais', repo: 'nightshift' }
      })
    })
    ...
    it("preference='upstream' + no upstream → falls back to origin with fellBack=true", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:solo/nightshift is:issue is:open')
      expect(result.issueSourceFellBack).toBe(true)
    })
    ...
    it("preference='origin' + upstream exists → queries origin (not upstream)", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:fork/nightshift is:issue is:open')
    })
