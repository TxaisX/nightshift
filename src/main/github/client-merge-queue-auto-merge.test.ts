...
describe('GitHub GraphQL rate-limit guard', () => {
  ...
  it('uses explicit PR repo for merge and title mutations', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/TxaisX/nightshift/pulls/7'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
  ...
  it('detects merge queues once per base branch and blocks direct merges', async () => {
    ...
    await expect(
      mergePR('/repo-root', 7, 'squash', undefined, { owner: 'txais', repo: 'nightshift' })
