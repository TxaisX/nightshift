...
describe('getPRForBranch', () => {
  ...
  it('resolves fork PR push target using the origin URL protocol', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'txais', repo: 'nightshift' })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'prateek/fix-sidebar-agents-toggle',
          repo: {
            full_name: 'prateek/nightshift',
            name: 'nightshift',
            clone_url: 'https://github.com/prateek/nightshift.git',
            ssh_url: 'git@github.com:prateek/nightshift.git',
            owner: { login: 'prateek' }
          }
        }
      })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/TxaisX/nightshift/pulls/1738'], {
      cwd: '/repo-root'
    })
  ...
  it('pins explicit origin push-target lookup when upstream has the same PR number', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'fork', repo: 'nightshift' })
    resolvePRRepositoryCandidatesMock.mockResolvedValue({
      candidates: [
        { owner: 'upstream', repo: 'nightshift' },
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'contributor/fix',
          repo: {
            full_name: 'contributor/nightshift',
            name: 'nightshift',
            clone_url: 'https://github.com/contributor/nightshift.git',
            ssh_url: 'git@github.com:contributor/nightshift.git',
            owner: { login: 'contributor' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:fork/nightshift.git')

    await getPullRequestPushTarget('/repo-root', 1738, null, {}, 'origin')

    expect(resolvePRRepositoryCandidatesMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/fork/nightshift/pulls/1738'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
    expect(ghExecFileAsyncMock).not.toHaveBeenCalledWith(
      ['api', 'repos/upstream/nightshift/pulls/1738'],
      expect.anything()
    )
  })
  ...
  it('keeps getRepoSlug origin-based on a fork checkout (#7331)', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'fsdwen', repo: 'nightshift' })
  ...
  it('resolves a distinct upstream remote as the repo upstream', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'tmchow', repo: 'nightshift' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'tmchow', repo: 'nightshift' }
        : { owner: 'txais', repo: 'nightshift' }
    )
  ...
  it('does not treat a same-repository upstream remote as a fork', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'Txais', repo: 'Nightshift' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'Txais', repo: 'Nightshift' }
        : { owner: 'txais', repo: 'nightshift' }
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ...
      ['repo', 'view', 'github.com/Txais/Nightshift', '--json', 'isFork,parent'],
  ...
  it('falls back to the GitHub parent when no upstream remote is configured', async () => {
    getOwnerRepoForRemoteMock
      .mockResolvedValueOnce({ owner: 'tmchow', repo: 'nightshift' })
      .mockResolvedValueOnce(null)
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        isFork: true,
        parent: { name: 'nightshift', owner: { login: 'txais' } }
      })
    })
  ...
  it('routes GHES push-target probes through the Enterprise host', async () => {
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
          repo: {
            full_name: 'team/nightshift',
            name: 'nightshift',
            clone_url: 'https://github.acme-corp.com/team/nightshift.git',
            ssh_url: 'git@github.acme-corp.com:team/nightshift.git',
            owner: { login: 'team' }
          }
        }
      })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/team/nightshift/pulls/7'],
      expect.objectContaining({ host: 'github.acme-corp.com' })
    )
  })

  it('does not confuse same-slug PR repositories across GitHub hosts', async () => {
    const enterprise = { owner: 'team', repo: 'nightshift', host: 'github.acme-corp.com' }
    const dotCom = { owner: 'team', repo: 'nightshift', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [enterprise, dotCom],
      headRepo: dotCom
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce(dotCom)
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:team/nightshift.git')
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
    ...
    await expect(getPullRequestPushTarget('/repo-root', 7)).resolves.toEqual({
      pushTarget: {
        remoteName: 'pr-team-nightshift',
        branchName: 'feature',
        remoteUrl: 'git@github.acme-corp.com:team/nightshift.git'
      }
    })
  })
  ...
  it('probes additional PR repo candidates when the first lookup is not found', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork', repo: 'nightshift' })
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          head: {
            ref: 'feature/test',
            repo: {
              full_name: 'fork/nightshift',
              name: 'nightshift',
              clone_url: 'https://github.com/fork/nightshift.git',
              ssh_url: 'git@github.com:fork/nightshift.git',
              owner: { login: 'fork' }
            }
          }
        })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/fork/nightshift/pulls/1849'],
      {
        cwd: '/repo-root'
      }
    )
  ...
  it('probes the next PR work-item candidate after a permission denial', async () => {
    const upstream = { owner: 'upstream', repo: 'nightshift', host: 'github.com' }
    const origin = { owner: 'fork', repo: 'nightshift', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Origin PR',
          state: 'OPEN',
          url: 'https://github.com/fork/nightshift/pull/42',
          labels: [],
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'upstream/nightshift'])
    )
    expect(ghExecFileAsyncMock.mock.calls[1][0]).toEqual([
      'api',
      'repos/upstream/nightshift/pulls/42'
    ])
    expect(ghExecFileAsyncMock.mock.calls[2][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'fork/nightshift'])
    )
  })
