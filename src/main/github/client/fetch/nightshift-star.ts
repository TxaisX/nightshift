...
export const NIGHTSHIFT_REPO = 'TxaisX/nightshift'
...
/**
 * Check if the authenticated user has starred the Nightshift repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export function checkNightshiftStarred(): Promise<boolean | null> {
  ...
  inFlightStarCheck ??= runNightshiftStarredCheck().finally(() => {
...
export function __resetNightshiftStarCheckForTests(): void {
  inFlightStarCheck = null
}

async function runNightshiftStarredCheck(): Promise<boolean | null> {
  await acquire()
  try {
    const { stdout, stderr } = await ghExecFileAsync(
      ['api', '--include', `user/starred/${NIGHTSHIFT_REPO}`],
...
/**
 * Star the Nightshift repo for the authenticated user.
 */
export async function starNightshift(): Promise<boolean> {
  await acquire()
  try {
    await ghExecFileAsync(['api', '-X', 'PUT', `user/starred/${NIGHTSHIFT_REPO}`], {
      encoding: 'utf-8',
