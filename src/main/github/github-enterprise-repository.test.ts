...
describe('getEnterpriseGitHubRepoSlug', () => {
  ...
  it('resolves a GHES remote whose host the user is gh-authenticated to (#8312)', async () => {
    mockOriginRemote('https://github.acme-corp.com/team/nightshift.git')
  ...
  it('resolves a GHES SCP-style SSH remote', async () => {
    mockOriginRemote('git@github.acme-corp.com:team/nightshift.git')
    mockHostAuthenticated()
  ...
  it('expands an SSH Host alias to the authenticated GHES HostName (#10284)', async () => {
    mockOriginRemote('git@ghe-work:team/nightshift.git')
  ...
  it('returns null for a Host alias that resolves to github.com (dotcom path owns it)', async () => {
    mockOriginRemote('git@github-work:team/nightshift.git')
  ...
  it('uses the unique ported auth host for a hostname-only SSH remote', async () => {
    mockOriginRemote('git@ghe.acme.com:team/nightshift.git')
  ...
  it.each([
    ...
    async (authenticatedHosts) => {
      mockOriginRemote('https://ghe.acme.com:8443/team/nightshift.git')
  ...
  it.each([
    'https://ghe.acme.com:8443/team/nightshift.git',
    'https://ghe.acme.com:80/team/nightshift.git',
    'http://ghe.acme.com:443/team/nightshift.git'
  ...
  it.each([
    'http://ghe.acme.com:80/team/nightshift.git',
    'https://ghe.acme.com:443/team/nightshift.git'
  ])('matches a protocol-default port to the portless auth host: %s', async (remoteUrl) => {
    mockOriginRemote(remoteUrl)
    mockHostAuthenticated('ghe.acme.com')
  ...
  it('preserves exact authentication for an HTTP endpoint on port 443', async () => {
    mockOriginRemote('http://ghe.acme.com:443/team/nightshift.git')
  ...
  it('preserves exact authentication for an HTTPS endpoint on port 80', async () => {
    mockOriginRemote('https://ghe.acme.com:80/team/nightshift.git')
  ...
  it('leaves github.com to getOwnerRepo without probing gh auth', async () => {
    mockOriginRemote('https://github.com/team/nightshift.git')
  ...
  it('declines a custom host the user is not gh-authenticated to (leaves it for Gitea)', async () => {
    mockOriginRemote('https://gitea.example.com/team/nightshift.git')
    mockHostNotAuthenticated()
