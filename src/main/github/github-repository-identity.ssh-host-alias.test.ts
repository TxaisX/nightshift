...
function sshProvider(hostname: string, remoteUrl = 'git@github-work:team/nightshift.git') {
  return {
...
describe('#10284 SSH Host alias → github.com owner/repo', () => {
  it('resolveGitHubOwnerRepoFromRemoteUrl expands HostName ssh.github.com', async () => {
    ...
    await expect(
      resolveGitHubOwnerRepoFromRemoteUrl('git@github-work:team/nightshift.git')
    ).resolves.toEqual({ owner: 'team', repo: 'nightshift' })
  ...
  it('getOwnerRepoForRemote resolves SCP alias remote used for multi-account GitHub', async () => {
    mockRemoteUrl('git@github-work:team/nightshift.git')
  ...
  it('preserves a one-shot SSH remote failure through candidate discovery', async () => {
    ...
    const provider = {
      exec: vi
        ...
        .mockResolvedValueOnce({ stdout: 'git@github.com:team/nightshift.git\n', stderr: '' })
    }
    getSshGitProviderMock.mockReturnValue(provider)
  ...
  it('isolates the same alias across native and WSL runtimes', async () => {
    ...
    await expect(
      resolveGitHubOwnerRepoFromRemoteUrl('git@forge-work:team/nightshift.git')
    ).resolves.toEqual({ owner: 'team', repo: 'nightshift' })
    await expect(
      resolveGitHubOwnerRepoFromRemoteUrl('git@forge-work:team/nightshift.git', {
        repoPath: REPO,
  ...
  it('invalidates alias resolution when an SSH provider reconnects', async () => {
    ...
    await expect(
      resolveGitHubOwnerRepoFromRemoteUrl('git@forge-work:team/nightshift.git', context)
    ).resolves.toEqual({ owner: 'team', repo: 'nightshift' })
  ...
  it('invalidates owner/repo identity when an SSH provider reconnects', async () => {
    ...
    getSshGitProviderMock.mockReturnValue(
      sshProvider('github.com', 'git@github-work:team/nightshift.git')
    )
  ...
  it('does not call ssh -G for literal github.com remotes', async () => {
    mockRemoteUrl('git@github.com:team/nightshift.git')
  ...
  it('does not call ssh -G for https remotes', async () => {
    mockRemoteUrl('https://github.com/team/nightshift.git')
  ...
  it('returns null when alias resolves to a non-GitHub host', async () => {
    mockRemoteUrl('git@gitlab-work:team/nightshift.git')
  ...
  it('does not rewrite transport: identity resolution only consumes HostName', async () => {
    const remote = 'git@github-work:team/nightshift.git'
  ...
  it('classifies ssh -G failure as indeterminate (not stable not-github)', async () => {
    resolveWithSshGMock.mockResolvedValueOnce(null)
    await expect(
      classifyGitHubOwnerRepoFromRemoteUrl('git@github-work:team/nightshift.git')
  ...
  it('does not pin an SSH-config-dependent miss to the Git config signature', async () => {
    vi.useFakeTimers()
    mockRemoteUrl('git@forge-work:team/nightshift.git')
    resolveWithSshGMock
  ...
  it('isolates case-sensitive OpenSSH Host aliases in the cache', async () => {
    ...
    await expect(
      ...
      kind: 'github',
      ownerRepo: { owner: 'team', repo: 'nightshift' }
    })
    await expect(
      classifyGitHubOwnerRepoFromRemoteUrl('git@github-work:team/nightshift.git')
    ).resolves.toEqual({
  ...
  it('classifies a resolved non-GitHub HostName as not-github', async () => {
    resolveWithSshGMock.mockResolvedValueOnce(sshConfig('gitlab.com'))
    await expect(
      classifyGitHubOwnerRepoFromRemoteUrl('git@gitlab-work:team/nightshift.git')
