...
const WINDOWS_GROK_HOOK_POST_COMMAND = buildWindowsAgentHookPostCommand('grok', [
  ...
  '  --data-urlencode "grokHome=%NIGHTSHIFT_GROK_HOME%" ^'
])
 ...
 * is unset (the default outside a Nightshift-managed terminal), length/trailing
...
export function buildWindowsGrokHookScript(): string {
  return [
    ...
    ...buildWindowsHookEnvironmentGuardLines(),
    'set "NIGHTSHIFT_GROK_HOME="',
    'if not defined GROK_HOME goto :nightshift_grok_home_ready',
    `if not "%GROK_HOME:~${GROK_HOME_ENVELOPE_MAX_LENGTH},1%"=="" goto :nightshift_grok_home_ready`,
    ...
    'set "NIGHTSHIFT_GROK_HOME=%GROK_HOME:"=%"',
    'if not defined NIGHTSHIFT_GROK_HOME goto :nightshift_grok_home_ready',
    ...
    `if not "%NIGHTSHIFT_GROK_HOME:~${GROK_HOME_ENVELOPE_MAX_LENGTH},1%"=="" set "NIGHTSHIFT_GROK_HOME="`,
    ':nightshift_grok_home_ready',
