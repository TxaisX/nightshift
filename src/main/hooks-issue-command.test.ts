...
describe('readIssueCommand', () => {
  it('prefers the local override over the shared nightshift.yaml command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_ISSUE_COMMAND_PATH || path === TEST_REPO_NIGHTSHIFT_YAML_PATH
    )
  ...
  it('falls back to the shared nightshift.yaml command when no local override exists', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation((path) => path === TEST_REPO_NIGHTSHIFT_YAML_PATH)
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_NIGHTSHIFT_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
...
describe('writeIssueCommand', () => {
  it('writes only the local override file and keeps .nightshift ignored locally', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_GITIGNORE_PATH || path === join(TEST_REPO_PATH, '.nightshift')
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
...
describe('createIssueCommandRunnerScript', () => {
  ...
  it('writes a POSIX issue-command runner when a shebang declares bash and setup resolves to Git Bash', async () => {
    ...
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\nightshift\\issue-command-runner.sh\n')
    const fs = await import('node:fs')
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ['rev-parse', '--git-path', 'nightshift/issue-command-runner.sh'],
        { cwd: 'C:\\repo-worktree' }
      )
  ...
  it('keeps a plain issue command on the cmd runner under a Git Bash terminal', async () => {
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ['rev-parse', '--git-path', 'nightshift/issue-command-runner.cmd'],
        { cwd: 'C:\\repo-worktree' }
      )
