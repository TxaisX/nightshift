...
describe('parseNightshiftYaml', () => {
  it('parses YAML with setup script only', () => {
    const yaml = `scripts:\n  setup: |\n    echo "setting up"\n    npm install\n`
    const result = parseNightshiftYaml(yaml)
  ...
  it('parses a project requirement to finish setup before agent startup', () => {
    ...
    expect(parseNightshiftYaml(yaml)).toEqual({
  ...
  it('returns null when there is no scripts block', () => {
    const yaml = `other:\n  key: value\n`
    expect(parseNightshiftYaml(yaml)).toBeNull()
  })
  ...
  it('returns null for empty string', () => {
    expect(parseNightshiftYaml('')).toBeNull()
  })
  ...
  it('parses default terminal tabs from nightshift.yaml', () => {
    const yaml = [
      'defaultTabs:',
      '  - title: Claude',
      '    color: "#f97316"',
  ...
  it('parses environmentRecipes from nightshift.yaml', () => {
    const yaml = [
      'environmentRecipes:',
      '  - id: cloud-sandbox',
      '    name: Cloud Sandbox',
      ...
      '    create: ./scripts/nightshift-vm/start-cloud-sandbox.sh',
      '    suspend: ./scripts/nightshift-vm/suspend-cloud-sandbox.sh',
      '    resume: ./scripts/nightshift-vm/resume-cloud-sandbox.sh',
      '    destroy: ./scripts/nightshift-vm/destroy-cloud-sandbox.sh'
    ].join('\n')
      ...
      environmentRecipeDiagnostics: [
        {
          ...
          field: 'checkoutMode',
          message:
            'Recipe "cloud-sandbox" checkoutMode must be "nightshift-worktree" or "provisioned-root".'
        }
      ]
    })
  })
  ...
  it('parses legacy environmentRecipes command and cleanup aliases', () => {
    const yaml = [
      ...
      '  - id: manual-sandbox',
      '    name: Manual Sandbox',
      '    command: ./scripts/nightshift-vm/start-manual-sandbox.sh',
      '    cleanup: none'
    ].join('\n')
      ...
      environmentRecipes: [
        {
          id: 'manual-sandbox',
          name: 'Manual Sandbox',
          create: './scripts/nightshift-vm/start-manual-sandbox.sh',
  ...
  it('drops invalid and duplicate environmentRecipes', () => {
    const yaml = [
      ...
      '    create: ./scripts/nightshift-vm/start-duplicate.sh',
      '  - id: missing-create',
      '    name: Missing Create',
      '  - name: Missing Id',
      '    create: ./scripts/nightshift-vm/start-missing-id.sh',
      '  - id: "Cloud Sandbox"',
      '    name: Unsafe Id',
      '    create: ./scripts/nightshift-vm/start-unsafe-id.sh',
      '  - 42'
    ].join('\n')
  ...
  it('parses worktree.sharedDirectories from nightshift.yaml', () => {
    const result = parseNightshiftYaml(
  ...
  it('returns null when sharedDirectories is the only key and holds nothing usable', () => {
    expect(parseNightshiftYaml('worktree:\n  sharedDirectories: []\n')).toBeNull()
    expect(parseNightshiftYaml('worktree:\n  sharedDirectories: node_modules\n')).toBeNull()
  })

  it('keeps sharedDirectories alongside other nightshift.yaml keys', () => {
    const result = parseNightshiftYaml(
      [
        'scripts:',
        '  setup: pnpm install',
...
describe('hasUnrecognizedNightshiftYamlKeys', () => {
  it('returns true when the file contains only keys this version does not handle', async () => {
    ...
    const { hasUnrecognizedNightshiftYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedNightshiftYamlKeys('/test/repo')).toBe(true)
  })
  ...
  it('returns false when the file contains only recognised keys', async () => {
    ...
    const { hasUnrecognizedNightshiftYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedNightshiftYamlKeys('/test/repo')).toBe(false)
  })
