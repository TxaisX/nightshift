...
describe('registerEphemeralVmHandlers', () => {
  ...
  beforeEach(() => {
    ...
    connectRuntimeOwnedSshTargetMock.mockResolvedValue({
      targetId: 'runtime-ssh-nightshift-instance-1',
      target: {
        id: 'runtime-ssh-nightshift-instance-1',
        label: 'Sandbox',
  ...
  it('lists recipes from local repo nightshift.yaml', async () => {
    const repoPath = makeDir('nightshift-ephemeral-vm-ipc-repo-')
    writeFileSync(
      join(repoPath, 'nightshift.yaml'),
      [
        'environmentRecipes:',
        '  - id: cloud-sandbox',
        '    name: Cloud Sandbox',
  ...
  it('merges approved plugin recipes while repository recipes shadow matching ids', async () => {
    ...
    const pluginService = pluginServiceWithRecipes([
      {
        pluginKey: 'nightshift-samples.recipes',
  ...
  it('uses an immutable plugin recipe snapshot after the plugin is removed', async () => {
    const userDataPath = makeDir('nightshift-ephemeral-vm-ipc-user-data-')
    const repoPath = makeDir('nightshift-ephemeral-vm-ipc-repo-')
  ...
  it('provisions an ssh recipe without creating a runtime environment', async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      connectionType: 'ssh',
      sshTargetId: 'runtime-ssh-nightshift-instance-1',
      runtime: {
        repoId: 'repo-1',
        status: 'running',
        connectionMode: 'ssh',
  ...
  it('retains the runtime-owned SSH target when destroy fails', async () => {
    ...
    expect(cleaned.sshTargetId).toBe('runtime-ssh-nightshift-instance-1')
  })
    ...
    upsertEphemeralVmRuntime(userDataPath, {
      ...
      recipeResult: {
        ...
        connection: {
          type: 'ssh',
          projectRoot: '/workspace/repo',
          target: { label: 'VM', host: 'host', port: 22, username: 'nightshift' }
        }
      }
    })
    registerEphemeralVmHandlers(makeStore(repoPath) as never)
