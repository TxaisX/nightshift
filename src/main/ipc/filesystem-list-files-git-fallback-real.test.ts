...
async function initRepo(repoPath: string): Promise<void> {
  ...
  await execFile('git', ['config', 'user.email', 'nightshift@example.invalid'], { cwd: repoPath })
  await execFile('git', ['config', 'user.name', 'Nightshift Test'], { cwd: repoPath })
}
...
describe('filesystem-list-files real git fallback', () => {
  ...
  beforeEach(() => {
    wslAwareSpawnMock.mockImplementation(
      (_command: string, _args: string[], options: SpawnOptions & { cwd?: string }) =>
        spawn('nightshift-definitely-missing-rg', [], {
          cwd: options.cwd,
          stdio: options.stdio
        })
    )
  ...
  it('returns real paths for UTF-8 filenames from the git fallback', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'nightshift-quick-open-git-fallback-'))
  ...
  it('fills nested git repos from gitlink and untracked embedded-repo entries', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'nightshift-quick-open-monorepo-'))
  ...
  it('walks a non-git root instead of returning an empty git fallback result', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'nightshift-quick-open-non-git-'))
  ...
  it('bounds a non-git readdir fallback without treating the limit as an error', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'nightshift-quick-open-bounded-non-git-'))
  ...
  it('rejects abnormal git ls-files failures instead of resolving an empty list', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'nightshift-quick-open-bad-index-'))
    const repoPath = join(tempDir, 'repo')
    await initRepo(repoPath)
  ...
  it('resolves an empty repo as an empty list', async () => {
    tempDir = await mkdtemp(join(tmpdir(), 'nightshift-quick-open-empty-repo-'))
    const repoPath = join(tempDir, 'repo')
    await initRepo(repoPath)
