    ...
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('uses the current account when the original auth recovers after a switch', async () => {
        ...
        expect(daemonSpawn.mock.calls[0]?.[0].env).toMatchObject({
          CODEX_HOME: nextHome,
          NIGHTSHIFT_CODEX_HOME: nextHome
        })
      })
      ...
      it('mirrors a user-provided OPENCODE_CONFIG_DIR into a source-scoped overlay on the daemon path', async () => {
        ...
        expect(env.NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR).toBe('/user/custom/opencode')
      })
      it('uses source OpenCode config env instead of remirroring a parent overlay', async () => {
        const env = await daemonSpawnAndGetEnv({
          ...
          NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR: '/user/custom/opencode'
        })
        expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
          expect.any(String),
          '/user/custom/opencode'
      ...
      it('installs Pi managed extensions without redirecting homes on the daemon path', async () => {
        ...
        expect(env.NIGHTSHIFT_PI_SOURCE_AGENT_DIR).toBe('/user/.pi/agent')
        expect(env.NIGHTSHIFT_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.NIGHTSHIFT_OMP_STATUS_EXTENSION).toBe(expectedOmpStatusExtension)
      })
      ...
      it('threads command: "omp" through to piBuildPtyEnv on the daemon path with OMP status metadata', async () => {
        ...
        expect(env.NIGHTSHIFT_OMP_STATUS_EXTENSION).toBe(
          '/user/.omp/agent/extensions/nightshift-agent-status.ts'
        )
        expect(env.NIGHTSHIFT_OMP_SOURCE_AGENT_DIR).toBe('/user/.omp/agent')
        expect(env.NIGHTSHIFT_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.NIGHTSHIFT_PI_SOURCE_AGENT_DIR).toBeUndefined()
      })
