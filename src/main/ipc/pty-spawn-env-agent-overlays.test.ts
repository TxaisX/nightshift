...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    ...
    it('injects the OpenCode hook env into Nightshift terminal PTYs', async () => {
      ...
      expect(env.NIGHTSHIFT_OPENCODE_HOOK_PORT).toBe('4567')
      expect(env.NIGHTSHIFT_OPENCODE_HOOK_TOKEN).toBe('opencode-token')
      expect(env.NIGHTSHIFT_OPENCODE_PTY_ID).toBe('test-pty')
      expect(env.OPENCODE_CONFIG_DIR).toEqual(expect.any(String))
      expect(env.NIGHTSHIFT_OPENCODE_CONFIG_DIR).toBe(env.OPENCODE_CONFIG_DIR)
    })
    it('mirrors the original OpenCode source dir when launched from a Nightshift overlay shell', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-nightshift-opencode-overlay',
        NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR: '/tmp/user-opencode-config'
      })
      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-opencode-config'
      )
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/nightshift-opencode-overlay')
      expect(env.NIGHTSHIFT_OPENCODE_CONFIG_DIR).toBe('/tmp/nightshift-opencode-overlay')
      expect(env.NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR).toBe('/tmp/user-opencode-config')
    })
    it('does not treat inherited Nightshift OpenCode config as user config without a source dir', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-nightshift-opencode-overlay',
        NIGHTSHIFT_OPENCODE_CONFIG_DIR: '/tmp/parent-nightshift-opencode-overlay'
      })

      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined)
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/nightshift-opencode-config')
      expect(env.NIGHTSHIFT_OPENCODE_CONFIG_DIR).toBe('/tmp/nightshift-opencode-config')
      expect(env.NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    it('restores user OpenCode config when agent status hooks are disabled in a nested Nightshift shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-nightshift-opencode-overlay',
          NIGHTSHIFT_OPENCODE_CONFIG_DIR: '/tmp/parent-nightshift-opencode-overlay',
    ...
    it('injects MiMo overlay env only when launch command is mimo', async () => {
      ...
      expect(env.MIMOCODE_HOME).toBe('/tmp/nightshift-mimocode-shared')
      expect(env.NIGHTSHIFT_MIMOCODE_HOME).toBe('/tmp/nightshift-mimocode-shared')
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested Nightshift shell', async () => {
      const env = await spawnAndGetEnv(
        {
          MIMOCODE_HOME: '/tmp/parent-nightshift-mimocode-overlay',
          NIGHTSHIFT_MIMOCODE_HOME: '/tmp/parent-nightshift-mimocode-overlay',
          NIGHTSHIFT_MIMOCODE_SOURCE_HOME: '/tmp/user-mimocode-home'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false }),
    ...
    posixOnlyIt(
      'reproduces issue #1534: GUI-launched Nightshift mirrors zshrc-only OpenCode config',
      async () => {
        ...
        const env = await spawnAndGetEnv(undefined, {
          ...
          NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR: undefined
        })
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/nightshift-opencode-overlay')
        expect(env.NIGHTSHIFT_OPENCODE_CONFIG_DIR).toBe('/tmp/nightshift-opencode-overlay')
        expect(env.NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR).toBe('/home/pim/company/opencode-config')
        expect(env.OPENCODE_CONFIG_DIR).not.toBe(env.NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR)
      }
    )
    it('installs Pi managed extensions without redirecting Nightshift terminal PTY homes', async () => {
      ...
      expect(env.NIGHTSHIFT_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    ...
    it('threads command: "omp" through to piBuildPtyEnv and emits OMP status metadata', async () => {
      // Why: OMP launches emit NIGHTSHIFT_OMP_* shadow vars, not Pi-named ones; only PI_CODING_AGENT_DIR stays (OMP's own binary reads it).
      const env = await spawnAndGetEnv(
        undefined,
      ...
      expect(env.NIGHTSHIFT_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.NIGHTSHIFT_OMP_STATUS_EXTENSION).toBe(
        '/tmp/user-omp-agent/extensions/nightshift-agent-status.ts'
      )
      expect(env.NIGHTSHIFT_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
    ...
    it('installs Prime status into its independent agent dir on explicit launch', async () => {
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.NIGHTSHIFT_PRIME_AGENT_SOURCE_AGENT_DIR).toBe('/tmp/user-prime-agent')
      expect(env.NIGHTSHIFT_PI_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.NIGHTSHIFT_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('mirrors the original Pi source dir when launched from a Nightshift overlay shell', async () => {
      const env = await spawnAndGetEnv({
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/parent-nightshift-pi-overlay')
      expect(env.NIGHTSHIFT_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.NIGHTSHIFT_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
  })
