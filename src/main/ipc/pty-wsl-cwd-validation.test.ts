...
describe('registerPtyHandlers', () => {
  ...
  it('spawns a plain POSIX login shell and queues startup commands for the live session', async () => {
    ...
    const originalNightshiftOrigZdotdir = process.env.NIGHTSHIFT_ORIG_ZDOTDIR
    ...
    try {
      ...
      // config dir to hand back and Nightshift must not invent one — the wrapper
      // leaves ZDOTDIR unset, exactly as an unwrapped login zsh would.
      expect(options.env.NIGHTSHIFT_ORIG_ZDOTDIR).toBeUndefined()
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
      ...
      if (originalNightshiftOrigZdotdir === undefined) {
        delete process.env.NIGHTSHIFT_ORIG_ZDOTDIR
      } else {
        process.env.NIGHTSHIFT_ORIG_ZDOTDIR = originalNightshiftOrigZdotdir
      }
