...
describe('repos:add + repos:clone', () => {
  ...
  it('inherits global non-Nightshift visibility while retaining the mixed-version safety marker', async () => {
  ...
  it('preserves the selected Enterprise host when aligning an existing folder', async () => {
    ...
    const selectedProject = {
      id: 'github:github.acme-corp.com/acme/nightshift',
  ...
  it('sets up a folder when the selected project exists only on another host', async () => {
    ...
    expect(result).toHaveProperty('project.id', 'github:github.acme.test/acme/nightshift')
  })
  ...
  it('rolls back a new repo when the supplied identity does not match the project', async () => {
    ...
    await expect(
      handlers.get('projectHostSetups:setupExistingFolder')!(null, {
        projectId: 'github:acme/nightshift',
        projectProviderIdentity: { provider: 'github', owner: 'other', repo: 'nightshift' },
        hostId: 'local',
