...
describe('usage provider IPC handlers', () => {
  it('registers every provider route and forwards query arguments', () => {
    ...
    call('claudeUsage', 'getSnapshot', { scope: 'nightshift', range: '30d', limit: 7 })
    call('claudeUsage', 'getSummary', { scope: 'all', range: '7d' })
    call('claudeUsage', 'getDaily', { scope: 'nightshift', range: '90d' })
    call('claudeUsage', 'getBreakdown', { scope: 'all', range: 'all', kind: 'model' })
    call('claudeUsage', 'getRecentSessions', { scope: 'nightshift', range: '30d', limit: 4 })
    ...
    expect(claudeUsage.getSnapshot).toHaveBeenCalledWith('nightshift', '30d', 7)
    expect(claudeUsage.getSummary).toHaveBeenCalledWith('all', '7d')
    expect(claudeUsage.getDaily).toHaveBeenCalledWith('nightshift', '90d')
    expect(claudeUsage.getBreakdown).toHaveBeenCalledWith('all', 'all', 'model')
    expect(claudeUsage.getRecentSessions).toHaveBeenCalledWith('nightshift', '30d', 4)
  })
})
