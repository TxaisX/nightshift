...
// Nightshift-provenance `pr-*` remote's refspec to only the branches Nightshift actually tracked
...
// alone isn't proof of Nightshift provenance the way a metadata entry is.
//
...
/** `pushTarget`-derived branches to keep per remote, gated on at least one entry proving Nightshift created it. */
...
export async function migrateForkRemoteRefspecsWithExec(
  ...
  for (const remoteName of candidateNames) {
    ...
    if (branches.size === 0) {
      ...
      // pre-#17828 Nightshift's), not a `pr`-prefixed remote a user configured by hand. Clears
