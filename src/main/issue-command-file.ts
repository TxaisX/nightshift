// Why: `.nightshift/issue-command` is the per-user override; `nightshift.yaml` is the tracked project default.
...
export function getIssueCommandFilePath(repoPath: string): string {
  return join(repoPath, NIGHTSHIFT_DIR, ISSUE_COMMAND_FILENAME)
}
...
/**
 * Write the per-user issue command override to `{repoRoot}/.nightshift/issue-command`.
 * Empty content deletes the override so the shared `nightshift.yaml` command applies again.
 */
export function writeIssueCommand(repoPath: string, content: string): void {
  ...
  try {
    ...
    const nightshiftDir = join(repoPath, NIGHTSHIFT_DIR)
    if (!existsSync(nightshiftDir)) {
      mkdirSync(nightshiftDir, { recursive: true })
    }
    ensureNightshiftDirIgnored(repoPath)
...
/** Ensure `.nightshift` is in `.gitignore` so the per-user directory is never committed. */
function ensureNightshiftDirIgnored(repoPath: string): void {
  const gitignorePath = join(repoPath, '.gitignore')
  try {
    if (existsSync(gitignorePath)) {
      const content = readFileSync(gitignorePath, 'utf-8')
      if (/^\.nightshift\/?$/m.test(content)) {
        return
      }
      const separator = content.endsWith('\n') ? '' : '\n'
      writeFileSync(gitignorePath, `${content}${separator}.nightshift\n`, 'utf-8')
    } else {
      writeFileSync(gitignorePath, '.nightshift\n', 'utf-8')
    }
  } catch {
    console.warn('[hooks] Could not update .gitignore to exclude .nightshift')
  }
}
