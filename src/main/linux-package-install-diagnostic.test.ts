...
describe('redactLinuxPackageInstallText', () => {
  ...
  it('replaces every cached package-path occurrence before the home directory', () => {
    ...
    const packagePath = `${home}/.cache/nightshift-updater/Nightshift-1.2.3.deb`
    const text = `${packagePath} failed; retry ${packagePath}; config ${home}/.config/nightshift`
    expect(diagnostic.redactLinuxPackageInstallText(text, packagePath)).toBe(
      '<package> failed; retry <package>; config <home>/.config/nightshift'
    )
  })
