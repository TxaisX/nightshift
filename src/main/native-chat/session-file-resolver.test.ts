...
describe('resolveSessionFilePath', () => {
  it('reads Claude last-prompt leaf metadata as the durable branch marker', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-leaf-')
  ...
  it('fails closed when a Claude transcript has no branch marker', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-no-leaf-')
    const transcript = join(root, 'session.jsonl')
    await writeFile(
      transcript,
  ...
  it('distinguishes an incomplete final Claude JSONL record from durable malformed content', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-torn-tail-')
    const transcript = join(root, 'session.jsonl')
  ...
  it('refuses a Claude marker on a sibling branch', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-sibling-')
    const transcript = join(root, 'session.jsonl')
    await writeFile(
      transcript,
      [
  ...
  it('refuses missing and cyclic Claude parent chains', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-invalid-ancestry-')
  ...
  it('rejects non-transcript and sidechain UUIDs as the durable leaf', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-leaf-filter-')
    const transcript = join(root, 'session.jsonl')
    await writeFile(
      transcript,
      [
  ...
  it('rejects a main leaf whose ancestry crosses a subagent sidechain', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-sidechain-ancestry-')
    const transcript = join(root, 'session.jsonl')
    await writeFile(
      transcript,
      [
  ...
  it('rejects a main leaf whose ancestry crosses a parent-tool-use sidechain', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-parent-tool-ancestry-')
  ...
  it('rejects a previous cursor descended from a parent-tool-use sidechain', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-parent-tool-cursor-')
    const transcript = join(root, 'transcript.jsonl')
    await writeFile(
      transcript,
      [
  ...
  it('rejects a latest marker descended from a parent-tool-use cursor sidechain', async () => {
    const root = await makeRoot(
      'nightshift-native-chat-resolve-claude-parent-tool-cursor-descendant-'
    )
    const transcript = join(root, 'transcript.jsonl')
    await writeFile(
      transcript,
  ...
  it('rejects a post-snapshot descendant whose parent row was observed later', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-post-snapshot-')
    const transcript = join(root, 'transcript.jsonl')
    await writeFile(
      transcript,
      [
  ...
  it('does not re-prove a divergent sibling after the sampled cursor rejects', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-sibling-reproof-')
    const transcript = join(root, 'transcript.jsonl')
    await writeFile(
      transcript,
      [
  ...
  it('globs Claude project subdirs for <sessionId>.jsonl', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-claude-')
  ...
  it('resolves OpenClaude sessions from the Claude transcript layout', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-openclaude-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
  ...
  it('resolves Grok chat_history.jsonl under encodeURIComponent(cwd)/sessionId', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-grok-')
  ...
  it('resolves Grok chat_history by session id under a long-cwd slug group', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-grok-long-')
    const grokSessionsDir = join(root, 'grok-sessions')
  ...
  it('ignores nested Grok decoys outside the direct group/session layout', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-grok-decoy-')
    const grokSessionsDir = join(root, 'grok-sessions')
    const decoy = join(
      grokSessionsDir,
      'group',
  ...
  it('rejects unsafe Grok session ids before filesystem discovery', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-grok-invalid-')
    const grokSessionsDir = join(root, 'grok-sessions')
  ...
  it('resolves Grok sessions under GROK_HOME when no override is passed', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-grok-home-')
  ...
  it('matches Codex rollout files by session id suffix', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-codex-')
  ...
  it('matches omp transcripts by session id suffix inside the per-cwd directory', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-omp-')
  ...
  it('never descends into an omp session artifact dir', async () => {
    ...
    const root = await makeRoot('nightshift-native-chat-resolve-omp-artifact-')
    const ompSessionsDir = join(root, 'omp-sessions')
    const cwdDir = join(ompSessionsDir, '-Users-ada-repo')
  ...
  it('honors OMP_CODING_AGENT_DIR when resolving omp transcripts', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-omp-env-')
  ...
  it('resolves a rollout from the nightshift-managed Codex home (NIGHTSHIFT_USER_DATA_PATH)', async () => {
    // Nightshift launches Codex with its own managed CODEX_HOME, so rollout files land
    // under <userData>/codex-runtime-home/home/sessions, NOT ~/.codex/sessions.
    const root = await makeRoot('nightshift-native-chat-resolve-managed-')
    ...
    const previous = process.env.NIGHTSHIFT_USER_DATA_PATH
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.NIGHTSHIFT_USER_DATA_PATH = previous
      }
    }
  })

  it('falls back to CODEX_HOME when the managed home has no match', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-codex-home-')
    ...
    const previousUserData = process.env.NIGHTSHIFT_USER_DATA_PATH
    process.env.CODEX_HOME = codexHome
    // Point the managed home at an empty dir so the fallback is exercised.
    process.env.NIGHTSHIFT_USER_DATA_PATH = managedRoot
    try {
      const resolved = await resolveSessionFilePath('codex', 'xyz-session')
      expect(resolved).toBe(target)
    } finally {
      restoreEnv('CODEX_HOME', previousCodex)
      restoreEnv('NIGHTSHIFT_USER_DATA_PATH', previousUserData)
    }
  })

  it('returns null when no transcript matches', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-missing-')
    const claudeProjectsDir = join(root, 'claude-projects')
  ...
  it('prefers the hook transcriptPath when it exists (Claude id != file name)', async () => {
    ...
    const root = await makeRoot('nightshift-native-chat-resolve-path-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
  ...
  it('falls back to the id glob when the hook transcriptPath does not exist', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-path-stale-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
  ...
  it('ignores a non-jsonl transcriptPath and falls back to the glob', async () => {
    const root = await makeRoot('nightshift-native-chat-resolve-path-ext-')
    const claudeProjectsDir = join(root, 'claude-projects')
    const projectDir = join(claudeProjectsDir, '-Users-ada-repo')
    await mkdir(projectDir, { recursive: true })
...
describe('the default Claude transcript root mobile falls back to', () => {
  it('follows CLAUDE_CONFIG_DIR, the same variable the pinned account home follows', async () => {
    const configDir = await makeRoot('nightshift-native-chat-claude-config-dir-')
