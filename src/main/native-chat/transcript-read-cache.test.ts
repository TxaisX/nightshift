...
async function seedSession(sessionId: string, turns: number): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'nightshift-native-chat-cache-'))
  tempRoots.push(root)
...
async function seedBigFile(name: string, bytes: number): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'nightshift-native-chat-cache-bytes-'))
  tempRoots.push(root)
...
describe('readNativeChatTranscriptCached', () => {
  ...
  it('never serves one file’s parse for a different file that shares a sessionId', async () => {
    const root = await mkdtemp(join(tmpdir(), 'nightshift-native-chat-cache-xwt-'))
    tempRoots.push(root)
