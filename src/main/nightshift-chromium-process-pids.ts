...
/**
 * PIDs of Nightshift's own Chromium processes — browser, renderers, GPU, utilities.
 *
 ...
 * kill in every field Nightshift records (#10680). A pid in this set is proof the
 ...
 * (main-process-preflight). The standalone daemon installs none and `nightshiftd`
 ...
 * the *killing* process's own pid. Nightshift's Chromium processes are children of
 * Electron main, so they never classify `own` from a daemon or nightshiftd host, and
 * on an SSH/serve host there is no Chromium on the machine at all.
 */
export function readNightshiftChromiumProcessPids(): ReadonlySet<number> {
