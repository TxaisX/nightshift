...
describe('profile artifact cloud cleanup', () => {
  it('preserves recovery state when the profile write fails', async () => {
    ...
    expect(() => linkNightshiftProfileToCloud(profileId, cloud('org-b'), userDataPath)).toThrow(
      'profile write failed'
    )
    expect(getArtifactCreateIntent(profileId, userDataPath, '/report.md', scope)).not.toBeNull()

    linkNightshiftProfileToCloud(profileId, cloud('org-b'), userDataPath)
  ...
  it('reconciles an interrupted transition before linking again', async () => {
    ...
    expect(() => linkNightshiftProfileToCloud(profileId, cloud('org-c'), userDataPath)).toThrow(
      'cleanup failed'
    )
    ...
    linkNightshiftProfileToCloud(profileId, cloud('org-c'), userDataPath)
  ...
  it('reconciles an interrupted transition before unlinking', async () => {
    ...
    expect(() => unlinkNightshiftProfileFromCloud(profileId, userDataPath)).toThrow(
      'cleanup failed'
    )
    expect(writeProfileIndex).not.toHaveBeenCalled()
    expect(currentCloud(userDataPath)?.activeOrgId).toBe('org-b')
  ...
  it('preserves an orphaned local marker for an unknown profile', async () => {
    ...
    const scope = shareScope('org-a')
    mkdirSync(getNightshiftProfileDirectory(orphanProfileId, userDataPath), { recursive: true })
    createIntent(userDataPath, scope, orphanProfileId)
    prepareArtifactCloudCleanup(orphanProfileId, userDataPath, undefined)

    const transitions = [
      () => linkNightshiftProfileToCloud(orphanProfileId, cloud('org-b'), userDataPath),
      () => unlinkNightshiftProfileFromCloud(orphanProfileId, userDataPath)
    ]
    for (const transition of transitions) {
      expect(transition).toThrow('unknown_nightshift_profile')
...
function cloud(activeOrgId: string): NightshiftProfileCloudSummary {
  return {
...
function shareScope(cloudOrganizationId: string): ArtifactShareScope {
  return {
    ...
    cloudProfileId: 'cloud-profile-a',
    cloudOrganizationId,
    apiOrigin: 'https://share.nightshift.invalid'
  }
}

async function createLinkedProfile(cloudSummary: NightshiftProfileCloudSummary): Promise<string> {
  const userDataPath = await mkdtemp(join(tmpdir(), 'nightshift-profile-artifact-cleanup-'))
  createdPaths.push(userDataPath)
  loadOrCreateProfileIndex(userDataPath)
  linkNightshiftProfileToCloud(profileId, cloudSummary, userDataPath)
  vi.clearAllMocks()
  return userDataPath
}
...
function currentCloud(userDataPath: string): NightshiftProfileCloudSummary | undefined {
  return readProfileIndex(getNightshiftProfileIndexPath(userDataPath))?.profiles.find(
