...
export type NightshiftCloudAuthConfig = {
  apiBaseUrl: string
...
const PRODUCTION_API_BASE_URL = 'https://login.nightshift.invalid'
const PRODUCTION_CLIENT_ID = 'nightshift-desktop'
const PRODUCTION_RELAY_DIRECTOR_URL = 'https://relay.nightshift.invalid'
...
function isPackagedNightshiftBuild(): boolean {
  try {
...
export function getNightshiftCloudAuthConfig(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedNightshiftBuild()
):
  | { configured: true; config: NightshiftCloudAuthConfig }
  ...
  const configuredApiBaseUrl = env.NIGHTSHIFT_CLOUD_API_URL?.trim()
  ...
  const apiBaseUrl = configuredApiBaseUrl
    ...
    : packaged
      ...
      : null
  const clientId =
    env.NIGHTSHIFT_CLOUD_CLIENT_ID?.trim() || (packaged ? PRODUCTION_CLIENT_ID : undefined)
  ...
  const authBaseUrl = cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_AUTH_URL) ?? apiBaseUrl
  return {
    configured: true,
    config: {
      apiBaseUrl,
      authorizeEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_AUTHORIZE_URL) ??
        endpoint(authBaseUrl, '/v1/desktop/auth/authorize'),
      sessionEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_SESSION_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/session'),
      refreshEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_REFRESH_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/refresh'),
      capabilitiesEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_CAPABILITIES_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/capabilities'),
      profileEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_PROFILE_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/profile'),
      orgEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_ORG_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/org'),
      logoutEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_LOGOUT_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/logout'),
      relayTokenEndpoint:
        cleanEndpointUrl(env.NIGHTSHIFT_CLOUD_RELAY_TOKEN_URL) ??
        endpoint(apiBaseUrl, '/v1/desktop/auth/relay-token'),
      relayDirectorUrl:
        cleanOrigin(env.NIGHTSHIFT_RELAY_URL, allowLoopbackHttp) ?? PRODUCTION_RELAY_DIRECTOR_URL,
      clientId,
      scope: env.NIGHTSHIFT_CLOUD_AUTH_SCOPE?.trim() || DEFAULT_SCOPE
    }
  }
}

export function allowsPlaintextNightshiftCloudSession(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedNightshiftBuild()
): boolean {
  return (
    env.NIGHTSHIFT_CLOUD_ALLOW_PLAINTEXT_SESSION === '1' &&
    env.NODE_ENV !== 'production' &&
    !packaged
  )
}

export function isNightshiftCloudDevAuthEnabled(
  env: NodeJS.ProcessEnv = process.env,
  packaged: boolean = isPackagedNightshiftBuild()
): boolean {
  return env.NIGHTSHIFT_CLOUD_DEV_AUTH === '1' && env.NODE_ENV !== 'production' && !packaged
}
