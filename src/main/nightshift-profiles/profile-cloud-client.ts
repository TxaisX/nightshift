...
export class NightshiftCloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_nightshift_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): NightshiftCloudCapabilities {
...
function normalizeOrganizations(value: unknown): NightshiftCloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): NightshiftProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_nightshift_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): NightshiftCloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_nightshift_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
export function isAmbiguousCloudRequestFailure(error: unknown): boolean {
  return !(error instanceof NightshiftCloudRequestError)
}
...
async function postJson<T>(url: string, body: unknown, options?: PostJsonOptions): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new NightshiftCloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeNightshiftCloudAuthCode(
  config: NightshiftCloudAuthConfig,
  args: ExchangeCodeArgs
): Promise<NightshiftCloudSessionExchangeResponse> {
...
export async function refreshNightshiftCloudCapabilities(
  config: NightshiftCloudAuthConfig,
...
export async function refreshNightshiftCloudSession(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession
): Promise<NightshiftCloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createNightshiftCloudProfile(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
...
export async function selectNightshiftCloudOrg(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
  orgId: string
...
export async function revokeNightshiftCloudSession(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession
): Promise<void> {
  await postJson(
