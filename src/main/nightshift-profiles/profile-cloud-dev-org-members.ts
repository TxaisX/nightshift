...
function devSelf(): NightshiftOrgMember {
  return {
    userId: cleanEnvString(process.env.NIGHTSHIFT_CLOUD_DEV_USER_ID, 'dev-user'),
    email: cleanEnvString(process.env.NIGHTSHIFT_CLOUD_DEV_EMAIL, 'dev@nightshift.local'),
    displayName: cleanEnvString(process.env.NIGHTSHIFT_CLOUD_DEV_DISPLAY_NAME, 'Nightshift Dev'),
    role: 'owner'
  }
}
  ...
  return {
    members: [
      devSelf(),
      {
        ...
        email: 'teammate@nightshift.local',
        displayName: 'Dev Teammate',
        role: 'admin'
      },
      // Why: userId null exercises the "hasn't signed in to Nightshift yet" disabled row.
      {
        userId: null,
        email: 'invited-member@nightshift.local',
        displayName: undefined,
        role: 'member'
      }
    ],
    pendingInvites: [{ email: 'pending@nightshift.local', role: 'member', createdAt: Date.now() }]
  }
}
...
export function listDevNightshiftCloudOrgMembers(orgId: string): NightshiftOrgMembersRoster {
...
export function inviteDevNightshiftCloudOrgMember(
...
export function revokeDevNightshiftCloudOrgInvite(
...
export function changeDevNightshiftCloudOrgMemberRole(
...
export function removeDevNightshiftCloudOrgMember(
