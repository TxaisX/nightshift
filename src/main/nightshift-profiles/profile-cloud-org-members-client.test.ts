...
const config: NightshiftCloudAuthConfig = {
...
const session: NightshiftCloudSession = {
...
describe('Nightshift cloud org members client', () => {
  beforeEach(() => {
    fetchMock.mockReset()
  ...
  it('normalizes the roster, dropping malformed rows and defaulting the viewer role', async () => {
    ...
    await expect(listNightshiftCloudOrgMembers(config, session, 'org-1')).resolves.toEqual({
      members: [
        { userId: 'user-1', email: 'nina@example.com', displayName: 'Nina', role: 'admin' },
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://nightshift-cloud.example/v1/desktop/orgs/org-1/members',
  ...
  it('percent-encodes the org id in the request URL', async () => {
    ...
    await listNightshiftCloudOrgMembers(config, session, 'org/with space')
    expect(fetchMock).toHaveBeenCalledWith(
      'https://nightshift-cloud.example/v1/desktop/orgs/org%2Fwith%20space/members',
      expect.any(Object)
    )
  })
  ...
  it('posts invites with an email and role body', async () => {
    ...
    await inviteNightshiftCloudOrgMember(config, session, {
      orgId: 'org-1',
      email: 'new@example.com',
      role: 'admin'
    })
    expect(fetchMock).toHaveBeenCalledWith(
      'https://nightshift-cloud.example/v1/desktop/orgs/org-1/invites',
      expect.objectContaining({
        method: 'POST',
  ...
  it('carries the server error code on a 409 conflict', async () => {
    ...
    await expect(
      inviteNightshiftCloudOrgMember(config, session, {
        orgId: 'org-1',
  ...
  it('surfaces a 403 as a NightshiftCloudRequestError without an error code when the body is empty', async () => {
    fetchMock.mockResolvedValue({
      ok: false,
      status: 403,
      json: async () => {
    ...
    await expect(
      changeNightshiftCloudOrgMemberRole(config, session, {
        orgId: 'org-1',
        userId: 'user-2',
        role: 'admin'
      })
    ).rejects.toBeInstanceOf(NightshiftCloudRequestError)
    await expect(
      removeNightshiftCloudOrgMember(config, session, { orgId: 'org-1', userId: 'user-2' })
    ).rejects.toMatchObject({ statusCode: 403, errorCode: undefined })
  })

  it('posts invite revocations by email', async () => {
    mockJsonResponse({ ok: true })
    await revokeNightshiftCloudOrgInvite(config, session, {
      orgId: 'org-1',
      email: 'gone@example.com'
    })
    expect(fetchMock).toHaveBeenCalledWith(
      'https://nightshift-cloud.example/v1/desktop/orgs/org-1/invites/revoke',
