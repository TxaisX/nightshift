import type {
  NightshiftOrgMember,
  NightshiftOrgMembersRoster,
  NightshiftOrgPendingInvite,
  NightshiftOrgRole
...
const ORG_ROLES: readonly NightshiftOrgRole[] = ['owner', 'admin', 'member']

function isOrgRole(value: unknown): value is NightshiftOrgRole {
  return typeof value === 'string' && (ORG_ROLES as readonly string[]).includes(value)
}

function normalizeRole(value: unknown, fallback: NightshiftOrgRole): NightshiftOrgRole {
...
function normalizeMember(value: unknown): NightshiftOrgMember | null {
...
function normalizePendingInvite(value: unknown): NightshiftOrgPendingInvite | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  const record = value as Record<string, unknown>
...
function normalizeRoster(value: unknown): NightshiftOrgMembersRoster {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_nightshift_org_members_roster')
  }
  const record = value as Record<string, unknown>
  const members = Array.isArray(record.members)
    ? record.members
        .map(normalizeMember)
        .filter((member): member is NightshiftOrgMember => member !== null)
    : []
  const pendingInvites = Array.isArray(record.pendingInvites)
    ? record.pendingInvites
        .map(normalizePendingInvite)
        .filter((invite): invite is NightshiftOrgPendingInvite => invite !== null)
    : []
  return {
    members,
    pendingInvites,
...
function orgMembersUrl(config: NightshiftCloudAuthConfig, orgId: string, path: string): string {
...
async function requestOrgMembers<T>(
  ...
  if (!response.ok) {
    throw new NightshiftCloudRequestError(response.status, await extractErrorCode(response))
  }
  return parse((await response.json()) as unknown)
}

export async function listNightshiftCloudOrgMembers(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
  orgId: string
): Promise<NightshiftOrgMembersRoster> {
...
export async function inviteNightshiftCloudOrgMember(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
  args: { orgId: string; email: string; role: NightshiftOrgRole }
): Promise<void> {
...
export async function revokeNightshiftCloudOrgInvite(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
...
export async function changeNightshiftCloudOrgMemberRole(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
  args: { orgId: string; userId: string; role: NightshiftOrgRole }
): Promise<void> {
  await requestOrgMembers(
...
export async function removeNightshiftCloudOrgMember(
  config: NightshiftCloudAuthConfig,
  session: NightshiftCloudSession,
