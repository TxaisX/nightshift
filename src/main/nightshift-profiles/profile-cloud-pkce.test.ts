...
async function startedFlow(): Promise<{
  authUrl: URL
  flow: ReturnType<typeof beginNightshiftCloudPkceFlow>
  nonce: string
  redirectUri: string
  state: string
}> {
  const flow = beginNightshiftCloudPkceFlow(config, 'local-default')
...
describe('Nightshift cloud PKCE flow', () => {
  beforeEach(() => {
  ...
  it('keeps the loopback listener alive after an invalid callback', async () => {
    ...
    expect(validResponse.body).toContain('<h1>Signed in to Nightshift</h1>')
  ...
  it('rejects a provider error that matches the flow state', async () => {
    ...
    await expect(observedFlow).resolves.toMatchObject({ message: 'nightshift_cloud_auth_denied' })
  })
