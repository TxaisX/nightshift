...
vi.mock('./profile-cloud-session-store', () => ({
  readNightshiftCloudSession: readMock,
  saveNightshiftCloudSessionIfCurrent: saveIfCurrentMock,
  clearNightshiftCloudSession: clearMock
}))
...
vi.mock('./profile-cloud-client', async (importOriginal) => {
  ...
  return { ...original, refreshNightshiftCloudSession: refreshMock }
})

vi.mock('./profile-cloud-index', () => ({ linkNightshiftProfileToCloud: linkMock }))

import { readFreshNightshiftCloudSession } from './profile-cloud-session-refresh'
import { NightshiftCloudRequestError } from './profile-cloud-client'
import { onNightshiftCloudSessionInvalidated } from './profile-cloud-session-invalidation'
import { forgetAmbiguousRefreshAttempt } from './profile-cloud-refresh-replay-guard'

const config = {} as NightshiftCloudAuthConfig
const active = {
  profile: {
    id: 'profile-1',
    cloud: {
...
} as ActiveNightshiftProfileState
const staleSession = {
...
describe('profile cloud session refresh', () => {
  ...
  it('does not publish a refresh whose persistent mutation snapshot became stale', async () => {
    ...
    const refreshing = readFreshNightshiftCloudSession(config, active, '/data')
  ...
  it('single-flights concurrent rotating refresh-token use per profile and store', async () => {
    ...
    const first = readFreshNightshiftCloudSession(config, active, '/data')
    const second = readFreshNightshiftCloudSession(config, active, '/data')
  ...
  it('notifies subscribers when an auth failure clears the stored session', async () => {
    ...
    const unsubscribe = onNightshiftCloudSessionInvalidated(invalidated)
    refreshMock.mockRejectedValue(new NightshiftCloudRequestError(401))

    await expect(readFreshNightshiftCloudSession(config, active, '/data')).resolves.toEqual({
...
describe('refresh-token replay after an ambiguous attempt', () => {
  ...
  it('never resends a refresh token whose attempt timed out', async () => {
    ...
    await expect(readFreshNightshiftCloudSession(config, active, '/data')).rejects.toThrow(
    ...
    await expect(readFreshNightshiftCloudSession(config, active, '/data')).rejects.toThrow(
      'nightshift_cloud_refresh_replay_blocked'
    )
    expect(refreshMock).toHaveBeenCalledTimes(1)
    expect(clearMock).not.toHaveBeenCalled()
  })
  ...
  it('retries once after a definitive 5xx, which cannot have rotated the token', async () => {
    refreshMock
      .mockRejectedValueOnce(new NightshiftCloudRequestError(503))
      .mockResolvedValueOnce(rotatedResponse)

    const result = await readFreshNightshiftCloudSession(config, active, '/data')
  ...
  it('gives a definitive 5xx exactly one retry', async () => {
    refreshMock.mockRejectedValue(new NightshiftCloudRequestError(503))

    await expect(readFreshNightshiftCloudSession(config, active, '/data')).rejects.toThrow(
      'nightshift_cloud_request_failed_503'
    )
    expect(refreshMock).toHaveBeenCalledTimes(2)
    expect(clearMock).not.toHaveBeenCalled()
  })
  ...
  it('marks a 401 that follows an ambiguous attempt as a possible self-replay', async () => {
    ...
    refreshMock.mockRejectedValueOnce(new NightshiftCloudRequestError(401))
    ...
    expect(refreshMock).toHaveBeenCalledTimes(2)
    expect(warn.mock.calls.flat().join(' ')).toContain('nightshift_cloud_refresh_possible_replay')
    expect(clearMock).toHaveBeenCalledTimes(1)
    expect(invalidated).toHaveBeenCalledTimes(1)
    unsubscribe()
  })
  ...
  it('does not mark a 401 that follows no ambiguous attempt', async () => {
    ...
    expect(warn.mock.calls.flat().join(' ')).not.toContain(
      'nightshift_cloud_refresh_possible_replay'
    )
    expect(clearMock).toHaveBeenCalledTimes(1)
  })
})
