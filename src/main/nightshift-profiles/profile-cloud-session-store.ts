...
import type {
  ...
  NightshiftCloudSessionPersistence
} from '../../shared/nightshift-profiles'
import { getNightshiftProfileDirectory } from './profile-storage-paths'
import { allowsPlaintextNightshiftCloudSession } from './profile-cloud-auth-config'
...
export type NightshiftCloudSession = {
  accessToken: string
  refreshToken: string
  expiresAt: number
...
export type NightshiftCloudSessionReadResult =
  | {
      status: 'found'
      session: NightshiftCloudSession
...
type CachedNightshiftCloudSession = {
  session: NightshiftCloudSession
  persistence: Exclude<NightshiftCloudSessionPersistence, 'none'>
}

const memorySessions = new Map<string, CachedNightshiftCloudSession>()
...
function isNightshiftCloudSession(value: unknown): value is NightshiftCloudSession {
  if (!isObject(value) || !isObject(value.capabilities) || !isObject(value.capabilities.flags)) {
    return false
  }
  if (value.organizations !== undefined && !isNightshiftCloudOrganizations(value.organizations)) {
    return false
  }
  return (
...
function isNightshiftCloudOrganizations(value: unknown): value is NightshiftCloudOrgSummary[] {
...
export function getNightshiftCloudSessionPath(profileId: string, userDataPath: string): string {
  return join(getNightshiftProfileDirectory(profileId, userDataPath), 'account-session.json.enc')
}

export function saveNightshiftCloudSession(
  profileId: string,
  userDataPath: string,
  session: NightshiftCloudSession
): NightshiftCloudSessionPersistence {
  ...
  if (safeStorage.isEncryptionAvailable()) {
    ...
    writeSecureJsonFile(getNightshiftCloudSessionPath(profileId, userDataPath), encrypted)
    memorySessions.set(cacheKey, { session, persistence: 'encrypted' })
    return 'encrypted'
  }

  if (allowsPlaintextNightshiftCloudSession()) {
    ...
    writeSecureJsonFile(getNightshiftCloudSessionPath(profileId, userDataPath), plaintext)
    memorySessions.set(cacheKey, { session, persistence: 'dev-plaintext' })
    return 'dev-plaintext'
  }

  // Why: Nightshift account refresh tokens must not silently fall back to plaintext
...
export function saveNightshiftCloudSessionExchange(
  profileId: string,
  userDataPath: string,
  exchange: NightshiftCloudSessionExchangeResponse
): NightshiftCloudSessionPersistence {
  recordSuccessfulCloudSessionLogin(cloudSessionIdentity(profileId, exchange.cloud), userDataPath)
  return saveNightshiftCloudSession(profileId, userDataPath, {
...
export function saveNightshiftCloudSessionIfCurrent(
  profileId: string,
  userDataPath: string,
  session: NightshiftCloudSession,
  snapshot: CloudSessionMutationSnapshot
): NightshiftCloudSessionPersistence | null {
  ...
  return saveNightshiftCloudSession(profileId, userDataPath, session)
}

export function readNightshiftCloudSession(
  profileId: string,
  userDataPath: string
): NightshiftCloudSessionReadResult {
  const cacheKey = sessionCacheKey(profileId, userDataPath)
  ...
  const path = getNightshiftCloudSessionPath(profileId, userDataPath)
  if (!existsSync(path)) {
  ...
  try {
    ...
    if (parsed.format === 'electron-safe-storage-v1') {
      ...
      const session = JSON.parse(decrypted) as NightshiftCloudSession
      if (!isNightshiftCloudSession(session)) {
    ...
    if (parsed.format === 'dev-plaintext-v1' && allowsPlaintextNightshiftCloudSession()) {
      if (!isNightshiftCloudSession(parsed.session)) {
        return { status: 'decrypt-failed', persistence: 'none', error: 'Invalid saved session.' }
      }
    ...
    if (isUnreadableError(error)) {
      return {
        status: 'unreadable',
        persistence: 'none',
        error: 'Cannot read the saved Nightshift account session: the read failed.'
      }
    }
    return {
      status: 'decrypt-failed',
      persistence: 'none',
      error: 'Could not decrypt saved Nightshift account session.'
    }
  }
}

export function clearNightshiftCloudSession(profileId: string, userDataPath: string): void {
  memorySessions.delete(sessionCacheKey(profileId, userDataPath))
  rmSync(getNightshiftCloudSessionPath(profileId, userDataPath), { force: true })
}
