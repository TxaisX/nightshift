...
import {
  ...
  type CreateLocalNightshiftProfileArgs,
  type CreateLocalNightshiftProfileResult,
  type NightshiftProfileIndex,
  type NightshiftProfileListState,
  type NightshiftProfileSummary
} from '../../shared/nightshift-profiles'
import {
  getNightshiftProfileBrowserSessionMetaFile,
  getNightshiftProfileDataFile,
  getNightshiftProfileDirectory,
...
export {
  ...
  getNightshiftProfileDirectory,
  getNightshiftProfileIndexPath,
  getNightshiftProfilesDirectory,
  initNightshiftProfilePaths
} from './profile-storage-paths'

export type ActiveNightshiftProfileState = {
  index: NightshiftProfileIndex
...
function isProfileSummary(value: unknown): value is NightshiftProfileSummary {
  if (!isObject(value)) {
    return false
  }
...
function normalizeProfileIndex(raw: unknown): NightshiftProfileIndex | null {
...
function readProfileIndexFile(indexPath: string): NightshiftProfileIndex | null {
  try {
...
export function readProfileIndex(indexPath: string): NightshiftProfileIndex | null {
...
export function writeProfileIndex(indexPath: string, index: NightshiftProfileIndex): void {
...
function copyLegacyStateToProfile(userDataPath: string, profileId: string): void {
  const profileDataFile = getNightshiftProfileDataFile(profileId, userDataPath)
  copyIfPresent(legacyDataFilePath(userDataPath), profileDataFile)
  copyIfPresent(
    legacyBrowserSessionMetaPath(userDataPath),
    getNightshiftProfileBrowserSessionMetaFile(profileId, userDataPath)
  )
...
export function seedNewNightshiftProfileTelemetryConsent(
  profileId: string,
  ...
  const dataFile = getNightshiftProfileDataFile(profileId, userDataPath)
...
function createInitialProfileIndex(now = Date.now()): NightshiftProfileIndex {
  const profile = createDefaultLocalNightshiftProfile(now)
  return {
    schemaVersion: NIGHTSHIFT_PROFILE_INDEX_SCHEMA_VERSION,
...
export function loadOrCreateProfileIndex(userDataPath: string): NightshiftProfileIndex {
  const indexPath = getNightshiftProfileIndexPath(userDataPath)
...
function getActiveProfile(index: NightshiftProfileIndex): NightshiftProfileSummary {
  return (
    index.profiles.find((profile) => profile.id === index.activeProfileId) ??
    index.profiles[0] ??
    createDefaultLocalNightshiftProfile(Date.now())
  )
}

export function ensureActiveNightshiftProfile(
  userDataPath = getProfileUserDataPath()
): ActiveNightshiftProfileState {
  const indexPath = getNightshiftProfileIndexPath(userDataPath)
  ...
  const profileDirectory = getNightshiftProfileDirectory(activeProfile.id, userDataPath)
  mkdirSync(profileDirectory, { recursive: true })
  if (activeProfile.id === DEFAULT_LOCAL_NIGHTSHIFT_PROFILE_ID) {
  ...
  return {
    ...
    dataFile: getNightshiftProfileDataFile(activeProfile.id, userDataPath),
    profileDirectory
  }
}

export function isDefaultLocalNightshiftProfileId(profileId: string): boolean {
  return profileId === DEFAULT_LOCAL_NIGHTSHIFT_PROFILE_ID
}

export function getNightshiftProfileListState(
  userDataPath = getProfileUserDataPath()
): NightshiftProfileListState {
  const { index } = ensureActiveNightshiftProfile(userDataPath)
  return {
...
export function createLocalNightshiftProfile(
  ...
  const profile: NightshiftProfileSummary = {
    id: `local-${randomUUID()}`,
    name,
    avatar: {
      kind: 'initials',
      initials: (
        name.match(/[A-Za-z0-9]/)?.[0] ?? DEFAULT_LOCAL_NIGHTSHIFT_PROFILE_NAME[0]
      ).toUpperCase(),
      color: 'neutral'
    },
    kind: 'local',
  ...
  const nextIndex: NightshiftProfileIndex = {
    ...index,
    profiles: [...index.profiles, profile]
  }
  mkdirSync(getNightshiftProfileDirectory(profile.id, userDataPath), { recursive: true })
  writeProfileIndex(getNightshiftProfileIndexPath(userDataPath), nextIndex)
  return {
...
export function setActiveNightshiftProfile(
  profileId: string,
  userDataPath = getProfileUserDataPath()
): NightshiftProfileListState {
  const index = loadOrCreateProfileIndex(userDataPath)
  ...
  if (!found) {
    throw new Error('unknown_nightshift_profile')
  }
  const nextIndex: NightshiftProfileIndex = {
    ...index,
  ...
  mkdirSync(getNightshiftProfileDirectory(profileId, userDataPath), { recursive: true })
  writeProfileIndex(getNightshiftProfileIndexPath(userDataPath), nextIndex)
  return {
    activeProfileId: nextIndex.activeProfileId,
    profiles: nextIndex.profiles
