...
function profile(
  id: string,
  name: string,
  kind: NightshiftProfileKind = 'local'
): NightshiftProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function writeProfileState(profileId: string, repos: Repo[]): void {
  ...
  const dataFile = join(testState.dir, 'profiles', profileId, 'nightshift-data.json')
...
describe('profile project presence', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'nightshift-profile-presence-'))
    writeIndex()
  })
  ...
  it('finds matching projects in other profiles while excluding the active profile', async () => {
    writeProfileState('personal', [
      makeRepo({
        id: 'personal-repo',
        path: 'C:\\Code\\Nightshift',
        displayName: 'Personal Nightshift'
      })
    ])
    writeProfileState('work', [
      makeRepo({ id: 'work-repo', path: 'C:\\Code\\Nightshift', displayName: 'Work Nightshift' })
    ])

    const { findNightshiftProfileProjectsByPath } = await loadPresenceModule()
    const result = findNightshiftProfileProjectsByPath(
      {
        path: 'c:/code/nightshift/',
  ...
  it('keeps SSH projects separate from local projects with the same path', async () => {
    writeProfileState('personal', [
      makeRepo({ id: 'local-repo', path: '/srv/nightshift', displayName: 'Local Nightshift' })
    ])
    writeProfileState('work', [
      makeRepo({
        id: 'ssh-repo',
        path: '/srv/nightshift',
        displayName: 'SSH Nightshift',
