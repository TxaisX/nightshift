...
function profile(id: string, name: string): NightshiftProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'nightshift-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'nightshift-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/nightshift-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:/workspace/nightshift-nautilus': { exhaustedTiers: 0, names: ['seahorse'] }
        },
        worktreeMeta: {
    ...
    const { transferNightshiftProfileProject } = await loadTransferModule()
    const result = transferNightshiftProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/nightshift-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/nightshift' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/nightshift-feature'
    ...
    writeProfileState(
      ...
      makeState({
        ...
        retiredWorktreeNamesByNamespace: {
          'ssh:ssh-1:posix:/srv/nightshift-nightshift-retirement-probe': {
            exhaustedTiers: 0,
            names: ['seahorse']
          }
        },
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:nightshift-profile-personal-deadbeef-browser-default',
    ...
    expect(target.retiredWorktreeNamesByNamespace).toEqual({
      'ssh:builder.example.com|22|dev:posix:/srv/nightshift-nightshift-retirement-probe': {
        exhaustedTiers: 0,
        names: ['seahorse']
      }
    })
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\Nightshift\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/nightshift' })]
      })
    )
