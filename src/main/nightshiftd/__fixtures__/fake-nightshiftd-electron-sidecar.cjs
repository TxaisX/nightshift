/**
 * Stands in for an installed Nightshift app running `--serve`, so the nightshiftd Electron
 ...
 * listen on a local transport, then publish `nightshift-runtime.json` there.
 *
 * Test-only channels (injected by the spawn wrapper, never by production code):
 *   NIGHTSHIFT_FAKE_SIDECAR_LOG      JSONL of every request received.
 *   NIGHTSHIFT_FAKE_SIDECAR_CONTROL  JSON re-read per request: { capabilities, errors }.
 *   NIGHTSHIFT_FAKE_SIDECAR_MODE     'exit-before-ready' to die without publishing.
 */
'use strict'
...
const logPath = process.env.NIGHTSHIFT_FAKE_SIDECAR_LOG
const controlPath = process.env.NIGHTSHIFT_FAKE_SIDECAR_CONTROL

if (!userDataDir) {
  process.stderr.write('fake sidecar: missing --user-data-dir\n')
  process.exit(2)
}
if (process.env.NIGHTSHIFT_FAKE_SIDECAR_MODE === 'exit-before-ready') {
  process.exit(3)
}
...
const endpoint =
  process.platform === 'win32'
    ? `\\\\.\\pipe\\nightshiftd-fake-sidecar-${process.pid}`
...
server.listen(endpoint, () => {
  const metadataPath = join(userDataDir, 'nightshift-runtime.json')
