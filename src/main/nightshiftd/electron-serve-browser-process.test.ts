...
import type { RuntimeBrowserCommandHost } from '../runtime/nightshift-runtime-browser'
...
const FAKE_SIDECAR = join(
  import.meta.dirname,
  '__fixtures__',
  'fake-nightshiftd-electron-sidecar.cjs'
)
const INSTALLED_EXECUTABLE = join(
  '/Applications',
  'Nightshift.app',
  'Contents',
  'MacOS',
...
/** Every key the provider must strip so the sidecar cannot inherit nightshiftd's own browser config. */
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'nightshiftd-electron-harness-'))
  ...
  spawnProcessMock.mockImplementation((spec: RunProcessModule.ProcessSpec) =>
    actual.spawnProcess({
      ...
      env: {
        ...spec.env,
        NIGHTSHIFT_FAKE_SIDECAR_LOG: logPath,
        NIGHTSHIFT_FAKE_SIDECAR_CONTROL: controlPath,
        ...(sidecarMode ? { NIGHTSHIFT_FAKE_SIDECAR_MODE: sidecarMode } : {})
      }
    })
  )
})
...
describe('ElectronServeBrowserProcess start-up', () => {
  it('launches the installed app in headless serve mode without nightshiftd browser env', async () => {
    for (const key of AGENT_BROWSER_ENVIRONMENT_KEYS) {
      vi.stubEnv(key, `leaked-${key}`)
    }
    vi.stubEnv('NIGHTSHIFT_HARNESS_UNRELATED', 'preserved')
    ...
    expect(spec.env?.NIGHTSHIFT_HARNESS_UNRELATED).toBe('preserved')
  ...
  it('stops the sidecar process and removes its user data directory', async () => {
    ...
    const metadata = JSON.parse(
      await readFile(join(userDataPath, 'nightshift-runtime.json'), 'utf8')
    ) as { pid: number }
