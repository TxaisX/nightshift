...
} from '../runtime/nightshift-runtime-browser'
...
import { sendNightshiftdSidecarRequest } from './nightshiftd-sidecar-runtime-client'
import {
...
export class ElectronServeBrowserProcess {
  ...
  async start(): Promise<void> {
    ...
    const userDataPath = await mkdtemp(join(temporaryRoot, 'nightshiftd-browser-'))
      ...
      if (metadata) {
        try {
          const status = RuntimeStatusResult.parse(
            await sendNightshiftdSidecarRequest(metadata, 'status.get', undefined, 5_000)
          )
  ...
  private async invoke(
    ...
    if (method === 'browserScreencast') {
      throw new BrowserError(
        ...
        'The nightshiftd Electron provider does not proxy screencast.'
      )
    }
    ...
    let result: unknown
    try {
      result = await sendNightshiftdSidecarRequest(metadata, rpcMethod, params)
    } catch (error) {
      if (
        targetPage &&
