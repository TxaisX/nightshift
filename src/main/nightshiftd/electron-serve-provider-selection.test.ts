...
let provider: NightshiftdBrowserProvider | null
...
beforeEach(async () => {
  harnessRoot = await mkdtemp(join(tmpdir(), 'nightshiftd-provider-selection-'))
...
describe('resolveNightshiftdBrowserProvider Electron preference', () => {
  it('uses the installed Electron app even when a Chromium executable is configured', async () => {
    provider = await resolveNightshiftdBrowserProvider({
      userDataPath: join(harnessRoot, 'state'),
      environment: { NIGHTSHIFT_BROWSER_EXECUTABLE: chromiumExecutable },
  ...
  it('falls back to the operator Chromium when the installed Electron app cannot start', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      '[nightshiftd] Installed Electron browser provider unavailable:',
      expect.anything()
    )
  })
