...
import { resolveNightshiftdBrowserProvider } from './nightshiftd-browser-provider'

const executablePath = process.env.NIGHTSHIFT_BROWSER_EXECUTABLE
...
describe('ExternalChromiumBrowserProcess integration', () => {
  it.runIf(Boolean(executablePath))(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'nightshiftd-external-browser-'))
      ...
      const provider = await resolveNightshiftdBrowserProvider({
        userDataPath: root,
        environment: { NIGHTSHIFT_BROWSER_EXECUTABLE: executablePath },
