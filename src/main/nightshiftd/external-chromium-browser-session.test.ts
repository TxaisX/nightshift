...
const BASE = {
  executablePath: '/opt/nightshift/chromium',
  profilePath: '/state/browser-chromium',
  sessionName: 'nightshift-nightshiftd-0123456789abcdef'
}
...
describe('nightshiftd external-chromium agent-browser environment', () => {
  beforeEach(() => {
  ...
  // Why (#16367): nightshiftd is the backend a remote user's Chromium hangs off, and this session
  // never passes --cdp, so a daemon an earlier nightshiftd left behind is not bound to the dead
  ...
  // that is the killed-nightshiftd case the stable session name exists to recover.
