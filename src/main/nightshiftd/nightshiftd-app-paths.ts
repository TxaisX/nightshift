/**
 * The path half of nightshiftd's `AppEnvironment`: every `AppPathName` gets a real answer for
...
/** XDG-ish data root. `$NIGHTSHIFT_USER_DATA` wins so a smoke test can isolate state. */
export function resolveUserDataPath(): string {
  const explicit = env('NIGHTSHIFT_USER_DATA')
  if (explicit) {
    return explicit
  }
  const xdg = env('XDG_DATA_HOME')
  return xdg ? join(xdg, 'Nightshift') : join(homedir(), '.nightshift')
}
...
/**
 * The directory nightshiftd was launched from — the bundle root that `nightshiftd.js` and the
 ...
 */
export function resolveNightshiftdInstallRoot(scriptPath = process.argv[1]): string {
  if (!scriptPath) {
    throw new Error(
      'nightshiftd_install_root_unavailable — process.argv[1] is unset, so this process has no ' +
...
export function resolveNightshiftdPath(name: AppPathName): string {
  switch (name) {
    case 'userData':
