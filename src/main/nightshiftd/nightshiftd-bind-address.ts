/**
 * The address `nightshiftd` binds its RPC listener to.
 *
 * Why loopback by default: the desktop stays on loopback until the user pairs, and the
 * shipping design reaches a remote nightshiftd over an SSH local port-forward — so the wide
 * bind nightshiftd had was both a departure from the desktop's posture and unnecessary for the
 * deploy model. Exposure is now something an operator asks for by name.
 */
import { isIP } from 'node:net'

export const NIGHTSHIFTD_LOOPBACK_BIND_HOST = '127.0.0.1'
const ALL_INTERFACES_V4 = '0.0.0.0'
const ALL_INTERFACES_V6 = '::'

export class NightshiftdBindAddressError extends Error {
  readonly code = 'nightshiftd_invalid_bind_address'
}
 ...
 */
export function resolveNightshiftdBindHost(raw?: string): string {
  if (raw === undefined) {
    return NIGHTSHIFTD_LOOPBACK_BIND_HOST
  }
  const value = raw.trim()
  if (value === '') {
    throw new NightshiftdBindAddressError('--bind expects an address')
  }
  if (value === 'localhost') {
    return NIGHTSHIFTD_LOOPBACK_BIND_HOST
  }
  if (isIP(value) === 0) {
    throw new NightshiftdBindAddressError(
...
export function describeNightshiftdBindExposure(host: string): string {
  return bindHostIsNetworkExposed(host)
    ? `nightshiftd is bound to ${host} and is reachable from the network. Anything that can reach ` +
        'this port can attempt pairing.'
    : `nightshiftd is bound to ${host} (local only). Reach it from another machine with an SSH ` +
