...
import { nightshiftdAgentBrowserNativeName } from './nightshiftd-agent-browser-binary'
import {
...
beforeEach(async () => {
  root = await mkdtemp(join(tmpdir(), 'nightshiftd-browser-provider-'))
...
describe('ExternalChromiumBrowserProcess', () => {
  it('navigates, evaluates, and screenshots through the operator browser', async () => {
    ...
    const commands = processHandle.createCommands({
      ...
      // NightshiftRuntimeService, which owns the client-hosted registries.
...
describe('cross-platform browser provider paths', () => {
  it('resolves installed Electron launchers on macOS, Linux, and Windows', () => {
    expect(installedElectronCandidates('darwin', '/Users/test', {})).toContain(
      '/Users/test/Applications/Nightshift.app/Contents/MacOS/Nightshift'
    )
    expect(installedElectronCandidates('linux', '/home/test', {})).toContain(
      '/home/test/.local/bin/nightshift-ide'
    )
    expect(
      installedElectronCandidates('win32', 'C:\\Users\\test', {
        LOCALAPPDATA: 'C:\\Users\\test\\AppData\\Local'
      })
    ).toContain('C:\\Users\\test\\AppData\\Local\\Programs\\Nightshift\\Nightshift.exe')
  })

  it('uses platform-specific bundled agent-browser names', () => {
    expect(nightshiftdAgentBrowserNativeName('darwin', 'arm64')).toBe('agent-browser-darwin-arm64')
    expect(nightshiftdAgentBrowserNativeName('linux', 'x64')).toBe('agent-browser-linux-x64')
    expect(nightshiftdAgentBrowserNativeName('win32', 'x64')).toBe('agent-browser-win32-x64.exe')
  })
})

describe('resolveNightshiftdBrowserProvider', () => {
  it('uses NIGHTSHIFT_BROWSER_EXECUTABLE when Electron is absent', async () => {
    ...
    const provider = await resolveNightshiftdBrowserProvider({
      userDataPath: root,
      environment: { NIGHTSHIFT_BROWSER_EXECUTABLE: executable },
  ...
  it('reports an NIGHTSHIFT_BROWSER_EXECUTABLE path that does not exist', async () => {
    const missing = join(root, 'absent-chromium')

    await expect(
      resolveNightshiftdBrowserProvider({
        userDataPath: root,
        environment: { NIGHTSHIFT_BROWSER_EXECUTABLE: missing },
        resolveInstalledElectronExecutable: async () => null,
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
  ...
  it.skipIf(process.platform === 'win32')(
    'separates a non-executable NIGHTSHIFT_BROWSER_EXECUTABLE from a missing one',
    async () => {
  ...
  it('reports a failed Electron start instead of staying invisible to the client', async () => {
    ...
    await expect(
      resolveNightshiftdBrowserProvider({
        userDataPath: root,
        environment: {},
        resolveInstalledElectronExecutable: async () => '/opt/Nightshift/nightshift-ide',
        resolveAgentBrowserBinary: () => '/agent-browser'
      })
    ).resolves.toBeNull()
    expect(runtimeBrowserUnavailableCause()).toEqual({
  ...
  it('prefers the configured Chromium fault over an earlier Electron failure', async () => {
    ...
    await expect(
      resolveNightshiftdBrowserProvider({
        userDataPath: root,
        environment: { NIGHTSHIFT_BROWSER_EXECUTABLE: join(root, 'absent-chromium') },
        resolveInstalledElectronExecutable: async () => '/opt/Nightshift/nightshift-ide',
        resolveAgentBrowserBinary: () => null
      })
    ).resolves.toBeNull()
