...
import { resolveNightshiftdAgentBrowserBinary } from './nightshiftd-agent-browser-binary'
import { ElectronServeBrowserProcess } from './electron-serve-browser-process'

export type NightshiftdBrowserProvider = {
...
export type NightshiftdBrowserProviderOptions = {
  userDataPath: string
...
export function installedElectronCandidates(
  ...
  if (platform === 'darwin') {
    return [
      ...
      joinPath(homePath, 'Applications', 'Nightshift.app', 'Contents', 'MacOS', 'Nightshift')
    ]
  }
  if (platform === 'win32') {
    return [
      ...(environment.LOCALAPPDATA
        ? [joinPath(environment.LOCALAPPDATA, 'Programs', 'Nightshift', 'Nightshift.exe')]
        : []),
      ...(environment.ProgramFiles
        ? [joinPath(environment.ProgramFiles, 'Nightshift', 'Nightshift.exe')]
        : [])
    ]
  }
  return [
    joinPath(homePath, '.local', 'bin', 'nightshift-ide'),
    '/usr/local/bin/nightshift-ide',
    '/usr/bin/orca-ide',
...
async function startProvider(
  ...
  userDataPath: string
): Promise<NightshiftdBrowserProvider> {
...
export async function resolveNightshiftdBrowserProvider(
  options: NightshiftdBrowserProviderOptions
): Promise<NightshiftdBrowserProvider | null> {
  ...
  if (installedElectronExecutable) {
    try {
      ...
      console.warn('[nightshiftd] Installed Electron browser provider unavailable:', error)
  ...
  // problem, not an unconfigured host. Telling someone to set NIGHTSHIFT_BROWSER_EXECUTABLE when
  // they already did is the misdirection this ordering exists to prevent.
  const chromiumExecutable = environment.NIGHTSHIFT_BROWSER_EXECUTABLE?.trim()
  if (!chromiumExecutable) {
    return declined(electronFailure ?? { reason: 'unconfigured' })
  }

  const agentBrowserPath = (
    options.resolveAgentBrowserBinary ?? resolveNightshiftdAgentBrowserBinary
  )()
  if (!agentBrowserPath) {
  ...
  try {
    ...
    console.warn('[nightshiftd] External Chromium browser provider unavailable:', error)
