 ...
 * lazy initializer because `nightshiftd-entry` reaches it through `await import('../ipc/pty')`,
 ...
 *
 * Why this builds the bundle instead of skipping without one: no CI job builds nightshiftd and
 * runs vitest. `smoke:nightshiftd-terminal` builds it in the static-analysis job, which never
 * runs vitest; the `nightshiftd_browser` job runs vitest but deliberately does not build nightshiftd.
 * A `runIf(existsSync(...))` guard therefore skips in every shard, forever — the same way
 * an unset NIGHTSHIFT_BROWSER_EXECUTABLE kept the browser provider uncovered.
 *
...
const BUNDLE = join(REPO_ROOT, 'out', 'nightshiftd', 'nightshiftd.js')
const BUILD_SCRIPT = join(REPO_ROOT, 'config', 'scripts', 'build-nightshiftd.mjs')
 ...
 */
function ensureNightshiftdBundle(): void {
...
describe('nightshiftd bundle native load order', () => {
  const dirs: string[] = []
  afterEach(() => {
  ...
  it('does not require node-pty in-process before the entry rejects its argv', () => {
    ensureNightshiftdBundle()
    const dir = mkdtempSync(join(tmpdir(), 'nightshiftd-load-order-'))
    dirs.push(dir)
    ...
    writeFileSync(
      ...
      [
        ...
        '}',
        "process.argv.push('--nightshiftd-load-order-check')",
    ...
    expect(output).toContain('Unknown argument: --nightshiftd-load-order-check')
