...
vi.mock('../daemon/daemon-init', () => ({
  ...
  // nightshiftd shutdown is what would make a nightshiftd restart destructive again.
...
describe('startNightshiftdDaemon', () => {
  it('reports live with the daemon pid once the provider is installed', async () => {
    await expect(startNightshiftdDaemon()).resolves.toEqual({ state: 'live', pid: 4242 })
  })

  it('does not arm the macOS login-session death watch', async () => {
    await startNightshiftdDaemon()
    // That watch retires the daemon when the spawning GUI login session dies. A nightshiftd
  ...
  it('reports degraded when fresh terminals would fall back to the local provider', async () => {
    ...
    const result = await startNightshiftdDaemon()
  ...
  it('fails open when the daemon cannot start at all', async () => {
    ...
    await expect(startNightshiftdDaemon()).resolves.toEqual({
      state: 'unavailable',
...
describe('stopNightshiftdDaemon', () => {
  it('disconnects and never shuts the daemon down', async () => {
    ...
    // The whole point of item 4: a nightshiftd restart is non-destructive only if the daemon
