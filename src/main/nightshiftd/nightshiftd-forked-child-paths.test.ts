...
import { installNightshiftdHostAdapters } from './nightshiftd-entry'

/**
 * The composition, not the pieces: nightshiftd's real host adapters against the real resolver
 * and a real deployment layout on disk. Both halves of this passed their own unit tests
 * while the running server forked a path that has never existed in a nightshiftd deployment.
 */
describe('nightshiftd forked-child paths', () => {
  let deployRoot: string
  let originalArgv: string[]

  beforeEach(() => {
    deployRoot = mkdtempSync(join(tmpdir(), 'nightshiftd-deploy-'))
    // The layout build-nightshiftd.mjs emits: the bundle and its children side by side.
    writeFileSync(join(deployRoot, 'nightshiftd.js'), '')
    writeFileSync(join(deployRoot, 'parcel-watcher-process-entry.js'), '')
    originalArgv = process.argv
    process.argv = [process.execPath, join(deployRoot, 'nightshiftd.js')]
  ...
  it('forks the watcher child shipped beside nightshiftd.js', () => {
  ...
  it('ignores a decoy child under the working directory', () => {
    ...
    const elsewhere = mkdtempSync(join(tmpdir(), 'nightshiftd-cwd-'))
