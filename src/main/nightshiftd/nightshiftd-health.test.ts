...
const { collectNightshiftdHealth, collectTerminalDaemonHealth, computeNightshiftdBuildHash } =
  await import('./nightshiftd-health')
...
describe('collectTerminalDaemonHealth', () => {
  it('reports live only when the daemon answered its own PTY spawn probe', async () => {
    ...
    // The build the LIVE daemon came from, which can legitimately predate this nightshiftd.
    expect(health.buildVersion).toBe('1.2.2')
    expect(health.entryPath).toBe('/opt/nightshiftd/daemon-entry.js')
...
describe('collectNightshiftdHealth', () => {
  it('carries build identity and the Node ABI native addons must match', async () => {
    const health = await collectNightshiftdHealth('1.2.3')
...
describe('computeNightshiftdBuildHash', () => {
  it('changes when the bundle bytes change, even at the same version string', () => {
    const dir = mkdtempSync(join(tmpdir(), 'nightshiftd-build-hash-'))
    const entry = join(dir, 'nightshiftd.js')
    writeFileSync(entry, 'build-a')
    const first = computeNightshiftdBuildHash(entry)
    ...
    expect(computeNightshiftdBuildHash(entry)).not.toBe(first)
  })

  it('answers unknown rather than throwing when the entry cannot be read', () => {
    expect(computeNightshiftdBuildHash(join(tmpdir(), 'definitely-absent-nightshiftd.js'))).toBe(
      'unknown'
    )
    // A process with no argv[1] (an embedded host) still has to publish a readiness payload.
    expect(computeNightshiftdBuildHash('')).toBe('unknown')
  })
})
