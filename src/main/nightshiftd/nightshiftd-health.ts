/**
 * nightshiftd's health surface: the facts a supervisor needs to decide whether this deployment
 * is actually serving, as opposed to merely listening.
 *
 * The load-bearing one is the terminal-daemon verdict. nightshiftd answers RPC from its own
 ...
 * to cross the process boundary: nightshiftd drives it, the daemon performs it, and the verdict
...
export type TerminalDaemonHealth = {
  ...
  /** True only when FRESH terminals are daemon-owned, i.e. survive a nightshiftd restart. */
  ownsFreshSessions: boolean
  pid: number | null
  /** The build the LIVE daemon was forked from, which may predate this nightshiftd after an update. */
...
export type NightshiftdHealth = {
  /** Content hash of the running nightshiftd bundle — the deployed build's identity. */
  buildHash: string
  buildVersion: string
  nodeVersion: string
 ...
 *
 * Why hash the entry and not read a version string: `NIGHTSHIFT_VERSION` is whatever the deploy
 ...
 */
export function computeNightshiftdBuildHash(entryPath = process.argv[1]): string {
  if (!entryPath) {
    return 'unknown'
  }
  try {
...
export async function collectNightshiftdHealth(buildVersion: string): Promise<NightshiftdHealth> {
  return {
    buildHash: computeNightshiftdBuildHash(),
    buildVersion,
