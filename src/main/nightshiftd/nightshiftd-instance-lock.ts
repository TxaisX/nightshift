/**
 * Single-instance ownership of nightshiftd's data root, taken BEFORE the profile is loaded.
 *
 * Two nightshiftds on one data root corrupt it quietly: both load the same profile, both write
 ...
 * long-lived process living under `<root>/daemon`, it deliberately outlives the nightshiftd that
...
export const NIGHTSHIFTD_LOCK_FILE_NAME = 'nightshiftd.lock'

export type NightshiftdInstanceLockCode =
  | 'nightshiftd_data_root_unusable'
  | 'nightshiftd_data_root_wrong_owner'
  | 'nightshiftd_data_root_shared'
  | 'nightshiftd_instance_lock_held'
  | 'nightshiftd_instance_lock_foreign_identity'

export class NightshiftdInstanceLockError extends Error {
  constructor(
    readonly code: NightshiftdInstanceLockCode,
    message: string
  ) {
    super(message)
    this.name = 'NightshiftdInstanceLockError'
  }
}

export type NightshiftdLockRecord = {
  pid: number
...
export type NightshiftdInstanceLock = {
  readonly path: string
  readonly record: NightshiftdLockRecord
  release(): void
}

export type NightshiftdInstanceLockHooks = {
  identity?: () => string
  version?: () => string
  now?: () => Date
...
function parseLockRecord(content: string): NightshiftdLockRecord | null {
  try {
    const parsed: unknown = JSON.parse(content)
    if (!parsed || typeof parsed !== 'object') {
      return null
    }
    const record = parsed as Partial<NightshiftdLockRecord>
 ...
 *
 * Why self-heal first and refuse second: nightshiftd stores credentials unsealed (there is no OS
...
function assertDataRootIsPrivate(dataRoot: string): void {
  ...
  try {
    ...
    throw new NightshiftdInstanceLockError(
      'nightshiftd_data_root_unusable',
      `Cannot stat the nightshiftd data root ${dataRoot}: ${(error as Error).message}`
    )
  }
  ...
  if (uid !== undefined && stats.uid !== uid) {
    throw new NightshiftdInstanceLockError(
      'nightshiftd_data_root_wrong_owner',
      `The nightshiftd data root ${dataRoot} is owned by uid ${stats.uid}, not by uid ${uid} running ` +
        'this process. Give nightshiftd its own data root (NIGHTSHIFT_USER_DATA) or chown this one.'
    )
  }
  ...
  if ((mode & 0o077) !== 0) {
    throw new NightshiftdInstanceLockError(
      'nightshiftd_data_root_shared',
      `The nightshiftd data root ${dataRoot} is accessible to other users (mode ` +
        `${(mode & 0o777).toString(8)}) and could not be tightened. nightshiftd stores credentials ` +
        'there unsealed, so it refuses to start. Run `chmod 700` on it, or point NIGHTSHIFT_USER_DATA ' +
...
/**
 * Take the lock, or throw an `NightshiftdInstanceLockError` naming why.
 *
 * A dead holder's record is reclaimed; a live one, or one belonging to a different identity,
 * is never touched.
 */
export function acquireNightshiftdInstanceLock(
  dataRoot: string,
  hooks: NightshiftdInstanceLockHooks = {}
): NightshiftdInstanceLock {
  ...
  try {
    ...
    throw new NightshiftdInstanceLockError(
      'nightshiftd_data_root_unusable',
      `Cannot create the nightshiftd data root ${dataRoot}: ${(error as Error).message}`
    )
  }
  assertDataRootIsPrivate(dataRoot)

  const lockPath = join(dataRoot, NIGHTSHIFTD_LOCK_FILE_NAME)
  const record: NightshiftdLockRecord = {
    pid: process.pid,
    startedAtMs: readStartedAt(process.pid),
    identity,
    version: (hooks.version ?? (() => process.env.NIGHTSHIFT_VERSION ?? 'unknown'))(),
  ...
  const publish = (): boolean => {
    try {
      ...
      throw new NightshiftdInstanceLockError(
        'nightshiftd_data_root_unusable',
        `Cannot write the nightshiftd instance lock ${lockPath}: ${(error as Error).message}`
      )
    }
  }
  ...
  if (existing && existing.identity !== identity) {
    throw new NightshiftdInstanceLockError(
      'nightshiftd_instance_lock_foreign_identity',
      `The nightshiftd data root ${dataRoot} is locked by identity ${existing.identity} (pid ` +
        `${existing.pid}); this process runs as ${identity}. Two identities sharing one data ` +
        'root corrupts it. Give each its own NIGHTSHIFT_USER_DATA.'
    )
  }
  if (existing && isAlive(existing.pid) && matchesStartTime(existing.pid, existing.startedAtMs)) {
    throw new NightshiftdInstanceLockError(
      'nightshiftd_instance_lock_held',
      `Another nightshiftd (pid ${existing.pid}, started ${existing.acquiredAt || 'unknown'}) already ` +
        `owns the data root ${dataRoot}. Stop it before starting another, or use a different ` +
        'NIGHTSHIFT_USER_DATA.'
    )
  }
  if (!existing) {
    console.warn(
      `[nightshiftd] The instance lock at ${lockPath} is unreadable; reclaiming it. If another nightshiftd ` +
  ...
  try {
    ...
    throw new NightshiftdInstanceLockError(
      'nightshiftd_instance_lock_held',
      `Could not reclaim the stale nightshiftd instance lock at ${lockPath}; another process is ` +
        'holding it. Retry, or stop the other nightshiftd.'
    )
  }
  if (!publish()) {
    ...
    throw new NightshiftdInstanceLockError(
      'nightshiftd_instance_lock_held',
      `Another nightshiftd took the data root ${dataRoot} while this one was reclaiming a stale lock.`
    )
  }
  try {
    unlinkSync(claimPath)
...
function makeLock(lockPath: string, record: NightshiftdLockRecord): NightshiftdInstanceLock {
  let released = false
  return {
    path: lockPath,
    record,
    release: () => {
      ...
      released = true
      // Why re-read before unlinking: a reclaim by a later nightshiftd (after, say, a SIGKILL that
