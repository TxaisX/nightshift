...
import {
  NIGHTSHIFTD_NATIVE_PRECONDITION_EXIT_CODE,
  runNightshiftdNativePreflight
} from './nightshiftd-native-preflight'
import {
...
const harness = (given: NodePtyPreconditionVerdict) => {
  ...
  const continued = runNightshiftdNativePreflight({
    check: () => given,
...
describe('runNightshiftdNativePreflight', () => {
  it('stops the boot on a proven-unloadable binary instead of reaching the require', () => {
    ...
    expect(exit).toHaveBeenCalledWith(NIGHTSHIFTD_NATIVE_PRECONDITION_EXIT_CODE)
  ...
  it('exits with EX_CONFIG so a supervisor does not restart an unequippable host forever', () => {
    expect(NIGHTSHIFTD_NATIVE_PRECONDITION_EXIT_CODE).toBe(78)
    expect(NIGHTSHIFTD_NATIVE_PRECONDITION_EXIT_CODE).not.toBe(1)
  })
