/**
 * The native precondition nightshiftd runs before its runtime loads anything native.
 *
 ...
 */
export const NIGHTSHIFTD_NATIVE_PRECONDITION_EXIT_CODE = 78
 ...
 */
export function runNightshiftdNativePreflight(hooks: NativePreflightHooks = {}): boolean {
  ...
  if (verdict.status === 'blocked') {
    ...
    fail(`nightshiftd: ${formatNodePtyPreconditionReport(verdict, message, hints)}`)
    exit(NIGHTSHIFTD_NATIVE_PRECONDITION_EXIT_CODE)
    return false
  }
  ...
  warn(`nightshiftd: ${formatNodePtyPreconditionReport(verdict, message)}`)
  return true
}
