...
import {
  ...
  resolveNightshiftdPrebuildsDir
...
const dirs: string[] = []
const temp = (): string => {
  const dir = mkdtempSync(join(tmpdir(), 'nightshiftd-slot-'))
  dirs.push(dir)
  return dir
}
afterEach(() => {
  ...
  delete process.env.NIGHTSHIFT_NIGHTSHIFTD_PREBUILDS_DIR
})
...
describe('resolveNightshiftdPrebuildsDir', () => {
  it('looks beside the running bundle', () => {
    expect(resolveNightshiftdPrebuildsDir('/opt/nightshiftd/nightshiftd.js')).toBe(
      join('/opt/nightshiftd', 'prebuilds')
    )
  })

  it('honours an explicit override', () => {
    process.env.NIGHTSHIFT_NIGHTSHIFTD_PREBUILDS_DIR = '/custom/prebuilds'
    expect(resolveNightshiftdPrebuildsDir('/opt/nightshiftd/nightshiftd.js')).toBe(
      '/custom/prebuilds'
    )
  })
...
describe('installPrebuiltSlot', () => {
  ...
  it('installs a Linux slot without claiming a spawn-helper it never execs', () => {
    ...
    // macOS is what made every Linux nightshiftd boot degraded on spawn_helper_missing (#17844).
    const prebuilds = temp()
    const nodePtyDir = temp()
