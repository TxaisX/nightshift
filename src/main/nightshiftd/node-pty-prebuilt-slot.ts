 ...
 * by config/scripts/build-nightshiftd-prebuilds.mjs. An upstream tarball would not do: the patch
 ...
 * `nightshiftd.js` itself, so this stays correct wherever the install directory ends up.
 */
export function resolveNightshiftdPrebuildsDir(entryScript = process.argv[1]): string | null {
  const override = process.env.NIGHTSHIFT_NIGHTSHIFTD_PREBUILDS_DIR
  if (override) {
    return override
  }
...
export function installPrebuiltSlot(options: {
  ...
  const prebuildsDir = options.prebuildsDir ?? resolveNightshiftdPrebuildsDir()
