export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not Nightshift's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Nightshift restart',
    ...
    '      // writeEndpointFile on the Nightshift side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future NIGHTSHIFT_AGENT_HOOK_*',
    ...
    '      console.warn("[nightshift-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after a Nightshift restart. The',
    '  // file is rewritten on every Nightshift start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Nightshift',
