...
function commandPlugin(
  ...
  return {
    pluginKey: `nightshift-samples.${id}`,
    rootDir: `/plugins/${id}`,
    manifest,
...
describe('PluginCommandRegistry', () => {
  ...
  it('uses saved effective bindings to recover conflicting plugins', () => {
    ...
    registry.reconcile(
      [first, second],
      () => true,
      { 'plugin:nightshift-samples.first/tasks': ['Mod+Shift+T'] },
      'linux'
    )
  ...
  it('rejects conflicting saved bindings within one plugin', () => {
    ...
    registry.reconcile(
      ...
      () => true,
      {
        'plugin:nightshift-samples.aliases/tasks': ['Mod+Alt+T'],
        'plugin:nightshift-samples.aliases/sidebar': ['Mod+Alt+T']
      },
      'linux'
    )
