...
describe('PluginContentPackRegistry', () => {
  it('activates all contributions from a plugin atomically', async () => {
    const rootDir = await mkdtemp(join(tmpdir(), 'nightshift-plugin-content-pack-registry-'))
    roots.push(rootDir)
    ...
    const plugin: ValidDiscoveredPlugin = {
      pluginKey: 'nightshift-samples.mixed-content',
      rootDir,
      manifest,
  ...
  it('rolls back valid packs when a VM recipe from the same plugin is invalid', async () => {
    const rootDir = await mkdtemp(join(tmpdir(), 'nightshift-plugin-content-pack-vm-'))
    roots.push(rootDir)
    ...
    const plugin: ValidDiscoveredPlugin = {
      pluginKey: 'nightshift-samples.mixed-recipes',
      rootDir,
      manifest,
  ...
  it('withholds content from a plugin killed during the awaited verification phase', async () => {
    const rootDir = await mkdtemp(join(tmpdir(), 'nightshift-plugin-content-pack-kill-race-'))
    roots.push(rootDir)
    await Promise.all([mkdir(join(rootDir, 'locales')), mkdir(join(rootDir, 'recipes'))])
    await Promise.all([
    ...
    const plugin: ValidDiscoveredPlugin = {
      pluginKey: 'nightshift-samples.kill-race',
      rootDir,
      manifest,
      consentFingerprint: fingerprintPluginConsent(manifest, content.hash),
      consentContentHash: content.hash,
