...
async function readInstallManifest(
  ...
  if (!satisfiesNightshiftEngineRange(hostVersion, parsed.manifest.engines.nightshift)) {
    return {
      ok: false,
      error: `plugin requires Nightshift ${parsed.manifest.engines.nightshift} (this is ${hostVersion})`
    }
  }
