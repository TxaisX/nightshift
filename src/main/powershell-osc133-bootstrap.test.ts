...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('NIGHTSHIFT_OPENCODE_CONFIG_DIR')
    expect(script).toContain('NIGHTSHIFT_MIMOCODE_HOME')
    expect(script).not.toContain('NIGHTSHIFT_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('NIGHTSHIFT_OMP_CODING_AGENT_DIR')
    expect(script).toContain('NIGHTSHIFT_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:NIGHTSHIFT_OMP_STATUS_EXTENSION')
    expect(script).toContain('NIGHTSHIFT_CODEX_HOME')
    expect(script).toContain('NIGHTSHIFT_CODEX_LAUNCH_PREFLIGHT')
    expect(script).toContain('function Global:codex')
    expect(script).not.toContain('$Global:__NightshiftCodexExecutable')
    ...
    const codexHomeRestore = script.indexOf('if ($env:NIGHTSHIFT_CODEX_HOME)')
