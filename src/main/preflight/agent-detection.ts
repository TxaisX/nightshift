/**
 * Agent/tool preflight detection. Split out of `ipc/preflight.ts` so the Nightshift
 ...
 * Refresh — handles the "installed a new CLI, Nightshift doesn't see it yet" case
...
async function executePreflightCheck(
  ...
  if (force) {
    ...
    // runs `glab auth login` for a self-hosted host after Nightshift starts would
