...
export function removeAppImageRuntimeEnv(env: Record<string, string>): void {
  ...
  // AppImage launch can make host commands think they were invoked as Nightshift.
  const appDir = normalizeAppDir(env.APPDIR)
  if (appDir) {
    // Why: AppImage mount paths are for Nightshift's own loader; user shells should
