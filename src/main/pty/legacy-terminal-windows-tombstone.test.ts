...
describe('legacy terminal Windows tombstone text', () => {
  ...
  const makeUserDataDir = (): string => {
    const userData = mkdtempSync(join(tmpdir(), 'nightshift-legacy-shim-win-'))
  ...
  it('rejects stale Windows real-command paths inside the wrapper directory', () => {
    ...
    expectOrdered(cmd, 'if not exist "%nightshift_real%"', ':nightshift_try_candidate')
  ...
  it('resolves Windows fallbacks against PATH only, never the current directory', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      ...
      expect(cmd).toContain('for %%P in ("%nightshift_clean_path:;=" "%") do (')
      expect(cmd).toContain(`if exist "%nightshift_candidate_dir%\\${command}.exe"`)
      ...
      expect(cmd).toContain('set "nightshift_sep=\\"')
      ...
      // Why: a captured NIGHTSHIFT_REAL_* may be relative, and both the existence test and the
      // invocation resolve it against the cwd.
      expect(cmd).toContain('if defined nightshift_real call :nightshift_check_rooted')
      expect(cmd).toContain(
      ...
      // Why the exact block, not an ordering pin: the first ':nightshift_append_path' in the file is
      // the top-level CALL, and the first 'if not defined nightshift_rooted' belongs to a different
      ...
      expect(cmd).not.toContain('call :nightshift_check_rooted "')
      expect(cmd).not.toContain('call :nightshift_try_candidate "')
      expect(cmd).not.toContain('call :nightshift_append_path "')
      expect(cmd).not.toContain('%~1')
      expect(cmd).toContain(
        'set "nightshift_probe=%nightshift_entry%"\r\ncall :nightshift_check_rooted\r\n'
      )
      // Why: nightshift_sep is compared against before the legacy dir is normalized; defining it later
      // made that strip silently no-op and left the legacy dir on PATH.
      expectOrdered(cmd, 'set "nightshift_sep=', 'if "%nightshift_legacy_norm:~-1%."')
      // Why: %nightshift_sep% expands before the line is parsed, so `=="%nightshift_sep%"` becomes `=="\"` —
      // the literal trap the variable exists to avoid. Both sides must carry the trailing dot.
      expect(cmd).not.toContain('=="%nightshift_sep%"')
      ...
      expectOrdered(
        cmd,
        'call :nightshift_check_rooted\r\nif not defined nightshift_rooted exit /b',
        'for %%G in ("%nightshift_entry%") do set "nightshift_candidate_dir='
      )
      expect(cmd).toContain('if not defined nightshift_rooted exit /b')
      expect(cmd).toContain('if "%nightshift_candidate_dir:~-1%."=="%nightshift_sep%."')
      expect(cmd).not.toContain(':\\#=#%')
      ...
      expect(cmd).toContain('if "%nightshift_probe:~1,2%"==":\\" set "nightshift_rooted=1"')
      expect(cmd).toContain('if "%nightshift_probe:~0,2%"=="\\\\" set "nightshift_rooted=1"')
      ...
      expect(cmd).toContain('if not defined nightshift_entry exit /b')
      ...
      expect(cmd).toContain('if "%nightshift_legacy_norm:~-1%."=="%nightshift_sep%."')
      ...
      expect(cmd).not.toContain('if defined nightshift_legacy_wrapper_dir for ')
  ...
  it('guards both cmd PATH walks so an empty variable cannot break parsing', () => {
    ...
    for (const command of ['git', 'gh'] as const) {
      const cmd = readFileSync(join(win32Dir, `${command}.cmd`), 'utf8')
      expect(cmd).toContain('if not defined PATH goto :nightshift_path_walked')
      expect(cmd).toContain(
        'if not defined nightshift_clean_path goto :nightshift_candidates_walked'
      )
      for (const [guard, loop] of [
        ['if not defined PATH goto :nightshift_path_walked', 'for %%P in ("%PATH:;='],
        [
          'if not defined nightshift_clean_path goto :nightshift_candidates_walked',
          'for %%P in ("%nightshift_clean_path:;='
        ]
      ] as const) {
