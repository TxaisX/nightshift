...
// Why: Nightshift launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Nightshift bash never sees.
...
function shellStartupFiles(
  ...
  if (!shell) {
    // Why: Nightshift's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // Nightshift's zsh/bash/fish shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Nightshift does not inherit interactive shell exports, but the
