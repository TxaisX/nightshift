...
function worktreeSetupWslenvEntries(env: Record<string, string | undefined>): string[] {
  return [
    ...
    ...[
      'NIGHTSHIFT_ROOT_PATH',
      'NIGHTSHIFT_WORKTREE_PATH',
      'CONDUCTOR_ROOT_PATH',
      'GHOSTX_ROOT_PATH'
    ].map((name) => `${name}/${env[name]?.startsWith('/') ? 'u' : 'p'}`),
    // A display name, never a path.
    'NIGHTSHIFT_WORKSPACE_NAME/u'
  ]
}

export function addNightshiftWslInteropEnv(env: Record<string, string>): void {
  ...
  env.NIGHTSHIFT_SHELL_READY_ROOT = getShellReadyWrapperRoot()
  ...
  const endpointFlag = env.NIGHTSHIFT_AGENT_HOOK_ENDPOINT?.startsWith('/') ? 'u' : 'p'
  ...
  const opencodeOverlayEntries = (
    ['OPENCODE_CONFIG_DIR', 'NIGHTSHIFT_OPENCODE_CONFIG_DIR'] as const
  )
  ...
  const passthroughEntries = [
    ...
    // and it cannot derive the hash segment from NIGHTSHIFT_USER_DATA_PATH alone.
    'NIGHTSHIFT_SHELL_READY_ROOT/p',
    'NIGHTSHIFT_CLI_COMMAND/u',
    'NIGHTSHIFT_CODEX_LAUNCH_PREFLIGHT/p',
    'NIGHTSHIFT_PANE_KEY/u',
    'NIGHTSHIFT_TAB_ID/u',
    'NIGHTSHIFT_WORKTREE_ID/u',
    'NIGHTSHIFT_AGENT_LAUNCH_TOKEN/u',
    `${SETUP_AGENT_SEQUENCE_STARTUP_COMMAND_ENV}/u`,
    `${SETUP_AGENT_SEQUENCE_STARTUP_SCRIPT_ENV}/u`,
    'NIGHTSHIFT_ORCHESTRATION_COMPATIBILITY_HOST_KIND/u',
    'NIGHTSHIFT_ORCHESTRATION_COMPATIBILITY_HOST_ID/u',
    'NIGHTSHIFT_ORCHESTRATION_COMPATIBILITY_HOST_INCARNATION/u',
    'NIGHTSHIFT_AGENT_HOOK_PORT/u',
    'NIGHTSHIFT_AGENT_HOOK_TOKEN/u',
    'NIGHTSHIFT_AGENT_HOOK_ENV/u',
    'NIGHTSHIFT_AGENT_HOOK_VERSION/u',
    'NIGHTSHIFT_AGENT_HOOK_TRANSPORT/u',
    `NIGHTSHIFT_AGENT_HOOK_ENDPOINT/${endpointFlag}`,
    ...opencodeOverlayEntries,
    'NIGHTSHIFT_WSL_HOOK_RELAY_VERSION/u',
    'NIGHTSHIFT_WSL_HOOK_INSTANCE/u',
    'NIGHTSHIFT_OMP_SOURCE_AGENT_DIR/p',
