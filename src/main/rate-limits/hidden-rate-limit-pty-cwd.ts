...
const WSL_RATE_LIMIT_PTY_CWD_DIR = 'nightshift-rate-limit-pty-cwd'
...
function resolveUserDataRoot(userDataPath?: string | null): string {
  const root = userDataPath?.trim() || process.env.NIGHTSHIFT_USER_DATA_PATH?.trim()
  if (root && !isRootLikePath(root)) {
    return root
  }
  return join(tmpdir(), 'nightshift-rate-limit-pty')
}
...
export function getHiddenRateLimitWslCwdSetupCommands(): string[] {
  return [
    `nightshift_rate_limit_cwd="\${TMPDIR:-/tmp}/${WSL_RATE_LIMIT_PTY_CWD_DIR}"`,
    'mkdir -p "$nightshift_rate_limit_cwd"',
    'cd "$nightshift_rate_limit_cwd"'
  ]
}
