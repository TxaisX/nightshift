...
// The base URL is overridable via the same env var the CLI honours so Nightshift
...
function expiredSessionMessage(home: KimiHomeResolution): string {
  const where =
    home.runtime === 'wsl'
      ...
      : 'on the computer running Nightshift'
 ...
 * Nightshift must NEVER refresh or rewrite that file — a rotated refresh token would
