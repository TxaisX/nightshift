...
describe('fetchMiniMaxRateLimits', () => {
  ...
  it('sets the cookie jar and sends browser-like request headers', async () => {
    ...
    expect(sessionFromPartitionMock).toHaveBeenCalledWith('nightshift-minimax-rate-limit-fetch')
    ...
    expect(init.headers['User-Agent']).not.toContain('nightshift-minimax-usage')
  })
