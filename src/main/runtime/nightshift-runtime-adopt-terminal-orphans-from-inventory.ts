...
import { NightshiftRuntimeWithSubscribeToTerminalResize } from './nightshift-runtime-subscribe-to-terminal-resize'
import type {
...
export class NightshiftRuntimeWithAdoptTerminalOrphansFromInventory extends NightshiftRuntimeWithSubscribeToTerminalResize {
  ...
  async resolveActiveTerminal(
    ...
    // structured worker is covered by the `NIGHTSHIFT_TERMINAL_HANDLE` its child is spawned with. That
    ...
    // pane the guess resolves. Such a child now carries `NIGHTSHIFT_STRUCTURED_SESSION` and the CLI
