...
async function fenceRemoteAgentSessionSpawn(runtime: NightshiftRuntimeService) {
...
describe('agent-session create operation ledger', () => {
  ...
  it('shapes the launch for the route it resolved, not a repo row on another host', async () => {
    ...
    expect(createTerminal).toHaveBeenCalledWith(
      'id:worktree-1',
      expect.objectContaining({ command: expect.stringContaining('nightshift-ide claude-teams') })
    )
  })
