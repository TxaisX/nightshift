...
describe('NightshiftRuntimeService automation methods', () => {
  ...
  it('rejects a mismatched run context when the workspace is the machine selector', async () => {
    ...
    await expect(
      runtime.createAutomation({
        ...
        prompt: 'Review changes',
        agentId: 'codex',
        workspace: 'id:repo-1::/tmp/nightshift',
        runContext: {
          kind: 'workspace-run',
          projectId: 'project-1',
          hostId: 'local',
  ...
  it('passes session reuse updates for existing-workspace automations', async () => {
    const existing = {
      ...existingAutomation,
      workspaceMode: 'existing',
      workspaceId: 'repo-1::/tmp/nightshift',
      baseBranch: null
    } satisfies Automation
  ...
  it('rejects repo-only updates for existing-workspace automations', async () => {
    const existing = {
      ...existingAutomation,
      workspaceMode: 'existing',
      workspaceId: 'repo-1::/tmp/nightshift-worktree',
      baseBranch: null
    } satisfies Automation
    const store = makeStore([existing])
    const runtime = new NightshiftRuntimeService(store as never)
