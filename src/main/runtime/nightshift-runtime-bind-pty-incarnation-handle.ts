...
import { NightshiftRuntimeWithBuildPtyTerminalSummary } from './nightshift-runtime-build-pty-terminal-summary'
import type { PtyIncarnationHandleRecord } from './nightshift-runtime-core'
import type { RuntimeLeafRecord, RuntimePtyWorktreeRecord } from './runtime-terminal-state-records'
import { randomUUID } from 'node:crypto'

export class NightshiftRuntimeWithBindPtyIncarnationHandle extends NightshiftRuntimeWithBuildPtyTerminalSummary {
  ...
  protected rememberDetachedPreAllocatedLeaves(): void {
    for (const leaf of this.leaves.values()) {
      if (leaf.ptyId && this.handleByPtyId.has(leaf.ptyId)) {
        // Why: NIGHTSHIFT_TERMINAL_HANDLE is an agent identity, so CLI control survives renderer graph loss while the PTY is alive.
