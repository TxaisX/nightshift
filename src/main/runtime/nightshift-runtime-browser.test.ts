...
import type { RuntimeBrowserCommandHost } from './nightshift-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:nightshift-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
  ...
  beforeEach(() => {
    ...
    browserSessionRegistryMock.resolveKnownPartition.mockImplementation(
      (profileId: string | null | undefined) => {
        if (!profileId) {
          return 'persist:nightshift-browser'
        }
  ...
  it('sends the resolved isolated profile partition when creating a renderer tab', async () => {
    ...
    expect(send).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        sessionPartition: 'persist:nightshift-browser-session-profile-isolated'
      })
    )
  })
