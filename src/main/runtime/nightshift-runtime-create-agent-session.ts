...
import { NightshiftRuntimeWithGetAgentSessionExecutionNamespace } from './nightshift-runtime-get-agent-session-execution-namespace'
import type {
...
export class NightshiftRuntimeWithCreateAgentSession extends NightshiftRuntimeWithGetAgentSessionExecutionNamespace {
  async createAgentSession(
    ...
    const operation = (async (): Promise<RuntimeCreateAgentSessionResult> => {
      ...
      // NIGHTSHIFT_TERMINAL_HANDLE, so it is the only name a lost spawn can be re-found by.
      reclaim.identity = {
