...
// Why: dedicated file for "one surface" separation (emulator), parallel to nightshift-runtime-browser.ts. Keeps NightshiftRuntimeService focused; emulator routing easy to scan. No max-lines disable (split further if grows; per AGENTS + plan Phase 3).
...
export class RuntimeEmulatorCommands {
  ...
  // devices/AVDs) for the cross-platform `nightshift emulator devices` command.
