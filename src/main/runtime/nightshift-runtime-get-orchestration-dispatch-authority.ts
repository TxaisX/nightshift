...
import { NightshiftRuntimeWithVerifyOrchestrationCompatibilityCaller } from './nightshift-runtime-verify-orchestration-compatibility-caller'
...
export class NightshiftRuntimeWithGetOrchestrationDispatchAuthority extends NightshiftRuntimeWithVerifyOrchestrationCompatibilityCaller {
  ...
  getTerminalOrchestrationCliCommand(handle: string): OrchestrationCliCommand {
    ...
    return resolveTerminalOrchestrationCliCommand({
      ...
      isWsl: pty.isWsl,
      worktreeId: pty.worktreeId,
      // Dev builds run the CLI as `nightshift-dev`; a packaged app must not advertise it.
      runtimeCliCommand: getAppEnvironment().isPackaged() ? undefined : 'nightshift-dev',
