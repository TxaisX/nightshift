import { NightshiftRuntimeWithGetRuntimeId } from './nightshift-runtime-get-runtime-id'
...
import type { RuntimeWorktreeLifecycleEvent } from './nightshift-runtime-core'
import { WORKTREE_CREATE_RESULT_TTL_MS } from './nightshift-runtime-core'
...
export class NightshiftRuntimeWithGetStatus extends NightshiftRuntimeWithGetRuntimeId {
  ...
  getStatus(): RuntimeStatus {
    ...
    const capabilities: RuntimeCapability[] = RUNTIME_CAPABILITIES.filter(
      (capability) =>
        ...
        (process.env.NIGHTSHIFT_E2E_DISABLE_RUNTIME_SHARED_CONTROL !== '1' ||
          capability !== REMOTE_RUNTIME_SHARED_CONTROL_CAPABILITY) &&
        (process.env.NIGHTSHIFT_E2E_DISABLE_PAIRED_TERMINAL_PARKING !== '1' ||
          capability !== TERMINAL_PAIRED_PARKING_RUNTIME_CAPABILITY) &&
        (process.env.NIGHTSHIFT_E2E_DISABLE_AUTHORITATIVE_SESSION_TABS_INVENTORY !== '1' ||
    ...
    // that actually applies, and a host that says "set NIGHTSHIFT_BROWSER_EXECUTABLE" to someone
    ...
    return {
      ...
      // Why: headless nightshift serve cannot create/stream BrowserViews, so clients
