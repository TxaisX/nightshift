// #11761: on a headless `nightshift serve` host the HTTP agent hook is the only carrier of
...
): Promise<NightshiftRuntimeService> {
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: NightshiftRuntimeService) => void
...
function observePaneTitle(runtime: NightshiftRuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: NightshiftRuntimeService): number {
  const pty = (
