...
import { NightshiftRuntimeWithBuildHeadlessMobileSessionBrowserTabs } from './nightshift-runtime-build-headless-mobile-session-browser-tabs'
...
export class NightshiftRuntimeWithPerformMobileSessionPtyRecordsRefresh extends NightshiftRuntimeWithBuildHeadlessMobileSessionBrowserTabs {
    ...
    if (tab.type === 'terminal') {
      ...
      if (shouldMaterializePendingTerminal) {
        ...
        let agentStartup: Awaited<
          ReturnType<NightshiftRuntimeService['resolveMobileSessionTerminalCommand']>
        > = {}
        if (tab.launchAgent) {
          try {
