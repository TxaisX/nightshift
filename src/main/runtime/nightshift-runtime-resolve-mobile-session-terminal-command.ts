...
import { NightshiftRuntimeWithRunCreateMobileSessionTerminal } from './nightshift-runtime-run-create-mobile-session-terminal'
...
export class NightshiftRuntimeWithResolveMobileSessionTerminalCommand extends NightshiftRuntimeWithRunCreateMobileSessionTerminal {
  protected async resolveMobileSessionTerminalCommand(
    ...
    // Why: SSH runs the CLI through the relay shim (plain `nightshift`), so the Linux-only `nightshift-ide` rename must not apply.
