...
function runtimeWith(settings: Record<string, unknown>): NightshiftRuntimeService {
  return new NightshiftRuntimeService({ getSettings: () => settings } as never)
}
...
describe('structured agent-session launch args wiring', () => {
  ...
  it('supplies the Claude env overlay so the launch resolver does not fall back to process.env', async () => {
    const deps = await installedDeps({
      agentDefaultArgs: {},
      agentDefaultEnv: {
        claude: { NIGHTSHIFT_CLAUDE_OVERLAY: 'claude-value' },
        codex: { NIGHTSHIFT_CODEX_OVERLAY: 'codex-value' }
      }
    })

    expect(deps.resolveClaudeLaunchEnv).toBeTypeOf('function')
    expect(deps.resolveClaudeLaunchEnv?.()).toMatchObject({
      NIGHTSHIFT_CLAUDE_OVERLAY: 'claude-value'
    })
    expect(deps.resolveClaudeLaunchEnv?.()).not.toHaveProperty('NIGHTSHIFT_CODEX_OVERLAY')
    expect(deps.resolveLaunchEnvOverlay()).toMatchObject({
      NIGHTSHIFT_CODEX_OVERLAY: 'codex-value'
    })
  })
})
