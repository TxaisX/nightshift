...
function syncSplit(runtime: NightshiftRuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('NightshiftRuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new NightshiftRuntimeService(
      runtimeStore({ getWorkspaceSession: () => session })
    )
    runtime.attachWindow(1)
