...
import { InMemoryOrchestrationMessages } from './nightshift-runtime-test-orchestration-messages.spec'
import type { OrchestrationDb } from './orchestration/db'
import type { PtyProcessInspection } from '../providers/pty-process-inspection'

type RuntimeService = InstanceType<typeof NightshiftRuntimeService>
...
function antigravityReadyScreen(model = 'Gemini 3.5 Flash (High)'): string {
  return [
    ...
    model,
    '~/nightshift/workspaces/nightshift/agy-dispatch-issue',
    '>'
  ].join('\n')
}
...
function expectStablePaneKeyEnv(env: Record<string, string>): string {
  expect(env.NIGHTSHIFT_TAB_ID).toMatch(UUID_RE)
  const leafId = env.NIGHTSHIFT_PANE_KEY?.slice(`${env.NIGHTSHIFT_TAB_ID}:`.length)
  expect(leafId).toMatch(UUID_RE)
  expect(env.NIGHTSHIFT_PANE_KEY).toBe(`${env.NIGHTSHIFT_TAB_ID}:${leafId}`)
  return env.NIGHTSHIFT_PANE_KEY
}

function createRuntime(): RuntimeService {
  return new NightshiftRuntimeService(store)
}
