...
// because nightshift-runtime.test.ts imports them. A fragment left out of that list never runs and
// nothing fails, so the import list is the coverage boundary and has to be checked.
const ENTRYPOINT = 'nightshift-runtime.test.ts'
const FRAGMENT_DIR = 'nightshift-runtime-tests'

function importedFragments(): string[] {
  const source = readFileSync(join(import.meta.dirname, ENTRYPOINT), 'utf8')
  return [...source.matchAll(/await import\('\.\/nightshift-runtime-tests\/([\w-]+)\.spec'\)/g)]
...
describe('Nightshift runtime test fragments', () => {
