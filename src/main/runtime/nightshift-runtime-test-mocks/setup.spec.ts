...
import type { NightshiftRuntimeService as NightshiftRuntimeServiceConstructor } from '../nightshift-runtime'
...
export function acknowledgeAgentPromptSubmit(
  runtime: InstanceType<typeof NightshiftRuntimeServiceConstructor>,
  ptyId: string,
  data: string
): void {
  if (data === '\r') {
...
vi.mock('../../hooks', () => ({
  ...
  parseNightshiftYaml: vi.fn().mockReturnValue(null)
}))
