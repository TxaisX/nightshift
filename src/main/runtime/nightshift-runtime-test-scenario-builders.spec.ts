import * as mocks from './nightshift-runtime-test-mocks.spec'
import type { Mock } from 'vitest'

const { HEADLESS_RUNTIME_WINDOW_ID, NightshiftRuntimeService, electronMocks } = mocks
const { getBrowserHostLeaseRegistry, getDefaultWorkspaceSession, getRuntimeBrowserPageRegistry } =
  mocks
const { gitRunner, vi } = mocks

import * as fixtures from './nightshift-runtime-test-fixtures.spec'
...
// Why (#10333): `nightshift serve` publishes a ready graph under
...
function createWorktreeRemovalRuntime(runtimeStore: unknown = store): RuntimeService {
  ...
  return new NightshiftRuntimeService(runtimeStore as never, undefined, {
