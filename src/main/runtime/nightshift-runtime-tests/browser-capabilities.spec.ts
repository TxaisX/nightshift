...
import type { RuntimeMobileSessionTabsResult } from '../nightshift-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  TEST_WINDOW_ID,
  TEST_WORKTREE_ID,
...
} from '../nightshift-runtime-test-scenario-builders.spec'
...
describe('NightshiftRuntimeService', () => {
  ...
  it('reports the driver as missing instead of telling a configured operator to configure it', () => {
    ...
    expect(degradation).toEqual({
      ...
      message:
        'NIGHTSHIFT_BROWSER_EXECUTABLE is set, but the bundled agent-browser driver is missing or not executable on this host, so Chromium cannot be driven.'
    })
    // The whole point: never send someone to set a variable they already set.
    expect(degradation?.message).not.toMatch(/set NIGHTSHIFT_BROWSER_EXECUTABLE/)
  })
  ...
  it('keeps the degradation wire-safe for peers that predate structured causes', () => {
    ...
    expect(degradation?.message).toBe(
      'NIGHTSHIFT_BROWSER_EXECUTABLE points at a path that does not exist. (/nope/chromium)'
    )
  })
