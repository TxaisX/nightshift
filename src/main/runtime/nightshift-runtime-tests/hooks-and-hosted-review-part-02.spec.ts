...
describe('NightshiftRuntimeService', () => {
  ...
  it('rejects hosted review worktree selectors outside the selected repo', async () => {
    ...
    const runtime = new NightshiftRuntimeService(multiRepoStore as never)
  ...
  it('routes local WSL project hosted review flows through runtime git options', async () => {
    ...
    const runtime = new NightshiftRuntimeService(wslStore as never)
    getHostedReviewForBranchMock.mockResolvedValueOnce({
      provider: 'github',
      number: 76,
      title: 'Feature WSL',
      state: 'open',
      url: 'https://github.com/acme/nightshift/pull/76',
      status: 'success',
    ...
    createHostedReviewMock.mockResolvedValueOnce({
      ok: true,
      number: 77,
      url: 'https://github.com/acme/nightshift/pull/77'
    })
