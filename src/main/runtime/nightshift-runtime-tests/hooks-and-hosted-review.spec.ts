...
import { TEST_REPO_ID, TEST_REPO_PATH, store } from '../nightshift-runtime-test-fixtures.spec'
...
describe('NightshiftRuntimeService', () => {
  describe('checkRepoHooks status', () => {
    ...
    it('reports ok for a missing remote nightshift.yaml and error for any other read failure', async () => {
  ...
  it('resolves SSH issue commands from shared nightshift.yaml and deletes empty overrides', async () => {
    const remoteStore = {
      ...store,
      getRepos: () => [
        {
    ...
    vi.mocked(parseNightshiftYaml).mockReturnValue({
      scripts: {},
    ...
    const fsProvider = {
      readFile: vi.fn(async (filePath: string) => {
        ...
        if (filePath.endsWith('nightshift.yaml')) {
    ...
    try {
      await expect(runtime.readRepoIssueCommand('id:repo-1')).resolves.toMatchObject({
        ...
        localFilePath: '/remote/repo/.nightshift/issue-command',
        source: 'shared'
      })
  ...
  it('allows host integration slug helpers for SSH repos through provider-aware GitHub clients', async () => {
    const prRepo = { owner: 'acme', repo: 'nightshift', host: 'github.acme.test' }
  ...
  it('routes runtime GitHub issue and work-item actions through the selected WSL project runtime', async () => {
    ...
    createGitHubIssueMock.mockResolvedValueOnce({
      ok: true,
      number: 12,
      url: 'https://github.com/acme/nightshift/issues/12'
    })
