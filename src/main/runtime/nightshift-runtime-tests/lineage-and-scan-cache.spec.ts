...
describe('NightshiftRuntimeService', () => {
  ...
  it('rejects SSH lineage updates when Nightshift worktree identity is missing', async () => {
    const remoteRepo = {
      id: 'remote-repo',
      path: '/home/user/repo',
      displayName: 'remote',
  ...
  it('rejects local lineage updates when Nightshift worktree identity is missing', async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), 'nightshift-runtime-lineage-'))
    ...
    try {
      ...
      await expect(lstat(join(childPath, '.nightshift'))).rejects.toThrow()
      await expect(lstat(join(parentPath, '.nightshift'))).rejects.toThrow()
      expect(setWorktreeLineage).not.toHaveBeenCalled()
    } finally {
