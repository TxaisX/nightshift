...
import { NightshiftRuntimeService, electronMocks } from '../nightshift-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
import { makePendingAgentTabActivationRuntime } from '../nightshift-runtime-test-scenario-builders.spec'
...
describe('NightshiftRuntimeService', () => {
  ...
  it('durably tears down a runtime-owned SSH headless tab when renderer cleanup fails', async () => {
    ...
    const runtime = new NightshiftRuntimeService({ ...runtimeStore, flushOrThrow } as never)
    runtime.setPtyController({
      write: () => true,
      kill,
      getForegroundProcess: async () => null,
  ...
  it('retires an SSH-owned surface when a stale renderer acknowledges close after relay recovery', async () => {
    ...
    runtime = new NightshiftRuntimeService(runtimeStore as never)
    runtime.setNotifier({ closeTerminal, closeTerminalTab } as never)
    runtime.setPtyController({
      write: () => true,
      kill,
