...
import { listWorktrees } from '../nightshift-runtime-test-mocks.spec'
import {
  TEST_REPO_ID,
  createRuntime,
  syncSinglePty
...
describe('NightshiftRuntimeService', () => {
  ...
  // clear stripping the spinner off Nightshift's synthesized title after 3s of quiet output, and
  ...
  it('only records the bare Cursor native title via the stale-working clear', async () => {
    ...
    try {
      ...
      // Nightshift's synthesized spinner, then quiet output: the clear strips it to the bare title.
      runtime.onPtyData(ptyId, '\x1b]0;⠋ Cursor Agent\x07', 101)
