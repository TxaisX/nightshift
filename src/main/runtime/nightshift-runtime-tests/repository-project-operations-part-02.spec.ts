...
describe('NightshiftRuntimeService', () => {
  it('adopts public clone repos into host-qualified project setup', async () => {
    const destination = await mkdtemp(join(tmpdir(), 'nightshift-runtime-project-clone-'))
    ...
    try {
      const cloned = await runtime.cloneRepo('https://example.com/nightshift.git', destination)
  ...
  it('keeps project clone repos split by runtime host on the same clone path', async () => {
    ...
    try {
      const result = await runtime.setupProjectClone({
        ...
        url: 'https://example.com/nightshift.git',
        destination
      })
  ...
  it('defaults runtime createRepo badgeColor to DEFAULT_REPO_BADGE_COLOR', async () => {
    ...
    const runtime = new NightshiftRuntimeService(colorStore as never)
    const parentDir = await mkdtemp('/tmp/nightshift-runtime-create-')
    try {
  ...
  it('creates a missing runtime parent before creating the project directory', async () => {
    ...
    const runtime = new NightshiftRuntimeService(createStore as never)
    const tempRoot = await mkdtemp(join(tmpdir(), 'nightshift-runtime-create-parent-'))
    const parentDir = join(tempRoot, 'nightshift', 'projects')
    try {
  ...
  it('prepares the runtime worktree root when creating a repo', async () => {
    ...
    const runtime = new NightshiftRuntimeService(runtimeStore as never)
    const parentDir = await mkdtemp(join(tmpdir(), 'nightshift-runtime-create-root-prep-'))
    try {
  ...
  it('drops a same-path negative submodule cache before runtime cloneRepo', async () => {
    const spawnSpy = vi.spyOn(gitRunner, 'gitSpawnAfterWindowsEnvironmentReady')
    const destination = await mkdtemp(join(tmpdir(), 'nightshift-runtime-reclone-'))
    ...
    const runtime = new NightshiftRuntimeService(cloneStore as never)
  ...
  it('rejects runtime cloneRepo dot-segment URLs before spawning git', async () => {
    ...
    const tempRoot = await mkdtemp(join(tmpdir(), 'nightshift-runtime-clone-'))
  ...
  it('removes an owned runtime clone target when git exits unsuccessfully', async () => {
    ...
    const runtime = createRuntime()
    const destination = await mkdtemp(join(tmpdir(), 'nightshift-runtime-clone-'))
