...
describe('NightshiftRuntimeService', () => {
  ...
  it('applies Settings agent defaults to bare agent command terminal creates', async () => {
    ...
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  // the CLI Nightshift can host (issue #11926).
  ...
  it.each([
    { platform: 'win32' as const, expected: 'nightshift.cmd claude-teams' },
    { platform: 'linux' as const, expected: 'nightshift-ide claude-teams' },
    { platform: 'darwin' as const, expected: 'nightshift claude-teams' }
  ])(
  ...
  it('resolves a startupAgent in a repo-less folder workspace', async () => {
    const folderPath = await mkdtemp(join(tmpdir(), 'nightshift-runtime-folder-startup-agent-'))
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-bg' })
