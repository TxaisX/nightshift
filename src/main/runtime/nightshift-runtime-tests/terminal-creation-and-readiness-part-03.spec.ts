...
} from '../nightshift-runtime-test-mocks.spec'
import {
  ...
  makeFolderWorkspace,
  store
} from '../nightshift-runtime-test-fixtures.spec'

describe('NightshiftRuntimeService', () => {
  it('does not use the local Windows shell setting for remote Windows bare agent creates', async () => {
    ...
    const runtime = new NightshiftRuntimeService(remoteStore as never)
  ...
  it('matches canonical bare agent commands when a command override is configured', async () => {
    ...
    const runtime = new NightshiftRuntimeService(runtimeStore)
    runtime.setPtyController({
      spawn,
      write: () => true,
      kill: () => true,
  ...
  it('keeps non-bare agent command terminal creates unchanged', async () => {
    ...
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_LAUNCH_TOKEN).toBeUndefined()
  })
  ...
  it('injects runtime hook receiver env into terminal sessions', async () => {
    ...
    const runtime = new NightshiftRuntimeService(store, undefined, {
      buildAgentHookPtyEnv: () => ({
        ...
        NIGHTSHIFT_AGENT_HOOK_ENV: 'remote',
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        NIGHTSHIFT_AGENT_HOOK_TRANSPORT: 'stale-transport',
        NIGHTSHIFT_AGENT_HOOK_ENDPOINT: '/tmp/stale-endpoint.env'
      },
      envToDelete: ['CODEX_HOME', 'NIGHTSHIFT_CODEX_HOME']
    })
    ...
    expect(spawnCall?.env).toEqual(
      expect.objectContaining({
        ...
        NIGHTSHIFT_AGENT_HOOK_ENV: 'remote',
        NIGHTSHIFT_AGENT_HOOK_VERSION: '1',
        NIGHTSHIFT_PANE_KEY: expect.any(String),
        NIGHTSHIFT_TAB_ID: expect.any(String),
        NIGHTSHIFT_WORKTREE_ID: TEST_WORKTREE_ID
      })
    )
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_HOOK_TRANSPORT).toBeUndefined()
    expect(spawnCall?.envToDelete).toEqual(['CODEX_HOME', 'NIGHTSHIFT_CODEX_HOME'])
  })
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'nightshift-runtime-folder-workspace-'))
    ...
    expect(spawnedEnv.NIGHTSHIFT_WORKSPACE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
    expect(spawnedEnv.NIGHTSHIFT_PROJECT_GROUP_ID).toBe(TEST_FOLDER_PROJECT_GROUP_ID)
    expect(spawnedEnv.NIGHTSHIFT_WORKSPACE_ROOT).toBe(folderPath)
    expect(spawnedEnv.NIGHTSHIFT_WORKTREE_ID).toBe(TEST_FOLDER_WORKSPACE_KEY)
  })
  ...
  it.each([
    ...
    expect(spawnCall?.env?.NIGHTSHIFT_WORKTREE_ID).toBe(FLOATING_TERMINAL_WORKTREE_ID)
    expect(spawnCall?.env?.NIGHTSHIFT_WORKSPACE_ID).toBeUndefined()
    expect(spawnCall?.env?.NIGHTSHIFT_PROJECT_GROUP_ID).toBeUndefined()
    expect(spawnCall?.env?.NIGHTSHIFT_WORKSPACE_ROOT).toBeUndefined()
  })

  it('rejects folder workspace terminal creation when the backing path is missing', async () => {
    const missingPath = join(tmpdir(), `nightshift-missing-folder-workspace-${randomUUID()}`)
    const spawn = vi.fn().mockResolvedValue({ id: 'pty-folder' })
  ...
  it('rejects folder workspace folderPath updates when the new path is missing', async () => {
    const missingPath = join(tmpdir(), `nightshift-missing-folder-update-${randomUUID()}`)
  ...
  it('reveals Claude Agent Teams launches with the rewritten launch config', async () => {
    ...
    const spawnedEnv = spawnCall?.env ?? {}
    const spawnedLeafId = spawnedEnv.NIGHTSHIFT_PANE_KEY.slice(
      `${spawnedEnv.NIGHTSHIFT_TAB_ID}:`.length
    )
    expect(revealTerminalSession).toHaveBeenCalledWith(TEST_WORKTREE_ID, {
      ...
      launchToken: spawnedEnv.NIGHTSHIFT_AGENT_LAUNCH_TOKEN,
      launchAgent: 'claude',
      activate: false,
      tabId: spawnedEnv.NIGHTSHIFT_TAB_ID,
