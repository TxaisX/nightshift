...
import { wireHeadlessServeRuntime } from '../nightshift-runtime-test-scenario-builders.spec'
...
describe('NightshiftRuntimeService', () => {
  it('restores captured native Claude Agent Teams mode with fresh service env', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      env: {
        ...
        NIGHTSHIFT_AGENT_TEAMS_SHIM_BIN: '/opt/nightshift/bin/nightshift-ide',
        NIGHTSHIFT_AGENT_TEAMS_TEAM_ID: 'stale-team',
        NIGHTSHIFT_AGENT_TEAMS_TOKEN: 'stale-token',
        TMUX: '/tmp/nightshift-claude-agent-teams/stale-team,0,1'
      },
      launchAgent: 'claude',
      launchConfig: {
        agentCommand: 'claude',
    ...
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_TEAMS_TEAM_ID).toMatch(/^team-/)
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(spawnCall?.env?.NIGHTSHIFT_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
    expect(spawnCall?.env?.TMUX).not.toBe('/tmp/nightshift-claude-agent-teams/stale-team,0,1')
    ...
    expect(revealedLaunchConfig?.agentEnv.NIGHTSHIFT_AGENT_TEAMS_TEAM_ID).not.toBe('stale-team')
    expect(revealedLaunchConfig?.agentEnv.NIGHTSHIFT_AGENT_TEAMS_TOKEN).not.toBe('stale-token')
  })
  ...
  it('adopts renderer pane identity for remote runtime terminal creates', async () => {
    ...
    await runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
      ...
      leafId,
      env: {
        NIGHTSHIFT_PANE_KEY: `${tabId}:${leafId}`,
    ...
    expect(spawnedEnv.NIGHTSHIFT_TAB_ID).toBe(tabId)
    expect(spawnedEnv.NIGHTSHIFT_PANE_KEY).toBe(`${tabId}:${leafId}`)
  })
  ...
  it('does not adopt web mirror ids as host terminal ids', async () => {
    ...
    const spawnedEnv =
      (spawn.mock.calls[0]?.[0] as { env?: Record<string, string> } | undefined)?.env ?? {}
    expect(spawnedEnv.NIGHTSHIFT_TAB_ID).not.toBe(tabId)
    expect(spawnedEnv.NIGHTSHIFT_TAB_ID).not.toMatch(/^web-terminal-/)
    expect(spawnedEnv.NIGHTSHIFT_PANE_KEY).toMatch(`${spawnedEnv.NIGHTSHIFT_TAB_ID}:`)
  })
  ...
  it('spawns focus-requested CLI terminal creates in background on headless serve', async () => {
    ...
    // `nightshift terminal create --worktree <wt> --command "echo test" --focus`
    await expect(
      runtime.createTerminal(`path:${TEST_WORKTREE_PATH}`, {
        command: 'echo test',
        focus: true
