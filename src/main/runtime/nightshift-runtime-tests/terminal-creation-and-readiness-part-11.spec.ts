...
import { store, syncSinglePty } from '../nightshift-runtime-test-fixtures.spec'
...
describe('NightshiftRuntimeService', () => {
  ...
  it('strips retained terminal path line and hash locators before matching', () => {
    const colonPath = '/tmp/nightshift report/result.json'
  ...
  it('keeps tmpdir artifact paths in bounded PTY path candidates', () => {
    const artifactPath = join(tmpdir(), 'nightshift-runtime-retained-result.json')
    const candidates = appendRecentPtyPathCandidates(undefined, `wrote ${artifactPath}\n`)
  ...
  it('matches terminal artifact paths only when they appear in recent terminal output', () => {
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/nightshift report/result.json:12:3',
        '/tmp/nightshift report/result.json',
        '/tmp/nightshift report/result.json'
      )
    ).toBe(true)
    expect(
      recentTerminalOutputIncludesPath(
        '\x1b]8;;file:///tmp/nightshift%20report/result.json\x1b\\result\x1b]8;;\x1b\\',
        '/tmp/nightshift report/result.json',
        '/tmp/nightshift report/result.json'
      )
    ).toBe(true)
    ...
    expect(
      recentTerminalOutputIncludesPath(
        'wrote /tmp/nightshift report/other.json',
        '/tmp/nightshift report/result.json',
        '/tmp/nightshift report/result.json'
      )
    ).toBe(false)
