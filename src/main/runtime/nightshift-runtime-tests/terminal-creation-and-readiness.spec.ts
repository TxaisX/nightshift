...
import { NightshiftRuntimeService, makePaneKey } from '../nightshift-runtime-test-mocks.spec'
import {
  HEADLESS_LEAF_ID,
...
describe('NightshiftRuntimeService', () => {
  it('creates visible terminal sessions without asking the renderer to focus a tab', async () => {
    ...
    expect(spawnedEnv.NIGHTSHIFT_WORKTREE_ID).toBe(TEST_WORKTREE_ID)
    expect(spawnedEnv.NIGHTSHIFT_AGENT_LAUNCH_TOKEN).toMatch(UUID_RE)
  ...
  it('retires inherited launch authority when the agent command exits', async () => {
    ...
    const evidence = {
      ...
      paneKey: spawnEnv.NIGHTSHIFT_PANE_KEY,
      launchToken: spawnEnv.NIGHTSHIFT_AGENT_LAUNCH_TOKEN
    }

    expect(runtime.verifyOrchestrationCompatibilityCaller(evidence)).not.toBeNull()
    expect(
      runtime.getAgentStatusLaunchConfigForPaneKey(spawnEnv.NIGHTSHIFT_PANE_KEY, {
        launchToken: spawnEnv.NIGHTSHIFT_AGENT_LAUNCH_TOKEN
      })
    ).toBeDefined()
    ...
    expect(retireAuthority).toHaveBeenCalledWith(spawnEnv.NIGHTSHIFT_PANE_KEY)
