...
import { createWorktreeRemovalRuntime } from '../nightshift-runtime-test-scenario-builders.spec'
...
describe('NightshiftRuntimeService', () => {
  ...
  it('treats forced runtime deletion of an already-missing unregistered worktree as cleanup', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'nightshift-runtime-remove-'))
  ...
  it('routes already-missing SSH runtime history cleanup through the PTY owner', async () => {
    ...
    const metaById: Record<string, WorktreeMeta> = {
      [worktreeId]: makeWorktreeMeta({ hostId: 'ssh:ssh-1', nightshiftCreationSource: 'ssh' })
    }
    ...
    const runtime = new NightshiftRuntimeService(runtimeStore as never, undefined, {
  ...
  it('force-removes a legacy Nightshift-created runtime orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'nightshift-runtime-orphan-'))
  ...
  it('prompts then force-removes a Nightshift-created runtime unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'nightshift-runtime-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects a Nightshift-created runtime unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'nightshift-runtime-standalone-'))
    const repoPath = join(parentDir, 'repo')
