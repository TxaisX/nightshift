...
describe('NightshiftRuntimeService', () => {
    ...
    vi.mocked(listWorktrees).mockResolvedValueOnce([
      {
        ...
        head: 'abc',
        branch: 'refs/heads/wolfiesch/nightshift-skill-smoke',
        isBare: false,
        isMainWorktree: false
      }
    ])
  ...
  it.each([
    ...
    const folderPath = await mkdtemp(join(tmpdir(), 'nightshift-runtime-folder-files-'))
  ...
  it('guides registered SSH repo ids without probing the provider', async () => {
    ...
    const runtime = new NightshiftRuntimeService(remoteStore)
  ...
  it('retires a generated name before a post-create listing failure', async () => {
    const addRetiredWorktreeName = vi.fn()
    const runtime = new NightshiftRuntimeService({ ...store, addRetiredWorktreeName })
