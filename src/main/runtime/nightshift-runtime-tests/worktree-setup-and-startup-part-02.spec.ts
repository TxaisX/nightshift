...
import { expectStablePaneKeyEnv, store } from '../nightshift-runtime-test-fixtures.spec'
...
describe('NightshiftRuntimeService', () => {
  it('sequences setup before startup for opted-in local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      runnerScriptPath: 'C:\\tmp\\repo\\.git\\nightshift\\setup-runner.sh',
      shell: { family: 'posix', executable: 'wsl.exe' },
      envVars: {
        NIGHTSHIFT_ROOT_PATH: '/tmp/repo',
        NIGHTSHIFT_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-startup-setup'
      },
    ...
    expect(startupScript).toContain('/mnt/c/tmp/repo/.git/nightshift/setup-runner.sh')
    expect(setupCommand).toContain('bash /mnt/c/tmp/repo/.git/nightshift/setup-runner.sh')
  ...
  it('starts setup and startup side by side by default for local headless worktree creates', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        NIGHTSHIFT_ROOT_PATH: '/tmp/repo',
        NIGHTSHIFT_WORKTREE_PATH: '/tmp/workspaces/runtime-headless-parallel'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(spawn).toHaveBeenNthCalledWith(
      ...
      expect.objectContaining({
        command: expect.stringContaining('__NIGHTSHIFT_SETUP_COMPLETE__:')
      })
    )
  ...
  it('observes setup completion through the launch shell the runner was written for', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        NIGHTSHIFT_ROOT_PATH: '/tmp/repo',
        NIGHTSHIFT_WORKTREE_PATH: '/tmp/workspaces/runtime-observed-wsl-shell'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    const setupCommand = (spawn.mock.calls[1]![0] as { command: string }).command
    expect(setupCommand).toContain('bash /mnt/c/tmp/repo/.git/nightshift/setup-runner.sh')
    expect(setupCommand).toContain('__NIGHTSHIFT_SETUP_COMPLETE__:')
  })
  ...
  it('creates the first terminal for CLI-created worktrees without activating them', async () => {
    ...
    const initialLeafId = initialSpawnEnv.NIGHTSHIFT_PANE_KEY.slice(
      `${initialSpawnEnv.NIGHTSHIFT_TAB_ID}:`.length
    )
    ...
    expect(revealTerminalSession).toHaveBeenCalledWith(result.worktree.id, {
      ...
      activate: false,
      surfaceOwner: false,
      tabId: initialSpawnEnv.NIGHTSHIFT_TAB_ID,
  ...
  it('does not surface the new workspace when a background CLI create launches its agent', async () => {
    ...
    // `nightshift worktree create --agent claude` without --activate: the agent tab is
  ...
  it('honors split setup placement for CLI-created worktrees without startup agents', async () => {
    ...
    vi.mocked(createSetupRunnerScript).mockReturnValue({
      ...
      envVars: {
        NIGHTSHIFT_ROOT_PATH: '/tmp/repo',
        NIGHTSHIFT_WORKTREE_PATH: '/tmp/workspaces/runtime-cli-setup-split'
      }
    })
    vi.mocked(listWorktrees).mockResolvedValue([
      {
    ...
    expect(setupEnv.NIGHTSHIFT_TAB_ID).toBe(mainEnv.NIGHTSHIFT_TAB_ID)
    const mainLeafId = mainEnv.NIGHTSHIFT_PANE_KEY!.slice(`${mainEnv.NIGHTSHIFT_TAB_ID!}:`.length)
    expect(revealTerminalSession).toHaveBeenLastCalledWith(
      result.worktree.id,
      expect.objectContaining({
        ptyId: 'pty-cli-setup-setup',
        tabId: mainEnv.NIGHTSHIFT_TAB_ID,
        activate: false,
