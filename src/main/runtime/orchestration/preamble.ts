...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `nightshift-ide`;
  // other execution hosts keep their existing bare `nightshift` bridge.
  ...
  // shells have no agent prompt for Nightshift to reuse.
...
// Why: the dispatch preamble teaches agents about Nightshift's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use nightshift-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Nightshift instance (Section 6.4).
  const cli = params.devMode ? 'nightshift-dev' : (params.cliCommand ?? 'nightshift')
  ...
  const header = `You are working inside Nightshift, a multi-agent IDE. You are a dispatched worker.
...
prompt for Nightshift to reuse; if the coordinator has more for you it will
