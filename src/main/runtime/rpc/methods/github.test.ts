...
describe('github RPC methods', () => {
  it('resolves the repo slug on the runtime server', async () => {
    const runtime = {
      ...
      getRepoSlug: vi.fn().mockResolvedValue({ owner: 'acme', repo: 'nightshift' })
    ...
    expect(response).toMatchObject({
      ok: true,
      result: { owner: 'acme', repo: 'nightshift' }
    })
  })
  ...
  it('lists slug-addressed issue metadata on the runtime server', async () => {
    ...
    const labels = await dispatcher.dispatch(
      makeRequest('github.project.listLabelsBySlug', { owner: 'acme', repo: 'nightshift' })
    )
    const users = await dispatcher.dispatch(
    ...
    const issueTypes = await dispatcher.dispatch(
      makeRequest('github.project.listIssueTypesBySlug', { owner: 'acme', repo: 'nightshift' })
    )

    expect(runtime.listGitHubLabelsBySlug).toHaveBeenCalledWith({
      owner: 'acme',
      repo: 'nightshift'
    })
