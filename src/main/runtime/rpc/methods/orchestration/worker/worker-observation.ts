...
export async function inspectWorkerTerminal(
  ...
  if (structured) {
    ...
    //
    // `agentWait` is deliberately ABSENT rather than null. Null is the contract's "Nightshift looked and
...
export function resolvePinnedFederatedServer(
  ...
  if (server.peerFingerprint !== federated.peer_fingerprint) {
    throw new OrchestrationError(
      'peer_changed',
      `Saved environment ${federated.environment_name} now identifies a different Nightshift server.`
    )
  }
  return server
}
