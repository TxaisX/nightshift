...
describe('orchestration new-worktree workers', () => {
  type CreateWorktreeResult = Awaited<ReturnType<NightshiftRuntimeService['createManagedWorktree']>>
  ...
  it('injects the execution host CLI command and Dispatch capability together', async () => {
    mockCreatedWorktree()
    vi.mocked(runtime.getTerminalOrchestrationCliCommand).mockReturnValue('nightshift-ide')

    await startWorker({ worktree: 'new-top-level' })

    const prompt = vi.mocked(runtime.sendTerminalAgentPrompt).mock.calls[0]?.[1] ?? ''
    expect(prompt).toContain('nightshift-ide orchestration send')
    expect(prompt).toMatch(/--dispatch-capability dcap_[A-Za-z0-9_-]+/)
    expect(prompt).not.toMatch(/(^|\s)nightshift orchestration send/)
  })
  ...
  it('replays a dispatch-input failure after restart without creating another worker', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nightshift-worker-start-replay-'))
    paths.push(dir)
    db.close()
    ...
    db = new OrchestrationDb(join(dir, 'orchestration.db'))
    const restartedRuntime = new NightshiftRuntimeService()
  ...
  it('persists pre-effect, post-effect, and post-input stages in order', async () => {
    mockCreatedWorktree({ hookFound: false })
    let finishWait:
      | ((value: Awaited<ReturnType<NightshiftRuntimeService['waitForTerminal']>>) => void)
      | undefined
    let finishPrompt:
      | ((value: Awaited<ReturnType<NightshiftRuntimeService['sendTerminalAgentPrompt']>>) => void)
      | undefined
