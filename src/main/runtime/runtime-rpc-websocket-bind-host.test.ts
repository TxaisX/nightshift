...
describe('NightshiftRuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  ...
  it('binds all interfaces at startup when exposeNetworkByDefault is set (nightshift serve)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'nightshift-runtime-rpc-'))
    const server = new NightshiftRuntimeRpcServer({
      runtime: new NightshiftRuntimeService(),
      userDataPath,
  ...
  it('upgrades a reused pending grant to network reach so its link survives a relaunch', async () => {
    ...
    const relaunched = new NightshiftRuntimeRpcServer({
      runtime: new NightshiftRuntimeService(),
      userDataPath,
      enableWebSocket: true,
      wsPort: 0
  ...
  it('honours a pinned bind host over exposeNetworkByDefault (nightshiftd --bind)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'nightshift-runtime-rpc-'))
    const server = new NightshiftRuntimeRpcServer({
      runtime: new NightshiftRuntimeService(),
      userDataPath,
  ...
  it('stays pinned to loopback even after a device has connected once', async () => {
    ...
    // network-reach device has connected. A loopback nightshiftd would therefore go wide one
