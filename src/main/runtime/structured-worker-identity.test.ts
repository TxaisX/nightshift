...
describe('structured workers stay outside the PTY-only fail-closed paths', () => {
  it('keeps the selector shut by never letting a structured pane key reach a hook status', () => {
    ...
    // NIGHTSHIFT_PANE_KEY is absent from its child's environment. That absence IS the guard: put the key
    ...
    try {
      ...
      expect(Object.keys(env)).not.toContain('NIGHTSHIFT_PANE_KEY')
    } finally {
