...
// Why: kept in sync with pty/wsl-nightshift-env.ts, which path-translates the same keys for WSL.
// NIGHTSHIFT_WORKSPACE_NAME is a display name and the credential-guard policy is an enum — never paths.
...
export function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    NIGHTSHIFT_ROOT_PATH: repo.path,
    NIGHTSHIFT_WORKTREE_PATH: worktreePath,
    NIGHTSHIFT_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
