...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'nightshift-skill-bundle-test-'))
...
describe('skill bundle creation and extraction', () => {
  ...
  it('rejects conflicting staging roots without changing their imported namespace', async () => {
    ...
    const importedManifest = join(destination, 'dev.nightshift.skill-sharing', 'manifest.json')
    await mkdir(join(destination, 'dev.nightshift.skill-sharing'), { recursive: true })
  ...
  it('rejects unknown top-level extension namespaces and removes fresh staging', async () => {
    ...
    await writeSkillTarGzip(archivePath, [
      ...
      {
        path: 'dev.nightshift.unexpected/manifest.json',
        size: unknown.length,
        executable: false,
        bytes: unknown
      }
