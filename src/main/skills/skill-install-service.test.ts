...
async function fixture(): Promise<{
  ...
  const root = await mkdtemp(join(tmpdir(), 'nightshift-skill-service-test-'))
  ...
  return {
    ...
    input: {
      ...
      nightshiftStateDirectory: join(root, 'nightshift-state'),
...
describe('skill install service', () => {
  ...
  it('never removes a byte-identical provider copy that was recorded as unowned', async () => {
    ...
    const removed = await removeSharedSkill({
      ...
      nightshiftStateDirectory: input.nightshiftStateDirectory,
