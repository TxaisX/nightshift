...
const CHILD = process.env.NIGHTSHIFT_SKILL_CONTENTION_CHILD === '1'
...
describe.runIf(CHILD)('skill process contention child', () => {
  it('runs one transaction process', async () => {
    const root = process.env.NIGHTSHIFT_SKILL_CONTENTION_ROOT
    const archivePath = process.env.NIGHTSHIFT_SKILL_CONTENTION_ARCHIVE
    const resultPath = process.env.NIGHTSHIFT_SKILL_CONTENTION_RESULT
    const role = process.env.NIGHTSHIFT_SKILL_CONTENTION_ROLE
    ...
    const result = await installLocalSkillPackage(
      ...
      },
      {
        onJournalTransition: async (phase, boundary) => {
          ...
          const readyPath = process.env.NIGHTSHIFT_SKILL_CONTENTION_READY
          const releasePath = process.env.NIGHTSHIFT_SKILL_CONTENTION_RELEASE
