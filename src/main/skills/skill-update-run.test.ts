...
describe('SkillUpdateRunner', () => {
  it('passes both non-interactive flags and the sorted skill names', () => {
    ...
    expect(runner.start(['orchestration', 'nightshift-cli'])).toEqual({ started: true })
  ...
  it('rejects names that could carry shell syntax', () => {
    ...
    expect(runner.start(['nightshift-cli; rm -rf /'])).toEqual({
      started: false,
  ...
  it('strips ANSI colour and carriage returns from captured output', async () => {
    ...
    runner.start(['nightshift-cli'])
    child.stdout.emit('data', Buffer.from('\x1b[36mChecking\x1b[0m\rUpdating nightshift-cli…'))
    child.emit('close', 0)
    await flush()

    const run = runner.getState()
    expect(run.state).toBe('success')
    expect(run.state === 'success' && run.output).toBe('Checking\nUpdating nightshift-cli…')
  })
  ...
  it('attributes failure to the names the re-scan says are still outdated', async () => {
    ...
    runner.start(['nightshift-cli', 'orchestration'])
    child.emit('close', 1)
    await flush()
  ...
  it('fails every requested name when the re-scan itself throws', async () => {
    ...
    const run = runner.getState()
    expect(run.state).toBe('error')
    expect(run.state === 'error' && run.failedNames).toEqual(['nightshift-cli'])
  ...
  it('keeps the spawn error when the failed child also emits close', async () => {
    ...
    const rescan = vi.fn(async () => ['nightshift-cli'])
  ...
  it('surfaces an unspawnable command path instead of silently doing nothing', async () => {
    ...
    const result = runner.start(['nightshift-cli'])
  ...
  it('spawns npx from a Program Files (x86) install instead of refusing it', () => {
    ...
    try {
      ...
      expect(runner.start(['nightshift-cli']).started).toBe(true)
