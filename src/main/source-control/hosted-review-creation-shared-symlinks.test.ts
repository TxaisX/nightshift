...
// cannot commit — it is a symlink Nightshift created.
describe('createHostedReview with shared symlinks', () => {
  ...
  beforeEach(() => {
    worktree = mkdtempSync(join(tmpdir(), 'nightshift-hosted-shared-'))
  ...
  // Why: only an *untracked* record can be Nightshift's artifact. A tracked change at a
