...
describe('ModelManager Windows model path handling', () => {
  beforeEach(() => {
    ...
    appGetPathMock.mockImplementation(() => join(tmpdir(), 'nightshift-speech-models-test'))
  })
  ...
  it('uses an ASCII cache path when the Windows default user data path has non-ASCII characters', () => {
    const dir = mkdtempSync(join(tmpdir(), 'nightshift-model-manager-'))
    try {
      setPlatform('win32')
      const programDataDir = join(dir, 'ProgramData')
      const userDataDir = join(dir, '用户', 'Nightshift')
      ...
      expect(manager.getModelsDir()).toContain(join(programDataDir, 'Nightshift', 'speech-models'))
