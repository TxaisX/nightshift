...
import { evaluateNightshiftdActivation } from './nightshiftd-activation-gate'
import type { ServeReadiness } from '../server/serve-readiness'
import type { NightshiftdHealth, TerminalDaemonHealth } from '../nightshiftd/nightshiftd-health'
...
function daemon(overrides: Partial<TerminalDaemonHealth> = {}): TerminalDaemonHealth {
  return {
    ...
    entryPath: '/home/u/.nightshift-remote/nightshiftd-0.2.0+bb01/daemon-entry.js',
    protocolVersion: 3,
...
function health(overrides: Partial<NightshiftdHealth> = {}): NightshiftdHealth {
  return {
...
describe('evaluateNightshiftdActivation', () => {
  it('activates a candidate that proved a real PTY round trip', () => {
    const verdict = evaluateNightshiftdActivation(readiness(), EXPECTED)
  ...
  it('refuses when the candidate never published readiness', () => {
    const verdict = evaluateNightshiftdActivation(null, EXPECTED)
  ...
  it('refuses a readiness payload with no health, rather than reading silence as healthy', () => {
    ...
    const verdict = evaluateNightshiftdActivation(withoutHealth as ServeReadiness, EXPECTED)
    expect(verdict).toMatchObject({ decision: 'reject', code: 'nightshiftd_activation_no_health' })
  })
  ...
  it('refuses a listening nightshiftd whose terminal daemon is absent', () => {
    const verdict = evaluateNightshiftdActivation(
      readiness({
        health: health({
          terminalDaemon: daemon({
  ...
  it('refuses a candidate that is not listening', () => {
    const verdict = evaluateNightshiftdActivation(readiness({ boundEndpoint: null }), EXPECTED)
    expect(verdict).toMatchObject({
      decision: 'reject',
  ...
  it('checks identity before health, so a wrong-build green payload cannot pass', () => {
    ...
    expect(verdict).toMatchObject({ code: 'nightshiftd_activation_build_mismatch' })
  })
})
