/**
 * Whether a freshly launched nightshiftd has earned the right to become the active one.
 *
 * The failure this exists to prevent is the one `docs/design/shipping-nightshiftd.html` names
 * throughout: a deployment that reports success because a port opened. nightshiftd answers RPC
...
export type NightshiftdActivationRejectCode =
  | 'nightshiftd_activation_no_readiness'
  | 'nightshiftd_activation_no_health'
  | 'nightshiftd_activation_build_mismatch'
  | 'nightshiftd_activation_not_listening'
  | 'nightshiftd_activation_daemon_absent'
  | 'nightshiftd_activation_daemon_degraded'
  | 'nightshiftd_activation_pty_self_test_failed'
  | 'nightshiftd_activation_no_persistent_terminals'

export type NightshiftdActivationVerdict =
  | {
      decision: 'activate'
      /**
  ...
  | { decision: 'reject'; code: NightshiftdActivationRejectCode; reason: string }

export type NightshiftdActivationExpectation = {
  /** sha256(nightshiftd.js).slice(0,16) computed from the bytes this client just uploaded. */
  buildHash: string
 ...
 */
export function evaluateNightshiftdActivation(
  readiness: ServeReadiness | null,
  expected: NightshiftdActivationExpectation
): NightshiftdActivationVerdict {
  if (!readiness) {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_no_readiness',
      reason:
        'The candidate nightshiftd never published an `nightshift_server_ready` line. It may have exited, ' +
  ...
  if (!health) {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_no_health',
      reason:
  ...
  // Why identity first: a stale nightshiftd already holding the port would answer readiness and
  ...
  if (health.buildHash !== expected.buildHash) {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_build_mismatch',
      reason:
  ...
  if (!readiness.boundEndpoint) {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_not_listening',
      reason:
  ...
  if (daemon.state === 'absent') {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_daemon_absent',
      reason:
        'The candidate has no terminal daemon. Every terminal on this host would run in the ' +
        'nightshiftd process and die with it, which is the exact regression the daemon exists to ' +
  ...
  if (daemon.state === 'degraded') {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_daemon_degraded',
      reason:
  ...
  if (!daemon.selfTest.ok) {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_pty_self_test_failed',
      reason:
  ...
  if (!daemon.ownsFreshSessions) {
    return {
      decision: 'reject',
      code: 'nightshiftd_activation_no_persistent_terminals',
      reason:
  ...
  if (health.buildVersion !== expected.fullVersion) {
    // Not a rejection: the hash already proved identity, and NIGHTSHIFT_VERSION is whatever the
    ...
    warnings.push(
      `The candidate reports version ${health.buildVersion} but was installed as ` +
        `${expected.fullVersion}; check NIGHTSHIFT_VERSION in the launch command.`
    )
  }
