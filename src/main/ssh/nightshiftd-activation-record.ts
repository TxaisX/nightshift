/**
 * Which installed nightshiftd is the live one, and which one a rollback goes back to.
 *
 ...
 *
 * It lives beside the version dirs (`~/.nightshift-remote/nightshiftd-active.json`), not inside one,
 * because it has to outlive whichever version GC removes.
 */
import { remoteInstallDirName, NIGHTSHIFTD_INSTALL_MODEL } from './remote-install-model'

export const NIGHTSHIFTD_ACTIVATION_FILENAME = 'nightshiftd-active.json'
export const NIGHTSHIFTD_ACTIVATION_SCHEMA_VERSION = 1

/** Where a pre-activation copy of the shared data root lives, relative to `.nightshift-remote/`. */
export const NIGHTSHIFTD_STATE_SNAPSHOT_DIR = 'nightshiftd-state-snapshots'

export type NightshiftdStateSnapshot = {
  /** Directory name under `NIGHTSHIFTD_STATE_SNAPSHOT_DIR`. */
  dirName: string
...
export type NightshiftdActivationRecord = {
  schemaVersion: typeof NIGHTSHIFTD_ACTIVATION_SCHEMA_VERSION
  ...
  snapshot: NightshiftdStateSnapshot | null
}

export function emptyNightshiftdActivationRecord(): NightshiftdActivationRecord {
  return {
    schemaVersion: NIGHTSHIFTD_ACTIVATION_SCHEMA_VERSION,
    active: null,
    previous: null,
    activatedAt: null,
    snapshot: null
 ...
 * install. Callers distinguish the two through `NightshiftdActivationReadResult`.
 */
export type NightshiftdActivationReadResult =
  | { state: 'absent' }
  | { state: 'ok'; record: NightshiftdActivationRecord }
  | { state: 'unreadable'; reason: string }

export function parseNightshiftdActivationRecord(
  raw: string | null
): NightshiftdActivationReadResult {
  ...
  const record = parsed as Partial<NightshiftdActivationRecord>
  if (record.schemaVersion !== NIGHTSHIFTD_ACTIVATION_SCHEMA_VERSION) {
    return {
      state: 'unreadable',
      reason:
        `activation record schemaVersion ${String(record.schemaVersion)} is not ` +
        `${NIGHTSHIFTD_ACTIVATION_SCHEMA_VERSION}; this client cannot safely interpret it`
    }
  }
  return {
    state: 'ok',
...
function parseSnapshot(value: unknown): NightshiftdStateSnapshot | null {
  if (typeof value !== 'object' || value === null) {
    return null
  }
  const snapshot = value as Partial<NightshiftdStateSnapshot>
...
export function serializeNightshiftdActivationRecord(record: NightshiftdActivationRecord): string {
 ...
 * OUTGOING bundle (see nightshiftd-update-plan.ts) — deleting the tree under a running process is
 * how a later respawn finds no entry point.
 */
export function nightshiftdGcPinnedDirNames(
  record: NightshiftdActivationRecord,
  ...
  return [...new Set(versions)].map((version) =>
    remoteInstallDirName(NIGHTSHIFTD_INSTALL_MODEL, version)
  )
}
