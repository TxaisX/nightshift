...
vi.mock('./nightshiftd-local-build-hash', () => ({
  computeLocalNightshiftdBuildHash: () => 'abc123def4567890'
}))

import { execCommand } from './ssh-relay-deploy-helpers'
import { acquireInstallLock } from './ssh-relay-install-lock'
import { uploadRelayDirectory, writeRelayFile } from './ssh-relay-install-transfers'
import { deployNightshiftd, type NightshiftdDeployOptions } from './nightshiftd-remote-deploy'
import {
...
function scriptHost(script: HostScript): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('nightshiftd-active.json')) {
      return script.activationRecord
    }
    if (text.includes('.nightshiftd-readiness') && text.startsWith('cat ')) {
...
function options(overrides: Partial<NightshiftdDeployOptions> = {}): NightshiftdDeployOptions {
  return {
    conn: {} as SshConnection,
    host: getRemoteHostPlatform('linux-x64'),
    remoteHome: '/home/u',
    localNightshiftdDir: '/local/out/nightshiftd',
...
const ACTIVE_OLD = JSON.stringify(
  withActivatedVersion(emptyNightshiftdActivationRecord(), OLD_VERSION, null, new Date(0))
)

describe('deployNightshiftd', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('installs under the nightshiftd namespace, not the relay one', async () => {
    ...
    scriptHost(script)
    await deployNightshiftd(options())
    expect(vi.mocked(acquireInstallLock).mock.calls[0][1]).toBe(
      `/home/u/.nightshift-remote/nightshiftd-${NEW_VERSION}`
    )
    expect(vi.mocked(uploadRelayDirectory).mock.calls[0][2]).toContain(`nightshiftd-${NEW_VERSION}`)
  })
  ...
  it('activates a healthy candidate and records the outgoing version as the rollback target', async () => {
    ...
    scriptHost(script)
    const result = await deployNightshiftd(options())
    expect(result).toMatchObject({ outcome: 'installed-and-activated', fullVersion: NEW_VERSION })
    const written = vi
      .mocked(writeRelayFile)
      .mock.calls.find((call) => String(call[2]).endsWith('nightshiftd-active.json'))
  ...
  it('installs but does not activate when terminals are running', async () => {
    ...
    scriptHost(script)
    const result = await deployNightshiftd(
  ...
  it('does not write the activation record when the candidate fails its health gate', async () => {
    ...
    scriptHost(script)
    const result = await deployNightshiftd(options())
    expect(result).toMatchObject({ code: 'nightshiftd_activation_daemon_degraded' })
    expect(
      vi
        .mocked(writeRelayFile)
        .mock.calls.some((call) => String(call[2]).endsWith('nightshiftd-active.json'))
    ).toBe(false)
  })
  ...
  it('puts the previous version back after a rejected candidate, rather than leaving the host down', async () => {
    ...
    expect(result.outcome === 'installed-not-activated' && result.reason).toContain(
      `nightshiftd ${OLD_VERSION} was restarted and is serving again`
    )
  })
  ...
  it('refuses to activate when a different build answered the port', async () => {
    ...
    scriptHost(script)
    const result = await deployNightshiftd(options())
    expect(result).toMatchObject({ code: 'nightshiftd_activation_build_mismatch' })
  })
  ...
  it('refuses to treat an unreadable activation record as an empty one', async () => {
    ...
    scriptHost(script)
    await expect(deployNightshiftd(options())).rejects.toThrow('activation record')
