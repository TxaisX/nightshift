/**
 * nightshiftd's garbage collection, and the half of §06 falsifier 1 that says who owns it.
 *
 * **Each model GCs only its own namespace, permanently.** nightshiftd removes `nightshiftd-<v>/`
 ...
 * to keep intact.
 *
 * On top of the ownership rule, nightshiftd pins three directories that are idle-looking but
...
export type NightshiftdGcOptions = {
  conn: SshConnection
...
export async function gcOldNightshiftdVersions(options: NightshiftdGcOptions): Promise<void> {
  await gcOldRemoteInstallVersions(
    ...
    options.host,
    {
      pinnedDirNames: nightshiftdGcPinnedDirNames(options.record, options.liveDaemonVersion),
      isDirLive: async (dir) => {
        try {
          const probe = await execCommand(
            options.conn,
            nightshiftdLivenessProbeCommand(options.host, dir),
            {
              wrapCommand: options.host.commandDialect !== 'powershell',
              signal: options.signal
            }
          )
          return nightshiftdLivenessBlocksGc(parseNightshiftdLiveness(probe))
        } catch {
