/**
 * Starting a candidate nightshiftd on the host and reading back what it says about itself.
 *
 * This is the piece `docs/design/shipping-nightshiftd.html` §02 marks **fork**, not reuse: the
 * relay launches detached and proves itself by printing an `NIGHTSHIFT-RELAY` sentinel, and nightshiftd
 * has no such interface. It publishes a single `nightshift_server_ready` JSON line on stdout,
...
import {
  assertPosixNightshiftdHost as assertPosixHost,
  NIGHTSHIFTD_PID_FILENAME,
  posixProcessAliveShellFunction
} from './nightshiftd-remote-host-support'
import type { ServeReadiness } from '../server/serve-readiness'

/** Stdout of the launched candidate: exactly one `nightshift_server_ready` line, then nothing. */
export const NIGHTSHIFTD_READINESS_FILENAME = '.nightshiftd-readiness'
/** Stderr, including the bind-exposure line and every supervision message. */
export const NIGHTSHIFTD_LOG_FILENAME = 'nightshiftd.log'
export {
  NIGHTSHIFTD_PID_FILENAME,
  NightshiftdRemoteLaunchUnsupportedError
} from './nightshiftd-remote-host-support'

export type NightshiftdLaunchSpec = {
 ...
 *
 * Why `--bind` is always passed explicitly: nightshiftd defaults to loopback, but a default is a
 ...
 */
export function nightshiftdLaunchCommand(
  ...
  const readiness = shellEscape(
    joinRemotePath(host, spec.remoteInstallDir, NIGHTSHIFTD_READINESS_FILENAME)
  )
  const log = shellEscape(joinRemotePath(host, spec.remoteInstallDir, NIGHTSHIFTD_LOG_FILENAME))
  const pidFile = shellEscape(joinRemotePath(host, spec.remoteInstallDir, NIGHTSHIFTD_PID_FILENAME))
  const entry = shellEscape(joinRemotePath(host, spec.remoteInstallDir, 'nightshiftd.js'))
  return [
    `cd ${dir} &&`,
    ...
    `: > ${readiness} &&`,
    'umask 077 &&',
    `NIGHTSHIFT_VERSION=${shellEscape(spec.fullVersion)}`,
    `NIGHTSHIFT_USER_DATA=${shellEscape(spec.userDataDir)}`,
...
export function readNightshiftdReadinessCommand(
  host: RemoteHostPlatform,
  remoteInstallDir: string
): string {
  assertPosixHost(host)
  const readiness = shellEscape(
    joinRemotePath(host, remoteInstallDir, NIGHTSHIFTD_READINESS_FILENAME)
  )
 ...
 */
export function nightshiftdLivenessProbeCommand(
  host: RemoteHostPlatform,
  remoteInstallDir: string
): string {
  assertPosixHost(host)
  const pidFile = shellEscape(joinRemotePath(host, remoteInstallDir, NIGHTSHIFTD_PID_FILENAME))
  return [
    posixProcessAliveShellFunction(),
    ...
    '* ) if nightshiftd_alive "$pid"; then echo LIVE;',
...
export type NightshiftdLiveness = 'LIVE' | 'DEAD' | 'UNKNOWN'

export function parseNightshiftdLiveness(output: string): NightshiftdLiveness {
...
export function nightshiftdLivenessBlocksGc(liveness: NightshiftdLiveness): boolean {
  return liveness !== 'DEAD'
}

export type NightshiftdReadinessParse =
  | { state: 'ready'; readiness: ServeReadiness }
  | { state: 'pending' }
  | { state: 'malformed'; reason: string }

/**
 * Pull the `nightshift_server_ready` payload out of whatever the candidate has written so far.
 *
 ...
 */
export function parseNightshiftdReadinessOutput(raw: string): NightshiftdReadinessParse {
  ...
  for (const line of lines) {
    ...
    if (payload.type !== 'nightshift_server_ready') {
      return {
        state: 'malformed',
        reason: `expected a nightshift_server_ready line, got type=${JSON.stringify(payload.type)}`
      }
    }
