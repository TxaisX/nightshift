...
import { rollbackNightshiftd, type NightshiftdRollbackOptions } from './nightshiftd-remote-rollback'
import {
...
function record(overrides: Partial<NightshiftdActivationRecord> = {}): NightshiftdActivationRecord {
  return {
    ...emptyNightshiftdActivationRecord(),
    active: ACTIVE,
    previous: TARGET,
...
function scriptHost(log: string[], overrides: { restore?: string } = {}): void {
  mockExec.mockImplementation(async (_conn, command: string) => {
    ...
    if (text.startsWith('cat ') && text.includes('.nightshiftd-readiness')) {
...
function options(overrides: Partial<NightshiftdRollbackOptions> = {}): NightshiftdRollbackOptions {
  return {
...
describe('rollbackNightshiftd', () => {
  ...
  it('stops, restores state, then starts the target — in that order', async () => {
    const log: string[] = []
    scriptHost(log)
    const result = await rollbackNightshiftd(options())
    expect(result).toMatchObject({ outcome: 'rolled-back', target: TARGET })
    // Restoring under a running nightshiftd would replace the store beneath a process holding it;
  ...
  it('refuses before touching anything when terminals started after activation', async () => {
    const log: string[] = []
    scriptHost(log)
    const result = await rollbackNightshiftd(
  ...
  it('does not start the old build when the restore failed', async () => {
    ...
    const result = await rollbackNightshiftd(options())
    expect(result).toMatchObject({ outcome: 'failed', code: 'nightshiftd_rollback_restore_failed' })
  ...
  it('leaves the record naming the newer version when the target fails to come up', async () => {
    ...
    const result = await rollbackNightshiftd(options())
    expect(result).toMatchObject({ outcome: 'failed', code: 'nightshiftd_activation_no_readiness' })
  ...
  it('records the rollback only after the target answers healthy', async () => {
    const log: string[] = []
    scriptHost(log)
    await rollbackNightshiftd(options())
    const written = vi
