 ...
 *
 * `docs/design/shipping-nightshiftd.html` §04's state-schema row asks for "backward-readable
 * migrations or a pre-activation snapshot". Only the second is available here, and not as a
 * preference: Nightshift's persisted state carries **no schema version**. Migrations are cohort
 ...
 * daemon's socket, PID record and auth token, and that daemon outlives every nightshiftd restart
...
import { assertPosixNightshiftdHost as assertPosixHost } from './nightshiftd-remote-host-support'
 ...
 */
export const NIGHTSHIFTD_SNAPSHOT_MEMBERS = [
  'nightshift-profile-index.json',
...
export const NIGHTSHIFTD_SNAPSHOT_EXCLUDED = ['daemon', 'logs'] as const

/**
 * The member names go into the command unquoted (see `captureNightshiftdStateSnapshotCommand`), so
...
function assertPlainMemberName(member: string): string {
  if (!/^[A-Za-z0-9._-]+$/.test(member)) {
    throw new Error(`Unsafe nightshiftd snapshot member name: ${JSON.stringify(member)}`)
  }
  return member
}

export function nightshiftdSnapshotDirName(fullVersion: string, takenAtMs: number): string {
 ...
 */
export function captureNightshiftdStateSnapshotCommand(
  ...
  const memberTests = NIGHTSHIFTD_SNAPSHOT_MEMBERS.map(
...
export type NightshiftdSnapshotCapture = 'captured' | 'empty' | 'failed'

export function parseNightshiftdSnapshotCapture(output: string): NightshiftdSnapshotCapture {
...
export function probeNightshiftdStateSnapshotCommand(
  host: RemoteHostPlatform,
  snapshotDir: string
): string {
  assertPosixHost(host)
 ...
 *
 * The caller must have stopped nightshiftd first. This does not check — it cannot, from a shell —
 * so `nightshiftd-remote-deploy.ts` owns that ordering.
 */
export function restoreNightshiftdStateSnapshotCommand(
  host: RemoteHostPlatform,
  userDataDir: string,
  snapshotDir: string
): string {
  ...
  const root = shellEscape(userDataDir)
  const archive = shellEscape(joinRemotePath(host, snapshotDir, 'state.tar'))
  const removals = NIGHTSHIFTD_SNAPSHOT_MEMBERS.map(
...
export type NightshiftdSnapshotRestore = 'restored' | 'missing' | 'failed'

export function parseNightshiftdSnapshotRestore(output: string): NightshiftdSnapshotRestore {
  const value = output.trim().split('\n').pop()?.trim()
  if (value === 'RESTORED') {
    return 'restored'
  }
 ...
 * caller compares; an `UNKNOWN` becomes `null`, which `assessNightshiftdRollback` treats as "yes,
 * assume writes".
 */
export function newestStateMtimeCommand(host: RemoteHostPlatform, userDataDir: string): string {
  assertPosixHost(host)
  const root = shellEscape(userDataDir)
  const paths = NIGHTSHIFTD_SNAPSHOT_MEMBERS.map((member) => `${root}/${shellEscape(member)}`).join(
    ' '
  )
  return [
