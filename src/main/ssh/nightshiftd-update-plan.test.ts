...
import { assessNightshiftdRollback, planNightshiftdUpdate } from './nightshiftd-update-plan'
import {
...
describe('planNightshiftdUpdate', () => {
  ...
  it('defers rather than restarting a host with live terminals', () => {
    ...
    expect(plan).toMatchObject({ action: 'defer', code: 'nightshiftd_update_terminals_running' })
...
describe('assessNightshiftdRollback', () => {
  ...
  it('refuses when there is no previous version to go back to', () => {
    ...
    expect(safety).toMatchObject({ safety: 'unsafe', code: 'nightshiftd_rollback_no_target' })
  })
})
