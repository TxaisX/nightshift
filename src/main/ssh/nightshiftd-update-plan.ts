/**
 * When an update may restart nightshiftd, and when going back is still sound.
 *
 * Two constraints shape everything here.
 *
 * **The daemon must outlive the restart.** nightshiftd forks the terminal daemon and deliberately
 * does not kill it on stop (`nightshiftd-daemon-supervision.ts` uses `disconnectDaemon`, never
 ...
 *
 * **The state root is shared across versions.** `~/.nightshift/` (or `$NIGHTSHIFT_USER_DATA`) is outside
 * every version dir, and Nightshift's persisted state carries no schema version — migrations run
 ...
 */
import type { NightshiftdActivationRecord } from './nightshiftd-activation-record'

export type NightshiftdTerminalCensus = {
  /**
...
export type NightshiftdUpdateDecision =
  ...
  | { action: 'defer'; code: NightshiftdUpdateDeferCode; reason: string }

export type NightshiftdUpdateDeferCode =
  | 'nightshiftd_update_terminals_running'
  | 'nightshiftd_update_terminal_census_unavailable'

/**
 * Decide whether to restart nightshiftd onto `candidateVersion`.
 *
 * Deferring on live terminals is a deliberate choice, not caution. The restart itself is
 * non-destructive, but it leaves the host running a NEW nightshiftd against an OLD daemon until
...
export function planNightshiftdUpdate(input: {
  ...
  if (liveSessions === null) {
    if (!input.force) {
      return {
        action: 'defer',
        code: 'nightshiftd_update_terminal_census_unavailable',
        reason:
  ...
  if (liveSessions > 0 && !input.force) {
    return {
      action: 'defer',
      code: 'nightshiftd_update_terminals_running',
      reason:
        ...
        `nightshiftd ${input.candidateVersion} against a daemon forked from ` +
...
export type NightshiftdRollbackSafety =
  | { safety: 'clean'; target: string; notes: string[] }
  | { safety: 'lossy'; target: string; discards: string[] }
  | { safety: 'unsafe'; code: NightshiftdRollbackUnsafeCode; reason: string }

export type NightshiftdRollbackUnsafeCode =
  | 'nightshiftd_rollback_no_target'
  | 'nightshiftd_rollback_snapshot_missing'
  | 'nightshiftd_rollback_orphans_live_terminals'
  | 'nightshiftd_rollback_census_unavailable'
 ...
 */
export function assessNightshiftdRollback(input: {
  record: NightshiftdActivationRecord
...
}): NightshiftdRollbackSafety {
  const target = input.record.previous
  if (!target) {
    return {
      safety: 'unsafe',
      code: 'nightshiftd_rollback_no_target',
      reason:
        'This host has no previous nightshiftd version recorded, so there is nothing to roll back ' +
  ...
  if (!input.record.snapshot || !input.snapshotPresent) {
    return {
      safety: 'unsafe',
      code: 'nightshiftd_rollback_snapshot_missing',
      reason:
        `The pre-activation state snapshot for ${input.record.active ?? 'the active version'} ` +
        'is gone, and Nightshift state carries no schema version that could prove the older build ' +
  ...
  if (startedSinceActivation === null) {
    return {
      safety: 'unsafe',
      code: 'nightshiftd_rollback_census_unavailable',
      reason:
  ...
  if (startedSinceActivation > 0) {
    return {
      safety: 'unsafe',
      code: 'nightshiftd_rollback_orphans_live_terminals',
      reason:
