...
vi.mock('./ssh-system-fallback', () => ({
  getNightshiftControlSocketPath: vi.fn().mockReturnValue(null),
...
const RELAY_DIR = '.nightshift-remote/relay-0.1.0+hash'
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  beforeEach(() => {
    ...
    localDir = realpathSync(mkdtempSync(join(tmpdir(), 'nightshift-relay-')))
