...
import {
  clientInstances,
  findSystemSshMock,
  getNightshiftControlSocketPathMock,
...
describe('SshConnection', () => {
  ...
  it('allows concurrent exec commands for system SSH with a Nightshift ControlMaster socket', async () => {
    getNightshiftControlSocketPathMock.mockReturnValue('/tmp/nightshift-ssh-501/live-socket')
    vi.mocked(resolveWithSshG).mockResolvedValueOnce(createResolvedConfig())
    const conn = new SshConnection(createTarget({ configHost: 'fdpass-host' }), createCallbacks())
  ...
  it('keeps concurrent exec commands disabled for system SSH without a reusable socket', async () => {
    getNightshiftControlSocketPathMock.mockReturnValue(null)
    vi.mocked(resolveWithSshG).mockResolvedValueOnce(createResolvedConfig())
  ...
  it('retries a failed system SSH probe without ControlMaster and disables mux for the session', async () => {
    getNightshiftControlSocketPathMock.mockImplementation(
      (_target: SshTarget, options?: { disableControlMaster?: boolean }) =>
        options?.disableControlMaster ? null : '/tmp/nightshift-ssh-501/stale-socket'
    )
    spawnSystemSshCommandMock
    ...
    expect(removeControlSocketPathMock).toHaveBeenCalledWith('/tmp/nightshift-ssh-501/stale-socket')
  ...
  it('retries a generic system SSH probe timeout without ControlMaster', async () => {
    ...
    try {
      ...
      expect(removeControlSocketPathMock).toHaveBeenCalledWith(
        '/tmp/nightshift-ssh-501/stale-socket'
      )
      expect(spawnSystemSshCommandMock).toHaveBeenNthCalledWith(
        2,
        expect.anything(),
  ...
  it('skips a second probe for a definite host failure', async () => {
    getNightshiftControlSocketPathMock.mockReturnValue('/tmp/nightshift-ssh-501/stale-socket')
  ...
  it('uses system SSH before ssh2 parses a security-key private key', async () => {
    ...
    const directory = mkdtempSync(join(tmpdir(), 'nightshift-security-key-connect-'))
  ...
  it('uses system SSH for an agent-backed security-key public identity', async () => {
    findSystemSshMock.mockReturnValue('/usr/bin/ssh')
    const directory = mkdtempSync(join(tmpdir(), 'nightshift-security-key-agent-connect-'))
...
describe('shouldUseSystemSshTransport', () => {
  ...
  it('allows an environment override for e2e coverage', () => {
    vi.stubEnv('NIGHTSHIFT_SSH_FORCE_SYSTEM_TRANSPORT', '1')
