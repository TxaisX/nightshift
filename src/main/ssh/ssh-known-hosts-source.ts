...
export type KnownHostsEvidence = {
   ...
   * state (ssh creates known_hosts on its own first connect, and most Nightshift profiles start without
 ...
 * ssh2 dials. Never the Nightshift label — bastions tunnelled through `localhost:port` depend on the
...
export function resolveKnownHostsLookupHost(
  ...
  // no Host block matches, which for a manual target is the Nightshift label. Keying on that consults a
