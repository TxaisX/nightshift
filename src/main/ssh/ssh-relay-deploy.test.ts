...
describe('deployAndLaunchRelay', () => {
  ...
  it('calls exec to detect remote platform', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__NIGHTSHIFT_REMOTE_PLATFORM__ Linux x86_64') // tagged POSIX platform probe
    mockExecCommand.mockResolvedValueOnce('/home/user') // echo $HOME
    mockExecCommand.mockResolvedValueOnce('NIGHTSHIFT-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(mockExecCommand).toHaveBeenCalledWith(
      conn,
      "printf '\\n%s ' '__NIGHTSHIFT_REMOTE_PLATFORM__'; uname -sm",
  ...
  it('reports progress via callback', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    mockExecCommand.mockResolvedValueOnce('__NIGHTSHIFT_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('resolves the remote node path once per deploy', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__NIGHTSHIFT_REMOTE_PLATFORM__ Linux x86_64')
    mockExecCommand.mockResolvedValueOnce('/home/user')
    mockExecCommand.mockResolvedValueOnce('NIGHTSHIFT-NATIVE-DEPS-OK')
    queueLaunchNamespaceAndDeadSocketProbe()
  ...
  it('uses a content-hashed versioned remote install directory', async () => {
    ...
    const sawVersionedDir = allCmds.some((cmd) =>
      cmd.includes('/.nightshift-remote/relay-0.1.0+abcdef012345')
    )
    ...
    try {
      ...
      vi.mocked(execCommand).mockImplementation((_conn, command) => {
        ...
        if (command.includes('__NIGHTSHIFT_UPLOAD_STAGE_SLOT__') && marker) {
          return Promise.resolve(`__NIGHTSHIFT_UPLOAD_STAGE_SLOT__${marker}:slot-0`)
        }
    ...
    try {
      ...
      mockExecCommand
        ...
        .mockImplementationOnce(
          () =>
            new Promise<string>((resolve) =>
              setTimeout(() => resolve('NIGHTSHIFT-NATIVE-DEPS-OK'), 899_900)
            )
        )
        .mockResolvedValueOnce('') // launch namespace marker
  ...
  it('launches Windows remotes via a named pipe endpoint', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__NIGHTSHIFT_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
      .mockResolvedValueOnce('C:\\Users\\me user') // remote home
      .mockResolvedValueOnce('NIGHTSHIFT-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(result.sockPath).toMatch(/^\\\\\.\\pipe\\nightshift-relay-[0-9a-f]{20}$/)
    ...
    expect(launchScript).toContain(
      '"C:/Users/me user/.nightshift-remote/relay-0.1.0+abcdef012345/relay.js"'
    )
    expect(launchScript).toContain(
      '"C:/Users/me user/.nightshift-remote/relay-0.1.0+abcdef012345/agent-hooks/nightshift-relay-'
    )
  ...
  it('relaunches Windows remotes on a fallback pipe when reconnecting the occupied pipe fails', async () => {
    ...
    expect(primaryPipe).toMatch(/^\\\\\.\\pipe\\nightshift-relay-[0-9a-f]{20}$/)
    expect(fallbackPipe).toMatch(/^\\\\\.\\pipe\\nightshift-relay-[0-9a-f]{20}$/)
  ...
  it('scopes persisted Windows active pipe markers by relay target', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__NIGHTSHIFT_REMOTE_PLATFORM__ Windows X64')
