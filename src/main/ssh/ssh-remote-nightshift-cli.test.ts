...
describe('runRemoteNightshiftCli', () => {
  ...
  it('uses the remote NIGHTSHIFT_TERMINAL_HANDLE as orchestration sender identity', async () => {
    ...
    const result = await runRemoteNightshiftCli(
      runtime,
      {
        ...
        env: {
          ...
          NIGHTSHIFT_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
    )
    ...
    try {
      const result = await runRemoteNightshiftCli(
        runtime,
        {
          ...
          cwd: '/home/alice/repo',
          env: { NIGHTSHIFT_PANE_KEY: 'tab_foreign:leaf_foreign' }
        },
        LEGACY_FALLBACK_OPTIONS
      )
    ...
    try {
      const result = await runRemoteNightshiftCli(
        runtime,
        {
          ...
          env: {
            NIGHTSHIFT_TERMINAL_HANDLE: 'term_ssh',
            NIGHTSHIFT_PANE_KEY: 'tab_owner:leaf_owner'
          }
        },
        LEGACY_FALLBACK_OPTIONS
      )
  ...
  it('uses the remote NIGHTSHIFT_TERMINAL_HANDLE as orchestration check identity', async () => {
    const { runtime, db } = createRuntime()
    db.insertMessage({
      from: 'term_windows',
      to: 'term_ssh',
  ...
  it('carries the remote pane key for an implicit orchestration check', async () => {
    ...
    const result = await runRemoteNightshiftCli(
      runtime,
      {
        ...
        cwd: '/home/alice/repo',
        env: {
          NIGHTSHIFT_TERMINAL_HANDLE: 'term_stale_ssh',
          NIGHTSHIFT_PANE_KEY: 'tab_ssh:leaf_ssh'
        }
      },
      LEGACY_FALLBACK_OPTIONS
  ...
  it('routes previously-unsupported commands through the full host CLI', async () => {
    ...
    const resultPromise = runRemoteNightshiftCli(
      runtime,
      {
        argv: ['worktree', 'create', '--repo', 'nightshift', '--branch', 'fix/x', '--json'],
        cwd: '/home/alice/repo',
        env: { NIGHTSHIFT_TERMINAL_HANDLE: 'term_ssh' }
      },
      {
  ...
  it('rejects host-interactive commands with a targeted error instead of bridging them', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('nightshift serve')
  ...
  it('rejects interactive account add but still bridges account list', async () => {
    ...
    const addResult = await runRemoteNightshiftCli(
      runtime,
    ...
    const listPromise = runRemoteNightshiftCli(
      runtime,
  ...
  it('bridges account add help because it does not start an interactive login', async () => {
    ...
    await Promise.resolve()
    child.stdout.emit('data', Buffer.from('Usage: nightshift account add\n'))
    child.emit('close', 0)

    await expect(resultPromise).resolves.toEqual({
      stdout: 'Usage: nightshift account add\n',
      stderr: '',
      exitCode: 0
    })
    expect(spawn).toHaveBeenCalledOnce()
  ...
  it('explains the root cause when falling back and the command is not in the legacy switch', async () => {
    ...
    expect(result.exitCode).toBe(1)
    expect(result.stderr).toContain('Unsupported SSH Nightshift CLI command: worktree list')
    expect(result.stderr).toContain('full Nightshift CLI bridge unavailable')
  })
  ...
  it('does not parse Android --activity values as Linear boolean flags', async () => {
    ...
    expect(result.stderr).toContain(
      'Unsupported SSH Nightshift CLI command: emulator launch com.acme.app'
    )
