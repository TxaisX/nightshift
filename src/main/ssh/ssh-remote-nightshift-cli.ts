...
import {
  ...
  type RemoteNightshiftCliRequest,
  type RemoteNightshiftCliResult
...
export type {
  RemoteNightshiftCliRequest,
  RemoteNightshiftCliResult
} from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'nightshift serve starts a foreground headless Nightshift server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Nightshift.',
  'claude-teams':
    'nightshift claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Nightshift host machine.',
  'agent-teams-tmux':
    'nightshift agent-teams-tmux is a tmux pane shim for the Nightshift host machine and cannot run through the SSH relay bridge.',
  'account add':
    'nightshift account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Nightshift host machine.'
}

export async function runRemoteNightshiftCli(
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteNightshiftCli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostNightshiftCliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteNightshiftCli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteNightshiftCli(
  runtime: NightshiftRuntimeService,
  request: RemoteNightshiftCliRequest,
...
async function dispatchRemoteCli(
  ...
  runtimeAuthority: RemoteNightshiftCliRequest['runtimeAuthority']
): Promise<RpcResponse> {
  const command = parsed.commandPath.join(' ')
  ...
  switch (command) {
    case 'status': {
      ...
      const cliStatus: CliStatusResult = {
        ...
        // Why: this answers for the Nightshift host the caller reached over SSH, not for the caller's
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.NIGHTSHIFT_PANE_KEY || undefined
        },
        {
      ...
      return await call(
        ...
        'orchestration.check',
        {
          ...
          terminalPaneKey: parsed.flags.has('terminal')
            ? undefined
            : env.NIGHTSHIFT_PANE_KEY || undefined,
          ...
          compatibilityCliCommand: 'nightshift',
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH Nightshift CLI command: ${command} (full Nightshift CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
