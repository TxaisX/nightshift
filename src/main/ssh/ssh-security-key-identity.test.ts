...
vi.mock('node:os', async (importOriginal) => {
  ...
  return {
    ...actual,
    homedir: () => process.env.NIGHTSHIFT_TEST_SSH_HOME || actual.homedir()
  }
})
...
async function writeKey(contents: Buffer, filename = 'security key'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'nightshift-security-key-'))
...
async function createDefaultKeyHome(files: Record<string, Buffer>): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'nightshift-default-key-home-'))
  tempDirs.push(directory)
...
describe('requiresSystemSshForSecurityKey', () => {
  it('uses default FIDO2 identities only when config resolution is unavailable', async () => {
    ...
    vi.stubEnv('NIGHTSHIFT_TEST_SSH_HOME', directory)
  ...
  it.each([ED25519_SECURITY_KEY, ECDSA_SECURITY_KEY])(
    ...
    async (keyType) => {
      const directory = await mkdtemp(join(tmpdir(), 'nightshift-security-key-agent-'))
      tempDirs.push(directory)
