...
export type SystemSshBuildArgsOptions = {
  ...
  suppressNightshiftControlMaster?: boolean
  gssapiOnly?: boolean
...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  if (options?.gssapiOnly) {
    ...
    // credential prompt; failure belongs to Nightshift's existing ssh2 prompt path.
  ...
  const controlPath = getNightshiftControlSocketPath(target, options)
  ...
  // as the wildcard's user, at the wildcard's host, discarding the endpoint Nightshift stored. Keep the
...
export function getNightshiftControlSocketPath(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): string | null {
  if (shouldDisableNightshiftControlMaster(target, options)) {
    return null
  }
...
export function getSystemSshBuildArgsFromOperationOptions(
  ...
  if (options?.suppressNightshiftControlMaster === true) {
    buildArgsOptions.suppressNightshiftControlMaster = true
  }
...
function shouldDisableNightshiftControlMaster(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): boolean {
  // Why: unresolved ssh_config aliases could otherwise share one Nightshift socket
  ...
  return (
    options?.disableControlMaster === true ||
    options?.suppressNightshiftControlMaster === true ||
    target.systemSshConnectionReuse === false ||
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master Nightshift needs.
  return (
