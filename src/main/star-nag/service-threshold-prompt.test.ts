...
describe('StarNagService', () => {
  ...
  it('shows the browser fallback when checkNightshiftStarred cannot determine star state', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkNightshiftStarredMock.mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not log a threshold exposure when checkNightshiftStarred returns true', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkNightshiftStarredMock.mockResolvedValue(true)
    const { service, emitAgentStarted } = createHarness()
