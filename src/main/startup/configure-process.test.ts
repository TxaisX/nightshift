...
describe('patchPackagedProcessPath', () => {
  ...
  it('prepends agent-CLI install dirs (~/.opencode/bin, ~/.vite-plus/bin) for packaged darwin runs', async () => {
    ...
    // Without them on PATH, GUI-launched Nightshift reports both as "Not installed"
...
describe('configureDevUserDataPath', () => {
  it('forces Electron home into the disposable E2E profile', async () => {
    ...
    const originalE2EUserDataDir = process.env.NIGHTSHIFT_E2E_USER_DATA_DIR
    const originalE2EHomeDir = process.env.NIGHTSHIFT_E2E_HOME_DIR
    const originalHome = process.env.HOME
    const originalUserProfile = process.env.USERPROFILE
    const tempRoot = mkdtempSync(join(tmpdir(), 'nightshift-configure-e2e-home-'))
    const e2eRoot = join(tempRoot, 'user-data')
    const e2eHome = join(tempRoot, 'home')
    process.env.NIGHTSHIFT_E2E_USER_DATA_DIR = e2eRoot
    process.env.NIGHTSHIFT_E2E_HOME_DIR = e2eHome
    ...
    try {
      ...
      if (originalE2EUserDataDir === undefined) {
        delete process.env.NIGHTSHIFT_E2E_USER_DATA_DIR
      } else {
        process.env.NIGHTSHIFT_E2E_USER_DATA_DIR = originalE2EUserDataDir
      }
      if (originalE2EHomeDir === undefined) {
        delete process.env.NIGHTSHIFT_E2E_HOME_DIR
      } else {
        process.env.NIGHTSHIFT_E2E_HOME_DIR = originalE2EHomeDir
      }
      if (originalHome === undefined) {
        delete process.env.HOME
      } else {
  ...
  it('rejects an E2E launch whose Node home escaped the disposable profile', async () => {
    ...
    const originalHome = process.env.HOME
    const originalUserProfile = process.env.USERPROFILE
    const e2eRoot = mkdtempSync(join(tmpdir(), 'nightshift-configure-e2e-escape-'))
    process.env.NIGHTSHIFT_E2E_USER_DATA_DIR = e2eRoot
    process.env.NIGHTSHIFT_E2E_HOME_DIR = join(e2eRoot, 'home')
    ...
    try {
      ...
      restoreEnv('NIGHTSHIFT_E2E_USER_DATA_DIR', originalE2EUserDataDir)
      restoreEnv('NIGHTSHIFT_E2E_HOME_DIR', originalE2EHomeDir)
  ...
  it('uses an explicit dev userData override when provided', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    const originalOverride = process.env.NIGHTSHIFT_DEV_USER_DATA_PATH
    process.env.NIGHTSHIFT_DEV_USER_DATA_PATH = '/tmp/nightshift-dev-repro'

    try {
      configureDevUserDataPath(true)
    } finally {
      if (originalOverride === undefined) {
        delete process.env.NIGHTSHIFT_DEV_USER_DATA_PATH
      } else {
        process.env.NIGHTSHIFT_DEV_USER_DATA_PATH = originalOverride
      }
    }

    expect(app.setPath).toHaveBeenCalledWith('userData', '/tmp/nightshift-dev-repro')
  })

  it('moves dev runs onto a nightshift-dev userData path', async () => {
    const { app } = await import('electron')
    const { configureDevUserDataPath } = await import('./configure-process')
    ...
    // Why: production code uses path.join(app.getPath('appData'), 'nightshift-dev')
    // which produces platform-specific separators.
    expect(app.setPath).toHaveBeenCalledWith('userData', join('/tmp/app-data', 'nightshift-dev'))
  })
...
describe('configureNightshiftUserDataPathEnv', () => {
  it('overwrites stale inherited NIGHTSHIFT_USER_DATA_PATH with Electron userData', async () => {
    const { app } = await import('electron')
    const { configureNightshiftUserDataPathEnv } = await import('./configure-process')
    const originalUserDataPath = process.env.NIGHTSHIFT_USER_DATA_PATH
    process.env.NIGHTSHIFT_USER_DATA_PATH = '/tmp/stale-nightshift-user-data'
    app.setPath('userData', '/tmp/current-nightshift-user-data')
    let configuredUserDataPath: string | undefined

    try {
      configureNightshiftUserDataPathEnv()
      configuredUserDataPath = process.env.NIGHTSHIFT_USER_DATA_PATH
    } finally {
      if (originalUserDataPath === undefined) {
        delete process.env.NIGHTSHIFT_USER_DATA_PATH
      } else {
        process.env.NIGHTSHIFT_USER_DATA_PATH = originalUserDataPath
      }
    }

    expect(configuredUserDataPath).toBe('/tmp/current-nightshift-user-data')
  })
})
...
describe('configureElectronNetworkCompatibility', () => {
  ...
  const originalEnvValue = process.env.NIGHTSHIFT_DISABLE_HTTP2

  function createUserDataDir(settings: Record<string, unknown>): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'nightshift-http1-compat-'))
    tempDirs.push(userDataPath)
    writeFileSync(join(userDataPath, 'nightshift-data.json'), JSON.stringify({ settings }), 'utf-8')
    return userDataPath
  }
  ...
  afterEach(() => {
    ...
    if (originalEnvValue === undefined) {
      delete process.env.NIGHTSHIFT_DISABLE_HTTP2
    } else {
      process.env.NIGHTSHIFT_DISABLE_HTTP2 = originalEnvValue
    }
  })
  ...
  it('lets the environment override force compatibility on', async () => {
    ...
    expect(
      shouldDisableHttp2ForElectronNetworking({
        env: { NIGHTSHIFT_DISABLE_HTTP2: 'true' },
  ...
  it('lets the environment override force compatibility off', async () => {
    ...
    expect(
      shouldDisableHttp2ForElectronNetworking({
        env: { NIGHTSHIFT_DISABLE_HTTP2: '0' },
  ...
  it('answers from the marker without reading the settings file', async () => {
    ...
    rmSync(join(userDataPath, 'nightshift-data.json'), { force: true })
...
describe('enableMainProcessGpuFeatures', () => {
  ...
  it('disables the GPU process for Linux E2E runs', async () => {
    ...
    setPlatform('linux')
    process.env.NIGHTSHIFT_E2E_USER_DATA_DIR = '/tmp/nightshift-e2e'
    vi.mocked(app.disableHardwareAcceleration).mockClear()
    vi.mocked(app.commandLine.appendSwitch).mockClear()
