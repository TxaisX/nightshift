...
const BASE_APP_NAME = 'Nightshift'
const BASE_APP_USER_MODEL_ID = 'com.txais.nightshift'
...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Nightshift'.
  appName: string
}
 ...
 * CFBundleName, which downstream forks ship differently ("Nightshift ALab Edition").
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.NIGHTSHIFT_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.NIGHTSHIFT_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.NIGHTSHIFT_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel =
    cleanEnvValue(env.NIGHTSHIFT_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.NIGHTSHIFT_DEV_DOCK_TITLE) ??
    `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Nightshift Dev Safe Storage') for all dev
