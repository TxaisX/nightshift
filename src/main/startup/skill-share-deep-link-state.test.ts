...
describe('SkillShareDeepLinkState', () => {
  it('queues a startup share until the renderer consumes it once', () => {
    ...
    expect(state.capture(['nightshift', 'https://app.nightshift.invalid/skills/share/share_startup'])).toBe(
      true
    )
  ...
  it('publishes a later second-instance intent and keeps it for renderer recovery', () => {
    ...
    state.capture(['nightshift', 'https://app.nightshift.invalid/skills/share/share_first'])
    expect(state.capture(['nightshift', 'nightshift://skills/share/share_second'], publish)).toBe(
      true
    )
  ...
  it('ignores untrusted URLs without replacing a pending intent', () => {
    const state = new SkillShareDeepLinkState()
    state.capture(['nightshift', 'https://app.nightshift.invalid/skills/share/share_safe'])

    expect(state.capture(['nightshift', 'https://attacker.test/skills/share/share_bad'])).toBe(
      false
    )
