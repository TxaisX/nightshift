  ...
  const started = repairWindowsInstallDirPackageAcl({
    ...
    onDone: (result) => {
      ...
      if (result.mode === 'repaired' && !claimDisproved) {
        ...
        // Never a user-confirmed marker: "keep safe graphics" is a choice, not Nightshift's latch.
...
const CAUSE =
  "Windows permissions on Nightshift's install folder are blocking its own sandboxed processes from reading the files it shipped with."
...
export function describeInstallDirAclPoison(): InstallDirAclPoisonDiagnosis | null {
  ...
  if (poison.stage === 'repaired') {
    return {
      detail: `${CAUSE}\n\nNightshift repaired the permissions. Reload to use them.`,
      commands
    }
  }
  const status =
    poison.stage === 'pending'
      ? 'Nightshift is repairing the permissions now.'
      : 'Nightshift could not repair them, which usually means the folder needs an administrator.'
  return {
    detail: `${CAUSE} ${status}\n\nRun these in an Administrator Command Prompt, then relaunch Nightshift:\n\n${commands.join('\n')}`,
    commands
  }
}
