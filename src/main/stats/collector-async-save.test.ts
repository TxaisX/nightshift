...
const statsPath = (): string => join(userDataDir, 'nightshift-stats.json')
...
describe('StatsCollector async debounced save', () => {
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'nightshift-stats-test-'))
    gate.blocked = false
  ...
  it('writes valid JSON via the async fs/promises path on the debounced timer, no stray temp files', async () => {
    ...
    await vi.waitFor(() => {
      expect(readdirSync(userDataDir)).toContain('nightshift-stats.json')
    })
