...
function makeHome(): string {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'nightshift-commit-env-'))
  tempDirs.push(dir)
...
describe('prepareLocalCommitMessageAgentEnv', () => {
  ...
  it('prefers the original OpenCode config root over inherited PTY overlays', async () => {
    process.env.OPENCODE_CONFIG_DIR = '/tmp/nightshift-opencode-overlay'
    process.env.NIGHTSHIFT_OPENCODE_SOURCE_CONFIG_DIR = '/Users/tester/company/opencode'
  ...
  it('prefers the original Pi agent root over inherited PTY overlays', async () => {
    process.env.PI_CODING_AGENT_DIR = '/tmp/nightshift-pi-overlay'
    process.env.NIGHTSHIFT_PI_SOURCE_AGENT_DIR = '/Users/tester/.pi/agent'
  ...
  it('sets CODEX_HOME for host managed Codex accounts', async () => {
    const result = await prepareLocalCommitMessageAgentEnv('codex', {
      prepareForCodexLaunch: () =>
        'C:\\Users\\tester\\AppData\\Roaming\\Nightshift\\codex-accounts\\a\\home'
    })

    expect(result).toEqual({
      ok: true,
      env: expect.objectContaining({
        CODEX_HOME: 'C:\\Users\\tester\\AppData\\Roaming\\Nightshift\\codex-accounts\\a\\home'
      })
    })
  })
  ...
  it('strips a nested-Nightshift CODEX_HOME override when the launch resolves to the real home', async () => {
    process.env.CODEX_HOME = '/managed/runtime/home'
    process.env.NIGHTSHIFT_CODEX_HOME = '/managed/runtime/home'
