...
function makeEntries(
  ...
  return items.map((item) => ({
    ...
    releaseNotesUrl: item.releaseNotesUrl ?? `https://nightshift.invalid/changelog/${item.version}`
  }))
}
...
describe('fetchChangelog', () => {
  ...
  it('returns exact match when the incoming version has rich content', async () => {
    const entries = makeEntries([
      {
        ...
        mediaUrl: 'https://nightshift.invalid/media/1.1.21.gif'
      },
      { version: '1.1.20' },
      { version: '1.1.19' }
    ])
    ...
    expect(result!.release.releaseNotesUrl).toBe('https://nightshift.invalid/changelog/1.1.21')
  ...
  it('falls back to the most recent rich entry when incoming version is not in JSON', async () => {
    ...
    const entries = makeEntries([
      {
        ...
        mediaUrl: 'https://nightshift.invalid/media/1.1.17.gif',
        releaseNotesUrl: 'https://nightshift.invalid/changelog/1.1.17'
      },
      { version: '1.1.16' },
      { version: '1.1.15' }
    ])
    ...
    expect(result!.release.releaseNotesUrl).toBe('https://nightshift.invalid/changelog')
    expect(result!.releasesBehind).toBe(2)
  })
  ...
  it('skips rich entries that the user has already passed', async () => {
    ...
    const entries = makeEntries([
      ...
      { version: '1.1.18' },
      {
        ...
        mediaUrl: 'https://nightshift.invalid/media/old.gif'
      }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
  ...
  it('shows rich entry at the same version as localVersion', async () => {
    ...
    const entries = makeEntries([
      { version: '1.1.20', description: '' },
      {
        ...
        mediaUrl: 'https://nightshift.invalid/media/current.gif'
      },
      { version: '1.1.17' }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
  ...
  it('shows rich entry when local version is not in JSON (very old user)', async () => {
    const entries = makeEntries([
      { version: '1.1.20', description: '' },
      {
        ...
        mediaUrl: 'https://nightshift.invalid/media/demo.gif'
      }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
    ...
    const entries = makeEntries([
      {
        ...
        mediaUrl: 'https://nightshift.invalid/media/latest.gif'
      },
      {
        version: '1.1.17',
  ...
  it('strips version from the returned release object', async () => {
    const entries = makeEntries([
      { version: '1.1.17', description: 'Feature', mediaUrl: 'https://nightshift.invalid/media/demo.gif' },
      { version: '1.1.15' }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
