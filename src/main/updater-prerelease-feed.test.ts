...
function buildAtomFeed(tags: string[]): string {
  const entries = tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/TxaisX/nightshift/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/TxaisX/nightshift/releases.atom') {
