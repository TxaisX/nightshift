...
const STAGING_ROOT = join(TEMP_ROOT, `nightshift-clipboard-files${UID_SUFFIX}`)
...
const LEGACY_EXPIRED = `nightshift-clipboard-file-${EXPIRED_TRANSFER}`
const LEGACY_FRESH = `nightshift-clipboard-file-${FRESH_TRANSFER}`
...
describe('legacy remote clipboard staging compatibility', () => {
  ...
  it('removes only exact expired legacy children encountered inside the bound', async () => {
    opendirMock.mockResolvedValue(
      openedDirectory([
        directoryEntry('nightshift-clipboard-file-lookalike'),
