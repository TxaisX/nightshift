...
function userDataWithProfiles(
  ...
  const root = mkdtempSync(join(tmpdir(), 'nightshift-gc-profiles-'))
  roots.push(root)
  mkdirSync(join(root, 'profiles'), { recursive: true })
  writeFileSync(
    join(root, 'nightshift-profile-index.json'),
    JSON.stringify({
      activeProfileId,
  ...
  for (const profile of profiles) {
    ...
    if (profile.raw !== undefined) {
      writeFileSync(join(root, 'profiles', profile.id, 'nightshift-data.json'), profile.raw)
    } else if (profile.state !== undefined) {
      writeFileSync(
        join(root, 'profiles', profile.id, 'nightshift-data.json'),
