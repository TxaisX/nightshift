...
const SNAPSHOT_FILE = 'nightshift-workspace-space-analysis.json'
...
describe('workspace space analysis snapshot', () => {
  beforeEach(async () => {
    ...
    userDataDirHolder.dir = await mkdtemp(join(tmpdir(), 'nightshift-space-snapshot-'))
  })
  ...
  it('keeps profile snapshots isolated', async () => {
    const otherProfile = await mkdtemp(join(tmpdir(), 'nightshift-space-snapshot-other-'))
    try {
      await persistWorkspaceSpaceAnalysisSnapshot(
        userDataDirHolder.dir,
