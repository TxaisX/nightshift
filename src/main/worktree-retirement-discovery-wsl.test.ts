...
const DISTRO_ROOT = '\\\\wsl.localhost\\Ubuntu\\home\\ada\\nightshift\\workspaces'
...
describe('retirement discovery on WSL', () => {
  ...
  it('reports a gate refusal as incomplete but still returns what the host side could read', async () => {
    ...
    const hostHome = await mkdtemp(join(tmpdir(), 'nightshift-wsl-host-home-'))
    try {
      await mkdir(
        join(
          hostHome,
  ...
  it('keeps a deleted WSL workspace name spent, so the next create cannot reuse its cwd', async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'nightshift-wsl-distro-home-'))
    const workspaceRoot = await mkdtemp(join(tmpdir(), 'nightshift-wsl-workspaces-'))
    try {
      await mkdir(
  ...
  it('reaches distro discovery from the real backfill, not only when called directly', async () => {
    ...
    const distroHome = await mkdtemp(join(tmpdir(), 'nightshift-wsl-backfill-home-'))
    try {
      await mkdir(
        join(distroHome, '.claude', 'projects', `-home-ada-nightshift-workspaces-${FIRST}`),
        {
