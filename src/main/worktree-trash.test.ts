...
beforeEach(async () => {
  scratchDir = await mkdtemp(join(tmpdir(), 'nightshift-worktree-trash-'))
})
...
describe('sweepStaleWorktreeTrash', () => {
  it('removes leftover entries from both workspace layouts and nothing else', async () => {
    ...
    // Not generated by Nightshift: a stray directory and file inside the trash root must survive.
...
describe('collectWorktreeTrashSweepRoots', () => {
  const settings = { workspaceDir: '/home/dev/nightshift/workspaces', nestWorkspaces: true }

  function repo(overrides: Partial<Repo>): Repo {
    return { id: 'repo-1', path: '/home/dev/code/nightshift', ...overrides } as unknown as Repo
  }
  ...
  it('collects one root per local git repo', () => {
    ...
    ).toEqual(['/home/dev/nightshift/workspaces'])
  })
  ...
  it('skips WSL repos that cannot create host trash', () => {
    ...
    try {
      expect(
        collectWorktreeTrashSweepRoots(
          [
            repo({ path: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\nightshift' }),
            repo({
              id: 'repo-2',
              path: 'C:\\code\\nightshift',
