 ...
 * that the rest of Nightshift's startup features arrive with it.
 *
 * macOS `/etc/zshrc` assigns `HISTFILE=${ZDOTDIR:-$HOME}/.zsh_history` with no
 * check-before-set. Nightshift used to fight that by keeping its own ZDOTDIR in place
 ...
 * Nightshift would not wrap cannot pass here by construction.
 */
...
const systemZshrcClobbersHistfile = (() => {
  ...
  const home = mkdtempSync(join(tmpdir(), 'nightshift-histfile-probe-'))
  try {
...
/** The launch config Nightshift produces for a pane with exactly these features. */
function launchPane(
  home: string,
  ...
  const env: Record<string, string> = {
    HOME: home,
    ...(scopedHistfile ? { HISTFILE: scopedHistfile, NIGHTSHIFT_HISTFILE: scopedHistfile } : {})
  }
  ...
  return {
    features,
    launch,
    // Why NIGHTSHIFT_ORIG_ZDOTDIR overridden: the launch config resolves the user's
    // config dir from the real process env, and these runs must resolve against
    // the sandbox home instead.
    env: { PATH: '/usr/bin:/bin', ...env, ...launch.env, NIGHTSHIFT_ORIG_ZDOTDIR: home }
  }
}
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    itWithZsh(
      'wraps a plain pane once Nightshift injected a worktree HISTFILE',
      withHome(USER_FILES, async (home) => {
        ...
        expect(launch.env.NIGHTSHIFT_SHELL_FEATURES).toBe('history')
      })
    )
    ...
    itWithZsh(
      ...
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'nightshift-history', 'zsh_history'))

        const { values } = await runZshPty({
          env,
          report: [
            'ZDOTDIR',
            'NIGHTSHIFT_TEST_USER_ZSHENV',
        ...
        expect(values.NIGHTSHIFT_TEST_USER_ZSHENV).toBe('1')
    ...
    itWithZsh(
      'never leaves history inside Nightshift’s own wrapper directory',
      withHome(USER_FILES, async (home) => {
        const { env, launch } = launchPane(home, join(home, 'nightshift-history', 'zsh_history'))
    ...
    itWithZsh(
      'consumes NIGHTSHIFT_HISTFILE so nothing the shell spawns can inherit it',
      withHome(USER_FILES, async (home) => {
        const scoped = join(home, 'nightshift-history', 'zsh_history')
        const { env } = launchPane(home, scoped)
        ...
        const { values } = await runZshPty({
          env,
          report: ['NIGHTSHIFT_HISTFILE', 'NIGHTSHIFT_SHELL_FEATURES', 'HISTFILE']
        })

        // Root-cause fix for #11146: the variables no longer exist after use.
        expect(values.NIGHTSHIFT_HISTFILE).toBe('UNSET')
    ...
    itWithZsh(
      'leaves HISTFILE exactly as an unwrapped zsh would when Nightshift injects nothing',
      withHome(USER_FILES, async (home) => {
        ...
        // The contract is that Nightshift's wrapper does not change it either way.
        const overlayEnv = { NIGHTSHIFT_CODEX_HOME: join(home, 'codex') }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
...
describe.skipIf(process.platform === 'win32')('the deferred hook delivers every feature', () => {
  ...
  itWithZsh(
    'restores Nightshift’s overlay values after the user’s config overwrites them',
    withHome(
      {
        ...USER_FILES,
      ...
      async (home) => {
        const overlayEnv = {
          NIGHTSHIFT_CODEX_HOME: '/nightshift/codex',
          NIGHTSHIFT_AGENT_TEAMS_SHIM_DIR: '/nightshift/shim'
        }
        const features = selectShellStartupFeatures({
          shellPath: ZSH_PATH,
          env: { HOME: home, ...overlayEnv },
        ...
        // the spawn env did, and Nightshift's values still win.
        expect(values.CODEX_HOME).toBe('/nightshift/codex')
        expect(values.PATH.startsWith('/nightshift/shim:')).toBe(true)
      }
    )
  )
})
...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
     ...
     * `.zshenv` or `.zprofile` cost the pane all of Nightshift's features. Only one
    ...
    it.each([
      ...
      try {
        ...
        const { values } = await runZshPty({ env, report: ['HISTFILE', 'NIGHTSHIFT_HISTFILE'] })
    ...
    itWithZsh(
      ...
      withHome({ ...USER_FILES, '.zshrc': 'precmd_functions=()\n' }, async (home) => {
        ...
        const report = ['HISTFILE', 'NIGHTSHIFT_HISTFILE', 'ZDOTDIR']
        ...
        // host's call, not Nightshift's. macOS /etc/zshrc overwrites HISTFILE, so it
        ...
        expect(values.HISTFILE).not.toContain(launch.env.ZDOTDIR)
        // NIGHTSHIFT_HISTFILE was consumed in .zshenv precisely so a dropped hook
    ...
    itWithZsh(
      ...
      withHome(
        ...
        async (home) => {
          const { env } = launchPane(home, join(home, 'nightshift-history', 'zsh_history'))

          const { values } = await runZshPty({
            env,
            report: ['ZDOTDIR', 'NIGHTSHIFT_TEST_USER_ZSHRC']
          })
...
describe.skipIf(process.platform === 'win32')('the relay variant of the hook', () => {
  itWithZsh(
    ...
    withHome(USER_FILES, async (home) => {
      const relayRoot = mkdtempSync(join(tmpdir(), 'nightshift-relay-wrapper-'))
      try {
        ...
        const { output, values } = await runZshPty({
          env: {
            ...
            NIGHTSHIFT_ORIG_ZDOTDIR: home,
            NIGHTSHIFT_SHELL_FEATURES: 'overlay,history,ready',
            NIGHTSHIFT_HISTFILE: scoped,
            NIGHTSHIFT_REMOTE_CLI_BIN_DIR: '/nightshift/remote-bin'
          },
          commands: ['true'],
          report: ['HISTFILE', 'ZDOTDIR', 'PATH', 'NIGHTSHIFT_HISTFILE']
        })

        expect(values.HISTFILE).toBe(scoped)
        expect(values.ZDOTDIR).toBe(home)
        expect(values.NIGHTSHIFT_HISTFILE).toBe('UNSET')
        expect(values.PATH.startsWith('/nightshift/remote-bin:')).toBe(true)
        expect(output).toContain(MARKERS.ready)
