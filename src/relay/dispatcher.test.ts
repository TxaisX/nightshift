...
describe('RelayDispatcher', () => {
  ...
  it('forwards relay-originated requests to an owning socket client instead of the caller', async () => {
    ...
    expect(request.method).toBe('nightshift.cli')
  ...
  it('detaches the primary client when its write throws (frame lost, trigger reconnect)', () => {
    ...
    // owning Nightshift's reconnect + PTY-reattach path never engaged until the ~20s
    ...
    try {
      ...
      // A frame the owning Nightshift must not silently miss (e.g. a pane exit).
      throwOnWrite = true
