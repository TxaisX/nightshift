...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const zshRoot = join(homeDir, '.nightshift-relay', 'shell-ready', 'zsh')

      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      const zshenv = readFileSync(join(zshRoot, '.zshenv'), 'utf8')
      // Why no NIGHTSHIFT_USER_ZDOTDIR: the relay used to republish the inherited
      ...
      expect(zshenv).not.toContain('NIGHTSHIFT_USER_ZDOTDIR')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      // bridge sets NIGHTSHIFT_REMOTE_CLI_BIN_DIR, which was already enough to wrap.
      const env = buildSshPtySpawnEnv({
        env: { HOME: homeDir, PATH: '/usr/bin:/bin' },
        remoteCliBridgeEnv: {
          binDir: '/home/remote/.nightshift-relay/bin',
          relayDir: '/home/remote/.nightshift-relay',
          nodePath: '/home/remote/.nightshift-relay/node',
          sockPath: '/home/remote/.nightshift-relay/relay.sock'
        }
      })
      env.NIGHTSHIFT_HISTFILE = join(homeDir, 'nightshift-history', 'zsh_history')

      const config = getRelayShellLaunchConfig('/bin/zsh', env)

      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.nightshift-relay', 'shell-ready', 'zsh'))
      expect(config.env.NIGHTSHIFT_SHELL_FEATURES).toBe('overlay,history,markers')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'keeps a remote zsh with nothing Nightshift-owned on the plain login path',
    () => {
      const env = buildSshPtySpawnEnv({ env: { HOME: homeDir, PATH: '/usr/bin:/bin' } })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env).toEqual({ NIGHTSHIFT_SHELL_FEATURES: '' })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.NIGHTSHIFT_SHELL_FEATURES).toContain('ready')
      ...
      expect(zshenv).toContain('builtin unset NIGHTSHIFT_SHELL_FEATURES')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.supportsReadyMarker).toBe(true)
      expect(config.env.NIGHTSHIFT_SHELL_FEATURES).toContain('ready')
      expect(bashRc).toContain('printf "\\033]777;nightshift-shell-start:%s\\007" "$$"')
      expect(bashRc).toContain('builtin unset NIGHTSHIFT_SHELL_FEATURES')
      expect(bashRc).toContain('__nightshift_append_prompt_command "__nightshift_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;nightshift-shell-ready\\007"')
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.ZDOTDIR).toBe(join(homeDir, '.nightshift-relay', 'shell-ready', 'zsh'))
      expect(config.env.NIGHTSHIFT_SHELL_FEATURES).toContain('identity')
      ...
      expect(config.env.NIGHTSHIFT_SHELL_FEATURES).not.toContain('ready')
  ...
  itWithBash('runs the relay bash wrapper without fake C/D markers before the first prompt', () => {
    ...
    expect(bashRc).toContain('[[ -z "${__nightshift_in_command:-}" ]] || return 0')
    expectBashOsc133Lifecycle(output)
  })

  itWithBash('emits lifecycle for foreground text ending like an internal hook', () => {
    const config = getRelayShellLaunchConfig('/bin/bash', { HOME: homeDir })
    const input = 'echo user:__nightshift_osc133_prompt_done\nfalse\nexit 0\n'
    const output = runInteractiveBashRcfile(config.args[1] as string, homeDir, input)

    expect(output).toContain('user:__nightshift_osc133_prompt_done')
    expectBashOsc133Lifecycle(output)
  })
  ...
  itWithBash('preserves relay bash prompt hooks and DEBUG traps without fake markers', () => {
    ...
    expect(output).not.toContain('USER_DEBUG_AFTER:<(( __nightshift_exit_code == 0 ))>')
    expect(output).not.toContain('USER_DEBUG_AFTER:<__nightshift_restore_prompt_status')
    expectBashOsc133Lifecycle(output)
  })
  ...
  // Nightshift inherits ends in a ";"+whitespace separator. Prepend/append must not
