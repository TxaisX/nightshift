...
export async function runRelayNightshiftCliChannel(
  sockPath: string,
  argv: string[],
  ...
  const stdin =
    ...
    (shouldReadRemoteCliStdin(argv) ? await readNightshiftCliStdin() : undefined)
      ...
      encodeJsonRpcFrame(
        {
          ...
          method: 'nightshift.cli.postOutput',
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(
      `[nightshift-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`
    )
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (error) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[nightshift-cli] Relay socket error: ${error.message}\n`)
    process.exit(1)
  })
}

async function readNightshiftCliStdin(): Promise<string | undefined> {
