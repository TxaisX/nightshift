...
describe('relay WSL shell history', () => {
  it('hands the guest a drvfs path for the host history file', async () => {
    ...
    expect(root).toBe('C:\\Users\\relay/.nightshift-remote/terminal-history')
    expect(env.HISTFILE).toBe(
      `/mnt/c/Users/relay/.nightshift-remote/terminal-history/${hashWorktreeId(worktreeId)}-bash_history`
    )
  ...
  it('leaves a host shell on the untranslated host path', async () => {
    ...
    expect(env.HISTFILE).toBe(
      `C:\\Users\\relay/.nightshift-remote/terminal-history/${hashWorktreeId(worktreeId)}-bash_history`
    )
  })
