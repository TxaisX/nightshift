...
// parsed hook envelope to Nightshift over this process's own stdin/stdout using the
...
async function main(): Promise<void> {
  const windowsPort = Number(process.env.NIGHTSHIFT_AGENT_HOOK_PORT ?? '')
  const token = process.env.NIGHTSHIFT_AGENT_HOOK_TOKEN ?? ''
  if (!Number.isInteger(windowsPort) || windowsPort <= 0 || token.length === 0) {
    process.stderr.write('[wsl-hook-relay] missing NIGHTSHIFT_AGENT_HOOK_PORT/TOKEN in env\n')
    process.exit(1)
  }
