...
const INSTALL_COMMAND =
  'sudo apt-get install -y /tmp/nightshift-updates/nightshift_1.4.200_amd64.deb'
const PACKAGE_FILE_NAME = 'nightshift_1.4.200_amd64.deb'
const SUMMARY =
  'Nightshift downloaded the system package. Quit Nightshift before finishing the update from a terminal.'
const COPIED_NOTE =
  `Command copied. Quit Nightshift, run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then reopen Nightshift.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard copy', () => {
  it('leads with the manual-install copy and recovery actions', () => {
    ...
    expect(
      screen.getByText(/a system terminal on the computer where Nightshift is installed/)
    ).toBeTruthy()
    expect(screen.getByText(/Copy the command, quit Nightshift/)).toBeTruthy()
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    const detail = screen.getByText(
      /Nightshift checks the downloaded file against the release metadata/
    )
    ...
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and Nightshift cannot vouch for the file ' +
        'after that point.'
    )
  })
...
describe('LinuxPackageInstallRecoveryCard without a usable command', () => {
  it('promotes Show Package and keeps the official-release fallback', async () => {
    ...
    expect(screen.getByText(/Quit Nightshift before finishing the update/)).toBeTruthy()
