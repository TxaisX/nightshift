...
const repo = { id: REPO_ID, displayName: 'nightshift', path: '/src/nightshift' } as Repo
...
describe('automation search row fields', () => {
  it('indexes every axis the design doc requires for a local automation', () => {
    ...
    expect(fields).toEqual({
      name: 'Nightly sweep',
      project: 'nightshift /src/nightshift',
...
describe('automation search row index', () => {
  it('builds one index per row and matches in a single pass', () => {
    ...
    expect(matchAutomationListSearchRowKeys(rows, 'nightshift')).toEqual([first, second])
