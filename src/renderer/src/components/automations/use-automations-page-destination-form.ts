...
export function useAutomationsPageDestinationForm({
  ...
  const isNightshiftForm = createTarget === 'nightshift' && local.editingExternalTarget === null
  ...
  const dialogRepos = isNightshiftForm
  ...
  useEffect(() => {
    if (!createOpen || editingAutomationId !== null || createTarget !== 'nightshift') {
      return
    }
    setDraft((current) =>
