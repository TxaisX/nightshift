...
describe('browser-focus', () => {
  ...
  it('requestBrowserFocus queues and dispatches the focus event', () => {
    ...
    window.addEventListener(NIGHTSHIFT_BROWSER_FOCUS_REQUEST_EVENT, onFocusRequest)
    requestBrowserFocus(detail)
    window.removeEventListener(NIGHTSHIFT_BROWSER_FOCUS_REQUEST_EVENT, onFocusRequest)
