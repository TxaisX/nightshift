...
const MARKUP_DRAW_HINT_SEEN_KEY = 'nightshift.browser.markup-draw-hint-seen'
...
vi.mock('@/lib/doc-preview-grants', () => ({
  ...
  ensureDocPreviewGrant: () =>
    Promise.resolve({
      grantId: GRANT_ID,
      url: `nightshift-preview://${GRANT_ID}/${ENTRY_RELATIVE_PATH}`
    }),
...
describe('HtmlDocPreview browser chrome', () => {
  ...
  it('never shows the internal preview scheme to the reader', async () => {
    ...
    expect(webview.getAttribute('src')).toContain('nightshift-preview')
    const withoutGuest = container.cloneNode(true) as HTMLElement
    for (const guest of withoutGuest.querySelectorAll('webview')) {
      guest.remove()
    }
    expect(withoutGuest.innerHTML).not.toContain('nightshift-preview')
  })
