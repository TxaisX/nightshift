...
if (typeof window !== 'undefined') {
  const guardedWindow = window as Window & {
    __nightshiftContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__nightshiftContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__nightshiftContextualTourGlobalKeyGuardInstalled = true
    window.addEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  }
}

const PANEL_BASE_CLASSES =
  'nightshift-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="nightshift-contextual-tour-target-rings fixed z-[75]"
