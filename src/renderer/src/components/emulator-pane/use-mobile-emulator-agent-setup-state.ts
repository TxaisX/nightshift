...
import { NIGHTSHIFT_CLI_SKILL_NAME } from '@/lib/agent-feature-install-commands'
import {
...
function getCliActionLabel(status: CliInstallStatus | null, busy: boolean): string {
  ...
  if (isNightshiftCliAvailableOnPath(status)) {
    return translate(
...
export function useMobileEmulatorAgentSetupState(enabled = true): {
  ...
  const cliEnabled = isNightshiftCliAvailableOnPath(cliInstallStatus)
  ...
  const recheckSetup = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const cliReady = isNightshiftCliAvailableOnPath(cliStatus)
      if (!mountedRef.current) {
        return
      }
      ...
      if (skillInstalled) {
        toast.message(
          translate(
            ...
            'Skill is installed. Enable the Nightshift CLI to finish setup.'
          )
        )
        return
      }
      if (cliReady) {
        toast.message(
          translate(
            ...
            'Nightshift CLI is ready. Install the skill to finish setup.'
          )
        )
        return
      }
  ...
  const handleEnableCli = useCallback(async (): Promise<void> => {
    setCliBusy(true)
    try {
      const next = await ensureNightshiftCliAvailableForAgentSkillTerminal({
