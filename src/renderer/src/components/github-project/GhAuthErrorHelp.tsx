...
export function reloadNightshiftRenderer(): void {
...
function unsetEnvVarCommand(varName: string): { label: string; command: string } {
  if (IS_WINDOWS) {
    ...
    // shell/Nightshift relaunch for the change to take effect.
    return {
      label: translate(
...
export function buildRemediation(
  ...
  if (!diag.ghAvailable) {
    return {
      ...
      detail:
        'Nightshift uses `gh` to talk to GitHub Projects. Install it from cli.github.com, then sign in.',
      commands: [
        {
          label: translate(
            'auto.components.github.project.GhAuthErrorHelp.9c2da6353b',
  ...
  if (active?.envToken) {
    ...
    return {
      ...
      detail: IS_WINDOWS
        ? `Find where \`${varName}\` is set (System or User environment variables, or your PowerShell profile), remove it, then restart Nightshift so the new environment is picked up.${fallback}`
        : `Find where \`${varName}\` is exported (commonly \`~/.zshrc\`, \`~/.zshenv\`, \`~/.bashrc\`, \`~/.profile\`, or your shell's secrets manager), remove it, then restart Nightshift so the new environment is picked up.${fallback}`,
  ...
  // separate terminal, Nightshift's gh subprocess sees the env var and uses it.
  if (diag.envTokenInProcess && (!active || diag.missingScopes.length > 0)) {
    const varName = diag.envTokenInProcess
    return {
      summary: `Nightshift inherited \`${varName}\` from your shell, and \`gh\` is using that token. \`gh auth refresh\` doesn't apply to env-supplied tokens.`,
      detail: `Unset \`${varName}\` in the shell that launches Nightshift${
    ...
    return (
      <div className="border-b border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-800 dark:text-amber-200">
        ...
        <div className="mt-1 flex flex-wrap gap-1">
          ...
          <button
            type="button"
            onClick={reloadNightshiftRenderer}
            className="inline-flex items-center gap-1 rounded border border-amber-500/30 px-1.5 py-0.5 text-[11px] hover:bg-amber-500/20"
          >
  ...
  return (
    <div className="flex flex-col gap-2 text-sm">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {/* Why: after running the refresh command in a terminal, users need to
            reload the renderer to pick up the new gh token state. */}
        <Button size="sm" variant="outline" onClick={reloadNightshiftRenderer}>
