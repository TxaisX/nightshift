...
export default function MobilePage(): React.JSX.Element {
  ...
  const signedIn = useAppStore((state) => state.nightshiftProfileAuthStatus?.state === 'connected')
  const refreshAuthStatus = useAppStore((state) => state.fetchNightshiftProfileAuthStatus)
