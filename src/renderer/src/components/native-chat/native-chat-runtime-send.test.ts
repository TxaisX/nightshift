...
describe('sendNativeChatMessage', () => {
  ...
  it('matches nightshift-runtime writeTerminalAction Enter gap (500ms)', () => {
...
describe('sendNativeChatMessageWithImageAttachments', () => {
  ...
  it('clears the line, then bracket-pastes image paths before prompt text', () => {
    const handle = sendNativeChatMessageWithImageAttachments(SETTINGS, PTY, 'what do you see?', [
      '/tmp/nightshift-paste-image.png'
    ])
    ...
    const framedImageWithSeparator = '\x1b[200~/tmp/nightshift-paste-image.png\x1b[201~ '
    expectWriteOrder(sendRuntimePtyInput.mock.calls, [
      NATIVE_CHAT_CLEAR_UNSUBMITTED_INPUT,
  ...
  it('does not append a trailing separator on an attachment-only send', () => {
    ...
    expectWriteOrder(sendRuntimePtyInput.mock.calls, [
      NATIVE_CHAT_CLEAR_UNSUBMITTED_INPUT,
      '\x1b[200~/tmp/nightshift-paste-image.png\x1b[201~'
    ])
  ...
  it('treats whitespace-only prompt input as attachment-only', () => {
    sendNativeChatMessageWithImageAttachments(SETTINGS, PTY, '   ', [
      '/tmp/nightshift-paste-image.png'
    ])
