...
import { showNightshiftCliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
...
const FEATURE_SKILL_NAMES: Record<OnboardingFeatureSetupId, string> = {
  browserUse: NIGHTSHIFT_CLI_SKILL_NAME,
  computerUse: COMPUTER_USE_SKILL_NAME,
  orchestration: ORCHESTRATION_SKILL_NAME,
  linearTickets: NIGHTSHIFT_LINEAR_SKILL_NAME
}
...
export function createOnboardingFeatureSetupDeps(
  ...
  // Register `nightshift` on the same PATH used by the skill install (#12103).
  ...
  return {
    ...
    showCliRegistrationPrompt: showNightshiftCliRegistrationPromptToast,
    installCli: () =>
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Nightshift CLI registration is not available on this platform.'
      })
    } else if (status.pathConfigured === null) {
      // Why: an unknown registry read cannot safely drive a PATH read-modify-write.
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Nightshift could not check your Windows user PATH.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Nightshift CLI registration needs attention.'
        })
