...
const dashboardPanel: ActivePluginPanel = {
  ...
  title: 'Dashboard',
  icon: 'gauge',
  tabKey: 'plugin:nightshift-samples.my-plugin/dashboard',
  pluginKey: 'nightshift-samples.my-plugin',
...
describe('PluginPanel', () => {
  it('renders the panel HTML in a scripts-only sandboxed iframe', async () => {
    ...
    await renderPanel('plugin:nightshift-samples.my-plugin/dashboard')
    ...
    expect(initialIframe?.getAttribute('name')).toBe(
      'nightshift-plugin-panel:plugin:nightshift-samples.my-plugin/dashboard'
    )
  ...
  it('remounts with fresh host theme tokens when the app theme changes', async () => {
    readPanelEntryMock.mockResolvedValue({
      html: '<html class="__NIGHTSHIFT_COLOR_SCHEME__"><head><style>:root{/*__NIGHTSHIFT_PANEL_TOKENS__*/}</style></head>',
      sessionToken: SESSION_TOKEN
    })
    await renderPanel('plugin:nightshift-samples.my-plugin/dashboard')
  ...
  it('recovers from a transient read failure with byte-identical HTML', async () => {
    ...
    expect(setPanelHealthMock).toHaveBeenLastCalledWith(
      'plugin:nightshift-samples.my-plugin/dashboard',
      'healthy'
    )
  })
  ...
  it('shows an unavailable state for a tab whose plugin is gone', async () => {
    ...
    await renderPanel('plugin:nightshift-samples.removed-plugin/dashboard')
