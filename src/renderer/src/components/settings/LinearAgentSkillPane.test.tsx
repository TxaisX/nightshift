...
import {
  NIGHTSHIFT_LINEAR_SKILL_INSTALL_COMMAND,
...
const UPDATE_COMMAND = 'npx skills update nightshift-linear --global'

const mocks = vi.hoisted(() => ({
  panelProps: [] as Record<string, unknown>[],
  runtime: 'native' as 'native' | 'wsl',
  skillInstalled: true,
  updateSkillName: 'nightshift-linear',
...
describe('LinearAgentSkillPane', () => {
  afterEach(async () => {
    ...
    mocks.updateSkillName = 'nightshift-linear'
  ...
  it('renders checklist with inlined skill panel and usage examples', () => {
    ...
    for (const example of examples) {
      ...
      expect(example.prompt).toContain('/nightshift-linear')
  ...
  it('passes the nightshift-linear install/update commands and freshness on a local runtime', async () => {
    await renderPane()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({
        command: NIGHTSHIFT_LINEAR_SKILL_INSTALL_COMMAND,
        installedCommand: UPDATE_COMMAND,
        freshnessSkillName: NIGHTSHIFT_LINEAR_SKILL_NAME
      })
    )
  })
