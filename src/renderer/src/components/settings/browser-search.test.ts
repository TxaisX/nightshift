...
describe('browser settings search copy', () => {
  ...
  it('keeps the pre-feature wording while inverting is off', () => {
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toBe(
      "Open http(s) links in Nightshift's built-in browser — from the terminal, markdown, and the editor. ⇧⌘-click always uses your system browser."
    )
  ...
  // Why: "always" would be a lie once the chord can land in Nightshift, so the nested row
...
describe('browser link routing modifier copy', () => {
  ...
  it('names the destination the modifier actually reaches', () => {
    expect(getLinkRoutingModifierTitle(false)).toBe('Hold Shift to open in Nightshift')
  ...
  it('points the description at Nightshift only when links currently open externally', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      "Nightshift's built-in browser"
    )
  ...
  // Why: the toggle is off by default, so present-tense "opens one in Nightshift" would
  // describe behavior the user does not have yet.
  it('phrases the Nightshift branch as enabled-state copy', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      'When enabled'
    )
  })
...
describe('Link Routing description localization', () => {
  ...
  it('renders the Korean copy with the shortcut interpolated', async () => {
    ...
    expect(description).not.toContain("Nightshift's built-in browser")
