      ...
      <DialogHeader className="text-left">
        ...
        <DialogDescription>
          {translate(
            ...
            'Pick a host to fill the form, or add every new host to Nightshift’s host list.'
          )}
        </DialogDescription>
      </DialogHeader>
          ...
          >
            {hosts.map((host) => (
              <li key={host.alias}>
                <button
                  type="button"
                  disabled={picksDisabled || host.alreadyInNightshift}
                  className={cn(
                ...
                >
                  ...
                  ) : host.alreadyInNightshift ? (
                    <Badge
                      variant="outline"
                      className="mt-0.5 shrink-0 border-emerald-500/40 text-[10.5px] text-emerald-400"
                    >
                      {translate(
                        'auto.components.sidebar.AddRemoteHostDialog.sshConfigPickerInNightshift',
                        'In Nightshift'
                      )}
                    </Badge>
          ...
          {isBulkImporting
            ...
            : newHostCount > 0
              ? translate(
                  ...
                  'Add all {{value0}} to Nightshift',
                  { value0: newHostCount }
                )
              : // Why: the remainder can be already-in-Nightshift or merely tombstoned, so the
