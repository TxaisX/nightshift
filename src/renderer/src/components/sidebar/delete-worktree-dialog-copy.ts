  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Nightshift. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Nightshift. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Nightshift workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Nightshift. The project folder on disk will not be deleted.'
