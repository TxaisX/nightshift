...
/** True when an existing Nightshift host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInNightshift` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In Nightshift" is still savable as a second target. */
