 ...
 * workspace from Nightshift because the SSH target is gone entirely.
 */
export type SshWorkspaceForgetResolution =
  ...
  // impossible, so forget-from-Nightshift is the only path.
...
export function resolveSshWorkspaceForget(args: {
  ...
  // the only escape is to forget it from Nightshift.
  if (!isConfigured) {
