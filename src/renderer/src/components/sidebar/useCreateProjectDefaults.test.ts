...
describe('useCreateProjectDefaults', () => {
  beforeEach(() => {
    ...
    mocks.getDefaultCreateProjectParent.mockResolvedValue('/Users/alice/nightshift/projects')
  })
  ...
  it('auto-fills the local default parent and reports Git availability', async () => {
    ...
    expect(setCreateParent).toHaveBeenCalledWith('/Users/alice/nightshift/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/Users/alice/nightshift/projects')
  ...
  it('keeps the local default marker after the auto-filled parent rerenders the hook', async () => {
    ...
    useHarness()
    await flushAsync()
    useHarness({ createParent: '/Users/alice/nightshift/projects' })
  ...
  it('resolves the runtime default parent from the host home directory', async () => {
    ...
    expect(setCreateParent).toHaveBeenCalledWith('/home/alice/nightshift/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/home/alice/nightshift/projects')
  ...
  it('replaces an untouched local default when switching to a runtime target', async () => {
    ...
    await flushAsync()
    expect(local.setCreateParent).toHaveBeenCalledWith('/Users/alice/nightshift/projects')

    const runtime = useHarness({
      activeRuntimeEnvironmentId: 'env-1',
      createParent: '/Users/alice/nightshift/projects'
    })
    ...
    expect(mocks.browseRuntimeServerDirectory).toHaveBeenCalledWith('env-1', '~')
    expect(resolvedRuntime.setCreateParent).toHaveBeenCalledWith('/home/alice/nightshift/projects')
    expect(mocks.stateValues[DEFAULT_PARENT_STATE]).toBe('/home/alice/nightshift/projects')
  ...
  it('does not replace a touched parent when switching to a runtime target', async () => {
    ...
    const local = useHarness({ createParent: '/Users/alice/nightshift/projects' })
    local.result.markCreateParentTouched('/Users/alice/nightshift/projects/pr5115-target-switch')

    const runtime = useHarness({
      activeRuntimeEnvironmentId: 'env-1',
      createParent: '/Users/alice/nightshift/projects/pr5115-target-switch'
    })
    await flushAsync()
