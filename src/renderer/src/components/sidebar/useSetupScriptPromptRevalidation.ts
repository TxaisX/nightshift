...
/**
 * Re-runs the setup-script prompt inspection when a shared `nightshift.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: nightshift.yaml is edited on disk or the hook runs in a terminal outside React
  // state. Re-inspect on window focus so returning to Nightshift detects it (mirrors
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable nightshift.yaml into a readable one,
