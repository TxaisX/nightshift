  ...
  if (result.status === 'deleted-group' && result.failedProjectRemovals.length > 0) {
    ...
    toast.error(
      translate(
        ...
        'Some projects could not be removed from Nightshift'
      ),
      {
        description: translate(
        ...
        toast.error(
          ...
          ),
          {
            description: translate(
              ...
              "Nightshift could not confirm the new name with the group's host. Recheck the group after reconnecting."
            )
          }
        )
      }
