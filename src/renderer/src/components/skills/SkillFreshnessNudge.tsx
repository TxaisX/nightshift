...
export function SkillFreshnessNudge(): null {
  ...
  useEffect(() => {
    ...
    const candidates = inventory.installations.flatMap((installation) =>
      ...
      // dismissed — and spends the bounded dismissal budget on copies Nightshift cannot update.
    ...
    nextActive.id = toast.info(
      names.size === 1
        ? translate(
            ...
            'An installed Nightshift skill is out of date'
          )
        : translate(
            'auto.components.skills.SkillFreshnessNudge.titleMany',
            '{{value0}} installed Nightshift skills are out of date',
            { value0: names.size }
          ),
      {
        description: translate(
          'auto.components.skills.SkillFreshnessNudge.description',
          'Update {{value0}} so agents follow the current instructions for this version of Nightshift.',
