...
} from './task-page-linear-in-nightshift-issues'
...
describe('filterLinearIssuesForInNightshiftWorkspace', () => {
  it('keeps issues without workspace metadata when a workspace is selected', () => {
    ...
    expect(
      filterLinearIssuesForInNightshiftWorkspace(issues, 'ws-a').map((item) => item.identifier)
    ).toEqual(['ENG-1', 'ENG-3'])
  })
})
