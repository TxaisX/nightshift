/**
 * Real-fish regression for #9993: Nightshift must never write `CSI ?997;Nn` into a PTY
...
const PROMPT_MARK = 'NIGHTSHIFT997> '
...
describe('fish never receives a color-scheme report it did not query (#9993)', () => {
  ...
  itWithFish(
    ...
    async () => {
      ...
      configHome = mkdtempSync(path.join(tmpdir(), 'nightshift-fish-2031-'))
