...
// (pty-startup-ingress.ts:26-29, 247-252), so the cooked-mode echo of Nightshift's
...
// Nightshift's One Dark terminal theme — the values that appear in the leaked text.
const NIGHTSHIFT_TERMINAL_THEME = { foreground: '#ffffff', background: '#282c34' }
 ...
 * comes from Nightshift's code, not from the harness.
 */
...
async function createRendererPane(): Promise<RendererPane> {
  ...
  terminal.options.theme = { ...NIGHTSHIFT_TERMINAL_THEME }
...
describe('#12112 opencode startup OSC 10/11 replies on the local path', () => {
  ...
  it('agent pane consumes the replies and renders nothing', async () => {
    ...
    const ingress = new PtyStartupIngress({
      intent: { colors: NIGHTSHIFT_TERMINAL_THEME, deadlineMs: 5_000 },
  ...
  it('renders no reply text when the tty echo is coalesced with program output', () => {
    ...
    // interactive shell, so bash's echo of Nightshift's reply shares a read with the shell's
