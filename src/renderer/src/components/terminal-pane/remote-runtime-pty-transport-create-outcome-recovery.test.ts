  ...
  it('retries an unknown terminal-create outcome exactly once with the same mutation id', async () => {
    ...
    runtimeCall.mockImplementation(async (args: { method: string; params?: unknown }) => {
      ...
      if (args.method === 'terminal.create') {
        createCalls += 1
        if (createCalls === 1) {
          throw Object.assign(new Error('Timed out waiting for the remote Nightshift runtime.'), {
        ...
        if (args.method === 'status.get') {
          if (statusTimesOut) {
            return new Promise((_, reject) => {
              setTimeout(() => {
                reject(
                  Object.assign(new Error('Timed out waiting for the remote Nightshift runtime.'), {
                    code: 'runtime_timeout'
                  })
                )
              }, args.timeoutMs)
