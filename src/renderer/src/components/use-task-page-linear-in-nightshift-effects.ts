...
import {
  ...
  filterLinearIssuesForInNightshiftWorkspace
...
export function useTaskPageLinearInNightshiftEffects(model: TaskPageLinearListEffectsModel) {
  const {
    fetchLinearIssue,
    refreshLinearIssue,
    linearConnected,
    ...
    inNightshiftLinkedLinearRefsSignature,
    inNightshiftLinkedLinearRefsRef
  } = model
  ...
  useEffect(() => {
    ...
    if (taskSource !== 'linear' || linearMode !== 'in-nightshift' || !linearConnected) {
      return
    }
    let cancelled = false
    const linkedRefs = inNightshiftLinkedLinearRefsRef.current
    const requestSignature = `in-nightshift::${selectedLinearWorkspaceId ?? 'default'}::${inNightshiftLinkedLinearRefsSignature}`
        ...
        if (loaded.length === 0) {
          setLinearError(
            translate(
              ...
              'Unable to load Linear issues linked to a Nightshift workspace.'
            )
          )
          setLinearIssues([])
          setLinearLoading(false)
        ...
        if (loaded.length !== results.length) {
          setLinearError(
            translate(
              ...
              'Some Linear issues linked to a Nightshift workspace could not be loaded. Refresh to try again.'
            )
          )
        }
        setLinearIssues(
          filterLinearIssuesForInNightshiftWorkspace(loaded, selectedLinearWorkspaceId)
        )
        setLinearLoading(false)
      })
      .catch((err) => {
...
export type TaskPageLinearInNightshiftEffectsModel = ReturnType<
  typeof useTaskPageLinearInNightshiftEffects
>
