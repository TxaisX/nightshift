...
import { registerNightshiftProfileAuthIpcBridge } from './nightshift-profile-auth-ipc-bridge'
...
const reconnectRequiredAuthStatus: NightshiftProfileAuthStatus = {
  activeProfileId: 'local-default',
  configured: true,
...
describe('nightshift profile auth IPC bridge', () => {
  ...
  it('re-fetches auth status on the push, flipping connected to reconnect-required', async () => {
    ...
    await store.getState().fetchNightshiftProfileAuthStatus()
    expect(store.getState().nightshiftProfileAuthStatus).toEqual(connectedAuthStatus)

    const unsubs: (() => void)[] = []
    registerNightshiftProfileAuthIpcBridge(unsubs)
    authStatus.mockResolvedValue(reconnectRequiredAuthStatus)
    listener?.()
    await vi.waitFor(() =>
      expect(store.getState().nightshiftProfileAuthStatus).toEqual(reconnectRequiredAuthStatus)
    )
  ...
  it('skips registration when the preload bridge does not expose the event', () => {
    vi.stubGlobal('window', { api: { nightshiftProfiles: { authStatus } } })
    const unsubs: (() => void)[] = []
