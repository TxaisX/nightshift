...
import {
  findNightshiftDispatchTaskMarkerIndex,
  NIGHTSHIFT_DISPATCH_STATUS_PREAMBLE_PREFIX,
  NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER
} from '../../../shared/nightshift-dispatch-status-prompt'

export const NIGHTSHIFT_DISPATCH_PREAMBLE_PREFIX = NIGHTSHIFT_DISPATCH_STATUS_PREAMBLE_PREFIX
const NIGHTSHIFT_DISPATCH_TASK_MARKER = NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER
const NIGHTSHIFT_DISPATCH_TASK_ID_MARKER = 'Your task ID is:'
...
const NIGHTSHIFT_DISPATCH_TASK_PREVIEW_SCAN_LIMIT = 512
// Why: task id lives near the top of the preamble; keep that scan tight.
const NIGHTSHIFT_DISPATCH_TASK_ID_SCAN_LIMIT = 1024
...
const NIGHTSHIFT_DISPATCH_TASK_MARKER_SCAN_LIMIT = 32_768

/** True when the live prompt is still a Nightshift dispatch turn (not sticky metadata alone). */
export function isNightshiftDispatchPrompt(prompt: string): boolean {
  return prompt.trimStart().startsWith(NIGHTSHIFT_DISPATCH_PREAMBLE_PREFIX)
}
...
export function orchestrationLabelsMatchLiveDispatch(
  ...
  if (!isNightshiftDispatchPrompt(entry.prompt)) {
    return false
  }
  ...
  const liveTaskId = getNightshiftDispatchTaskId(entry.prompt)
  if (!liveTaskId) {
    return true
  }
...
export function getAgentRowPrimaryText(
  ...
  // first characters are boilerplate ("You are working inside Nightshift…").
  if (orchestrationLabelsMatchLiveDispatch(entry)) {
    return (
      entry.orchestration?.displayName?.trim() ||
      entry.orchestration?.taskTitle?.trim() ||
      getNightshiftDispatchTaskPreview(entry.prompt)
    )
  }
  if (isNightshiftDispatchPrompt(entry.prompt)) {
    return getNightshiftDispatchTaskPreview(entry.prompt)
  }
...
export function getNightshiftDispatchTaskId(prompt: string): string | null {
  if (!isNightshiftDispatchPrompt(prompt)) {
    return null
  }
  const scan = prompt.trimStart().slice(0, NIGHTSHIFT_DISPATCH_TASK_ID_SCAN_LIMIT)
  const markerIndex = scan.indexOf(NIGHTSHIFT_DISPATCH_TASK_ID_MARKER)
  if (markerIndex === -1) {
    return null
  }
  ...
  const afterMarker = scan
    .slice(markerIndex + NIGHTSHIFT_DISPATCH_TASK_ID_MARKER.length)
    .trimStart()
...
function getNightshiftDispatchTaskPreview(prompt: string): string {
  ...
  const scan = prompt
    ...
    .slice(
      0,
      NIGHTSHIFT_DISPATCH_TASK_MARKER_SCAN_LIMIT + NIGHTSHIFT_DISPATCH_TASK_PREVIEW_SCAN_LIMIT
    )
  ...
  const taskMarkerIndex = findNightshiftDispatchTaskMarkerIndex(scan)
  if (taskMarkerIndex === -1) {
    return ''
  }
  const taskBodyStart = taskMarkerIndex + NIGHTSHIFT_DISPATCH_TASK_MARKER.length
  const taskBody = scan.slice(
    taskBodyStart,
    taskBodyStart + NIGHTSHIFT_DISPATCH_TASK_PREVIEW_SCAN_LIMIT
  )
