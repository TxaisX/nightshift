...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Nightshift pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Nightshift session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Nightshift status hints:', ...statusHints] : []),
    '',
