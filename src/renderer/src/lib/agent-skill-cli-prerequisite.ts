...
type EnsureNightshiftCliAvailableOptions = {
...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, Nightshift may show a system prompt to register the Nightshift CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'Nightshift needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the Nightshift CLI command.'

export function isNightshiftCliAvailableOnPath(
  status: CliInstallStatus | null | undefined
): boolean {
  return status?.state === 'installed' && status.pathConfigured === true
}

export async function ensureNightshiftCliAvailableForAgentSkillTerminal({
  onStatusChange,
  registrationPromptDelayMs = 700
}: EnsureNightshiftCliAvailableOptions = {}): Promise<CliInstallStatus | null> {
  try {
    ...
    if (status.state !== 'installed' || status.pathConfigured === false) {
      ...
      await showNightshiftCliRegistrationPromptToast(registrationPromptDelayMs)
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the Nightshift CLI in PATH.'
          )
    )
    return null
  }
...
export async function showNightshiftCliRegistrationPromptToast(delayMs = 700): Promise<void> {
...
function showCliPrerequisiteWarning(status: CliInstallStatus): void {
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'Nightshift CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'Nightshift CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'Nightshift could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    // Nightshift affordances when that shell can resolve the Nightshift CLI command.
    toast.warning(
      translate(
        'auto.lib.agent.skill.cli.prerequisite.79371593b0',
        'Nightshift CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the Nightshift CLI directory to PATH before setup.'
          )
      }
    )
  }
