  ...
  it('keeps loading and missing states ahead of freshness', () => {
    expect(
      getAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: true,
        inventory: inventory([placement('nightshift-linear', 'current')])
      })
    ).toBe('checking')
    expect(
      getAgentSkillNavInstallStatus({
...
describe('getLinearAgentSkillNavInstallStatus', () => {
  it('reports canonical installs under nightshift-linear', () => {
    const skills = [skill('nightshift-linear')]
    ...
    expect(
      getLinearAgentSkillNavInstallStatus({
        ...
        installed: true,
        loading: false,
        inventory: inventory([placement('nightshift-linear', 'outdated')], ['nightshift-linear'])
      })
  ...
  it('prefers canonical freshness when both names are installed', () => {
    expect(
      getLinearAgentSkillNavInstallStatus({
        skills: [skill('nightshift-linear'), skill('linear-tickets')],
        installed: true,
        loading: false,
        inventory: inventory(
          [placement('nightshift-linear', 'current'), placement('linear-tickets', 'outdated')],
          ['linear-tickets']
        )
      })
    ).toBe('up-to-date')
  ...
  it('falls back to presence-only status when freshness does not apply', () => {
    expect(
      getLinearAgentSkillNavInstallStatus({
        skills: [skill('nightshift-linear')],
        installed: true,
        loading: false,
        inventory: null
      })
