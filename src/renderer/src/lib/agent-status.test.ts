...
describe('detectAgentStatusFromTitle', () => {
  ...
  // Why: cursor-agent's native title stays "Cursor Agent" all turn, so Nightshift synthesizes decorated titles for the spinner/unread pipeline.
...
describe('getAgentLabel', () => {
  ...
  it('treats Claude Code prefixed task titles as Claude even when they mention another CLI', () => {
    ...
    expect(getAgentLabel('. Compare Opencode Vs Nightshift')).toBe('Claude Code')
  ...
  it('labels Cursor by identity, not a bare "cursor" token in another agent title', () => {
    ...
    expect(getAgentLabel('Terminal Cursor and Nightshift slows down')).toBeNull()
  })
})
