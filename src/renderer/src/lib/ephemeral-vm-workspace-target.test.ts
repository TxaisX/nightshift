...
describe('prepareEphemeralVmWorkspaceTarget', () => {
  ...
  it('rejects and cleans up a Nightshift-server provisioned root before project import', async () => {
    vi.mocked(window.api.ephemeralVm.provision).mockResolvedValue({
      ok: true,
      connectionType: 'nightshift-server',
      stderr: 'creating sandbox',
  ...
  it('cleans up the runtime when required project setup capability is missing', async () => {
    vi.mocked(assertRuntimeEnvironmentCapability).mockRejectedValue(
      new Error('The recipe-created Nightshift server does not support project setup.')
    )
    vi.mocked(window.api.ephemeralVm.provision).mockResolvedValue({
      ok: true,
      connectionType: 'nightshift-server',
    ...
    expect(result).toEqual({
      ok: false,
      error: 'The recipe-created Nightshift server does not support project setup.',
      stderr: 'creating sandbox'
    })
  })
