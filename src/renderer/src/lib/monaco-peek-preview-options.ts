...
type ReferenceWidgetConstructor = {
  prototype: ReferenceWidgetInstance & {
    __nightshiftPeekPreviewOptionsInstalled?: true
  }
}

function applyPeekReferencesPreviewOptions(editor: PeekPreviewEditor | undefined): void {
  // Why: Monaco embedded editors inherit Nightshift's full-editor options first.
...
export function installMonacoPeekReferencesPreviewOptions(
  ...
  if (prototype.__nightshiftPeekPreviewOptionsInstalled) {
    return
  }
  ...
  prototype.__nightshiftPeekPreviewOptionsInstalled = true
}
