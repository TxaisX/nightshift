...
describe('new workspace SSH gate', () => {
  ...
  it('never gates a runtime-owned (per-workspace-env) SSH target', () => {
    ...
    expect(
      getSelectedRepoSshGate({ connectionId: 'runtime-ssh-nightshift-1', status: 'disconnected' })
    ).toEqual({
      selectedRepoConnectionId: null,
      selectedRepoSshStatus: null,
      selectedRepoRequiresConnection: false,
    ...
    expect(
      canUseRepoBackedComposerSources({ connectionId: 'runtime-ssh-nightshift-1', status: null })
    ).toBe(true)
  })
