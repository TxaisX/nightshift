...
const SKILL_NAME = 'nightshift-cli'
...
describe('getSkillFreshnessDisplayStatus', () => {
  ...
  it.each([
    ...
    // Why: these are Nightshift's own bounds. A large but healthy plugin cache would
  ...
  // bound Nightshift chose, so the only one a person can actually clear.
...
describe('hasSkillCopyNeedingAttention', () => {
  ...
  // copy inside a work directory. Nightshift only updates global skills, so the project copy
  ...
  // presence rather than a freshness claim about a copy Nightshift does not manage.
  ...
  // Why: an unreadable plugin path could hide a copy of anything, but a skill Nightshift
