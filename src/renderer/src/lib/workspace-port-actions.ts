...
export function shouldOpenWorkspacePortInNightshiftBrowser(
...
export function resolvePortOpenInNightshiftBrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInNightshiftBrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  openInNightshiftBrowser?: boolean
  ...
  if (args.openInNightshiftBrowser === false && args.runtimeTarget.kind === 'local') {
    try {
