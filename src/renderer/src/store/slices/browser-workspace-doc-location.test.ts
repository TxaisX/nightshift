...
const LIVE_GRANT_URL = `nightshift-preview://${'a'.repeat(32)}/report/index.html`
...
describe('a browser page that shows a workspace document', () => {
  it('is created blank, with the document as its identity', () => {
    ...
    expect(page?.url).toBe(NIGHTSHIFT_BROWSER_BLANK_URL)
  ...
  it('refuses a grant url arriving as the document title', () => {
    ...
    expect(JSON.stringify(persistedSession(store))).not.toContain('nightshift-preview://')
  })
  ...
  it('still names an ordinary blank browser tab New Tab', () => {
    const store = createStoreWithWorktree()
    const tab = store.getState().createBrowserTab(WORKTREE_ID, NIGHTSHIFT_BROWSER_BLANK_URL)
    const pageId = store.getState().browserPagesByWorkspace[tab.id]?.[0]?.id ?? ''
  ...
  it('never asks for the address bar it does not have', () => {
    ...
    const newTab = store.getState().createBrowserTab(WORKTREE_ID, NIGHTSHIFT_BROWSER_BLANK_URL)
  ...
  it('reopens a closed document page with its document identity', () => {
    ...
    expect(reopened?.docLocation).toEqual(DOC_LOCATION)
    expect(reopened?.url).toBe(NIGHTSHIFT_BROWSER_BLANK_URL)
  })
  ...
  it('writes the document and not the grant to the session', () => {
    ...
    expect(session.browserPagesByWorkspace?.[tab.id]?.[0]?.url).toBe(NIGHTSHIFT_BROWSER_BLANK_URL)
    expect(written).not.toContain('nightshift-preview://')
  })
