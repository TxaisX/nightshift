...
// Mirrors the real report: one project name ("nightshift") set up on the local Mac and on a
// remote Nightshift server, where only the local repo row carries the user's chosen color.
...
function runtimeResult(method: string): unknown {
  ...
  if (method === 'projectHostSetup.list') {
    return { setups: [setup(SHARED_PROJECT_ID, 'remote-repo', '/srv/nightshift')] }
  }
  return {}
}
...
beforeEach(() => {
  ...
  listHostSetups.mockResolvedValue([setup(SHARED_PROJECT_ID, 'local-repo', '/local/nightshift')])
