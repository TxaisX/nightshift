...
const pairingInput = process.env.NIGHTSHIFT_REMOTE_ACCESS_PAIRING_INPUT
const proxyEndpoint = process.env.NIGHTSHIFT_REMOTE_ACCESS_PROXY_ENDPOINT
const secondPairingInput = process.env.NIGHTSHIFT_REMOTE_ACCESS_SECOND_PAIRING_INPUT
...
async function exerciseCatalogAndFilesystem(client: WebRuntimeClient): Promise<void> {
  ...
  expect(preview.content).toContain('"name": "nightshift"')
...
describe.runIf(Boolean(pairingInput && proxyEndpoint))(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      if (!parsed) {
        throw new Error('NIGHTSHIFT_REMOTE_ACCESS_PAIRING_INPUT is not a valid pairing input.')
      }
      directOffer = parsed
    ...
    it.runIf(Boolean(secondPairingInput))(
      ...
      async () => {
        ...
        if (!secondOffer) {
          throw new Error('NIGHTSHIFT_REMOTE_ACCESS_SECOND_PAIRING_INPUT is not valid.')
        }
