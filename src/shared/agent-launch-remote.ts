...
/**
 * Why: a repo reached over SSH runs the Nightshift CLI through the relay shim, which is always deployed
 * as plain `nightshift` (Unix) / `nightshift.cmd` (Windows). The Linux-only `nightshift-ide` rename — which exists
 * solely to avoid shadowing the GNOME Orca screen reader on a local desktop — must not be applied
 * to those remotes, or `nightshift-ide claude-teams` lands on a PATH where it does not exist.
 *
 ...
 *
 *   - a row carrying only `executionHostId: 'ssh:<target>'` reads as local and gets the `nightshift-ide`
 ...
 * shim), while a runtime host with no nested target is a full Nightshift install and stays false — as do
