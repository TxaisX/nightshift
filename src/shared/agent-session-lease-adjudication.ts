...
export type AgentSessionOwnerProbe =
  /** Nightshift watched this exact process exit. */
...
export function evaluateAgentSessionAcquisition(args: {
  ...
  if (lease.ownerProcess !== null) {
    if (!isProvenDeadProbe(probe)) {
      // Why: a lapsed deadline means Nightshift stopped hearing from the owner, not that the child
...
/**
 * A process carrying a Nightshift spawn token with no matching lease is an orphan: stop it, never
