...
describe('collectAgentTitleEvidence', () => {
  describe('an anchored name outranks a name in task text', () => {
    ...
    // the pane owner is named by Nightshift's `- <agent>` suffix, the competitor only by task text.
    it.each([
  ...
  ] as const)('recognizes Nightshift-controlled synthetic title %s', (title, agent) => {
    expect(agentFor(title)).toBe(agent)
    expect(reasonFor(title)).toBe('anchored')
  })
