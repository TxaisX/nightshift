...
const EPHEMERAL_VM_RUNTIMES_FILE = 'nightshift-ephemeral-vm-runtimes.json'
...
export function upsertEphemeralVmRuntimeRollbackRecovery(
  ...
  try {
    ...
    if (error instanceof JsonStringifyByteLimitError) {
      throw new EphemeralVmRuntimeStoreError(
        'runtime_error',
        `Could not write Nightshift ephemeral VM runtimes at ${path}; the store exceeds its durable capacity.`
      )
    }
    throw error
  }
...
function readEphemeralVmRuntimeStore(userDataPath: string): LoadedEphemeralVmRuntimeStore {
  ...
  try {
    ...
    throw new EphemeralVmRuntimeStoreError(
      'runtime_error',
      `Could not read Nightshift ephemeral VM runtimes at ${path}; the file is invalid.`
    )
  }
}
