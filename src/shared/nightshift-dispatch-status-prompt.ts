// Why: full Nightshift dispatch preambles are multi-KB (CLI instructions before
...
export const NIGHTSHIFT_DISPATCH_STATUS_PREAMBLE_PREFIX =
  'You are working inside Nightshift, a multi-agent IDE.'
export const NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER = '=== TASK ==='
const NIGHTSHIFT_DISPATCH_STATUS_TASK_ID_MARKER = 'Your task ID is:'
...
const NIGHTSHIFT_DISPATCH_STATUS_SOURCE_SCAN_LIMIT = 24_576

export function isNightshiftDispatchStatusPrompt(value: string): boolean {
  ...
  const scanEnd = Math.min(value.length, NIGHTSHIFT_DISPATCH_STATUS_SOURCE_SCAN_LIMIT)
  let start = 0
  while (start < scanEnd && isEcmaTrimWhitespace(value.charCodeAt(start))) {
    start++
  }
  return (
    start + NIGHTSHIFT_DISPATCH_STATUS_PREAMBLE_PREFIX.length <= scanEnd &&
    value.startsWith(NIGHTSHIFT_DISPATCH_STATUS_PREAMBLE_PREFIX, start)
  )
}
...
export function compactDispatchPromptForStatus(
  ...
  let taskId = ''
  const idMarkerIndex = scan.indexOf(NIGHTSHIFT_DISPATCH_STATUS_TASK_ID_MARKER)
  if (idMarkerIndex !== -1) {
    const afterId = scan.slice(idMarkerIndex + NIGHTSHIFT_DISPATCH_STATUS_TASK_ID_MARKER.length)
    let idStart = 0
  ...
  if (taskMarkerIndex !== -1) {
    const body = scan.slice(taskMarkerIndex + NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER.length)
  ...
  // Why: keep the dispatch prefix (isNightshiftDispatchPrompt) + task id (label match)
  // + task body (fallback preview) so UI helpers still work on the 200-char field.
  let compact = NIGHTSHIFT_DISPATCH_STATUS_PREAMBLE_PREFIX
  if (taskId) {
    compact += ` ${NIGHTSHIFT_DISPATCH_STATUS_TASK_ID_MARKER} ${taskId}`
  }
  if (taskBody) {
    compact += ` ${NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER} ${taskBody}`
  }
  return normalizeSingleLine(compact, maxLength)
}

/**
 * Locate the Nightshift task separator in a dispatch prompt scan window.
 * Why: base-drift commit subjects are repository-controlled and may mention
 * `=== TASK ===`. Raw multi-line preambles must use the standalone line Nightshift
 ...
 */
export function findNightshiftDispatchTaskMarkerIndex(value: string): number {
  let searchFrom = 0
  while (searchFrom < value.length) {
    const markerIndex = value.indexOf(NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER, searchFrom)
    if (markerIndex === -1) {
      break
    }
    const markerEnd = markerIndex + NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER.length
  ...
  return value.includes('\n') || value.includes('\r')
    ? -1
    : value.indexOf(NIGHTSHIFT_DISPATCH_STATUS_TASK_MARKER)
}
