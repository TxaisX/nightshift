...
import type {
  NightshiftDefaultTabTemplate,
  NightshiftHooks,
  NightshiftVmRecipe,
  NightshiftVmRecipeDiagnostic
} from './nightshift-yaml-hook-types'
import {
  isNightshiftYamlFieldWithinLimit,
  isNightshiftYamlTextWithinLimit,
...
function asTrimmedString(value: unknown): string | undefined {
  if (typeof value !== 'string' || !isNightshiftYamlFieldWithinLimit(value)) {
    return undefined
  }
...
export const NIGHTSHIFT_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const NIGHTSHIFT_VM_RECIPE_ID_RULE =
...
function normalizeDefaultTabs(value: unknown): NightshiftDefaultTabTemplate[] {
  if (!Array.isArray(value) || value.length > MAX_NIGHTSHIFT_YAML_COLLECTION_ENTRIES) {
    return []
  }
  ...
  return value
    ...
    .filter((entry): entry is NightshiftDefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: NightshiftVmRecipe[]
  diagnostics: NightshiftVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: NightshiftVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
    return { recipes: [], diagnostics }
  }
  if (value.length > MAX_NIGHTSHIFT_YAML_COLLECTION_ENTRIES) {
    return {
      recipes: [],
      diagnostics: [
        {
          index: MAX_NIGHTSHIFT_YAML_COLLECTION_ENTRIES,
          message: `At most ${MAX_NIGHTSHIFT_YAML_COLLECTION_ENTRIES} environment recipes are supported.`
        }
      ]
    }
  }
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!NIGHTSHIFT_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${NIGHTSHIFT_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
      ...
      if (
        ...
        diagnostics.push({
          index,
          field: 'checkoutMode',
          message: `Recipe "${id}" checkoutMode must be "nightshift-worktree" or "provisioned-root".`
        })
        return null
      }
    ...
    .filter((entry): entry is NightshiftVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `nightshift.yaml`.
 */
export function parseNightshiftYaml(content: string): NightshiftHooks | null {
  if (!isNightshiftYamlTextWithinLimit(content)) {
    return null
  }
  ...
  try {
    ...
    root = document.toJS({ maxAliasCount: MAX_NIGHTSHIFT_YAML_ALIAS_COUNT })
  } catch {
    return null
  }
