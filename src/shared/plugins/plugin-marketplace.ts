...
export function isOfficialOrganizationGitSource(url: string): boolean {
  ...
  // Why: this checks the real GitHub org (casing "TxaisX"), a different thing
  // from OFFICIAL_PLUGIN_PUBLISHER, which is a kebab-case manifest slug and
  // need not match the org's casing. Share OFFICIAL_MARKETPLACE_OWNER, the
  // other lowercased-owner comparison, instead of hardcoding it twice.
