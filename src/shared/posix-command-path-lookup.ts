...
export function buildPosixCommandPathLookupScript(
  ...
  const mountPrelude = options.skipWindowsMountDirs
    ? [
        '[ "${_nightshift_win_mounts+set}" = set ] || _nightshift_win_mounts=$(awk \'$3 == "drvfs" || $3 == "9p" || $3 == "virtiofs" { print $2 }\' /proc/mounts 2>/dev/null)'
      ]
    : []
  const skipMountComponent = options.skipWindowsMountDirs
    ? [
        '      for _nightshift_win_mount in $_nightshift_win_mounts; do',
        '        case "$_nightshift_lookup_component/" in',
        '          "$_nightshift_win_mount"/*) _nightshift_lookup_component= ;;',
        '        esac',
        '      done',
        '      if [ -z "$_nightshift_lookup_component" ]; then',
        '        [ -n "$_nightshift_lookup_has_more" ] || break',
        '        continue',
        '      fi'
      ]
    : []
  ...
  return [
    `_nightshift_lookup_command=${commandAssignment}`,
    'resolved=',
    ...mountPrelude,
    'case "$_nightshift_lookup_command" in',
    '  */*)',
    '    case "$_nightshift_lookup_command" in',
    '      /*) _nightshift_lookup_candidate=$_nightshift_lookup_command ;;',
    '      *) _nightshift_lookup_candidate=${PWD%/}/$_nightshift_lookup_command ;;',
    '    esac',
    '    if [ -x "$_nightshift_lookup_candidate" ] && [ ! -d "$_nightshift_lookup_candidate" ]; then',
    '      resolved=$_nightshift_lookup_candidate',
    '    fi',
    '    ;;',
    '  *)',
    '    _nightshift_lookup_remaining=${PATH-}',
    '    while :; do',
    '      case "$_nightshift_lookup_remaining" in',
    '        *:*)',
    '          _nightshift_lookup_component=${_nightshift_lookup_remaining%%:*}',
    '          _nightshift_lookup_remaining=${_nightshift_lookup_remaining#*:}',
    '          _nightshift_lookup_has_more=1',
    '          ;;',
    '        *)',
    '          _nightshift_lookup_component=$_nightshift_lookup_remaining',
    '          _nightshift_lookup_has_more=',
    '          ;;',
    '      esac',
    ...skipMountComponent,
    '      [ -n "$_nightshift_lookup_component" ] || _nightshift_lookup_component=.',
    '      case "$_nightshift_lookup_component" in',
    '        /*) _nightshift_lookup_candidate=$_nightshift_lookup_component/$_nightshift_lookup_command ;;',
    '        *) _nightshift_lookup_candidate=${PWD%/}/$_nightshift_lookup_component/$_nightshift_lookup_command ;;',
    '      esac',
    '      if [ -x "$_nightshift_lookup_candidate" ] && [ ! -d "$_nightshift_lookup_candidate" ]; then',
    '        resolved=$_nightshift_lookup_candidate',
    '        break',
