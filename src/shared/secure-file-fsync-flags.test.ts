...
describe('secure file fsync flags', () => {
  ...
  windowsIt('opens files read/write before fsync on Windows', () => {
    const directory = mkdtempSync(join(tmpdir(), 'nightshift-file-fsync-'))
  ...
  posixIt('fsyncs files without owner-write permission', () => {
    const directory = mkdtempSync(join(tmpdir(), 'nightshift-file-fsync-read-only-'))
    createdPaths.push(directory)
    const path = join(directory, 'record.json')
    writeFileSync(path, '{}')
  ...
  posixIt('durably writes when umask removes owner-write from the temporary file', () => {
    const directory = mkdtempSync(join(tmpdir(), 'nightshift-durable-file-fsync-read-only-'))
    createdPaths.push(directory)
    const path = join(directory, 'record.json')
  ...
  posixIt('opens directories read-only before fsync', () => {
    const directory = mkdtempSync(join(tmpdir(), 'nightshift-directory-fsync-'))
    createdPaths.push(directory)
