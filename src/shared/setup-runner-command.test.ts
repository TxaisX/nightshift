...
describe('buildSetupRunnerCommand', () => {
  ...
  it('uses bash with Linux paths for forward-slash WSL UNC runner scripts', () => {
    expect(
      buildSetupRunnerCommand(
        '//wsl.localhost/Ubuntu/home/jin/repo/.git/worktrees/feature/nightshift/setup-runner.sh',
        'windows'
      )
    ).toBe('bash /home/jin/repo/.git/worktrees/feature/nightshift/setup-runner.sh')
  })
  ...
  it('keeps generic forward-slash UNC runner scripts on cmd.exe', () => {
    expect(
      buildSetupRunnerCommand('//server/share/repo/.git/nightshift/setup-runner.cmd', 'windows')
    ).toBe('cmd.exe /c "//server/share/repo/.git/nightshift/setup-runner.cmd"')
  })

  it('uses POSIX launch semantics for native Windows runners when the setup shell is POSIX', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\.git\\nightshift\\setup-runner.sh', 'windows', {
        family: 'posix'
      })
    ).toBe('bash /c/repo/.git/nightshift/setup-runner.sh')
  })
  ...
  it('keeps the batch runner path in native form for a POSIX pane launch', () => {
    ...
    expect(
      resolveSetupRunnerCommand('C:\\repo\\.git\\nightshift\\setup-runner.cmd', 'windows', {
        family: 'posix'
      })
    ).toMatchObject({
      runnerScriptPathForShell: 'C:\\repo\\.git\\nightshift\\setup-runner.cmd',
      shell: 'windows'
    })
  })
...
describe('buildSetupRunnerCommand cmd metacharacter guard', () => {
  const cmdRunner = (segment: string) => `C:\\repo${segment}\\.git\\nightshift\\setup-runner.cmd`
  ...
  it.each([
    ['plain', 'C:\\repo\\.git\\nightshift\\setup-runner.cmd'],
    ['spaces', 'C:\\Program Files\\repo\\.git\\nightshift\\setup-runner.cmd'],
    ['single quote', "C:\\o'brien\\.git\\nightshift\\setup-runner.cmd"],
    ['brackets and dash', 'C:\\repo-[2]\\.git\\nightshift\\setup-runner.cmd']
  ...
  it('escapes single quotes when embedding the path in the PowerShell literal', () => {
    const script = decodePowerShellCommand(
      buildSetupRunnerCommand("C:\\o'brien&co\\.git\\nightshift\\setup-runner.cmd", 'windows', {
        family: 'cmd'
      })
    )

    expect(script).toContain("$runner = 'C:\\o''brien&co\\.git\\nightshift\\setup-runner.cmd'")
  })
  ...
  it.each([
    ['native POSIX runner', 'C:\\repo\\a&b\\.git\\nightshift\\setup-runner.sh', undefined],
    [
      'WSL UNC runner',
      '\\\\wsl.localhost\\Ubuntu\\home\\a&b\\nightshift\\setup-runner.sh',
      undefined
    ]
  ...
  it('does not disturb the wsl.exe POSIX launch', () => {
    expect(
      buildSetupRunnerCommand('C:\\repo\\a&b\\.git\\nightshift\\setup-runner.sh', 'windows', {
        family: 'posix',
        executable: 'wsl.exe'
      })
    ).toBe("bash '/mnt/c/repo/a&b/.git/nightshift/setup-runner.sh'")
  })
})
...
describe('getSetupRunnerCommandPlatformForPath', () => {
  it('prefers POSIX for absolute POSIX runner paths even from Windows clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath(
        '/remote/repo/.git/nightshift/setup-runner.sh',
        'windows'
      )
    ).toBe('posix')
  })
  ...
  it('prefers Windows for native Windows runner paths even from POSIX clients', () => {
    expect(
      getSetupRunnerCommandPlatformForPath('C:\\repo\\.git\\nightshift\\setup-runner.cmd', 'posix')
    ).toBe('windows')
  })
  ...
  it('keeps WSL UNC paths on the Windows resolver so they can be converted', () => {
    expect(
      getSetupRunnerCommandPlatformForPath(
        '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo\\.git\\nightshift\\setup-runner.sh',
        'posix'
      )
    ).toBe('windows')
  })
  ...
  it('keeps forward-slash UNC paths on the Windows resolver', () => {
    expect(
      getSetupRunnerCommandPlatformForPath(
        '//wsl.localhost/Ubuntu/home/jin/repo/.git/nightshift/setup-runner.sh',
        'posix'
      )
    ).toBe('windows')
    expect(
      getSetupRunnerCommandPlatformForPath(
        '//server/share/repo/.git/nightshift/setup-runner.cmd',
        'posix'
      )
    ).toBe('windows')
  })
