...
// or a plugin's bundled copy. Nightshift's global update never writes here, so the owner's
...
// Why: Nightshift's updater only ever passes --global, so a copy a project owns is not
// something Nightshift can act on — it has no remedy by design. Reporting it as global drift
 ...
 * the only update Nightshift runs.
 */
...
// Why: a real read failure is a fact about the user's disk and stays actionable. Nightshift's
