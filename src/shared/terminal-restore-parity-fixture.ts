...
import { activateNightshiftTerminalUnicodeProvider } from './terminal-unicode-provider'
 ...
 *  (pane-dom-creation.ts) and the Nightshift ZWJ provider (pane-lifecycle.ts).
...
export function createRendererParityTerminal(dims: { cols: number; rows: number }): ParityTerminal {
  ...
  activateNightshiftTerminalUnicodeProvider(terminal)
 ...
 *    while Nightshift's WebGL glyph renderer skips decorations on null cells.
 ...
 *  NightshiftRuntime.serializeHeadlessTerminalBuffer (normal buffer separated from
