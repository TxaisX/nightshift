...
/**
 * `'posix'` covers every Unix shell Nightshift can type into, fish included — not
 ...
 * is deliberately no fish member: a dialect Nightshift has to detect is a dialect it
...
export function tokenizeStartupCommand(
  ...
  // Why one Unix parse: the input is a string the user typed into a Nightshift
...
/**
 * Quotes one argument so the SAME text is literal in every Unix shell Nightshift can
 ...
 * of them, and Nightshift never has to know which shell will read the line.
 *
 ...
 *
 * Why not a helper function defined by Nightshift's shell wrappers: Nightshift only wraps
 ...
 * shells Nightshift never spawned — in all of those a helper would be `command not
 ...
 * clipboard command, which a user may run with no Nightshift-injected value at all.
