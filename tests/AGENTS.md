...
whenever `NIGHTSHIFT_E2E_HEADLESS=1`, `NIGHTSHIFT_E2E_HEADFUL=1`, or `NIGHTSHIFT_BACKGROUND_LAUNCH=1`:
...
- Launch through `tests/e2e/helpers/nightshift-app.ts` (or `nightshift-restart.ts`) — they already set the env.
- A raw `electron.launch()` outside those helpers must pass `NIGHTSHIFT_BACKGROUND_LAUNCH: '1'`.
...
- Native-focus tests belong on an isolated display or CI. Do not set `NIGHTSHIFT_E2E_FOREGROUND=1`
