...
- Headless (`NIGHTSHIFT_E2E_HEADLESS=1`) does not exempt you from this rule — Playwright drives the real DOM via CDP regardless of window visibility. The rare cases that need focus or pointer capture use `NIGHTSHIFT_E2E_HEADFUL=1` via `project.metadata.nightshiftHeadful`.
