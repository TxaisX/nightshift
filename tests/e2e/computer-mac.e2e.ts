...
describe.skipIf(!isMac || !e2eOptIn)('computer-use macOS e2e (TextEdit)', () => {
  ...
  test('click, type-text, and re-observe show inserted text', async () => {
    const before = parseJsonOutput<{ result: ComputerSnapshotResult }>(
      (await runNightshiftCli(['computer', 'get-app-state', '--app', 'TextEdit', '--json'])).stdout
    )
    ...
    const marker = `nightshift computer e2e ${Date.now()}`
    await runNightshiftCli(['computer', 'type-text', '--app', 'TextEdit', '--text', marker])
  ...
  test('paste-text and hotkey verify TextEdit text replacement', async () => {
    ...
    const marker = `nightshift paste final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('nightshift paste first')
  })
  ...
  test('accessibility text actions work when TextEdit is not frontmost', async () => {
    ...
    const marker = `nightshift unfocused final ${Date.now()}`
    ...
    expect(after.result.snapshot.treeText).toContain(marker)
    expect(after.result.snapshot.treeText).not.toContain('nightshift unfocused first')
  })
