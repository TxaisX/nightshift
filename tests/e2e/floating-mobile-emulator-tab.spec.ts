...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(nightshiftPage)
  await openFloatingPanelIfNeeded(nightshiftPage)

  const openPanel = nightshiftPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(nightshiftPage, testInfo)
