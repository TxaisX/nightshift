...
test('@golden opens an unstaged file diff from Source Control', async ({
  ...
  const changedFile = nightshiftPage
  ...
  await expect(nightshiftPage.locator('.monaco-diff-editor')).toBeVisible({ timeout: 20_000 })
  await expect(
    nightshiftPage
  ...
  await expect(nightshiftPage.locator('.editor-header-path').first()).toHaveAttribute(
    'title',
    `${realpathSync(path.join(fixture.worktreePath, GOLDEN_CHANGED_PATH)).replaceAll('\\', '/')} (diff)`
  )

  const probe = nightshiftPage.getByRole('button', { name: /Source Control/ })
  await probe.focus()
