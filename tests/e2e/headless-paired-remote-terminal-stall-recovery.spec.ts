...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'nightshift-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless Nightshift host @headful', async ({
  testRepoPath
}) => {
