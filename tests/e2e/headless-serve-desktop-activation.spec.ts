...
import { test, expect, forwardElectronProcessLogs } from './helpers/nightshift-app'
import { TEST_REPO_PATH_FILE } from './global-setup'
import { getE2ECompletedOnboardingProfile } from './helpers/e2e-completed-onboarding-profile'
import { getNightshiftElectronLaunchArgs } from './helpers/electron-launch-args'
...
function createHeadlessLaunchIsolation(userDataDir: string): ElectronHomeIsolation {
  ...
  return createElectronHomeIsolation({
    ...
    launchEnv: {
      ...
      NIGHTSHIFT_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1'
    },
    extraEnv: {},
    userDataDir
  })
...
{}, testInfo) => {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'nightshift-e2e-serve-promotion-'))
  ...
  try {
    serveApp = await electron.launch({
      args: [...getNightshiftElectronLaunchArgs(mainPath, false), '--serve', '--serve-no-pairing'],
      env
    })
    ...
    const forwardAppLogs = process.env.NIGHTSHIFT_E2E_FORWARD_APP_LOGS === '1'
    activatingProcess = spawn(electronPath, getNightshiftElectronLaunchArgs(mainPath, false), {
      env,
