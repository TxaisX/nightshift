...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'nightshift-e2e-retired-worker-'))
...
const ledger = process.env.NIGHTSHIFT_E2E_CODEX_LIFECYCLE_LEDGER
...
process.stdin.on('data', (chunk) => {
  ...
  if (input.includes('NIGHTSHIFT_E2E_EXIT_AFTER_DONE')) {
...
export const completedWorkerLaunchEnv = {
  ...
  NIGHTSHIFT_E2E_CODEX_LIFECYCLE_LEDGER: lifecycleLedgerPath
}
...
export function runBuiltNightshiftCli(
  args: string[],
  options: { userDataDir: string; cwd: string }
): unknown {
  const {
    NIGHTSHIFT_ENVIRONMENT: _environment,
    NIGHTSHIFT_PAIRING_CODE: _pairingCode,
    NIGHTSHIFT_USER_DATA_PATH: _userDataPath,
    ...cleanEnv
  } = process.env
  void _environment
  void _pairingCode
  ...
  const output = execFileSync(
    ...
    {
      cwd: options.cwd,
      env: { ...cleanEnv, NIGHTSHIFT_USER_DATA_PATH: options.userDataDir },
      encoding: 'utf8',
      timeout: 30_000
    }
  )
