/**
 * Shared Electron fixture for Nightshift E2E tests.
 *
...
type NightshiftTestFixtures = {
  ...
  nightshiftAppExtraEnv: Record<string, string>
  ...
  // developer's shell or already-running Nightshift instance.
  launchEnv: NodeJS.ProcessEnv
}

type NightshiftWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad NIGHTSHIFT_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const NIGHTSHIFT_E2E_SLOWMO_MS_RAW = process.env.NIGHTSHIFT_E2E_SLOWMO_MS
const NIGHTSHIFT_E2E_SLOWMO_MS = ((): number => {
  if (NIGHTSHIFT_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(NIGHTSHIFT_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[nightshift-e2e] NIGHTSHIFT_E2E_SLOWMO_MS="${NIGHTSHIFT_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: NIGHTSHIFT_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.NIGHTSHIFT_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.nightshiftHeadful === true
}
...
// same NIGHTSHIFT_E2E_FORWARD_APP_LOGS-gated stdout/stderr capture.
export function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.NIGHTSHIFT_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
...
/**
 * Extended Playwright test with Nightshift-specific fixtures.
 *
 * `nightshiftPage` — the main Nightshift renderer window.
 *
 ...
 */
export const test = base.extend<NightshiftTestFixtures, NightshiftWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'nightshift-e2e-userdata-'))
    ...
    // Nightshift's own agent runtime) set it so Electron behaves as a plain Node
    ...
    const homeIsolation = createElectronHomeIsolation({
      ...
      launchEnv,
      extraEnv: nightshiftAppExtraEnv,
      userDataDir
    })
    // Why: NIGHTSHIFT_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with NIGHTSHIFT_E2E_FORCE_HEADFUL=1 can actually watch what
    // the test does. Defaults to 0 (no slowdown) for normal runs.
    const slowMo = NIGHTSHIFT_E2E_SLOWMO_MS
    // Why: NIGHTSHIFT_E2E_RECORD_VIDEO=1 captures a webm of the renderer so a
    ...
    const recordVideoDir =
      process.env.NIGHTSHIFT_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: [...nightshiftAppExtraArgs, ...getNightshiftElectronLaunchArgs(mainPath, headful)],
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. NIGHTSHIFT_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: NIGHTSHIFT_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: NIGHTSHIFT_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. NIGHTSHIFT_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        ...((process.env.NIGHTSHIFT_E2E_SSH_LOCALHOST === '1' ||
          process.env.NIGHTSHIFT_E2E_SSH_DOCKER === '1' ||
          process.env.NIGHTSHIFT_E2E_NESTED_RUNTIME_SSH === '1') &&
        !cleanEnv.NIGHTSHIFT_RELAY_PATH
          ? { NIGHTSHIFT_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { NIGHTSHIFT_E2E_HEADFUL: '1' } : { NIGHTSHIFT_E2E_HEADLESS: '1' })
      }
    })
  ...
  nightshiftAppExtraEnv: [{}, { option: true }],
  nightshiftAppExtraArgs: [[], { option: true }],
  ...
  nightshiftPage: async ({ sharedPage }, provideFixture) => {
