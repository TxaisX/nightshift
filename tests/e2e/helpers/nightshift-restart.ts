 ...
 *
 * Why: the default `nightshiftPage` fixture creates a fresh `userDataDir` per test
...
type RestartSession = {
  ...
  launch: (options?: LaunchOptions) => Promise<LaunchedNightshift>
...
export function createRestartSession(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'nightshift-e2e-restart-'))
  ...
  const launch = async (options?: LaunchOptions): Promise<LaunchedNightshift> => {
    runtimeWsPort ??= await reserveRestartRuntimeWsPort()
    const app = await electron.launch({
      args: getNightshiftElectronLaunchArgs(mainPath, headful),
      env: {
        ...homeIsolation.env,
        ...options?.extraEnv,
        NIGHTSHIFT_E2E_RUNTIME_WS_PORT: String(runtimeWsPort)
      }
    })
  ...
  const dispose = async (): Promise<void> => {
    ...
    if (process.env.NIGHTSHIFT_E2E_PRESERVE_RESTART_PROFILE === '1') {
      ...
      () =>
        readRestartRendererState(() =>
          page.evaluate(async (repoId) => {
            ...
            // worktree is external to Nightshift's workspace root after the visibility rollout.
