...
const WSL_STUB_BACKUP_PATH = '/usr/local/bin/golden-stub-agent.nightshift-e2e-backup'
/** mkdir is atomic in the distro, so the lock dir serializes overlapping invocations. */
const WSL_STUB_LOCK_PATH = '/usr/local/bin/golden-stub-agent.nightshift-e2e-lock'
...
export async function useWslRuntimeForActiveProject(page: Page, distro: string): Promise<void> {
  await page.evaluate(async (wslDistro) => {
    ...
    if (!store) {
      throw new Error('Nightshift store is unavailable')
    }
