...
test('Linear filter chips keep readable names after the dropdown closes', async ({
  ...
  await expect
    .poll(() => getStoreState<string>(nightshiftPage, 'activeView'), { timeout: 5_000 })
    .toBe('tasks')
  const filtersButton = nightshiftPage.getByRole('button', { name: 'Filters', exact: true })
  await expect(filtersButton).toBeVisible()
  await expect(nightshiftPage.getByText(FIXTURE.issue.title, { exact: true })).toBeVisible()

  await filtersButton.click()
  const popover = nightshiftPage.locator('[data-slot="popover-content"]')
