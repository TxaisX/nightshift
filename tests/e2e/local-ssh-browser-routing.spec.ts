...
test.skip(
  process.env.NIGHTSHIFT_E2E_LOCAL_SSH_BROWSER !== '1',
  'Run with NIGHTSHIFT_E2E_LOCAL_SSH_BROWSER=1 (requires Docker)'
)
...
test('routes SSH-workspace browsing through the SSH host, fail-closed across a real drop', async ({
  ...
  try {
    ...
    await installBrowserPaneMountCensus(nightshiftPage)

    // (1) Remote-only origin renders -- the causal egress oracle.
    const loginTab = await createBrowserTab(nightshiftPage, worktreeId, LOGIN_URL, 'SSH login')
    ...
    const loginProbe = await probeTabWebview(nightshiftPage, loginTab.id)
    expect(
      loginProbe?.partition,
      'SSH-workspace pages must mount on a derived route partition'
    ).toMatch(ROUTE_PARTITION_RE)

    const routedCensus = await readBrowserPaneMountCensus(nightshiftPage)
    ...
    const echoTab = await createBrowserTab(nightshiftPage, worktreeId, ECHO_URL, 'SSH echo')
    ...
    expect(
      (await probeTabWebview(nightshiftPage, echoTab.id))?.partition,
      'both SSH-workspace tabs must share one route partition'
    ).toBe(loginProbe?.partition)

    const beforeDrop = await readSshState(nightshiftPage, remote.targetId)
    ...
    await expect
      .poll(
        async () => {
          dropOutcome = await reloadTab(nightshiftPage, echoTab.id, 30_000)
    ...
    expect(
      await readPageLoadError(nightshiftPage, echoTab.id),
      'the fenced page must surface a load failure in the product state'
    ).not.toBeNull()
    expect(
      (await probeTabWebview(nightshiftPage, echoTab.id))?.marker ?? null,
    ...
    const afterDrop = await reconnectSshTarget(nightshiftPage, remote.targetId)
    expect(
    ...
    await expect
      .poll(
        async () => {
          recoveryOutcome = await reloadTab(nightshiftPage, echoTab.id, 30_000)
    ...
    const optOutTab = await createBrowserTab(nightshiftPage, worktreeId, OPT_OUT_URL, 'SSH opt-out')
    await expect
      .poll(async () => (await probeTabWebview(nightshiftPage, optOutTab.id))?.partition ?? null, {
        timeout: 60_000,
        message: 'the opt-out tab never attached a guest'
      })
      .not.toBeNull()
    const optOutPartition = (await probeTabWebview(nightshiftPage, optOutTab.id))?.partition
    expect(
      optOutPartition,
      'with routing disabled a new tab must not mount on a route partition'
    ).not.toMatch(ROUTE_PARTITION_RE)

    const optOutCensus = censusFor(await readBrowserPaneMountCensus(nightshiftPage), optOutTab.id)
    expect(
      optOutCensus.some((entry) => entry.kind === 'gate-preparing'),
      'the opt-out pane must skip the SSH-routing gate entirely'
    ).toBe(false)
    await expect
      .poll(async () => (await reloadTab(nightshiftPage, optOutTab.id, 30_000)).outcome, {
        timeout: 120_000,
        intervals: [1_000, 2_000],
...
test('holds the mount and offers a working local escape hatch when the SSH host is unavailable', async ({
  ...
  try {
    ...
    await expect
      .poll(async () => (await readSshState(nightshiftPage, remote.targetId)).status, {
        timeout: 60_000,
    ...
    const strandedPane = nightshiftPage.locator(`[data-browser-overlay-tab-id="${strandedTab.id}"]`)
    await expect(
    ...
    const strandedCensus = await readBrowserPaneMountCensus(nightshiftPage)
    ...
    await expect
      .poll(async () => (await probeTabWebview(nightshiftPage, strandedTab.id))?.marker ?? null, {
        timeout: 120_000,
        intervals: [250, 500, 1_000],
        message: 'the escape hatch never produced a page that could load from this device'
      })
      .toBe(LOCAL_DEVICE_MARKER)
    const escapedPartition = (await probeTabWebview(nightshiftPage, strandedTab.id))?.partition
    expect(
      escapedPartition,
      'the explicit local-browsing choice must mount off the route partition'
    ).not.toMatch(ROUTE_PARTITION_RE)
    for (const entry of censusFor(
      await readBrowserPaneMountCensus(nightshiftPage),
      strandedTab.id
    )) {
      if (entry.kind === 'webview') {
        expect(
...
test('classifies a real AllowTcpForwarding no refusal and keeps Try anyway routed', async ({
  ...
  try {
    ...
    const remote = await connectDockerSshRelayTarget(nightshiftPage, target)
    expect(
      (await readSshState(nightshiftPage, remote.targetId)).status,
    ...
    const blockedPane = nightshiftPage.locator(`[data-browser-overlay-tab-id="${blockedTab.id}"]`)
    ...
    const blockedCensus = censusFor(await readBrowserPaneMountCensus(nightshiftPage), blockedTab.id)
    ...
    await expect
      .poll(async () => (await probeTabWebview(nightshiftPage, blockedTab.id))?.partition ?? null, {
        timeout: 120_000,
        intervals: [250, 500, 1_000],
        message: '"Try anyway" never mounted a guest'
      })
      .not.toBeNull()
    const overrideCensus = censusFor(
      await readBrowserPaneMountCensus(nightshiftPage),
      blockedTab.id
    )
    ...
    await expect
      .poll(() => readPageLoadError(nightshiftPage, blockedTab.id), {
        timeout: 180_000,
        intervals: [500, 1_000, 2_000],
    ...
    await testInfo.attach('try-anyway-load-error', {
      body: JSON.stringify(await readPageLoadError(nightshiftPage, blockedTab.id)),
      contentType: 'application/json'
    })
    expect(
      (await probeTabWebview(nightshiftPage, blockedTab.id))?.marker ?? null,
