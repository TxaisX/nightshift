/**
 * Regression: a worktree created via the CLI (`nightshift worktree
...
test.describe('worktree visibility with a remote runtime active', () => {
  test('a CLI-created worktree appears in the sidebar while a remote runtime is active', async ({
    ...
    const repoId = await nightshiftPage.evaluate(() => {
    ...
    // dir — exactly what `nightshift worktree create` does from a terminal.
    ...
    const worktreeRow = (worktreeId: string) =>
      nightshiftPage.locator(`[data-worktree-id=${JSON.stringify(worktreeId)}]`).first()
