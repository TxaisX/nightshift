...
const isPairedBrowserRun = process.env.NIGHTSHIFT_E2E_WEB_CLIENT === '1'
...
test('keeps two paired browser clients and the host on independent worktrees', async ({
  ...
  await expect
    .poll(() => loadTestWorktreeIds(nightshiftPage, branchA, branchB), {
      timeout: 30_000,
  ...
  const ids = await loadTestWorktreeIds(nightshiftPage, branchA, branchB)
  if (!ids) {
    throw new Error('Test worktrees disappeared after discovery')
  }

  await selectWorktree(nightshiftPage, ids.host)

  let clientA: Page | null = null
  let clientB: Page | null = null
  try {
    const offerA = await createPairingOffer(nightshiftPage)
    ...
    const offerB = await createPairingOffer(nightshiftPage)
    ...
    await expectActiveWorktree(nightshiftPage, ids.host)
...
test('shows only provider-backed creation actions in paired web', async ({
  ...
  const visibleWorktreeId = await nightshiftPage.evaluate(
  ...
  const offer = await createPairingOffer(nightshiftPage)
...
test('routes Add Project folder browsing through the paired host', async ({
  ...
  const hostFolder = mkdtempSync(path.join(os.tmpdir(), 'nightshift-paired-web-folder-'))
