...
const relativeFilePath =
  'packages/nightshift/src/renderer/src/components/navigation/worktree/quick-open/long-path-fixtures/very-deeply-nested-folder/QuickOpenTarget.tsx'
...
test('cmd+p quick open prioritizes the filename and reveals the full path on hover', async ({
  ...
  const dialog = nightshiftPage.getByRole('dialog', { name: 'Go to file' })
  ...
  const proofPath = process.env.NIGHTSHIFT_QUICK_OPEN_PROOF_PATH
  if (proofPath) {
