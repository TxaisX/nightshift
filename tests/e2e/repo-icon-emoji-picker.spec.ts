...
async function openRepoSettings(page: Page, repoId: string): Promise<void> {
  // Why: the host OS locale (e.g. ko-KR) drives Nightshift's default UI language.
...
test.describe('Repository icon emoji picker', () => {
  test('search selects a native emoji and persists it as the repo icon', async ({
    ...
    await attachScreenshot(nightshiftPage, testInfo, 'repo-icon-emoji-picker-search')
    ...
    await attachScreenshot(nightshiftPage, testInfo, 'repo-icon-emoji-picker-selected')
  })
})
