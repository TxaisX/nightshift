...
const SETUP_SCRIPT_COMMAND = 'echo nightshift-e2e-setup'
...
/** Repo whose shared nightshift.yaml carries a real setup script, plus a second worktree. */
function createRepoWithSharedSetupScript(repoPath: string, featureWorktreePath: string): void {
  ...
  writeFileSync(path.join(repoPath, 'README.md'), '# Unreadable nightshift.yaml E2E\n')
  writeFileSync(
...
/**
 * Makes the main process report the failure the fix now surfaces: nightshift.yaml could
 ...
 */
async function installUnreadableNightshiftYamlFault(
  electronApp: ElectronApplication
): Promise<void> {
  await electronApp.evaluate(({ ipcMain }) => {
    type InvokeHandler = (event: unknown, ...args: unknown[]) => unknown
    const faultState = globalThis as typeof globalThis & {
      __nightshiftE2eNightshiftYamlUnreadable?: boolean
    }
    ...
    faultState.__nightshiftE2eNightshiftYamlUnreadable = true
    ipcMain.removeHandler('hooks:check')
    ipcMain.handle('hooks:check', async (event, ...args) => {
      if (faultState.__nightshiftE2eNightshiftYamlUnreadable) {
...
/** nightshift.yaml becomes readable again — every later check runs the production handler. */
async function healNightshiftYamlRead(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(() => {
    ;(
      globalThis as typeof globalThis & { __nightshiftE2eNightshiftYamlUnreadable?: boolean }
    ).__nightshiftE2eNightshiftYamlUnreadable = false
  })
}
...
test.describe('Setup script prompt', () => {
  ...
  test('recovers from an unreadable nightshift.yaml instead of pinning the failed verdict', async ({
    electronApp,
    nightshiftPage
  }, testInfo) => {
    const repoPath = testInfo.outputPath('unreadable-nightshift-yaml-repo')
    const featureWorktreePath = testInfo.outputPath('unreadable-nightshift-yaml-feature')
    createRepoWithSharedSetupScript(repoPath, featureWorktreePath)

    await installUnreadableNightshiftYamlFault(electronApp)
    ...
    const promptCard = nightshiftPage.locator('[data-setup-script-prompt-layer]')
    const inspectionError = promptCard.getByText(INSPECTION_ERROR_TEXT)
    await expect(inspectionError).toBeVisible({ timeout: 20_000 })

    // nightshift.yaml is readable again; the card is still pinned to the failed verdict.
    await healNightshiftYamlRead(electronApp)
    const healthyCheck = await nightshiftPage.evaluate(
    ...
    await nightshiftPage.waitForTimeout(RECORDING_DWELL_MS)

    // Activating another worktree in the same repo must re-inspect.
    const featureRow = worktreeRow(nightshiftPage, featureWorktreeId)
    await expect(featureRow).toBeVisible()
    await worktreeRowSurface(nightshiftPage, featureWorktreeId).click()
    await expect(featureRow).toHaveAttribute('aria-current', 'page')

    // The repo has a valid nightshift.yaml scripts.setup, so no prompt may remain.
