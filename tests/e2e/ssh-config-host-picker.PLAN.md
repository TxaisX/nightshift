...
secondary **Add all N to Nightshift** action (no re-adopt). Settings → SSH → **Import**
...
| Search, result limit, suppressed aliases, alreadyInNightshift  | `ssh-config-host-picker.test.ts`                     |
...
1. **Isolated HOME** — E2E already sets `HOME` to `{userDataDir}/home`. Seed config with:
   ...
   ```
2. **Unique aliases** — prefix Host entries and Nightshift labels with
...
| **Expect** | Dialog title **Choose from ~/.ssh/config**; body **No hosts in ~/.ssh/config**; **Add all to Nightshift** disabled; **Back** returns to form. |
...
| **Steps**  | Open picker.                                                                                                          |
| **Expect** | Host list `SSH config hosts` shows both aliases; subtitle `user@hostname:port`; button **Add all 2 to Nightshift** enabled. |
...
### P5 — Already-in-Nightshift badge + disabled row

|            |                                                                                                              |
| ---------- | ------------------------------------------------------------------------------------------------------------ |
| **Setup**  | Config hosts alpha + bravo. Seed Nightshift target with `configHost`/`label` matching alpha (via `ssh.addTarget`). |
| **Steps**  | Open picker.                                                                                                 |
| **Expect** | Alpha shows **In Nightshift** badge and is not clickable; bravo still selectable; **Add all 1 to Nightshift** (not 2).   |

### P6 — Add all N to Nightshift imports new hosts only

|            |                                                                                                                                     |
| ---------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **Setup**  | Config with 2 new hosts; no Nightshift targets for them.                                                                                  |
| **Steps**  | **Add all 2 to Nightshift** → wait for success toast / return to form or list refresh.                                                    |
| **Expect** | Both targets exist (DOM in Settings SSH and/or listTargets); re-open picker shows **All hosts already in Nightshift** / both **In Nightshift**. |
...
| **Expect** | Deleted config host reappears as a Nightshift target; toast sync count ≥ 1. |
...
- **N2** Case-insensitive alias: config `Prod`, existing label `prod` → **In Nightshift**.
...
| P5 In Nightshift badge / count              | `ssh-config-host-import.spec.ts` |
