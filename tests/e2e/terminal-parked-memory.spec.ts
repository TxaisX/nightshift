...
// scoped to this spec's app launches via nightshiftAppExtraEnv (same pattern as
...
test.use({
  ...
  nightshiftAppExtraArgs: ['--enable-precise-memory-info']
})

// Why: 8 hidden tabs is below the 12-tab hot-retain limit, but that limit
// never retains anything here — the NIGHTSHIFT_E2E_TERMINAL_PARKING_DELAY_MS
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.nightshift-parked-memory-${runId}.mjs`)
    ...
    try {
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(nightshiftPage, worktreeId)
      ...
      const metrics = await sampleParkedMemoryMetrics(nightshiftPage)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(nightshiftPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(nightshiftPage, lastActiveTab.tabId)).hasManager).toBe(
        true
      )
      const visibleState = await readTerminalTabViewState(nightshiftPage, visibleTab.tabId)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    try {
      ...
      expect(await countMountedPaneManagers(nightshiftPage, scrollbackTabIds)).toBe(
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(nightshiftPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(nightshiftPage, visibleTab.tabId)).hasManager).toBe(
        true
      )
...
test.describe('Terminal hidden worktree retention budget', () => {
  test.use({
    nightshiftAppExtraEnv: {
      NIGHTSHIFT_E2E_TERMINAL_PARKING_DELAY_MS: String(PARKING_DELAY_MS),
      ...
      NIGHTSHIFT_E2E_TERMINAL_RETENTION_LIMIT: '1'
    },
    nightshiftAppExtraArgs: ['--enable-precise-memory-info']
  })
  ...
  test('releases un-parkable hidden worktree buffers only once the retention budget engages', async ({
    ...
    await waitForSessionReady(nightshiftPage)
    const victimWorktreeId = await waitForActiveWorktree(nightshiftPage)
    await skipUnlessParkingWired(nightshiftPage)

    const decoyWorktreeId = (await getAllWorktreeIds(nightshiftPage)).find(
    ...
    await updateTerminalSettings(nightshiftPage, {
      terminalHiddenWorktreeRetentionBudget: false,
      terminalScrollbackRows: RETENTION_SCROLLBACK_ROWS
    })
    await waitForRetentionBudgetSetting(nightshiftPage, false)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.nightshift-retention-memory-${runId}.mjs`)
    ...
    try {
      ...
      const baselineSnapshot = await waitForPaneIdentitySnapshot(nightshiftPage, 1)
      const baselinePtyId = baselineSnapshot.panes[0]?.ptyId
      if (!baselinePtyId) {
      ...
      for (let tabIndex = 0; tabIndex < RETENTION_TAB_COUNT; tabIndex += 1) {
        if (tabIndex > 0) {
          victimTabs.push(await createActiveTerminalTab(nightshiftPage, victimWorktreeId))
        }
        ...
        await stageUnparkableWorktreeTabs(nightshiftPage, victimWorktreeId)
      }
      ...
      await switchToWorktree(nightshiftPage, decoyWorktreeId)
      await expect
      ...
      const decoySnapshot = await waitForPaneIdentitySnapshot(nightshiftPage, 1)
      const decoyTabIds = [decoySnapshot.tabId]
      expect(await countMountedPaneManagers(nightshiftPage, decoyTabIds)).toBe(1)
      ...
      await stageUnparkableWorktreeTabs(nightshiftPage, victimWorktreeId)
      await stageUnparkableWorktreeTabs(nightshiftPage, decoyWorktreeId)
      await waitForUnparkableWorktreeTabs(nightshiftPage, victimWorktreeId)
      await waitForUnparkableWorktreeTabs(nightshiftPage, decoyWorktreeId)
      ...
      await expect
        .poll(
          async () => {
            await stageUnparkableWorktreeTabs(nightshiftPage, victimWorktreeId)
            await stageUnparkableWorktreeTabs(nightshiftPage, decoyWorktreeId)
            const victimMounted = await countMountedPaneManagers(nightshiftPage, victimTabIds)
            const decoyMounted = await countMountedPaneManagers(nightshiftPage, decoyTabIds)
      ...
      const before = await readRetentionMemorySample(nightshiftPage)
      expect(before.bufferMb).toBeGreaterThan(MIN_STAGED_BUFFER_MB)

      const flipStartedAt = Date.now()
      await updateTerminalSettings(nightshiftPage, { terminalHiddenWorktreeRetentionBudget: true })
      await waitForRetentionBudgetSetting(nightshiftPage, true)
      await expect
        .poll(() => countMountedPaneManagers(nightshiftPage, victimTabIds), {
          timeout: 30_000,
      ...
      const after = await readRetentionMemorySample(nightshiftPage)
      testInfo.annotations.push({
      ...
      expect(await countMountedPaneManagers(nightshiftPage, decoyTabIds)).toBe(decoyTabIds.length)
