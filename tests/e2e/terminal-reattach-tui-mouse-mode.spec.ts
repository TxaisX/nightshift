/**
 * Regression proof for #8291: a real alt-screen TUI survives a Nightshift quit/relaunch, and after the
...
function createIsolatedProofRepo(): string {
  // Why realpathSync: macOS tmpdir symlinks through /private and Nightshift canonicalizes repo.path.
  const repoDir = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), 'nightshift-mouse-reattach-repo-'))
  )
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Nightshift mouse-mode reattach proof repo\n')
  git('add', '-A')
...
// Why: this suite quits and relaunches Nightshift against one userDataDir, and the
...
test.describe('terminal reattach mouse mode', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      // Why a screenshot and not the video fixture: this spec quits and relaunches Nightshift,
      ...
      const proofShot = process.env.NIGHTSHIFT_E2E_PROOF_SCREENSHOT
      if (proofShot) {
