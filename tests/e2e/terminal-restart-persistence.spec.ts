...
const REQUIRE_WINDOWS_TERMINAL_RESTART_E2E =
  process.env.NIGHTSHIFT_REQUIRE_WINDOWS_TERMINAL_RESTART_E2E === '1'
...
test.describe('Terminal restart persistence', () => {
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      const prompt = `NIGHTSHIFT_RESTART_PROMPT_${Date.now()}_GT `
      const marker = `NIGHTSHIFT_CURSOR_RESTART_${Date.now()}`
      const promptCommand =
