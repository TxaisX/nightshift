...
test.describe('terminal hidden-worktree retention budget', () => {
  ...
  test('force-parks the older hidden un-parkable worktree and spares the newest', async ({
    ...
    try {
      ...
      const older = await connectDockerSshRelayTarget(nightshiftPage, target)
      await expect
      ...
      const olderPtyId = await waitForActivePanePtyId(nightshiftPage, 60_000)
      const olderTabId = await getActiveTabId(nightshiftPage)
      if (!olderTabId) {
      ...
      await sendToTerminal(nightshiftPage, olderPtyId, `echo "${olderMarker}:"\r`)
      await expect
        .poll(() => getTerminalContent(nightshiftPage, 20_000), {
          timeout: 30_000,
      ...
      const newerTabId = await getActiveTabId(nightshiftPage)
      if (!newerTabId) {
      ...
      await waitForTabParked(nightshiftPage, olderTabId, { parkDelayMs: PARKING_DELAY_MS })
      // …while the newest hidden worktree keeps its mounted panes (last-active exemption).
      const newerStillMounted = await nightshiftPage.evaluate(
