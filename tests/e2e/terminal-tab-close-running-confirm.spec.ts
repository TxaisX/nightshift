...
test('the tab X button applies the same running-process confirmation as Cmd+W', async ({
  ...
  await execInTerminal(nightshiftPage, ptyId, 'echo repro-10142-ready')
  await waitForTerminalOutput(nightshiftPage, 'repro-10142-ready', 20_000)
  await execInTerminal(nightshiftPage, ptyId, 'sleep 300')
  ...
  const busyTabId = (await getActiveTabId(nightshiftPage))!
  ...
  expect(
    {
      confirmDialogVisible: await closeDialogTitle(nightshiftPage).isVisible(),
      tabStillPresent: (await countRenderedTabs(nightshiftPage)) === tabsBefore
    },
