...
> `win-unpacked` build so that a **copied `Nightshift.exe`** — run with
...
The daemon is `fork()`ed from the app's own Electron binary (`Nightshift.exe`) with
...
round-trips the nonce, the daemon's main module is the copied `Nightshift.exe`, and
**no** loaded module resolves under `--app-dir`.

## Tiers (defined as data in `tier-file-set.mjs`)

Every tier includes the irreducible core: `Nightshift.exe`, `icudtl.dat`, both V8
...
### `NIGHTSHIFT_NODE_PTY_NATIVE_DIR`

The reverted #7421 added a `node-pty` patch that reads
`NIGHTSHIFT_NODE_PTY_NATIVE_DIR` to override the native dir. **The current branch's
...
spike still _sets_ `NIGHTSHIFT_NODE_PTY_NATIVE_DIR` to the relocated native dir so it
...
will need to **re-add the `NIGHTSHIFT_NODE_PTY_NATIVE_DIR` patch** from #7421. If it
...
process's **main module** is the copied `Nightshift.exe`, not the install-dir one.
...
- Whether `TIER_MINIMAL` (no top-level DLLs) boots `Nightshift.exe` as node at all, or
...
ship minimal; if `Nightshift.exe` fails to boot without the non-GPU DLLs, fall back to
