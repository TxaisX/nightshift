...
// installed, packaged Nightshift.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Nightshift's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Nightshift.exe>] [options]
...
Options:
  --exe-path <path>        Installed Nightshift.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Nightshift). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Nightshift.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Nightshift.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Nightshift.exe>'
    )
