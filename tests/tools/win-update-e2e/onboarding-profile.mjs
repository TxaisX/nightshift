// Persistence seed for a fresh packaged Nightshift profile: dismisses onboarding and
...
// nightshift-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites nightshift-data.json
 ...
 * for it. A real checkout (init + one commit) is required — Nightshift treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@nightshift.test')
