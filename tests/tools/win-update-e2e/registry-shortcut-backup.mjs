...
/**
 * Read-only discovery of the machine's current Nightshift install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Nightshift' -or ($il -and $il -match '\\\\Programs\\\\nightshift')) {`,
    ...
    `  if ($il -and ($sn -eq 'Nightshift' -or $il -match 'nightshift')) {`,
...
/** The two directories NSIS writes Nightshift shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Nightshift *.lnk shortcuts under the given dirs (recursive). */
export function discoverNightshiftShortcuts(dirs = shortcutDirs()) {
  const found = []
  for (const dir of dirs) {
    collectNightshiftLnks(dir, found)
  }
  return found
}

function collectNightshiftLnks(dir, out) {
  if (!existsSync(dir)) {
    return
  }
  let entries = []
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectNightshiftLnks(full, out)
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /nightshift/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + Nightshift shortcuts before an isolated
...
export function backupInstallState(runDir) {
  ...
  let i = 0
  for (const lnk of discoverNightshiftShortcuts(dirs)) {
 ...
 * Pre-existing shortcuts are copied back; test-created Nightshift shortcuts are removed.
...
function restoreShortcuts(manifest, result) {
  ...
  for (const lnk of discoverNightshiftShortcuts(manifest.shortcutDirs)) {
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Nightshift install pointer may be hijacked to the test directory.')
  console.error('!! The next real Nightshift update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Nightshift shortcuts that would be backed up:')
  console.log(JSON.stringify(discoverNightshiftShortcuts(), null, 2))
}
