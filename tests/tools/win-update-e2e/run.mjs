...
//
// Given two Nightshift Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `NIGHTSHIFT-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `nightshift-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir Nightshift.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: a Nightshift install existed before this run. It was OVERWRITTEN and the\n' +
